...
document.location.pathname.split("/"),e=Page.find(f[f.length-1]),h={};jQuery("iframe").contents().find("div.editable").each(function(){var l=jQuery(this),n=l.attr("id").split("_")[0];h[n]=l.html()});jQuery.each(h,function(l,n){n=Editor.textilize(n);e.setPartAttributes(l,{content:n})});e.save()})},textilize:function(c,a){c=c.replace(/\s*<p>((.|[\r\n])*?)<\/p>\s*/gi,"\n\n$1\n\n\n");c=c.replace(/\s*<h1>((.|[\r\n])*?)<\/h1>\s*/gi,"h1. $1\n\n");c=c.replace(/\s*<h2>((.|[\r\n])*?)<\/h2>\s*/gi,"h2. $1\n\n");
...
part:v.attributes},"mustache").appendTo("body").then(function(){jQuery("ul.assets .image a").click(function(D){var C=jQuery(D.currentTarget).find("img").attr("src").split("?")[0];D=jQuery(D.currentTarget).attr("id").split("-")[2];var O=jQuery("iframe#page-iframe-"+n.id()).contents().find('img[src^="'+v.path+'"]').first();C=O.attr("src").replace(O.attr("src").split("?")[0],C);O.attr("src",C);jQuery("#overlay").fadeOut("fast",function(){jQuery(this).remove()});n.setPartAttributes("image",{asset_id:D,
path:C});console.log(n.attr("contents"));return false})})})});c.bind("toggle-sidebar",function(){var a=jQuery("div#sidebar"),f=jQuery("div#editor"),e=jQuery("p#toggle");e.find("a").click(function(h){h.preventDefault();if(a.hasClass("closed")){h="300";jQuery.cookie("sidebar_toggled","")}else{h="24";jQuery.cookie("sidebar_toggled","closed")}Modernizr.cssanimations&&console.log("Move.js goes here");a.animate({width:h},200).toggleClass("closed");f.animate({left:h},200).toggleClass("open");e.animate({left:h},
...
a.params.id;Part.create(a.params.part,function(){a.redirect("/admin/templates/"+f+"/edit")})})})}),Assets=Sammy(function(c){var a=this;c.helpers({send_files:function(f,e,h){jQuery.each(f,function(l,n){Asset.create({file:n,folder_id:e.folder_id},{before:function(v){jQuery(".progress").append('<p id="progress-'+v+'">'+n.name+'<span class="percentage"></span></p>')},progress:function(v,D){jQuery("#progress-"+v+" .percentage").text(" "+D+"%")},success:function(v){h&&h.call(this,v)}})})}});c.bind("render-asset",
function(f,e,h){this.load(jQuery("script#admin-assets-edit")).interpolate(e.toMustacheWithNeighbors(h),"mustache").appendTo("body").then(function(){Utilities.loadModal("div#asset-editor",function(){jQuery("div#asset-editor").fadeIn("fast");window.modal=true})})});c.bind("render-index",function(f,e){var h=e.query,l=e.folder_id,n=this,v=jQuery("script#admin-assets-asset").html();n.load(jQuery("script#admin-assets-index")).interpolate({assets:Asset.toMustache(h),folder_id:l,query:h,partials:{asset:v}},
"mustache").replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple").bind("change",function(){jQuery(this).parents("form:first").submit()});n.trigger("set_draggable_assets");n.trigger("set_droppable_folders")})});c.bind("set_draggable_assets",function(){jQuery("li.asset").draggable({revert:true,stack:".asset"})});c.bind("set_droppable_folders",function(){jQuery("li.folder").droppable({hoverClass:"active",drop:function(f,e){var h=f.target.id.split("-")[1],l=Folder.find(h),
n=e.draggable[0].id.split("-")[1];n=Asset.find(n);n.attr("folder_id",h);e.draggable.fadeOut("fast");n.save(function(v){v?Utilities.notice("Asset added to "+l.attr("name")):e.draggable.fadeIn("fast")})}})});c.bind("show_info",function(f,e){var h=Asset.find(e.current_asset_id),l=jQuery("#asset-"+h.id()),n=l.find(".info");jQuery(".modal-strip").remove();n.length||this.load(jQuery("#admin-assets-info")).interpolate({asset:h.toMustache()},"mustache").appendTo(l).then(function(){jQuery(".modal-strip").fadeIn("fast")})});
c.bind("show_remove_dialog",function(f,e){var h=Asset.find(e.current_asset_id),l=jQuery("#asset-"+h.id()),n=l.find(".remove");jQuery(".modal-strip").remove();n.length||this.load(jQuery("script#admin-assets-remove")).interpolate({asset:h.toMustache()},"mustache").appendTo(jQuery("body")).then(function(){var v=jQuery(".modal-strip");v.css({top:l.offset().top+l.height()/2,left:l.offset().left+l.width()/2});v.fadeIn("fast")})});c.get("/admin/assets",function(f){jQuery("#overlay").remove();var e=f.params.query,
h={query:e};h.limit=f.params.limit||96;h.page=f.params.page||1;if(e){f.renderFolderTree();Asset.searchAdmin(h,function(){f.trigger("render-index",{query:e})})}else f.redirect(Folder.first().attr("admin_path"))});c.get("/admin/assets/new",function(f){f.load(jQuery("script#admin-assets-new")).interpolate({},"mustache").replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple")});a.first_run=false});c.post("/admin/assets",function(f){var e=document.getElementById("ajax_uploader").files,
h=f.params.asset.folder_id,l=f.params.query?f.params.query:null;jQuery("form#new_asset");l=l?{query:l}:{};l.limit=f.params.limit||48;l.page=f.params.page||1;l.folder_id=h;this.send_files(e,l,function(n){f.load(jQuery("script#admin-assets-asset")).interpolate(n.attr(),"mustache").prependTo("#assets")});return false});c.get("/admin/assets/:id/edit",function(f){jQuery("#overlay").remove();var e=f.params.query?f.params.query:"",h=e?{query:f.params.query}:{},l=Asset.find(f.params.id);l?f.trigger("render-asset",
l,e):Asset.searchAdmin(h,function(){var n=Asset.find(f.params.id);console.log(n.attr("folder_id"));f.trigger("render-index",{folder_id:n.attr("folder_id")});f.renderFolderTree();f.trigger("render-asset",n,e)})});c.put("/admin/assets/:id",function(f){var e=Asset.find(f.params.id);if(e){e.attr(f.params.asset);e.save(function(h){h&&Utilities.notice("Successfully saved asset")})}else{e=new Asset({id:f.params.id});e.load(function(){e.attr(f.params.asset);e.save(function(h){h&&Utilities.notice("Successfully saved asset")})})}});
...
"mustache").replace("#sidebar").then(function(){a&&a.call(this)})}});this.get("/admin/folders/new/?",function(a){$("#modal").length==0&&Galerie.open();a.renderFolderTree(function(){jQuery("ul#folders li").removeClass("active")});a.load(jQuery("#admin-folders-new")).interpolate({},"mustache").replace("#modal")});c.get("/admin/folders/:id",function(a){jQuery("#overlay").remove();var f=a.params.id,e=Folder.find(f);a.renderFolderTree(function(){jQuery("ul#folders li").removeClass("active");jQuery("#folder-"+
f).addClass("active")});e.loadAssets(function(){a.trigger("render-index",{folder_id:e.id()})})});this.post("/admin/folders",function(a){var f=new Folder(a.params.folder);f.save(function(e,h){var l=JSON.parse(h.responseText);if(l.errors)alert(JSON.stringify(l));else{alert(f.attr("admin_path"));a.load(jQuery("script#admin-folders-index")).interpolate({folders:Folder.asJSON()},"mustache").replace("#sidebar");a.redirect(f.attr("admin_path"))}})})}),Pages=Sammy(function(c){c.helpers({renderPagePreview:function(a,
f){var e=this;e.load(jQuery("script#admin-pages-show")).interpolate({page:a.asJSON(),layouts:Layout.asLayoutJSON(a.attr("layout_id")),base_page_id:a.id()},"mustache").replace("#editor").then(function(){iFramer.initialize(".preview iframe",function(){f&&f.call(this)});e.trigger("set-active-page",a);Editor.initialize()})},renderPageEditor:function(a,f){var e=this;if(jQuery("#page-editor").length){f&&f.call(this);e.trigger("set-active-page",a)}else e.load(jQuery("script#admin-pages-edit")).interpolate({page:a.asJSON()},
