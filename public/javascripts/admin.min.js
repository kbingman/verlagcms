...
j=f.css(d,"display");h.removeChild(Ta)}db[c]=j}return db[c]}function i(c,h){var d={};f.each(Ka.concat.apply([],Ka.slice(0,h)),function(){d[this]=c});return d}function l(){qb=a}function n(){setTimeout(l,0);return qb=f.now()}function w(){try{return new b.XMLHttpRequest}catch(c){}}function C(c,h,d,j){if(f.isArray(h))f.each(h,function(m,o){d||eb.test(c)?j(c,o):C(c+"["+(typeof o=="object"||f.isArray(o)?m:"")+"]",o,d,j)});else if(!d&&h!=null&&typeof h=="object")for(var k in h)C(c+"["+k+"]",h[k],d,j);else j(c,
h)}function B(c,h,d,j,k,m){k=k||h.dataTypes[0];m=m||{};m[k]=true;k=c[k];for(var o=0,q=k?k.length:0,s=c===bb,u;o<q&&(s||!u);o++){u=k[o](h,d,j);typeof u=="string"&&(!s||m[u]?(u=a):(h.dataTypes.unshift(u),u=B(c,h,d,j,u,m)))}(s||!u)&&!m["*"]&&(u=B(c,h,d,j,"*",m));return u}function K(c){return function(h,d){typeof h!="string"&&(d=h,h="*");if(f.isFunction(d))for(var j=h.toLowerCase().split(cc),k=0,m=j.length,o,q;k<m;k++){o=j[k];(q=/^\+/.test(o))&&(o=o.substr(1)||"*");o=c[o]=c[o]||[];o[q?"unshift":"push"](d)}}}
...
function(k){return k.nodeType===1});if(Fa.test(h))return f.filter(h,j,!d);h=f.filter(h,j)}return f.grep(c,function(k){return f.inArray(k,h)>=0===d})}function ba(c,h){return(c&&c!=="*"?c+".":"")+h.replace(cb,"`").replace(gb,"&")}function ga(c){var h,d,j,k,m,o,q,s,u,y,D,P=[];k=[];m=f._data(this,"events");if(!(c.liveFired===this||!m||!m.live||c.target.disabled||c.button&&c.type==="click")){c.namespace&&(D=new RegExp("(^|\\.)"+c.namespace.split(".").join("\\.(?:.*\\.)?")+"(\\.|$)"));c.liveFired=this;
var Y=m.live.slice(0);for(q=0;q<Y.length;q++){m=Y[q];m.origType.replace(ua,"")===c.type?k.push(m.selector):Y.splice(q--,1)}k=f(c.target).closest(k,c.currentTarget);s=0;for(u=k.length;s<u;s++){y=k[s];for(q=0;q<Y.length;q++){m=Y[q];if(y.selector===m.selector&&(!D||D.test(m.namespace))&&!y.elem.disabled){o=y.elem;j=null;if(m.preType==="mouseenter"||m.preType==="mouseleave"){c.type=m.preType;(j=f(c.relatedTarget).closest(m.selector)[0])&&f.contains(o,j)&&(j=o)}(!j||j!==o)&&P.push({elem:o,handleObj:m,
...
function oa(){return false}function Ca(c,h,d){var j=h+"defer",k=h+"queue",m=h+"mark",o=f.data(c,j,a,true);o&&(d==="queue"||!f.data(c,k,a,true))&&(d==="mark"||!f.data(c,m,a,true))&&setTimeout(function(){!f.data(c,k,a,true)&&!f.data(c,m,a,true)&&(f.removeData(c,j,true),o.resolve())},0)}function Ua(c){for(var h in c)if(h!=="toJSON")return false;return true}function Va(c,h,d){if(d===a&&c.nodeType===1){d="data-"+h.replace(S,"$1-$2").toLowerCase();d=c.getAttribute(d);if(typeof d=="string"){try{d=d==="true"?
true:d==="false"?false:d==="null"?null:f.isNaN(d)?L.test(d)?f.parseJSON(d):d:parseFloat(d)}catch(j){}f.data(c,h,d)}else d=a}return d}var V=b.document,Q=b.navigator,U=b.location,f=function(){function c(){if(!h.isReady){try{V.documentElement.doScroll("left")}catch(x){setTimeout(c,1);return}h.ready()}}var h=function(x,J){return new h.fn.init(x,J,k)},d=b.jQuery,j=b.$,k,m=/^(?:[^<]*(<[\w\W]+>)[^>]*$|#([\w\-]*)$)/,o=/\S/,q=/^\s+/,s=/\s+$/,u=/\d/,y=/^<(\w+)\s*\/?>(?:<\/\1>)?$/,D=/^[\],:{}\s]*$/,P=/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,
...
J,Z){b.DOMParser?(Z=new DOMParser,J=Z.parseFromString(x,"text/xml")):(J=new ActiveXObject("Microsoft.XMLDOM"),J.async="false",J.loadXML(x));Z=J.documentElement;(!Z||!Z.nodeName||Z.nodeName==="parsererror")&&h.error("Invalid XML: "+x);return J},noop:function(){},globalEval:function(x){x&&o.test(x)&&(b.execScript||function(J){b.eval.call(b,J)})(x)},camelCase:function(x){return x.replace(r,v)},nodeName:function(x,J){return x.nodeName&&x.nodeName.toUpperCase()===J.toUpperCase()},each:function(x,J,Z){var W,
...
true);o.test("\u00a0")&&(q=/^[\s\xA0]+/,s=/[\s\xA0]+$/);k=h(V);V.addEventListener?(E=function(){V.removeEventListener("DOMContentLoaded",E,false);h.ready()}):V.attachEvent&&(E=function(){V.readyState==="complete"&&(V.detachEvent("onreadystatechange",E),h.ready())});return h}(),A="done fail isResolved isRejected promise then always pipe".split(" "),M=[].slice;f.extend({_Deferred:function(){var c=[],h,d,j,k={done:function(){if(!j){var m=arguments,o,q,s,u,y;h&&(y=h,h=0);o=0;for(q=m.length;o<q;o++){s=
m[o];u=f.type(s);u==="array"?k.done.apply(k,s):u==="function"&&c.push(s)}y&&k.resolveWith(y[0],y[1])}return this},resolveWith:function(m,o){if(!j&&!h&&!d){o=o||[];d=1;try{for(;c[0];)c.shift().apply(m,o)}finally{h=[m,o];d=0}}return this},resolve:function(){k.resolveWith(this,arguments);return this},isResolved:function(){return!!d||!!h},cancel:function(){j=1;c=[];return this}};return k},Deferred:function(c){var h=f._Deferred(),d=f._Deferred(),j;f.extend(h,{then:function(k,m){h.done(k).fail(m);return this},
always:function(){return h.done.apply(h,arguments).fail.apply(this,arguments)},fail:d.done,rejectWith:d.resolveWith,reject:d.resolve,isRejected:d.isResolved,pipe:function(k,m){return f.Deferred(function(o){f.each({done:[k,"resolve"],fail:[m,"reject"]},function(q,s){var u=s[0],y=s[1],D;f.isFunction(u)?h[q](function(){(D=u.apply(this,arguments))&&f.isFunction(D.promise)?D.promise().then(o.resolve,o.reject):o[y](D)}):h[q](o[y])})}).promise()},promise:function(k){if(k==null){if(j)return j;j=k={}}for(var m=
A.length;m--;)k[A[m]]=h[A[m]];return k}});h.done(d.cancel).fail(h.cancel);delete h.cancel;c&&c.call(h,h);return h},when:function(c){function h(q){return function(s){d[q]=arguments.length>1?M.call(arguments,0):s;--m||o.resolveWith(o,M.call(d,0))}}var d=arguments,j=0,k=d.length,m=k,o=k<=1&&c&&f.isFunction(c.promise)?c:f.Deferred();if(k>1){for(;j<k;j++)d[j]&&f.isFunction(d[j].promise)?d[j].promise().then(h(j),o.reject):--m;m||o.resolveWith(o,d)}else o!==c&&o.resolveWith(o,k?[c]:[]);return o.promise()}});
f.support=function(){var c=V.createElement("div"),h=V.documentElement,d,j,k,m,o,q,s;c.setAttribute("className","t");c.innerHTML="   <link/><table></table><a href='/a' style='top:1px;float:left;opacity:.55;'>a</a><input type='checkbox'/>";d=c.getElementsByTagName("*");j=c.getElementsByTagName("a")[0];if(!d||!d.length||!j)return{};k=V.createElement("select");m=k.appendChild(V.createElement("option"));d=c.getElementsByTagName("input")[0];q={leadingWhitespace:c.firstChild.nodeType===3,tbody:!c.getElementsByTagName("tbody").length,
...
"<div style='width:4px;'></div>",q.shrinkWrapBlocks=c.offsetWidth!==2);c.innerHTML="<table><tr><td style='padding:0;border:0;display:none'></td><td>t</td></tr></table>";k=c.getElementsByTagName("td");d=k[0].offsetHeight===0;k[0].style.display="";k[1].style.display="none";q.reliableHiddenOffsets=d&&k[0].offsetHeight===0;c.innerHTML="";V.defaultView&&V.defaultView.getComputedStyle&&(o=V.createElement("div"),o.style.width="0",o.style.marginRight="0",c.appendChild(o),q.reliableMarginRight=(parseInt((V.defaultView.getComputedStyle(o,
null)||{marginRight:0}).marginRight,10)||0)===0);j.innerHTML="";h.removeChild(j);if(c.attachEvent)for(s in{submit:1,change:1,focusin:1}){o="on"+s;(d=o in c)||(c.setAttribute(o,"return;"),d=typeof c[o]=="function");q[s+"Bubbles"]=d}j=j=k=m=k=o=c=d=null;return q}();f.boxModel=f.support.boxModel;var L=/^(?:\{.*\}|\[.*\])$/,S=/([a-z])([A-Z])/g;f.extend({cache:{},uuid:0,expando:"jQuery"+(f.fn.jquery+Math.random()).replace(/\D/g,""),noData:{embed:true,object:"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000",
applet:true},hasData:function(c){c=c.nodeType?f.cache[c[f.expando]]:c[f.expando];return!!c&&!Ua(c)},data:function(c,h,d,j){if(f.acceptData(c)){var k=f.expando,m=typeof h=="string",o=c.nodeType,q=o?f.cache:c,s=o?c[f.expando]:c[f.expando]&&f.expando;if(!((!s||j&&s&&!q[s][k])&&m&&d===a)){s||(o?(c[f.expando]=s=++f.uuid):(s=f.expando));q[s]||(q[s]={},o||(q[s].toJSON=f.noop));if(typeof h=="object"||typeof h=="function")j?(q[s][k]=f.extend(q[s][k],h)):(q[s]=f.extend(q[s],h));c=q[s];j&&(c[k]||(c[k]={}),c=
c[k]);d!==a&&(c[f.camelCase(h)]=d);if(h==="events"&&!c[h])return c[k]&&c[k].events;return m?c[f.camelCase(h)]||c[h]:c}}},removeData:function(c,h,d){if(f.acceptData(c)){var j=f.expando,k=c.nodeType,m=k?f.cache:c,o=k?c[f.expando]:f.expando;if(m[o]){if(h){var q=d?m[o][j]:m[o];if(q){delete q[h];if(!Ua(q))return}}if(d){delete m[o][j];if(!Ua(m[o]))return}h=m[o][j];f.support.deleteExpando||m!=b?delete m[o]:(m[o]=null);h?(m[o]={},k||(m[o].toJSON=f.noop),m[o][j]=h):k&&(f.support.deleteExpando?delete c[f.expando]:
c.removeAttribute?c.removeAttribute(f.expando):(c[f.expando]=null))}}},_data:function(c,h,d){return f.data(c,h,d,true)},acceptData:function(c){if(c.nodeName){var h=f.noData[c.nodeName.toLowerCase()];if(h)return h!==true&&c.getAttribute("classid")===h}return true}});f.fn.extend({data:function(c,h){var d=null;if(typeof c=="undefined"){if(this.length){d=f.data(this[0]);if(this[0].nodeType===1)for(var j=this[0].attributes,k,m=0,o=j.length;m<o;m++){k=j[m].name;k.indexOf("data-")===0&&(k=f.camelCase(k.substring(5)),
...
return this.queue(h,function(){var d=this;setTimeout(function(){f.dequeue(d,h)},c)})},clearQueue:function(c){return this.queue(c||"fx",[])},promise:function(c,h){function d(){--o||j.resolveWith(k,[k])}typeof c!="string"&&(h=c,c=a);c=c||"fx";for(var j=f.Deferred(),k=this,m=k.length,o=1,q=c+"defer",s=c+"queue",u=c+"mark",y;m--;)if(y=f.data(k[m],q,a,true)||(f.data(k[m],s,a,true)||f.data(k[m],u,a,true))&&f.data(k[m],q,f._Deferred(),true)){o++;y.done(d)}d();return j.promise()}});var ea=/[\n\t\r]/g,da=
...
a;delete this[c]}catch(h){}})},addClass:function(c){var h,d,j,k,m,o,q;if(f.isFunction(c))return this.each(function(s){f(this).addClass(c.call(this,s,this.className))});if(c&&typeof c=="string"){h=c.split(da);d=0;for(j=this.length;d<j;d++){k=this[d];if(k.nodeType===1)if(!k.className&&h.length===1)k.className=c;else{m=" "+k.className+" ";o=0;for(q=h.length;o<q;o++)~m.indexOf(" "+h[o]+" ")||(m+=h[o]+" ");k.className=f.trim(m)}}}return this},removeClass:function(c){var h,d,j,k,m,o,q;if(f.isFunction(c))return this.each(function(s){f(this).removeClass(c.call(this,
s,this.className))});if(c&&typeof c=="string"||c===a){h=(c||"").split(da);d=0;for(j=this.length;d<j;d++){k=this[d];if(k.nodeType===1&&k.className)if(c){m=(" "+k.className+" ").replace(ea," ");o=0;for(q=h.length;o<q;o++)m=m.replace(" "+h[o]+" "," ");k.className=f.trim(m)}else k.className=""}}return this},toggleClass:function(c,h){var d=typeof c,j=typeof h=="boolean";if(f.isFunction(c))return this.each(function(k){f(this).toggleClass(c.call(this,k,this.className,h),h)});return this.each(function(){if(d===
"string")for(var k,m=0,o=f(this),q=h,s=c.split(da);k=s[m++];){q=j?q:!o.hasClass(k);o[q?"addClass":"removeClass"](k)}else if(d==="undefined"||d==="boolean"){this.className&&f._data(this,"__className__",this.className);this.className=this.className||c===false?"":f._data(this,"__className__")||""}})},hasClass:function(c){for(var h=0,d=this.length;h<d;h++)if((" "+this[h].className+" ").replace(ea," ").indexOf(" "+c+" ")>-1)return true;return false},val:function(c){var h,d,j=this[0];if(!arguments.length){if(j){if((h=
f.valHooks[j.nodeName.toLowerCase()]||f.valHooks[j.type])&&"get"in h&&(d=h.get(j,"value"))!==a)return d;d=j.value;return typeof d=="string"?d.replace(pa,""):d==null?"":d}return a}var k=f.isFunction(c);return this.each(function(m){var o=f(this),q;if(this.nodeType===1){k?(q=c.call(this,m,o.val())):(q=c);q==null?(q=""):typeof q=="number"?(q+=""):f.isArray(q)&&(q=f.map(q,function(s){return s==null?"":s+""}));h=f.valHooks[this.nodeName.toLowerCase()]||f.valHooks[this.type];if(!h||!("set"in h)||h.set(this,
q,"value")===a)this.value=q}})}});f.extend({valHooks:{option:{get:function(c){var h=c.attributes.value;return!h||h.specified?c.value:c.text}},select:{get:function(c){var h,d=c.selectedIndex,j=[],k=c.options;c=c.type==="select-one";if(d<0)return null;for(var m=c?d:0,o=c?d+1:k.length;m<o;m++){h=k[m];if(h.selected&&(f.support.optDisabled?!h.disabled:h.getAttribute("disabled")===null)&&(!h.parentNode.disabled||!f.nodeName(h.parentNode,"optgroup"))){h=f(h).val();if(c)return h;j.push(h)}}if(c&&!j.length&&
k.length)return f(k[d]).val();return j},set:function(c,h){var d=f.makeArray(h);f(c).find("option").each(function(){this.selected=f.inArray(f(this).val(),d)>=0});d.length||(c.selectedIndex=-1);return d}}},attrFn:{val:true,css:true,html:true,text:true,data:true,width:true,height:true,offset:true},attrFix:{tabindex:"tabIndex"},attr:function(c,h,d,j){var k=c.nodeType;if(!c||k===3||k===8||k===2)return a;if(j&&h in f.attrFn)return f(c)[h](d);if(!("getAttribute"in c))return f.prop(c,h,d);var m,o;(j=k!==
1||!f.isXMLDoc(c))&&(h=f.attrFix[h]||h,o=f.attrHooks[h],o||(Ma.test(h)?(o=Ja):ta&&h!=="className"&&(f.nodeName(c,"form")||Wa.test(h))&&(o=ta)));if(d!==a){if(d===null){f.removeAttr(c,h);return a}if(o&&"set"in o&&j&&(m=o.set(c,d,h))!==a)return m;c.setAttribute(h,""+d);return d}if(o&&"get"in o&&j&&(m=o.get(c,h))!==null)return m;m=c.getAttribute(h);return m===null?a:m},removeAttr:function(c,h){var d;c.nodeType===1&&(h=f.attrFix[h]||h,f.support.getSetAttribute?c.removeAttribute(h):(f.attr(c,h,""),c.removeAttributeNode(c.getAttributeNode(h))),
...
cb=/\./g,gb=/ /g,kb=/[^\w\s.|`]/g,Fb=function(c){return c.replace(kb,"\\$&")};f.event={add:function(c,h,d,j){if(c.nodeType!==3&&c.nodeType!==8){if(d===false)d=oa;else if(!d)return;var k,m;d.handler&&(k=d,d=k.handler);d.guid||(d.guid=f.guid++);if(m=f._data(c)){var o=m.events,q=m.handle;o||(m.events=o={});q||(m.handle=q=function(Y){return typeof f!="undefined"&&(!Y||f.event.triggered!==Y.type)?f.event.handle.apply(q.elem,arguments):a});q.elem=c;h=h.split(" ");for(var s,u=0,y;s=h[u++];){m=k?f.extend({},
k):{handler:d,data:j};s.indexOf(".")>-1?(y=s.split("."),s=y.shift(),m.namespace=y.slice(0).sort().join(".")):(y=[],m.namespace="");m.type=s;m.guid||(m.guid=d.guid);var D=o[s],P=f.event.special[s]||{};if(!D){D=o[s]=[];if(!P.setup||P.setup.call(c,j,y,q)===false)c.addEventListener?c.addEventListener(s,q,false):c.attachEvent&&c.attachEvent("on"+s,q)}P.add&&(P.add.call(c,m),m.handler.guid||(m.handler.guid=d.guid));D.push(m);f.event.global[s]=true}c=null}}},global:{},remove:function(c,h,d,j){if(c.nodeType!==
3&&c.nodeType!==8){d===false&&(d=oa);var k,m,o=0,q,s,u,y,D,P,Y=f.hasData(c)&&f._data(c),ca=Y&&Y.events;if(Y&&ca){h&&h.type&&(d=h.handler,h=h.type);if(!h||typeof h=="string"&&h.charAt(0)===".")for(k in ca)f.event.remove(c,k+(h||""));else{for(h=h.split(" ");k=h[o++];){y=k;q=k.indexOf(".")<0;s=[];q||(s=k.split("."),k=s.shift(),u=new RegExp("(^|\\.)"+f.map(s.slice(0).sort(),Fb).join("\\.(?:.*\\.)?")+"(\\.|$)"));if(D=ca[k])if(d){y=f.event.special[k]||{};for(m=j||0;m<D.length;m++){P=D[m];if(d.guid===P.guid){if(q||
...
changeData:true},trigger:function(c,h,d,j){var k=c.type||c,m=[],o;k.indexOf("!")>=0&&(k=k.slice(0,-1),o=true);k.indexOf(".")>=0&&(m=k.split("."),k=m.shift(),m.sort());if(d&&!f.event.customEvent[k]||f.event.global[k]){c=typeof c=="object"?c[f.expando]?c:new f.Event(k,c):new f.Event(k);c.type=k;c.exclusive=o;c.namespace=m.join(".");c.namespace_re=new RegExp("(^|\\.)"+m.join("\\.(?:.*\\.)?")+"(\\.|$)");if(j||!d){c.preventDefault();c.stopPropagation()}if(d){if(!(d.nodeType===3||d.nodeType===8)){c.result=
a;c.target=d;h=h!=null?f.makeArray(h):[];h.unshift(c);m=d;j=k.indexOf(":")<0?"on"+k:"";do{o=f._data(m,"handle");c.currentTarget=m;o&&o.apply(m,h);j&&f.acceptData(m)&&m[j]&&m[j].apply(m,h)===false&&(c.result=false,c.preventDefault());m=m.parentNode||m.ownerDocument||m===c.target.ownerDocument&&b}while(m&&!c.isPropagationStopped());if(!c.isDefaultPrevented()){var q;m=f.event.special[k]||{};if((!m._default||m._default.call(d.ownerDocument,c)===false)&&(k!=="click"||!f.nodeName(d,"a"))&&f.acceptData(d)){try{j&&
d[k]&&(q=d[j],q&&(d[j]=null),f.event.triggered=k,d[k]())}catch(s){}q&&(d[j]=q);f.event.triggered=a}}return c.result}}else f.each(f.cache,function(){var u=this[f.expando];u&&u.events&&u.events[k]&&f.event.trigger(c,h,u.handle.elem)})}},handle:function(c){c=f.event.fix(c||b.event);var h=((f._data(this,"events")||{})[c.type]||[]).slice(0),d=!c.exclusive&&!c.namespace,j=Array.prototype.slice.call(arguments,0);j[0]=c;c.currentTarget=this;for(var k=0,m=h.length;k<m;k++){var o=h[k];if(d||c.namespace_re.test(o.namespace)){c.handler=
o.handler;c.data=o.data;c.handleObj=o;o=o.handler.apply(this,j);o!==a&&(c.result=o,o===false&&(c.preventDefault(),c.stopPropagation()));if(c.isImmediatePropagationStopped())break}}return c.result},props:"altKey attrChange attrName bubbles button cancelable charCode clientX clientY ctrlKey currentTarget data detail eventPhase fromElement handler keyCode layerX layerY metaKey newValue offsetX offsetY pageX pageY prevValue relatedNode relatedTarget screenX screenY shiftKey srcElement target toElement view wheelDelta which".split(" "),
...
false;for(X=X[r];X;){if(X.sizcache===G){ja=H[X.sizset];break}X.nodeType===1&&!I&&(X.sizcache=G,X.sizset=E);if(X.nodeName.toLowerCase()===v){ja=X;break}X=X[r]}H[E]=ja}}}var d=/((?:\((?:\([^()]+\)|[^()]+)+\)|\[(?:\[[^\[\]]*\]|['"][^'"]*['"]|[^\[\]'"]+)+\]|\\.|[^ >+~,(\[\\]+)+|[>+~])(\s*,\s*)?((?:.|\r|\n)*)/g,j=0,k=Object.prototype.toString,m=false,o=true,q=/\\/g,s=/\W/;[0,0].sort(function(){o=false;return 0});var u=function(r,v,G,H){G=G||[];var E=v=v||V;if(v.nodeType!==1&&v.nodeType!==9)return[];if(!r||
...
c),m,o,q;d=0;for(j=this.length;d<j;d++){m=k.length;f.find(c,this[d],k);if(d>0)for(o=m;o<k.length;o++)for(q=0;q<m;q++)if(k[q]===k[o]){k.splice(o--,1);break}}return k},has:function(c){var h=f(c);return this.filter(function(){for(var d=0,j=h.length;d<j;d++)if(f.contains(this,h[d]))return true})},not:function(c){return this.pushStack(F(this,c,false),"not",c)},filter:function(c){return this.pushStack(F(this,c,true),"filter",c)},is:function(c){return!!c&&(typeof c=="string"?f.filter(c,this).length>0:this.filter(c).length>
0)},closest:function(c,h){var d=[],j,k,m=this[0];if(f.isArray(c)){var o,q={},s=1;if(m&&c.length){j=0;for(k=c.length;j<k;j++){o=c[j];q[o]||(q[o]=sb.test(o)?f(o,h||this.context):o)}for(;m&&m.ownerDocument&&m!==h;){for(o in q){j=q[o];(j.jquery?j.index(m)>-1:f(m).is(j))&&d.push({selector:o,elem:m,level:s})}m=m.parentNode;s++}}return d}o=sb.test(c)||typeof c!="string"?f(c,h||this.context):0;j=0;for(k=this.length;j<k;j++)for(m=this[j];m;){if(o?o.index(m)>-1:f.find.matchesSelector(m,c)){d.push(m);break}m=
...
h,d){var j,k,m,o=c[0],q=[];if(!f.support.checkClone&&arguments.length===3&&typeof o=="string"&&tb.test(o))return this.each(function(){f(this).domManip(c,h,d,true)});if(f.isFunction(o))return this.each(function(y){var D=f(this);c[0]=o.call(this,y,h?D.html():a);D.domManip(c,h,d)});if(this[0]){m=o&&o.parentNode;f.support.parentNode&&m&&m.nodeType===11&&m.childNodes.length===this.length?(j={fragment:m}):(j=f.buildFragment(c,this,q));m=j.fragment;m.childNodes.length===1?(k=m=m.firstChild):(k=m.firstChild);
if(k){h=h&&f.nodeName(k,"tr");k=0;for(var s=this.length,u=s-1;k<s;k++)d.call(h?f.nodeName(this[k],"table")?this[k].getElementsByTagName("tbody")[0]||this[k].appendChild(this[k].ownerDocument.createElement("tbody")):this[k]:this[k],j.cacheable||s>1&&k<u?f.clone(m,true,true):m)}q.length&&f.each(q,aa)}return this}});f.buildFragment=function(c,h,d){var j,k,m,o;h&&h[0]&&(o=h[0].ownerDocument||h[0]);o.createDocumentFragment||(o=V);c.length===1&&typeof c[0]=="string"&&c[0].length<512&&o===V&&c[0].charAt(0)===
"<"&&!$a.test(c[0])&&(f.support.checkClone||!tb.test(c[0]))&&(k=true,m=f.fragments[c[0]],m&&m!==1&&(j=m));j||(j=o.createDocumentFragment(),f.clean(c,o,j,d));k&&(f.fragments[c[0]]=m?j:1);return{fragment:j,cacheable:k}};f.fragments={};f.each({appendTo:"append",prependTo:"prepend",insertBefore:"before",insertAfter:"after",replaceAll:"replaceWith"},function(c,h){f.fn[c]=function(d){var j=[];d=f(d);var k=this.length===1&&this[0].parentNode;if(k&&k.nodeType===11&&k.childNodes.length===1&&d.length===1){d[h](this[0]);
return this}k=0;for(var m=d.length;k<m;k++){var o=(k>0?this.clone(true):this).get();f(d[k])[h](o);j=j.concat(o)}return this.pushStack(j,c,d.selector)}});f.extend({clone:function(c,h,d){var j=c.cloneNode(true),k,m,o;if((!f.support.noCloneEvent||!f.support.noCloneChecked)&&(c.nodeType===1||c.nodeType===11)&&!f.isXMLDoc(c)){t(c,j);k=p(c);m=p(j);for(o=0;k[o];++o)t(k[o],m[o])}if(h){z(c,j);if(d){k=p(c);m=p(j);for(o=0;k[o];++o)z(k[o],m[o])}}return j},clean:function(c,h,d,j){h=h||V;typeof h.createElement==
"undefined"&&(h=h.ownerDocument||h[0]&&h[0].ownerDocument||V);for(var k=[],m,o=0,q;(q=c[o])!=null;o++){typeof q=="number"&&(q+="");if(q){if(typeof q=="string")if(fc.test(q)){q=q.replace(Ub,"<$1></$2>");m=(Vb.exec(q)||["",""])[1].toLowerCase();var s=Ra[m]||Ra._default,u=s[0],y=h.createElement("div");for(y.innerHTML=s[1]+q+s[2];u--;)y=y.lastChild;if(!f.support.tbody){u=Wb.test(q);s=m==="table"&&!u?y.firstChild&&y.firstChild.childNodes:s[1]==="<table>"&&!u?y.childNodes:[];for(m=s.length-1;m>=0;--m)f.nodeName(s[m],
"tbody")&&!s[m].childNodes.length&&s[m].parentNode.removeChild(s[m])}!f.support.leadingWhitespace&&Jb.test(q)&&y.insertBefore(h.createTextNode(Jb.exec(q)[0]),y.firstChild);q=y.childNodes}else q=h.createTextNode(q);var D;if(!f.support.appendChecked)if(q[0]&&typeof(D=q.length)=="number")for(m=0;m<D;m++)ia(q[m]);else ia(q);q.nodeType?k.push(q):(k=f.merge(k,q))}}if(d){c=function(P){return!P.type||gc.test(P.type)};for(o=0;k[o];o++)if(j&&f.nodeName(k[o],"script")&&(!k[o].type||k[o].type.toLowerCase()===
"text/javascript"))j.push(k[o].parentNode?k[o].parentNode.removeChild(k[o]):k[o]);else{if(k[o].nodeType===1){h=f.grep(k[o].getElementsByTagName("script"),c);k.splice.apply(k,[o+1,0].concat(h))}d.appendChild(k[o])}}return k},cleanData:function(c){for(var h,d,j=f.cache,k=f.expando,m=f.event.special,o=f.support.deleteExpando,q=0,s;(s=c[q])!=null;q++)if(!(s.nodeName&&f.noData[s.nodeName.toLowerCase()]))if(d=s[f.expando]){if((h=j[d]&&j[d][k])&&h.events){for(var u in h.events)m[u]?f.event.remove(s,u):f.removeEvent(s,
u,h.handle);h.handle&&(h.handle.elem=null)}o?delete s[f.expando]:s.removeAttribute&&s.removeAttribute(f.expando);delete j[d]}}});var Xb=/alpha\([^)]*\)/i,hc=/opacity=([^)]*)/,mc=/([A-Z]|^ms)/g,Kb=/^-?\d+(?:px)?$/i,Yb=/^-?\d/,Zb=/^[+\-]=/,qa=/[^+\-\.\de]+/g,ic={position:"absolute",visibility:"hidden",display:"block"},fa=["Left","Right"],Qb=["Top","Bottom"],fb,mb,ma;f.fn.css=function(c,h){if(arguments.length===2&&h===a)return this;return f.access(this,c,h,true,function(d,j,k){return k!==a?f.style(d,
j,k):f.css(d,j)})};f.extend({cssHooks:{opacity:{get:function(c,h){if(h){var d=fb(c,"opacity","opacity");return d===""?"1":d}return c.style.opacity}}},cssNumber:{fillOpacity:true,fontWeight:true,lineHeight:true,opacity:true,orphans:true,widows:true,zIndex:true,zoom:true},cssProps:{"float":f.support.cssFloat?"cssFloat":"styleFloat"},style:function(c,h,d,j){if(c&&c.nodeType!==3&&c.nodeType!==8&&c.style){var k,m=f.camelCase(h),o=c.style,q=f.cssHooks[m];h=f.cssProps[m]||m;if(d===a){if(q&&"get"in q&&(k=
q.get(c,false,j))!==a)return k;return o[h]}j=typeof d;if(!(j==="number"&&isNaN(d)||d==null)){j==="string"&&Zb.test(d)&&(d=+d.replace(qa,"")+parseFloat(f.css(c,h)),j="number");j==="number"&&!f.cssNumber[m]&&(d+="px");if(!q||!("set"in q)||(d=q.set(c,d))!==a)try{o[h]=d}catch(s){}}}},css:function(c,h,d){var j,k;h=f.camelCase(h);k=f.cssHooks[h];h=f.cssProps[h]||h;h==="cssFloat"&&(h="float");if(k&&"get"in k&&(j=k.get(c,true,d))!==a)return j;if(fb)return fb(c,h)},swap:function(c,h,d){var j={};for(var k in h){j[k]=
...
(f.isFunction(h)?(d=h,h=a):typeof h=="object"&&(h=f.param(h,f.ajaxSettings.traditional),j="POST"));var m=this;f.ajax({url:c,type:j,dataType:"html",data:h,complete:function(o,q,s){s=o.responseText;o.isResolved()&&(o.done(function(u){s=u}),m.html(k?f("<div>").append(s.replace(Na,"")).find(k):s));d&&m.each(d,[s,q,o])}});return this},serialize:function(){return f.param(this.serializeArray())},serializeArray:function(){return this.map(function(){return this.elements?f.makeArray(this.elements):this}).filter(function(){return this.name&&
...
ha["* "+Bb];if(!Cb){Ob=a;for(Ib in ha){jc=Ib.split(" ");if(jc[0]===Nb||jc[0]==="*")if(Ob=ha[jc[1]+" "+Bb]){Ib=ha[Ib];Ib===true?(Cb=Ob):Ob===true&&(Cb=Ib);break}}}!Cb&&!Ob&&f.error("No conversion from "+nc.replace(" "," to "));Cb!==true&&(R=Cb?Cb(R):Ob(Ib(R)))}}Ga=R;I="success";ja=true}catch(pc){I="parsererror";Ea=pc}}else{Ea=I;if(!I||E){I="error";E<0&&(E=0)}}v.status=E;v.statusText=I;ja?o.resolveWith(k,[Ga,I,v]):o.rejectWith(k,[v,I,Ea]);v.statusCode(s);s=a;wa&&m.trigger("ajax"+(ja?"Success":"Error"),
[v,j,ja?Ga:Ea]);q.resolveWith(k,[v,I]);wa&&(m.trigger("ajaxComplete",[v,j]),--f.active||f.event.trigger("ajaxStop"))}}typeof c=="object"&&(h=c,c=a);h=h||{};var j=f.ajaxSetup({},h),k=j.context||j,m=k!==j&&(k.nodeType||k instanceof f)?f(k):f.event,o=f.Deferred(),q=f._Deferred(),s=j.statusCode||{},u,y={},D={},P,Y,ca,Da,ra,va=0,wa,r,v={readyState:0,setRequestHeader:function(E,I){if(!va){var R=E.toLowerCase();E=D[R]=D[R]||E;y[E]=I}return this},getAllResponseHeaders:function(){return va===2?P:null},getResponseHeader:function(E){var I;
if(va===2){if(!Y)for(Y={};I=Mb.exec(P);)Y[I[1].toLowerCase()]=I[2];I=Y[E.toLowerCase()]}return I===a?null:I},overrideMimeType:function(E){va||(j.mimeType=E);return this},abort:function(E){E=E||"abort";ca&&ca.abort(E);d(0,E);return this}};o.promise(v);v.success=v.done;v.error=v.fail;v.complete=q.done;v.statusCode=function(E){if(E){var I;if(va<2)for(I in E)s[I]=[s[I],E[I]];else{I=E[v.status];v.then(I,I)}}return this};j.url=((c||j.url)+"").replace(nb,"").replace(Xa,N[1]+"//");j.dataTypes=f.trim(j.dataType||
...
function(m,o){o=f.isFunction(o)?o():o;d[d.length]=encodeURIComponent(m)+"="+encodeURIComponent(o)};h===a&&(h=f.ajaxSettings.traditional);if(f.isArray(c)||c.jquery&&!f.isPlainObject(c))f.each(c,function(){j(this.name,this.value)});else for(var k in c)C(k,c[k],h,j);return d.join("&").replace(Ab,"+")}});f.extend({active:0,lastModified:{},etag:{}});var ub=f.now(),Db=/(\=)\?(&|$)|\?\?/i;f.ajaxSetup({jsonp:"callback",jsonpCallback:function(){return f.expando+"_"+ub++}});f.ajaxPrefilter("json jsonp",function(c,
h,d){h=c.contentType==="application/x-www-form-urlencoded"&&typeof c.data=="string";if(c.dataTypes[0]==="jsonp"||c.jsonp!==false&&(Db.test(c.url)||h&&Db.test(c.data))){var j,k=c.jsonpCallback=f.isFunction(c.jsonpCallback)?c.jsonpCallback():c.jsonpCallback,m=b[k],o=c.url,q=c.data,s="$1"+k+"$2";c.jsonp!==false&&(o=o.replace(Db,s),c.url===o&&(h&&(q=q.replace(Db,s)),c.data===q&&(o+=(/\?/.test(o)?"&":"?")+c.jsonp+"="+k)));c.url=o;c.data=q;b[k]=function(u){j=[u]};d.always(function(){b[k]=m;j&&f.isFunction(m)&&
...
function(c){if(c.crossDomain){var h,d=V.head||V.getElementsByTagName("head")[0]||V.documentElement;return{send:function(j,k){h=V.createElement("script");h.async="async";c.scriptCharset&&(h.charset=c.scriptCharset);h.src=c.url;h.onload=h.onreadystatechange=function(m,o){if(o||!h.readyState||/loaded|complete/.test(h.readyState)){h.onload=h.onreadystatechange=null;d&&h.parentNode&&d.removeChild(h);h=a;o||k(200,"success")}};d.insertBefore(h,d.firstChild)},abort:function(){h&&h.onload(0,1)}}}});var ab=
b.ActiveXObject?function(){for(var c in pb)pb[c](0,1)}:false,Qa=0,pb;f.ajaxSettings.xhr=b.ActiveXObject?function(){var c;if(!(c=!this.isLocal&&w()))a:{try{c=new b.ActiveXObject("Microsoft.XMLHTTP");break a}catch(h){}c=void 0}return c}:w;(function(c){f.extend(f.support,{ajax:!!c,cors:!!c&&"withCredentials"in c})})(f.ajaxSettings.xhr());f.support.ajax&&f.ajaxTransport(function(c){if(!c.crossDomain||f.support.cors){var h;return{send:function(d,j){var k=c.xhr(),m,o;c.username?k.open(c.type,c.url,c.async,
c.username,c.password):k.open(c.type,c.url,c.async);if(c.xhrFields)for(o in c.xhrFields)k[o]=c.xhrFields[o];c.mimeType&&k.overrideMimeType&&k.overrideMimeType(c.mimeType);!c.crossDomain&&!d["X-Requested-With"]&&(d["X-Requested-With"]="XMLHttpRequest");try{for(o in d)k.setRequestHeader(o,d[o])}catch(q){}k.send(c.hasContent&&c.data||null);h=function(s,u){var y,D,P,Y,ca;try{if(h&&(u||k.readyState===4)){h=a;m&&(k.onreadystatechange=f.noop,ab&&delete pb[m]);if(u)k.readyState!==4&&k.abort();else{y=k.status;
...
var m=f.extend({},k),o=this.nodeType===1,q=o&&f(this).is(":hidden"),s,u,y,D,P,Y,ca,Da;m.animatedProperties={};for(y in c){s=f.camelCase(y);y!==s&&(c[s]=c[y],delete c[y]);u=c[s];f.isArray(u)?(m.animatedProperties[s]=u[1],u=c[s]=u[0]):(m.animatedProperties[s]=m.specialEasing&&m.specialEasing[s]||m.easing||"swing");if(u==="hide"&&q||u==="show"&&!q)return m.complete.call(this);o&&(s==="height"||s==="width")&&(m.overflow=[this.style.overflow,this.style.overflowX,this.style.overflowY],f.css(this,"display")===
"inline"&&f.css(this,"float")==="none"&&(f.support.inlineBlockNeedsLayout?(D=g(this.nodeName),D==="inline"?(this.style.display="inline-block"):(this.style.display="inline",this.style.zoom=1)):(this.style.display="inline-block")))}m.overflow!=null&&(this.style.overflow="hidden");for(y in c){o=new f.fx(this,m,y);u=c[y];ib.test(u)?o[u==="toggle"?q?"show":"hide":u]():(P=kc.exec(u),Y=o.cur(),P?(ca=parseFloat(P[2]),Da=P[3]||(f.cssNumber[y]?"":"px"),Da!=="px"&&(f.style(this,y,(ca||1)+Da),Y=(ca||1)/o.cur()*
Y,f.style(this,y,Y+Da)),P[1]&&(ca=(P[1]==="-="?-1:1)*ca+Y),o.custom(Y,ca,Da)):o.custom(Y,u,""))}return true})},stop:function(c,h){c&&this.queue([]);this.each(function(){var d=f.timers,j=d.length;for(h||f._unmark(true,this);j--;)d[j].elem===this&&(h&&d[j](true),d.splice(j,1))});h||this.dequeue();return this}});f.each({slideDown:i("show",1),slideUp:i("hide",1),slideToggle:i("toggle",1),fadeIn:{opacity:"show"},fadeOut:{opacity:"hide"},fadeToggle:{opacity:"toggle"}},function(c,h){f.fn[c]=function(d,j,
...
parseFloat(h))?!h||h==="auto"?0:h:c},custom:function(c,h,d){function j(q){return k.step(q)}var k=this,m=f.fx,o;this.startTime=qb||n();this.start=c;this.end=h;this.unit=d||this.unit||(f.cssNumber[this.prop]?"":"px");this.now=this.start;this.pos=this.state=0;j.elem=this.elem;j()&&f.timers.push(j)&&!jb&&(Eb?(jb=true,o=function(){jb&&(Eb(o),m.tick())},Eb(o)):(jb=setInterval(m.tick,m.interval)))},show:function(){this.options.orig[this.prop]=f.style(this.elem,this.prop);this.options.show=true;this.custom(this.prop===
"width"||this.prop==="height"?1:0,this.cur());f(this.elem).show()},hide:function(){this.options.orig[this.prop]=f.style(this.elem,this.prop);this.options.hide=true;this.custom(this.cur(),0)},step:function(c){var h=qb||n(),d=true,j=this.elem,k=this.options,m,o;if(c||h>=k.duration+this.startTime){this.now=this.end;this.pos=this.state=1;this.update();k.animatedProperties[this.prop]=true;for(m in k.animatedProperties)k.animatedProperties[m]!==true&&(d=false);if(d){k.overflow!=null&&!f.support.shrinkWrapBlocks&&
f.each(["","X","Y"],function(s,u){j.style["overflow"+u]=k.overflow[s]});k.hide&&f(j).hide();if(k.hide||k.show)for(var q in k.animatedProperties)f.style(j,q,k.orig[q]);k.complete.call(j)}return false}k.duration==Infinity?(this.now=h):(o=h-this.startTime,this.state=o/k.duration,this.pos=f.easing[k.animatedProperties[this.prop]](this.state,o,0,1,k.duration),this.now=this.start+(this.end-this.start)*this.pos);this.update();return true}};f.extend(f.fx,{tick:function(){for(var c=f.timers,h=0;h<c.length;++h)c[h]()||
...
(function(b,a){function e(i,l){var n=i.nodeName.toLowerCase();if("area"===n){n=i.parentNode;var w=n.name;if(!i.href||!w||n.nodeName.toLowerCase()!=="map")return false;n=b("img[usemap=#"+w+"]")[0];return!!n&&g(n)}return(/input|select|textarea|button|object/.test(n)?!i.disabled:"a"==n?i.href||l:l)&&g(i)}function g(i){return!b(i).parents().andSelf().filter(function(){return b.curCSS(this,"visibility")==="hidden"||b.expr.filters.hidden(this)}).length}b.ui=b.ui||{};if(!b.ui.version){b.extend(b.ui,{version:"1.8.16",
...
this;setTimeout(function(){b(n).focus();l&&l.call(n)},i)}):this._focus.apply(this,arguments)},scrollParent:function(){var i;i=b.browser.msie&&/(static|relative)/.test(this.css("position"))||/absolute/.test(this.css("position"))?this.parents().filter(function(){return/(relative|absolute|fixed)/.test(b.curCSS(this,"position",1))&&/(auto|scroll)/.test(b.curCSS(this,"overflow",1)+b.curCSS(this,"overflow-y",1)+b.curCSS(this,"overflow-x",1))}).eq(0):this.parents().filter(function(){return/(auto|scroll)/.test(b.curCSS(this,
...
"mousedown")+".ui-disableSelection",function(i){i.preventDefault()})},enableSelection:function(){return this.unbind(".ui-disableSelection")}});b.each(["Width","Height"],function(i,l){function n(K,O,aa,ia){b.each(w,function(){O-=parseFloat(b.curCSS(K,"padding"+this,true))||0;if(aa)O-=parseFloat(b.curCSS(K,"border"+this+"Width",true))||0;if(ia)O-=parseFloat(b.curCSS(K,"margin"+this,true))||0});return O}var w=l==="Width"?["Left","Right"]:["Top","Bottom"],C=l.toLowerCase(),B={innerWidth:b.fn.innerWidth,
innerHeight:b.fn.innerHeight,outerWidth:b.fn.outerWidth,outerHeight:b.fn.outerHeight};b.fn["inner"+l]=function(K){if(K===a)return B["inner"+l].call(this);return this.each(function(){b(this).css(C,n(this,K)+"px")})};b.fn["outer"+l]=function(K,O){if(typeof K!=="number")return B["outer"+l].call(this,K);return this.each(function(){b(this).css(C,n(this,K,true,O)+"px")})}});b.extend(b.expr[":"],{data:function(i,l,n){return!!b.data(i,n[3])},focusable:function(i){return e(i,!isNaN(b.attr(i,"tabindex")))},
tabbable:function(i){var l=b.attr(i,"tabindex"),n=isNaN(l);return(n||l>=0)&&e(i,!n)}});b(function(){var i=document.body,l=i.appendChild(l=document.createElement("div"));b.extend(l.style,{minHeight:"100px",height:"auto",padding:0,borderWidth:0});b.support.minHeight=l.offsetHeight===100;b.support.selectstart="onselectstart"in l;i.removeChild(l).style.display="none"});b.extend(b.ui,{plugin:{add:function(i,l,n){i=b.ui[i].prototype;for(var w in n){i.plugins[w]=i.plugins[w]||[];i.plugins[w].push([l,n[w]])}},
call:function(i,l,n){if((l=i.plugins[l])&&i.element[0].parentNode)for(var w=0;w<l.length;w++)i.options[l[w][0]]&&l[w][1].apply(i.element,n)}},contains:function(i,l){return document.compareDocumentPosition?i.compareDocumentPosition(l)&16:i!==l&&i.contains(l)},hasScroll:function(i,l){if(b(i).css("overflow")==="hidden")return false;var n=l&&l==="left"?"scrollLeft":"scrollTop",w=false;if(i[n]>0)return true;i[n]=1;w=i[n]>0;i[n]=0;return w},isOverAxis:function(i,l,n){return i>l&&i<l+n},isOver:function(i,
l,n,w,C,B){return b.ui.isOverAxis(i,n,C)&&b.ui.isOverAxis(l,w,B)}})}})(jQuery);
(function(b,a){if(b.cleanData){var e=b.cleanData;b.cleanData=function(i){for(var l=0,n;(n=i[l])!=null;l++)try{b(n).triggerHandler("remove")}catch(w){}e(i)}}else{var g=b.fn.remove;b.fn.remove=function(i,l){return this.each(function(){if(!l)if(!i||b.filter(i,[this]).length)b("*",this).add([this]).each(function(){try{b(this).triggerHandler("remove")}catch(n){}});return g.call(b(this),i,l)})}}b.widget=function(i,l,n){var w=i.split(".")[0],C;i=i.split(".")[1];C=w+"-"+i;if(!n){n=l;l=b.Widget}b.expr[":"][C]=
function(B){return!!b.data(B,i)};b[w]=b[w]||{};b[w][i]=function(B,K){arguments.length&&this._createWidget(B,K)};l=new l;l.options=b.extend(true,{},l.options);b[w][i].prototype=b.extend(true,l,{namespace:w,widgetName:i,widgetEventPrefix:b[w][i].prototype.widgetEventPrefix||i,widgetBaseClass:C},n);b.widget.bridge(i,b[w][i])};b.widget.bridge=function(i,l){b.fn[i]=function(n){var w=typeof n==="string",C=Array.prototype.slice.call(arguments,1),B=this;n=!w&&C.length?b.extend.apply(null,[true,n].concat(C)):
n;if(w&&n.charAt(0)==="_")return B;w?this.each(function(){var K=b.data(this,i),O=K&&b.isFunction(K[n])?K[n].apply(K,C):K;if(O!==K&&O!==a){B=O;return false}}):this.each(function(){var K=b.data(this,i);K?K.option(n||{})._init():b.data(this,i,new l(n,this))});return B}};b.Widget=function(i,l){arguments.length&&this._createWidget(i,l)};b.Widget.prototype={widgetName:"widget",widgetEventPrefix:"",options:{disabled:false},_createWidget:function(i,l){b.data(l,this.widgetName,this);this.element=b(l);this.options=
b.extend(true,{},this.options,this._getCreateOptions(),i);var n=this;this.element.bind("remove."+this.widgetName,function(){n.destroy()});this._create();this._trigger("create");this._init()},_getCreateOptions:function(){return b.metadata&&b.metadata.get(this.element[0])[this.widgetName]},_create:function(){},_init:function(){},destroy:function(){this.element.unbind("."+this.widgetName).removeData(this.widgetName);this.widget().unbind("."+this.widgetName).removeAttr("aria-disabled").removeClass(this.widgetBaseClass+
"-disabled ui-state-disabled")},widget:function(){return this.element},option:function(i,l){var n=i;if(arguments.length===0)return b.extend({},this.options);if(typeof i==="string"){if(l===a)return this.options[i];n={};n[i]=l}this._setOptions(n);return this},_setOptions:function(i){var l=this;b.each(i,function(n,w){l._setOption(n,w)});return this},_setOption:function(i,l){this.options[i]=l;if(i==="disabled")this.widget()[l?"addClass":"removeClass"](this.widgetBaseClass+"-disabled ui-state-disabled").attr("aria-disabled",
l);return this},enable:function(){return this._setOption("disabled",false)},disable:function(){return this._setOption("disabled",true)},_trigger:function(i,l,n){var w=this.options[i];l=b.Event(l);l.type=(i===this.widgetEventPrefix?i:this.widgetEventPrefix+i).toLowerCase();n=n||{};if(l.originalEvent){i=b.event.props.length;for(var C;i;){C=b.event.props[--i];l[C]=l.originalEvent[C]}}this.element.trigger(l,n);return!(b.isFunction(w)&&w.call(this.element[0],l,n)===false||l.isDefaultPrevented())}}})(jQuery);
...
this._mouseStart(e)!==false;if(!this._mouseStarted){e.preventDefault();return true}}true===b.data(e.target,this.widgetName+".preventClickEvent")&&b.removeData(e.target,this.widgetName+".preventClickEvent");this._mouseMoveDelegate=function(n){return g._mouseMove(n)};this._mouseUpDelegate=function(n){return g._mouseUp(n)};b(document).bind("mousemove."+this.widgetName,this._mouseMoveDelegate).bind("mouseup."+this.widgetName,this._mouseUpDelegate);e.preventDefault();return a=true}},_mouseMove:function(e){if(b.browser.msie&&
...
!(this.scrollParent[0]!=document&&b.ui.contains(this.scrollParent[0],this.offsetParent[0]))?this.offsetParent:this.scrollParent,i=/(html|body)/i.test(g[0].tagName),l=a.pageX,n=a.pageY;if(this.originalPosition){var w;if(this.containment){if(this.relative_container){w=this.relative_container.offset();w=[this.containment[0]+w.left,this.containment[1]+w.top,this.containment[2]+w.left,this.containment[3]+w.top]}else w=this.containment;if(a.pageX-this.offset.click.left<w[0])l=w[0]+this.offset.click.left;
if(a.pageY-this.offset.click.top<w[1])n=w[1]+this.offset.click.top;if(a.pageX-this.offset.click.left>w[2])l=w[2]+this.offset.click.left;if(a.pageY-this.offset.click.top>w[3])n=w[3]+this.offset.click.top}if(e.grid){n=e.grid[1]?this.originalPageY+Math.round((n-this.originalPageY)/e.grid[1])*e.grid[1]:this.originalPageY;n=w?!(n-this.offset.click.top<w[1]||n-this.offset.click.top>w[3])?n:!(n-this.offset.click.top<w[1])?n-e.grid[1]:n+e.grid[1]:n;l=e.grid[0]?this.originalPageX+Math.round((l-this.originalPageX)/
e.grid[0])*e.grid[0]:this.originalPageX;l=w?!(l-this.offset.click.left<w[0]||l-this.offset.click.left>w[2])?l:!(l-this.offset.click.left<w[0])?l-e.grid[0]:l+e.grid[0]:l}}return{top:n-this.offset.click.top-this.offset.relative.top-this.offset.parent.top+(b.browser.safari&&b.browser.version<526&&this.cssPosition=="fixed"?0:this.cssPosition=="fixed"?-this.scrollParent.scrollTop():i?0:g.scrollTop()),left:l-this.offset.click.left-this.offset.relative.left-this.offset.parent.left+(b.browser.safari&&b.browser.version<
...
width:g.outerWidth(),height:g.outerHeight(),top:i.top,left:i.left})})},drag:function(a,e){for(var g=b(this).data("draggable"),i=g.options,l=i.snapTolerance,n=e.offset.left,w=n+g.helperProportions.width,C=e.offset.top,B=C+g.helperProportions.height,K=g.snapElements.length-1;K>=0;K--){var O=g.snapElements[K].left,aa=O+g.snapElements[K].width,ia=g.snapElements[K].top,ka=ia+g.snapElements[K].height;if(O-l<n&&n<aa+l&&ia-l<C&&C<ka+l||O-l<n&&n<aa+l&&ia-l<B&&B<ka+l||O-l<w&&w<aa+l&&ia-l<C&&C<ka+l||O-l<w&&
w<aa+l&&ia-l<B&&B<ka+l){if(i.snapMode!="inner"){var p=Math.abs(ia-B)<=l,t=Math.abs(ka-C)<=l,z=Math.abs(O-w)<=l,F=Math.abs(aa-n)<=l;if(p)e.position.top=g._convertPositionTo("relative",{top:ia-g.helperProportions.height,left:0}).top-g.margins.top;if(t)e.position.top=g._convertPositionTo("relative",{top:ka,left:0}).top-g.margins.top;if(z)e.position.left=g._convertPositionTo("relative",{top:0,left:O-g.helperProportions.width}).left-g.margins.left;if(F)e.position.left=g._convertPositionTo("relative",{top:0,
left:aa}).left-g.margins.left}var ba=p||t||z||F;if(i.snapMode!="outer"){p=Math.abs(ia-C)<=l;t=Math.abs(ka-B)<=l;z=Math.abs(O-n)<=l;F=Math.abs(aa-w)<=l;if(p)e.position.top=g._convertPositionTo("relative",{top:ia,left:0}).top-g.margins.top;if(t)e.position.top=g._convertPositionTo("relative",{top:ka-g.helperProportions.height,left:0}).top-g.margins.top;if(z)e.position.left=g._convertPositionTo("relative",{top:0,left:O}).left-g.margins.left;if(F)e.position.left=g._convertPositionTo("relative",{top:0,
...
a,this.ui(g));return this.element}return false},ui:function(a){return{draggable:a.currentItem||a.element,helper:a.helper,position:a.position,offset:a.positionAbs}}});b.extend(b.ui.droppable,{version:"1.8.16"});b.ui.intersect=function(a,e,g){if(!e.offset)return false;var i=(a.positionAbs||a.position.absolute).left,l=i+a.helperProportions.width,n=(a.positionAbs||a.position.absolute).top,w=n+a.helperProportions.height,C=e.offset.left,B=C+e.proportions.width,K=e.offset.top,O=K+e.proportions.height;switch(g){case "fit":return C<=
i&&l<=B&&K<=n&&w<=O;case "intersect":return C<i+a.helperProportions.width/2&&l-a.helperProportions.width/2<B&&K<n+a.helperProportions.height/2&&w-a.helperProportions.height/2<O;case "pointer":return b.ui.isOver((a.positionAbs||a.position.absolute).top+(a.clickOffset||a.offset.click).top,(a.positionAbs||a.position.absolute).left+(a.clickOffset||a.offset.click).left,K,C,e.proportions.height,e.proportions.width);case "touch":return(n>=K&&n<=O||w>=K&&w<=O||n<K&&w>O)&&(i>=C&&i<=B||l>=C&&l<=B||i<C&&l>B);
default:return false}};b.ui.ddmanager={current:null,droppables:{"default":[]},prepareOffsets:function(a,e){var g=b.ui.ddmanager.droppables[a.options.scope]||[],i=e?e.type:null,l=(a.currentItem||a.element).find(":data(droppable)").andSelf(),n=0;a:for(;n<g.length;n++)if(!(g[n].options.disabled||a&&!g[n].accept.call(g[n].element[0],a.currentItem||a.element))){for(var w=0;w<l.length;w++)if(l[w]==g[n].element[0]){g[n].proportions.height=0;continue a}g[n].visible=g[n].element.css("display")!="none";if(g[n].visible){i==
...
nw:".ui-resizable-nw"});if(this.handles.constructor==String){if(this.handles=="all")this.handles="n,e,s,w,se,sw,ne,nw";var l=this.handles.split(",");this.handles={};for(var n=0;n<l.length;n++){var w=b.trim(l[n]),C=b('<div class="ui-resizable-handle '+("ui-resizable-"+w)+'"></div>');/sw|se|ne|nw/.test(w)&&C.css({zIndex:++i.zIndex});"se"==w&&C.addClass("ui-icon ui-icon-gripsmall-diagonal-se");this.handles[w]=".ui-resizable-"+w;this.element.append(C)}}this._renderAxis=function(B){B=B||this.element;for(var K in this.handles){if(this.handles[K].constructor==
...
false;for(var l in this.handles)if(b(this.handles[l])[0]==g.target)i=true;return!this.options.disabled&&i},_mouseStart:function(g){var i=this.options,l=this.element.position(),n=this.element;this.resizing=true;this.documentScroll={top:b(document).scrollTop(),left:b(document).scrollLeft()};if(n.is(".ui-draggable")||/absolute/.test(n.css("position")))n.css({position:"absolute",top:l.top,left:l.left});b.browser.opera&&/relative/.test(n.css("position"))&&n.css({position:"relative",top:"auto",left:"auto"});
this._renderProxy();l=a(this.helper.css("left"));var w=a(this.helper.css("top"));if(i.containment){l+=b(i.containment).scrollLeft()||0;w+=b(i.containment).scrollTop()||0}this.offset=this.helper.offset();this.position={left:l,top:w};this.size=this._helper?{width:n.outerWidth(),height:n.outerHeight()}:{width:n.width(),height:n.height()};this.originalSize=this._helper?{width:n.outerWidth(),height:n.outerHeight()}:{width:n.width(),height:n.height()};this.originalPosition={left:l,top:w};this.sizeDiff=
{width:n.outerWidth()-n.width(),height:n.outerHeight()-n.height()};this.originalMousePosition={left:g.pageX,top:g.pageY};this.aspectRatio=typeof i.aspectRatio=="number"?i.aspectRatio:this.originalSize.width/this.originalSize.height||1;i=b(".ui-resizable-"+this.axis).css("cursor");b("body").css("cursor",i=="auto"?this.axis+"-resize":i);n.addClass("ui-resizable-resizing");this._propagate("start",g);return true},_mouseDrag:function(g){var i=this.helper,l=this.originalMousePosition,n=this._change[this.axis];
...
_mouseStop:function(g){this.resizing=false;var i=this.options;if(this._helper){var l=this._proportionallyResizeElements,n=l.length&&/textarea/i.test(l[0].nodeName);l=n&&b.ui.hasScroll(l[0],"left")?0:this.sizeDiff.height;n=n?0:this.sizeDiff.width;n={width:this.helper.width()-n,height:this.helper.height()-l};l=parseInt(this.element.css("left"),10)+(this.position.left-this.originalPosition.left)||null;var w=parseInt(this.element.css("top"),10)+(this.position.top-this.originalPosition.top)||null;i.animate||
this.element.css(b.extend(n,{top:w,left:l}));this.helper.height(this.size.height);this.helper.width(this.size.width);this._helper&&!i.animate&&this._proportionallyResize()}b("body").css("cursor","auto");this.element.removeClass("ui-resizable-resizing");this._propagate("stop",g);this._helper&&this.helper.remove();return false},_updateVirtualBoundaries:function(g){var i=this.options,l,n,w;i={minWidth:e(i.minWidth)?i.minWidth:0,maxWidth:e(i.maxWidth)?i.maxWidth:Infinity,minHeight:e(i.minHeight)?i.minHeight:
0,maxHeight:e(i.maxHeight)?i.maxHeight:Infinity};if(this._aspectRatio||g){g=i.minHeight*this.aspectRatio;n=i.minWidth/this.aspectRatio;l=i.maxHeight*this.aspectRatio;w=i.maxWidth/this.aspectRatio;if(g>i.minWidth)i.minWidth=g;if(n>i.minHeight)i.minHeight=n;if(l<i.maxWidth)i.maxWidth=l;if(w<i.maxHeight)i.maxHeight=w}this._vBoundaries=i},_updateCache:function(g){this.offset=this.helper.offset();if(e(g.left))this.position.left=g.left;if(e(g.top))this.position.top=g.top;if(e(g.height))this.size.height=
g.height;if(e(g.width))this.size.width=g.width},_updateRatio:function(g){var i=this.position,l=this.size,n=this.axis;if(e(g.height))g.width=g.height*this.aspectRatio;else if(e(g.width))g.height=g.width/this.aspectRatio;if(n=="sw"){g.left=i.left+(l.width-g.width);g.top=null}if(n=="nw"){g.top=i.top+(l.height-g.height);g.left=i.left+(l.width-g.width)}return g},_respectSize:function(g){var i=this._vBoundaries,l=this.axis,n=e(g.width)&&i.maxWidth&&i.maxWidth<g.width,w=e(g.height)&&i.maxHeight&&i.maxHeight<
g.height,C=e(g.width)&&i.minWidth&&i.minWidth>g.width,B=e(g.height)&&i.minHeight&&i.minHeight>g.height;if(C)g.width=i.minWidth;if(B)g.height=i.minHeight;if(n)g.width=i.maxWidth;if(w)g.height=i.maxHeight;var K=this.originalPosition.left+this.originalSize.width,O=this.position.top+this.size.height,aa=/sw|nw|w/.test(l);l=/nw|ne|n/.test(l);if(C&&aa)g.left=K-i.minWidth;if(n&&aa)g.left=K-i.maxWidth;if(B&&l)g.top=O-i.minHeight;if(w&&l)g.top=O-i.maxHeight;if((i=!g.width&&!g.height)&&!g.left&&g.top)g.top=
null;else if(i&&!g.top&&g.left)g.left=null;return g},_proportionallyResize:function(){if(this._proportionallyResizeElements.length)for(var g=this.helper||this.element,i=0;i<this._proportionallyResizeElements.length;i++){var l=this._proportionallyResizeElements[i];if(!this.borderDif){var n=[l.css("borderTopWidth"),l.css("borderRightWidth"),l.css("borderBottomWidth"),l.css("borderLeftWidth")],w=[l.css("paddingTop"),l.css("paddingRight"),l.css("paddingBottom"),l.css("paddingLeft")];this.borderDif=b.map(n,
...
{resize:function(){var g=b(this).data("resizable"),i=g.options,l=g.size,n=g.originalSize,w=g.originalPosition,C=g.axis;i.grid=typeof i.grid=="number"?[i.grid,i.grid]:i.grid;var B=Math.round((l.width-n.width)/(i.grid[0]||1))*(i.grid[0]||1);i=Math.round((l.height-n.height)/(i.grid[1]||1))*(i.grid[1]||1);if(/^(se|s|e)$/.test(C)){g.size.width=n.width+B;g.size.height=n.height+i}else if(/^(ne)$/.test(C)){g.size.width=n.width+B;g.size.height=n.height+i;g.position.top=w.top-i}else{if(/^(sw)$/.test(C)){g.size.width=
n.width+B;g.size.height=n.height+i}else{g.size.width=n.width+B;g.size.height=n.height+i;g.position.top=w.top-i}g.position.left=w.left-B}}});var a=function(g){return parseInt(g,10)||0},e=function(g){return!isNaN(parseInt(g,10))}})(jQuery);
...
this.options,i=this.opos[0],l=this.opos[1],n=a.pageX,w=a.pageY;if(i>n){var C=n;n=i;i=C}if(l>w){C=w;w=l;l=C}this.helper.css({left:i,top:l,width:n-i,height:w-l});this.selectees.each(function(){var B=b.data(this,"selectable-item");if(!(!B||B.element==e.element[0])){var K=false;if(g.tolerance=="touch")K=!(B.left>n||B.right<i||B.top>w||B.bottom<l);else if(g.tolerance=="fit")K=B.left>i&&B.right<n&&B.top>l&&B.bottom<w;if(K){if(B.selected){B.$element.removeClass("ui-selected");B.selected=false}if(B.unselecting){B.$element.removeClass("ui-unselecting");
...
this]],i=this._connectWith();if(i)for(var l=i.length-1;l>=0;l--)for(var n=b(i[l]),w=n.length-1;w>=0;w--){var C=b.data(n[w],"sortable");if(C&&C!=this&&!C.options.disabled){g.push([b.isFunction(C.options.items)?C.options.items.call(C.element[0],a,{item:this.currentItem}):b(C.options.items,C.element),C]);this.containers.push(C)}}for(l=g.length-1;l>=0;l--){a=g[l][1];i=g[l][0];w=0;for(n=i.length;w<n;w++){C=b(i[w]);C.data("sortable-item",a);e.push({item:C,instance:a,width:0,height:0,left:0,top:0})}}},refreshPositions:function(a){if(this.offsetParent&&
...
(function(b,a){var e,g=/:([\w\d]+)/g,i=/\?([^#]*)$/,l=function(p){return Array.prototype.slice.call(p)},n=function(p){return Object.prototype.toString.call(p)==="[object Function]"},w=function(p){return Object.prototype.toString.call(p)==="[object Array]"},C=function(p){return decodeURIComponent((p||"").replace(/\+/g," "))},B=encodeURIComponent,K=function(p){return String(p).replace(/&(?!\w+;)/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")},O=function(p){return function(t,
z){return this.route.apply(this,[p,t,z])}},aa={},ia=!!(a.history&&history.pushState),ka=[];e=function(){var p=l(arguments),t,z;e.apps=e.apps||{};if(p.length===0||p[0]&&n(p[0]))return e.apply(e,["body"].concat(p));else if(typeof(z=p.shift())=="string"){t=e.apps[z]||new e.Application;t.element_selector=z;p.length>0&&b.each(p,function(F,ba){t.use(ba)});t.element_selector!=z&&delete e.apps[z];return e.apps[t.element_selector]=t}};e.VERSION="0.7.0";e.addLogger=function(p){ka.push(p)};e.log=function(){var p=
l(arguments);p.unshift("["+Date()+"]");b.each(ka,function(t,z){z.apply(e,p)})};if(typeof a.console!="undefined")n(a.console.log.apply)?e.addLogger(function(){a.console.log.apply(a.console,arguments)}):e.addLogger(function(){a.console.log(arguments)});else typeof console!="undefined"&&e.addLogger(function(){console.log.apply(console,arguments)});b.extend(e,{makeArray:l,isFunction:n,isArray:w});e.Object=function(p){return b.extend(this,p||{})};b.extend(e.Object.prototype,{escapeHTML:K,h:K,toHash:function(){var p=
{};b.each(this,function(t,z){n(z)||(p[t]=z)});return p},toHTML:function(){var p="";b.each(this,function(t,z){n(z)||(p+="<strong>"+t+"</strong> "+z+"<br />")});return p},keys:function(p){var t=[];for(var z in this)if(!n(this[z])||!p)t.push(z);return t},has:function(p){return this[p]&&b.trim(this[p].toString())!==""},join:function(){var p=l(arguments),t=p.shift();return p.join(t)},log:function(){e.log.apply(e,arguments)},toString:function(p){var t=[];b.each(this,function(z,F){if(!n(F)||p)t.push('"'+
...
this;return this.then(function(){var ba,ga,la;if(n(t)){z=t;t={}}else t=b.extend({},t);z&&this.then(z);if(typeof p==="string"){ba=(la=p.match(/\.json$/)||t.json)&&t.cache===true||t.cache!==false;F.next_engine=F.event_context.engineFor(p);delete t.cache;delete t.json;if(t.engine){F.next_engine=t.engine;delete t.engine}if(ba&&(ga=this.event_context.app.templateCache(p)))return ga;this.wait();b.ajax(b.extend({url:p,data:{},dataType:la?"json":null,type:"get",success:function(na){ba&&F.event_context.app.templateCache(p,
na);F.next(na)}},t));return false}else{if(p.nodeType)return p.innerHTML;if(p.selector){F.next_engine=p.attr("data-engine");return t.clone===false?p.remove()[0].innerHTML.toString():p[0].innerHTML.toString()}}})},loadPartials:function(p){if(p){this.partials=this.partials||{};for(name in p)this.load(p[name]).then(function(t){this.partials[name]=t})}return this},render:function(p,t,z,F){return n(p)&&!t?this.then(p):this.loadPartials(F).load(p).interpolate(t,p).then(z)},partial:function(p,t){return this.render(p,
t).swap()},send:function(){var p=this,t=l(arguments),z=t.shift();if(w(t[0]))t=t[0];return this.then(function(){t.push(function(F){p.next(F)});p.wait();z.apply(z,t);return false})},collect:function(p,t,z){var F=this,ba=function(){if(n(p)){t=p;p=this.content}var ga=[],la=false;b.each(p,function(na,oa){var Ca=t.apply(F,[na,oa]);if(Ca.jquery&&Ca.length==1){Ca=Ca[0];la=true}ga.push(Ca);return Ca});return la?ga:ga.join("")};return z?ba():this.then(ba)},renderEach:function(p,t,z,F){if(w(t)){F=z;z=t;t=null}return this.load(p).then(function(ba){var ga=
...
(function(b){function a(g){g=unescape(g);return g==="true"?true:g==="false"?false:g}function e(g,i,l){var n;if(i.match(/^[^\[]+$/))g[i]=a(l);else if(n=i.match(/^([^\[]+)\[\](.*)$/)){i=n[1];n=n[2];if(g[i]&&!b.isArray(g[i]))throw"400 Bad Request";if(n){n=n.match(/^\[([^\]]+)\](.*)$/);if(!n)throw"400 Bad Request";if(g[i])g[i][g[i].length-1][n[1]]?g[i].push(e({},n[1]+n[2],l)):b.extend(true,g[i][g[i].length-1],e({},n[1]+n[2],l));else g[i]=[e({},n[1]+n[2],l)]}else if(g[i])g[i].push(a(l));else g[i]=[a(l)]}else if(n=
i.match(/^([^\[]+)\[([^\[]+)\](.*)$/)){i=n[1];n=n[2]+n[3];if(g[i]&&b.isArray(g[i]))throw"400 Bad Request";if(g[i])b.extend(true,g[i],e(g[i],n,l));else g[i]=e({},n,l)}return g}Sammy=Sammy||{};Sammy.NestedParams=function(g){g._parseParamPair=e}})(jQuery);
...
String(z):"null";case "boolean":case "null":return String(z);case "object":if(!z)return"null";l+=n;t=[];if(Object.prototype.toString.apply(z)==="[object Array]"){ka=z.length;for(O=0;O<ka;O+=1)t[O]=e(O,z)||"null";ia=t.length===0?"[]":l?"[\n"+l+t.join(",\n"+l)+"\n"+p+"]":"["+t.join(",")+"]";l=p;return ia}if(C&&typeof C==="object"){ka=C.length;for(O=0;O<ka;O+=1){aa=C[O];if(typeof aa==="string")if(ia=e(aa,z))t.push(a(aa)+(l?": ":":")+ia)}}else for(aa in z)if(Object.hasOwnProperty.call(z,aa))if(ia=e(aa,
...
i=/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,l,n,w={"\u0008":"\\b","\t":"\\t","\n":"\\n","\u000c":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},C;if(typeof JSON.stringify!=="function")JSON.stringify=function(B,K,O){var aa;n=l="";if(typeof O==="number")for(aa=0;aa<O;aa+=1)n+=" ";else if(typeof O==="string")n=O;if((C=K)&&typeof K!=="function"&&(typeof K!=="object"||typeof K.length!=="number"))throw new Error("JSON.stringify");
...
true)},render_section:function(a,e,g){if(!this.includes("#",a)&&!this.includes("^",a))return a;var i=this;return a.replace(new RegExp(this.otag+"(\\^|\\#)\\s*(.+)\\s*"+this.ctag+"\n*([\\s\\S]+?)"+this.otag+"\\/\\s*\\2\\s*"+this.ctag+"\\s*","mg"),function(l,n,w,C){l=i.find(w,e);if(n=="^")return!l||i.is_array(l)&&l.length===0?i.render(C,e,g,true):"";else if(n=="#")return i.is_array(l)?i.map(l,function(B){return i.render(C,i.create_context(B),g,true)}).join(""):i.is_object(l)?i.render(C,i.create_context(l),
g,true):typeof l==="function"?l.call(e,C,function(B){return i.render(B,e,g,true)}):l?i.render(C,e,g,true):""})},render_tags:function(a,e,g,i){var l=this,n=function(){return new RegExp(l.otag+"(=|!|>|\\{|%)?([^\\/#\\^]+?)\\1?"+l.ctag+"+","g")},w=n(),C=function(K,O,aa){switch(O){case "!":return"";case "=":l.set_delimiters(aa);w=n();return"";case ">":return l.render_partial(aa,e,g);case "{":return l.find(aa,e);default:return l.escape(l.find(aa,e))}};a=a.split("\n");for(var B=0;B<a.length;B++){a[B]=a[B].replace(w,
...
l){localStorage.removeItem(i.uid);var n=i.uid,w=e();n=jQuery.inArray(n,w);if(n>-1){w.splice(n,1);localStorage.setItem(a,JSON.stringify(w))}l(true)},read:function(i){if(!i)return false;for(var l=b.map(function(){return this.uid}),n=e(),w=[],C,B,K=0,O=n.length;K<O;K++){B=n[K];if(jQuery.inArray(B,l)==-1){C=JSON.parse(localStorage[B]);C=new b(C);C.uid=B;w.push(C)}}i(w)},update:function(i,l){g(i);l(true)}}};Model.Log=function(){window.console&&window.console.log.apply(window.console,arguments)};
...
Model.REST=function(b,a,e){var g=/:([\w\d]+)/g,i=function(){for(var l=[],n;(n=g.exec(a))!==null;)l.push(n[1]);return l}();return jQuery.extend({path:function(l){var n=a;jQuery.each(i,function(w,C){n=n.replace(":"+C,l.attributes[C])});return n},create:function(l,n){return this.xhr("POST",this.create_path(l),l,n)},create_path:function(l){return this.path(l)},destroy:function(l,n){return this.xhr("DELETE",this.destroy_path(l),l,n)},destroy_path:function(l){return this.update_path(l)},params:function(l){var n;
if(l){var w=l.asJSON();delete w[l.constructor.unique_key];n={};n[l.constructor._name.toLowerCase()]=w}else n=null;if(jQuery.ajaxSettings.data)n=jQuery.extend({},jQuery.ajaxSettings.data,n);return n},read:function(l){return this.xhr("GET",this.read_path(),null,function(n,w,C){C=jQuery.makeArray(C);n=[];w=0;for(var B=C.length;w<B;w++)n.push(new b(C[w]));l(n)})},read_path:function(){return a},update:function(l,n){return this.xhr("PUT",this.update_path(l),l,n)},update_path:function(l){return[this.path(l),
l.id()].join("/")},xhr:function(l,n,w,C){var B=this,K=l=="GET"?undefined:this.params(w);return jQuery.ajax({type:l,url:n+".json",dataType:"json",data:K,dataFilter:function(O){return/\S/.test(O)?O:undefined},complete:function(O,aa){B.xhrComplete(O,aa,w,C)}})},xhrComplete:function(l,n,w,C){var B=Model.REST["handle"+l.status];B&&B.call(this,l,n,w);n=n==="success";B=Model.REST.parseResponseData(l);n&&w&&B&&w.attr(B);C&&C.call(w,n,l,B)}},e)};Model.RestPersistence=Model.REST;
...
Model.SinatraREST=function(b,a,e){var g=/:([\w\d]+)/g,i=function(){for(var l=[],n;(n=g.exec(a))!==null;)l.push(n[1]);return l}();return jQuery.extend({path:function(l){var n=a;jQuery.each(i,function(w,C){n=n.replace(":"+C,l.attributes[C])});return n},create:function(l,n){return this.xhr("POST",this.create_path(l),l,n)},create_path:function(l){return this.path(l)},destroy:function(l,n){return this.xhr("DELETE",this.destroy_path(l),l,n)},destroy_path:function(l){return this.update_path(l)},params:function(l){var n;
if(l){var w=l.asJSON();delete w[l.constructor.unique_key];n={};n[l.constructor._name.toLowerCase()]=w}else n=null;if(jQuery.ajaxSettings.data)n=jQuery.extend({},jQuery.ajaxSettings.data,n);return n},read:function(l){return this.xhr("GET",this.read_path(),null,function(n,w,C){C=jQuery.makeArray(C);n=[];w=0;for(var B=C.length;w<B;w++)n.push(new b(C[w]));l(n)})},read_path:function(){return a},update:function(l,n){return this.xhr("PUT",this.update_path(l),l,n)},update_path:function(l){return[this.path(l),
l.id()].join("/")},xhr:function(l,n,w,C){var B=this,K=l=="GET"?undefined:this.params(w);return jQuery.ajax({type:l,url:n+".json",dataType:"json",data:K,complete:function(O,aa){B.xhrComplete(O,aa,w,C)}})},xhrComplete:function(l,n,w,C){var B=Model.REST["handle"+l.status];B&&B.call(this,l,n,w);n=n==="success";B=Model.REST.parseResponseData(l);n&&w&&B&&w.attr(B);C&&C.call(w,n,l,B)}},e)};
var CodeMirror=function(){function b(f,A){function M(d){Ya=null;var j={line:0,ch:0};ea(j,{line:T.length-1,ch:T[T.length-1].text.length},V(d),j,j);Ya=new n}function L(){if(fa.readOnly!="nocursor"){!Sa&&fa.onFocus&&fa.onFocus(Za);Sa=true;ua();if(ma.className.search(/\bCodeMirror-focused\b/)==-1)ma.className+=" CodeMirror-focused";dc()}}function S(){Sa&&fa.onBlur&&fa.onBlur(Za);clearInterval(bc);hb=null;Sa=false;ma.className=ma.className.replace(" CodeMirror-focused","")}function ea(d,j,k,m,o){if(Ya){for(var q=
[],s=d.line,u=j.line+1;s<u;++s)q.push(T[s].text);for(Ya.addChange(d.line,k.length,q);Ya.done.length>fa.undoDepth;)Ya.done.shift()}La(d,j,k,m,o)}function da(d,j){var k=d.pop();if(k){for(var m=[],o=k.start+k.added,q=k.start;q<o;++q)m.push(T[q].text);j.push({start:k.start,added:k.old.length,old:m});q=k.start+k.old.length-1;m=m[m.length-1];var s=k.old[k.old.length-1];if(s)if(m){for(var u=m.length,y=s.length;u>=0&&y>=0;--u,--y)if(m.charAt(u)!=s.charAt(y))break;m=y+1}else m=s.length;else m=m?m.length:0;
q=Fa({line:q,ch:m});La({line:k.start,ch:0},{line:o-1,ch:T[o-1].text.length},k.old,q,q)}}function pa(){da(Ya.done,Ya.undone)}function Ha(){da(Ya.undone,Ya.done)}function La(d,j,k,m,o){for(var q=false,s=Eb.length,u=d.line;u<j.line;++u)if(T[u].text.length==s){q=true;break}var y=j.line-d.line;u=T[d.line];var D=T[j.line];if(u==D)if(k.length==1)u.replace(d.ch,j.ch,k[0]);else{D=u.split(j.ch,k[k.length-1]);var P=[d.line+1,y];u.replace(d.ch,u.text.length,k[0]);u=1;for(var Y=k.length-1;u<Y;++u)P.push(new i(k[u]));
...
bb[u];if(s<d.line)q.push(s);else s>j.line&&q.push(s+y)}k.length&&q.push(d.line);bb=q;Kb(100);Qa.push({from:d.line,to:j.line+1,diff:y});pb={from:d,to:j,text:k};xb(m,o,N.from.line<=Math.min(j.line,j.line+y)?N.from.line:N.from.line+y,N.to.line<=Math.min(j.line,j.line+y)?N.to.line:N.to.line+y);Ab.style.height=T.length*$a()+2*Xa.offsetTop+"px"}function Ia(d,j,k){function m(q){if(p(q,j))return q;if(!p(k,q))return o;var s=q.line+d.length-(k.line-j.line)-1,u=q.ch;if(q.line==k.line)u+=d[d.length-1].length-
(k.ch-(k.line==j.line?j.ch:0));return{line:s,ch:u}}j=Fa(j);k=k?Fa(k):j;d=V(d);var o;Wa(d,j,k,function(q){o=q;return{from:m(N.from),to:m(N.to)}});return o}function Ma(d,j){Wa(V(d),N.from,N.to,function(k){return j=="end"?{from:k,to:k}:j=="start"?{from:N.from,to:N.from}:{from:N.from,to:k}})}function Wa(d,j,k,m){m=m({line:j.line+d.length-1,ch:d.length==1?d[0].length+j.ch:d[d.length-1].length});ea(j,k,d,m.from,m.to)}function ta(d,j){var k=d.line,m=j.line;if(k==m)return T[k].text.slice(d.ch,j.ch);var o=
[T[k].text.slice(d.ch)];for(k=k+1;k<m;++k)o.push(T[k].text);o.push(T[m].text.slice(0,j.ch));return o.join("\n")}function Ja(){return ta(N.from,N.to)}function ua(){Pb||ac.set(2E3,function(){Yb();cb();Sa&&ua();Zb()})}function ya(d){function j(){Yb();var m=cb();if(m=="moved"&&d)Ua[d]=true;if(!m&&!k){k=true;ac.set(80,j)}else{Pb=false;ua()}Zb()}var k=false;Pb=true;ac.set(20,j)}function cb(){function d(va,wa){for(var r=0;;){var v=k.indexOf("\n",r);if(v==-1||(k.charAt(v-1)=="\r"?v-1:v)>=va)return{line:wa,
ch:va-r};++wa;r=v+1}}if(!Ta){var j=false,k=za.value,m=Q(za);if(!m)return false;j=Ka.text!=k;var o=ub,q=j||m.start!=Ka.start||m.end!=(o?Ka.start:Ka.end);if(!q&&!o)return false;if(j){hb=ub=null;if(fa.readOnly){ab=true;return"changed"}}var s=d(m.start,Ka.from),u=d(m.end,Ka.from);if(o){s=m.start==o.anchor?u:s;u=hb?N.to:m.start==o.anchor?s:u;if(!p(s,u)){ub=null;N.inverted=false;o=s;s=u;u=o}}if(s.line==u.line&&s.line==N.from.line&&s.line==N.to.line&&!hb)ab=false;if(j){o=0;for(var y=k.length,D=Math.min(y,
Ka.text.length),P,Y=Ka.from,ca=-1;o<D&&(P=k.charAt(o))==Ka.text.charAt(o);){++o;if(P=="\n"){Y++;ca=o}}D=ca>-1?o-ca:o;for(var Da=Ka.to-1,ra=Ka.text.length;;){P=Ka.text.charAt(ra);P=="\n"&&Da--;if(k.charAt(y)!=P){++y;++ra;break}if(ra<=o||y<=o)break;--y;--ra}ca=Ka.text.lastIndexOf("\n",ra-1);ea({line:Y,ch:D},{line:Da,ch:ca==-1?ra:ra-ca-1},V(k.slice(o,y)),s,u);if(Y!=Da||s.line!=Y)ab=true}else xb(s,u);Ka.text=k;Ka.start=m.start;Ka.end=m.end;return j?"changed":q?"moved":false}}function gb(){for(var d=[],
j=Math.max(0,N.from.line-1),k=Math.min(T.length,N.to.line+2),m=j;m<k;++m)d.push(T[m].text);d=za.value=d.join(na);var o=N.from.ch,q=N.to.ch;for(m=j;m<N.from.line;++m)o+=na.length+T[m].text.length;for(m=j;m<N.to.line;++m)q+=na.length+T[m].text.length;Ka={text:d,from:j,to:k,start:o,end:q};U(za,o,ub?o:q)}function kb(){fa.readOnly!="nocursor"&&za.focus()}function Fb(){var d=Wb(N.inverted?N.from:N.to);return Rb(d.x,d.y,d.x,d.yBot)}function Rb(d,j,k,m){var o=Xa.offsetLeft,q=Xa.offsetTop,s=$a();j+=q;m+=q;
d+=o;k+=o;var u=ma.clientHeight,y=ma.scrollTop;o=false;q=true;if(j<y){ma.scrollTop=Math.max(0,j-2*s);o=true}else if(m>y+u){ma.scrollTop=m+s-u;o=true}j=ma.clientWidth;m=ma.scrollLeft;if(d<m){ma.scrollLeft=Math.max(0,d-10);o=true}else if(k>j+m){ma.scrollLeft=k+10-j;o=true;if(k>Ab.clientWidth)q=false}o&&fa.onScroll&&fa.onScroll(Za);return q}function Sb(){var d=$a(),j=ma.scrollTop-Xa.offsetTop;return{from:Math.min(T.length,Math.max(0,Math.floor(j/d))),to:Math.min(T.length,Math.ceil((j+ma.clientHeight)/
d))}}function lb(d){if(ma.clientWidth){for(var j=d===true?[]:[{from:Ba,to:ib,domStart:0}],k=0,m=d.length||0;k<m;++k){for(var o=d[k],q=[],s=o.diff||0,u=0,y=j.length;u<y;++u){var D=j[u];if(o.to<=D.from)q.push({from:D.from+s,to:D.to+s,domStart:D.domStart});else if(D.to<=o.from)q.push(D);else{o.from>D.from&&q.push({from:D.from,to:o.from,domStart:D.domStart});o.to<D.to&&q.push({from:o.to+s,to:D.to+s,domStart:D.domStart+(o.to-D.from)})}}j=q}s=Sb();o=Math.min(Ba,Math.max(s.from-3,0));q=Math.min(T.length,
Math.max(ib,s.to+3));d=[];u=0;y=ib-Ba;var P=o,Y=0;k=0;for(m=j.length;k<m;++k){D=j[k];if(!(D.to<=o)){if(D.from>=q)break;if(D.domStart>u||D.from>P){d.push({from:P,to:D.from,domSize:D.domStart-u,domStart:u});Y+=D.from-P}P=D.to;u=D.domStart+(D.to-D.from)}}if(u!=y||P!=q){Y+=Math.abs(q-P);d.push({from:P,to:q,domSize:y-u,domStart:u})}if(d.length){Na.style.display="none";if(Y>(s.to-s.from)*0.3){D=o=Math.max(s.from-10,0);j=q=Math.min(s.to+7,T.length);k=[];m={line:D,ch:0};m=p(N.from,m)&&!p(N.to,m);for(D=D;D<
...
"";Da=Da.nextSibling}}m+=P}}Na.style.display="";j=o!=Ba||q!=ib||kc!=ma.clientHeight;Ba=o;ib=q;Lb.style.top=o*$a()+"px";if(j){kc=ma.clientHeight;Ab.style.height=T.length*$a()+2*Xa.offsetTop+"px";Gb()}o=Jb(Eb);Xa.style.width=o>ma.clientWidth?o+"px":"";if(Na.childNodes.length!=ib-Ba)throw new Error("BAD PATCH! "+JSON.stringify(d)+" size="+(ib-Ba)+" nodes="+Na.childNodes.length);Hb()}}else Ba=ib=0}function Gb(){if(fa.gutter||fa.lineNumbers){var d=Lb.offsetHeight,j=ma.clientHeight;nb.style.height=(d-j<
...
Math.min(d,T.length-1))}function Fa(d){if(d.line<0)return{line:0,ch:0};if(d.line>=T.length)return{line:T.length-1,ch:T[T.length-1].text.length};var j=d.ch,k=T[d.line].text.length;return j==null||j>k?{line:d.line,ch:k}:j<0?{line:d.line,ch:0}:d}function lc(d){rb({line:d,ch:0},{line:d,ch:T[d].text.length})}function sb(d,j){if(j=="smart")if(Pa.indent)var k=hc(d);else j="prev";var m=T[d],o=m.indentation(),q=m.text.match(/^\s*/)[0],s;if(j=="prev")s=d?T[d-1].indentation():0;else if(j=="smart")s=Pa.indent(k,
m.text.slice(q.length));else if(j=="add")s=o+fa.indentUnit;else if(j=="subtract")s=o-fa.indentUnit;s=Math.max(0,s);if(s-o){o="";k=0;if(fa.indentWithTabs)for(m=Math.floor(s/oa);m;--m){k+=oa;o+="\t"}for(;k<s;){++k;o+=" "}}else{if(N.from.line!=d&&N.to.line!=d)return;o=q}Ia(o,{line:d,ch:0},{line:d,ch:q.length})}function ec(){Pa=b.getMode(fa,fa.mode);for(var d=0,j=T.length;d<j;++d)T[d].stateAfter=null;bb=[0];Kb()}function Tb(d,j,k){function m(u,y,D,P){u=T[u];y=u.addMark(y,D,P);y.line=u;o.push(y)}d=Fa(d);
j=Fa(j);var o=[];if(d.line==j.line)m(d.line,d.ch,j.ch,k);else{m(d.line,d.ch,null,k);for(var q=d.line+1,s=j.line;q<s;++q)m(q,0,null,k);m(j.line,0,j.ch,k)}Qa.push({from:d.line,to:j.line+1});return function(){for(var u,y,D=0;D<o.length;++D){var P=o[D],Y=z(T,P.line);P.line.removeMark(P);if(Y>-1){if(u==null)u=Y;y=Y}}u!=null&&Qa.push({from:u,to:y+1})}}function Jb(d){eb.innerHTML="<pre><span>x</span></pre>";eb.firstChild.firstChild.firstChild.nodeValue=d;return eb.firstChild.firstChild.offsetWidth||10}function Ub(d,
j){if(j==0)return 0;eb.innerHTML="<pre><span>"+T[d].getHTML(null,null,false,j)+"</span></pre>";return eb.firstChild.firstChild.offsetWidth}function Vb(d,j){function k(P){eb.innerHTML="<pre><span>"+m.getHTML(null,null,false,P)+"</span></pre>";return eb.firstChild.firstChild.offsetWidth}if(j<=0)return 0;for(var m=T[d],o=0,q=0,s=m.text.length,u,y=Math.min(s,Math.ceil(j/Jb("x")));;){var D=k(y);if(D<=j&&y<s)y=Math.min(s,Math.ceil(y*1.2));else{u=D;s=y;break}}if(j>u)return s;y=Math.floor(s*0.8);D=k(y);if(D<
j){o=y;q=D}for(;;){if(s-o<=1)return u-j>j-q?o:s;y=Math.ceil((o+s)/2);D=k(y);if(D>j){s=y;u=D}else{o=y;q=D}}}function Wb(d,j){var k=$a(),m=d.line-(j?Ba:0);return{x:Ub(d.line,d.ch),y:m*k,yBot:(m+1)*k}}function fc(d){d=Wb(d,true);var j=ia(Xa);return{x:j.left+d.x,y:j.top+d.y,yBot:j.top+d.yBot}}function $a(){var d=Na.childNodes.length;if(d)return Na.offsetHeight/d||1;eb.innerHTML="<pre>x</pre>";return eb.firstChild.offsetHeight||1}function tb(d,j){var k=ia(ma,true),m=d.e.clientX,o=d.e.clientY;if(!j&&(m-
k.left>ma.clientWidth||o-k.top>ma.clientHeight))return null;k=ia(Xa,true);o=Ba+Math.floor((o-k.top)/$a());return Fa({line:o,ch:Vb(zb(o),m-k.left)})}function gc(d){function j(){za.value!=o&&qa(Ma)(za.value,"end");za.style.cssText=m;Ta=false;gb();ua()}var k=tb(d);if(!(!k||window.opera)){if(ka(N.from,N.to)||p(k,N.from)||!p(k,N.to))yb(k.line,k.ch);var m=za.style.cssText;za.style.cssText="position: fixed; width: 30px; height: 30px; top: "+(d.pageY()-1)+"px; left: "+(d.pageX()-1)+"px; z-index: 1000; background: white; border-width: 0; outline: none; overflow: hidden;";
var o=za.value=Ja();kb();U(za,0,za.value.length);Ta=true;if(la){d.stop();var q=K(window,"mouseup",function(){q();setTimeout(j,20)},true)}else setTimeout(j,50)}}function dc(){clearInterval(bc);var d=true;ob.style.visibility="";bc=setInterval(function(){ob.style.visibility=(d=!d)?"":"hidden"},650)}function Ra(d){function j(wa,r,v){if(wa.text){var G=wa.styles;wa=s?0:wa.text.length-1;for(var H,E=s?0:G.length-2,I=s?G.length:-2;E!=I;E+=2*u){var R=G[E];if(G[E+1]!=null&&G[E+1]!=Y)wa+=u*R.length;else for(var X=
s?0:R.length-1,ja=s?R.length:-1;X!=ja;X+=u,wa+=u)if(wa>=r&&wa<v&&Da.test(H=R.charAt(X))){var Ga=c[H];if(Ga.charAt(1)==">"==s)ca.push(H);else if(ca.pop()!=Ga.charAt(0))return{pos:wa,match:false};else if(!ca.length)return{pos:wa,match:true}}}}}var k=N.inverted?N.from:N.to,m=T[k.line],o=k.ch-1,q=o>=0&&c[m.text.charAt(o)]||c[m.text.charAt(++o)];if(q){q.charAt(0);var s=q.charAt(1)==">",u=s?1:-1,y=m.styles,D=o+1;q=0;for(var P=y.length;q<P;q+=2)if((D-=y[q].length)<=0){var Y=y[q+1];break}var ca=[m.text.charAt(o)],
Da=/[(){}[\]]/;q=k.line;for(P=s?Math.min(q+50,T.length):Math.max(-1,q-50);q!=P;q+=u){m=T[q];y=q==k.line;if(m=j(m,y&&s?o+1:0,y&&!s?o:m.text.length)){Y=m.match?"CodeMirror-matchingbracket":"CodeMirror-nonmatchingbracket";var ra=Tb({line:k.line,ch:o},{line:k.line,ch:o+1},Y),va=Tb({line:q,ch:m.pos},{line:q,ch:m.pos+1},Y);k=qa(function(){ra();va()});if(d)setTimeout(k,800);else qb=k;break}}}}function Xb(d){for(var j,k,m=d;m>d-40;--m){if(m==0)return 0;var o=T[m-1];if(o.stateAfter)return m;o=o.indentation();
if(k==null||j>o){k=m;j=o}}return k}function hc(d){var j=Xb(d),k=j&&T[j-1].stateAfter;k=k?a(Pa,k):e(Pa);for(j=j;j<d;++j){var m=T[j];m.highlight(Pa,k);m.stateAfter=a(Pa,k)}T[d].stateAfter||bb.push(d);return k}function mc(){for(var d=+new Date+fa.workTime,j=false;bb.length;){var k=T[Ba].stateAfter?bb.pop():Ba;if(!(k>=T.length)){j=true;var m=Xb(k),o=m&&T[m-1].stateAfter;o=o?a(Pa,o):e(Pa);m=m;for(var q=T.length;m<q;++m){var s=T[m],u=s.stateAfter;if(+new Date>d){bb.push(m);Kb(fa.workDelay);Qa.push({from:k,
to:m});return}var y=s.highlight(Pa,o);s.stateAfter=a(Pa,o);if(u&&!y&&s.text)break}Qa.push({from:k,to:m})}}j&&fa.onHighlightComplete&&fa.onHighlightComplete(Za)}function Kb(d){bb.length&&cc.set(d,qa(mc))}function Yb(){ab=null;Qa=[];pb=db=false}function Zb(){var d=false;if(db)d=!Fb();if(Qa.length)lb(Qa);else db&&Hb();d&&Fb();db&&dc();if(!Ta&&(ab===true||ab!==false&&db))gb();db&&fa.matchBrackets&&setTimeout(qa(function(){if(qb){qb();qb=null}Ra(false)}),20);d=pb;db&&fa.onCursorActivity&&fa.onCursorActivity(Za);
...
T[s.line].text.slice(s.ch);D=(y=u.match(d))&&s.ch+u.indexOf(y[0])}if(y)return{from:{line:s.line,ch:D},to:{line:s.line,ch:D+y[0].length},match:y}};else{if(k)d=d.toLowerCase();var m=k?function(q){return q.toLowerCase()}:function(q){return q},o=d.split("\n");this.matches=o.length==1?function(q,s){var u=m(T[s.line].text),y=d.length,D;if(q?s.ch>=y&&(D=u.lastIndexOf(d,s.ch-y))!=-1:(D=u.indexOf(d,s.ch))!=-1)return{from:{line:s.line,ch:D},to:{line:s.line,ch:D+y}}}:function(q,s){var u=s.line,y=q?o.length-
1:0,D=o[y],P=m(T[u].text),Y=q?P.indexOf(D)+D.length:P.lastIndexOf(D);if(!(q?Y>=s.ch||Y!=D.length:Y<=s.ch||Y!=P.length-D.length))for(;;){if(q?!u:u==T.length-1)return;P=m(T[u+=q?-1:1].text);D=o[q?--y:++y];if(y>0&&y<o.length-1)if(P!=D)return;else continue;y=q?P.lastIndexOf(D):P.indexOf(D)+D.length;if(q?y!=P.length-D.length:y!=D.length)return;D={line:s.line,ch:Y};u={line:u,ch:y};return{from:q?u:D,to:q?D:u}}}}}var fa={},Qb=b.defaults;for(var fb in Qb)if(Qb.hasOwnProperty(fb))fa[fb]=(A&&A.hasOwnProperty(fb)?
...
K(ma,"dragover",function(d){d.stop()});K(ma,"drop",qa(function(d){var j=tb(d,true),k=d.e.dataTransfer.files;if(!(!j||fa.readOnly))if(k&&k.length&&window.FileReader&&window.File){var m=k.length,o=Array(m),q=0;for(d=0;d<m;++d)s(k[d],d);var s=function(y,D){var P=new FileReader;P.onload=function(){o[D]=P.result;++q==m&&Ia(o.join(""),Fa(j),Fa(j))};P.readAsText(y)}}else try{(o=d.e.dataTransfer.getData("Text"))&&Ia(o,j,j)}catch(u){}}));K(ma,"paste",function(){kb();ya()});K(za,"paste",function(){ya()});K(za,
...
getRange:function(d,j){return ta(Fa(d),Fa(j))},operation:function(d){return qa(d)()},refresh:function(){lb(true)},getInputField:function(){return za},getWrapperElement:function(){return ma}},Pb=false,c={"(":")>",")":"(<","[":"]>","]":"[<","{":"}>","}":"{<"},h=0;ic.prototype={findNext:function(){return this.find(false)},findPrevious:function(){return this.find(true)},find:function(d){function j(o){o={line:o,ch:0};k.pos={from:o,to:o};return k.atOccurrence=false}for(var k=this,m=Fa(d?this.pos.from:this.pos.to);;){if(this.pos=
...
pa+Ma.length,gb=Ia;if(ya)gb=Ia?Ia+ya:ya;S(cb>ua?Ma.slice(0,ua-pa):Ma,gb);if(cb>=ua){Ma=Ma.slice(ua-pa);pa=ua;break}pa=cb;Ma=da[L++];Ia=da[L++]}}f!=null&&A==null&&S(" ","CodeMirror-selected")}M&&ea.push("</pre>");return ea.join("")}};n.prototype={addChange:function(f,A,M){this.undone.length=0;var L=+new Date,S=this.done[this.done.length-1];if(L-this.time>400||!S||S.start>f+A||S.start+S.added<f-S.added+S.old.length)this.done.push({start:f,added:A,old:M});else{var ea=0;if(f<S.start){for(var da=S.start-
...
query:b});return e.attr()}),query:b}},asJSON:function(){return Asset.map(function(b){return b.attr()})},tags:function(){tags=[];Asset.map(function(){tags=tags.concat(this.attr("tags"))});return tags},searchRemote:function(b,a){var e=b!=null?decodeURIComponent(jQuery.param({query:b})):"";Asset.each(function(){Asset.remove(this)});jQuery.ajax({type:"get",url:"/search.json",contentType:"application/json",dataType:"json",data:e,success:function(g){jQuery.each(g,function(i,l){var n=new Asset({id:l.id});
n.merge(l);Asset.add(n)});a.call(this)}})},searchAdmin:function(b,a){Asset.each(function(){Asset.remove(this)});jQuery.ajax({type:"get",url:"/admin/assets.json",contentType:"application/json",dataType:"json",data:b,success:function(e){$.each(e,function(g,i){var l=new Asset({id:i.id});l.merge(i);Asset.add(l)});a.call(this)}})},keys:{},create:function(b,a){Asset.callback=a;var e=new XMLHttpRequest,g=Asset.generate_uuid();e.upload.uuid=g;e.upload.filename=b.name;e.upload.addEventListener("loadstart",
...
this.extend({asJSON:function(){return Folder.map(function(b){return b.attr()})}})}),Page=Model("page",function(){this.persistence(Model.SinatraREST,"/admin/pages");this.include({initialize:function(){},children:function(){return Page.find_all_by_parent_id(this.id())},childrenAsJSON:function(){return this.children().map(function(b){return b.attr()})},setPartContent:function(b,a){var e=this.attr("contents"),g;jQuery.each(e,function(i,l){if(b==l.name){l.content=a;g=l}});return g},findPartByName:function(b){var a=
this.attr("contents"),e;jQuery.each(a,function(g,i){if(b==i.name){e=i;return false}});return e},findPartById:function(b){var a=this.attr("contents"),e;jQuery.each(a,function(g,i){if(b==i.id){e=i;return false}});return e},getChildren:function(b){var a="/admin/pages/"+this.id()+"/children.json";jQuery.ajax({type:"GET",url:a,dataType:"json",success:function(e){$.each(e,function(g,i){var l=new Page({id:i.id});l.merge(i);Page.add(l)});b&&b.call(this,e)}})},has_children:function(){if(this.children().all().length>
0)return true},parent:function(){return Page.find(this.attr("parent_id"))},assets:function(){for(var b=this.attr("assets"),a=0,e=b.length;a<e;++a){var g=b[a],i=new Asset({id:g.id});i.merge(g);Asset.add(i)}return Asset},parts:function(){var b=this.attr("contents"),a=b.length;Part.each(function(){Part.remove(this)});for(var e=0;e<a;++e){var g=b[e],i=new Part({id:g.id});i.merge(g);i.attr("page_id",this.id());Part.add(i)}return Part},load:function(b){var a=this,e="/admin/pages/"+a.id()+".json";jQuery.ajax({type:"GET",
url:e,dataType:"json",success:function(g){a.merge(g);b.call(this)}})}});this.extend({toMustache:function(){return{pages:this.map(function(b){return b.attr()})}},root:function(){return this.detect(function(){return this.attr("parent_id")==null})},find_by_parent_id:function(b){return this.detect(function(){return this.attr("parent_id")==b})},find_all_by_parent_id:function(b){return this.select(function(){return this.attr("parent_id")==b})},find_by_path:function(b){return this.detect(function(){return this.attr("path")==
b})},load_by_id:function(b,a){jQuery.ajax({type:"GET",url:"/admin/pages/"+b+".json",dataType:"json",success:function(e){var g=Page.find(e.id);g||(g=new Page({id:e.id}));g.merge(e);Page.add(g);a&&a.call(this,e)}})},create:function(b,a){jQuery.ajax({type:"post",url:"/admin/pages.json",dataType:"json",data:{page:b},success:function(e){var g=new Page({id:e.id});g.merge(e);Page.add(g);a.call(this,e)}})}})}),Layout=Model("layout",function(){this.persistence(Model.SinatraREST,"/admin/templates");this.include({load:function(b){var a=
this,e="/admin/templates/"+a.id()+".json";jQuery.ajax({type:"GET",url:e,dataType:"json",success:function(g){a.merge(g);b.call(this,g)}})}});this.extend({toMustache:function(){return{layouts:this.map(function(b){return b.attr()})}},asLayoutJSON:function(b){return Layout.find_all_by_class("Layout").map(function(a){b==a.id()?a.attr("selected",'selected="selected"'):a.attr("selected","");return a.attr()})},find_by_class:function(b){return this.detect(function(){return this.attr("klass")==b})},find_all_by_class:function(b){return this.select(function(){return this.attr("klass")==
b})},asJSON_by_class:function(b){return this.find_all_by_class(function(){return b.map(function(a){return a.attributes})})}})}),Part=Model("part",function(){this.include({deleteRemote:function(b,a){var e="/admin/pages/"+b.id()+"/parts/"+this.id()+".json";jQuery.ajax({type:"DELETE",url:e,dataType:"json",success:function(g){b.merge(g);a.call(this)}})}});this.extend({toMustache:function(){return{parts:this.map(function(b){return b.attr()})}},create:function(b,a){var e="/admin/templates/"+b.template_id+
"/parts.json",g=Layout.find(b.template_id);jQuery.ajax({type:"post",url:e,dataType:"json",data:{part:b},success:function(i){g.merge(i);a.call(this)}})}})}),Site=Model("site",function(){this.persistence(Model.SinatraREST,"/admin/sites");this.include({});this.extend({toMustache:function(){return{sites:this.map(function(b){return b.attr()})}}})}),User=Model("user",function(){this.persistence(Model.SinatraREST,"/admin/users");this.include({});this.extend({toMustache:function(){return{users:this.map(function(b){return b.attr()})}}})}),
Galerie={open:function(b,a){$("body").append('<div id="overlay"></div>').append('<div id="modal_wrapper"><div id="modal"></div></div>');var e=$("#modal");e.html(b);var g=$("#modal_wrapper"),i=$(window).height();e=e.height()+42;e=(i-e)/2;$("#overlay").css({height:i}).hide().fadeIn("fast");g.hide().fadeIn("fast").css({top:e});a&&a.call(this)},close:function(){$("#modal_wrapper").fadeOut("fast",function(){$(this).remove();$("#overlay").fadeOut("fast",function(){$(this).remove()})})}},logger={info:function(b){window.console&&
console.log(b)},debug:function(b){window.console&&console.log(b)}},Utilities={setNonHistoryLinks:function(){Modernizr.history||jQuery("a").click(function(b){b.preventDefault();if((b=jQuery(this).attr("href"))&&!b.match("http://")&&!b.match(/^#/)){console.log("#"+b);document.location.hash=$(this).attr("#href")}})},loadModal:function(b,a){var e=this,g=jQuery(b);if(g.length){Loader.start();var i=g.find("img");setTimeout(function(){g.height()<20?i.load(function(){e.resizeModal(g,a)}):e.resizeModal(g,
a)},13)}},resizeModal:function(b,a){var e=b.width(),g=b.height();e=e/g;jQuery(window).width();var i=jQuery(window).height();g>i-40?b.height(i-40).width((i-40)*e):b.css({"margin-top":(i-g)/2});Loader.stop();a&&a.call(this)},notice:function(b,a){a=a||{};var e=jQuery(".notice"),g=a["class"]||"message";e.html(b).addClass(g).fadeIn("fast",function(){setTimeout(function(){a.persist||e.fadeOut("slow")},1800)})},hideNotice:function(){jQuery(".notice").fadeOut("slow")},setTimestamp:function(){window.timestamp=
(new Date).getTime()},keyboard_nav:function(){jQuery("body").keydown(function(b){switch(b.keyCode){case 37:$("a.previous").click();break;case 39:$("a.next").click();break}})},check_browser_version:function(){},formObserver:function(){}},Loader={start:function(b){b=jQuery("#loader");if(b.length){b.show();Loader.timer=setInterval(function(){var a=b.css("background-position-y").replace("px","");b.css({"background-position-y":a-40+"px"})},67)}},stop:function(b){b=jQuery("#loader");if(b.length){b.hide();
clearInterval(Loader.timer)}}},Updater={setup:function(b,a){jQuery.each(b,function(e,g){var i=new a({id:g.id});i.merge(g);a.add(i)})},update:function(){window.ninja=true;jQuery.ajax({url:"/admin/activity.json",type:"POST",data:{updated:window.timestamp},success:function(b){jQuery.each(b.models,function(a,e){Page.find(e.id).merge(e);Utilities.setTimestamp()});window.ninja=false}})}},aceModes={javascript:require("ace/mode/javascript").Mode,html:require("ace/mode/html").Mode,css:require("ace/mode/css").Mode,
scss:require("ace/mode/scss").Mode,sass:require("ace/mode/scss").Mode,textile:require("ace/mode/textile").Mode,none:require("ace/mode/text").Mode},delay=function(){var b=0;return function(a,e){clearTimeout(b);b=setTimeout(a,e)}}(),Editor={initialize:function(){var b=jQuery("#testForm input[type=button]"),a=jQuery("#testForm input#save");b.click(function(){var e=jQuery(this).data("cmd"),g=jQuery(this).data("value");if(g=="promptUser")g=prompt(jQuery(this).data("text"));var i=jQuery("iframe")[0];e&&
i.contentDocument.execCommand(e,false,g)});a.click(function(){var e=document.location.pathname.split("/"),g=Page.find(e[e.length-1]),i={};jQuery("iframe").contents().find("div.editable").each(function(){var l=jQuery(this),n=l.attr("id").split("_")[0];i[n]=l.html()});jQuery.each(i,function(l,n){n=Editor.textilize(n);g.setPartContent(l,n);console.log(n)});g.save()})},textilize:function(b,a){b=b.replace(/\s*<p>((.|[\r\n])*?)<\/p>\s*/gi,"\n\n$1\n\n\n");b=b.replace(/\s*<h1>((.|[\r\n])*?)<\/h1>\s*/gi,"h1. $1\n\n");
b=b.replace(/\s*<h2>((.|[\r\n])*?)<\/h2>\s*/gi,"h2. $1\n\n");b=b.replace(/\s*<h3>((.|[\r\n])*?)<\/h3>\s*/gi,"h3. $1\n\n");b=b.replace(/\s*<h4>((.|[\r\n])*?)<\/h4>\s*/gi,"h4. $1\n\n");b=b.replace(/\s*<h5>((.|[\r\n])*?)<\/h5>\s*/gi,"h5. $1\n\n");b=b.replace(/\s*<h6>((.|[\r\n])*?)<\/h6>\s*/gi,"h6. $1\n\n");b=b.replace(/<(?:b|strong)>((.|[\r\n])*?)<\/(?:b|strong)>/gi,"*$1*");b=b.replace(/<(?:i|em)>((.|[\r\n])*?)<\/(?:i|em)>/gi,"_$1_");b=b.replace(/<(?:strike|del)>((.|[\r\n])*?)<\/(?:strike|del)>/gi,"-$1-");
b=b.replace(/<(?:u|ins)>((.|[\r\n])*?)<\/(?:u|ins)>/gi,"+$1+");b=b.replace(/<a href="(.*?)">((.|[\r\n])*?)<\/a>/gi,'"$2":$1');b=b.replace(/<code.*?>((.|[\r\n])*?)<\/code>/gi,"@$1@");if(a)b=b.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");b=b.replace(/(\r\n|\n){3,}/g,"\n\n");return b=b.replace(/^[\r\n]+|[\r\n]+$/g,"")}},iFramer={initialize:function(b,a){var e=jQuery(b);jQuery("#loader");if(e.length){Loader.start();e.load(function(){var g=$(this),i=g.contents();i.find("div.editable").attr("contenteditable",
"true").css({background:"hsla(30, 17.7%, 61%, 0.3)"});Loader.stop();g.fadeIn("fast");i.find("a").click(function(l){var n=jQuery(this).attr("href").split("?")[0].replace("/preview","");if(n=Page.find_by_path(n)){l.preventDefault();history.pushState({path:n.attr("title")},n.attr("title"),n.attr("admin_path"))}});a&&a.call(this)})}},setEditFlags:function(b){b.css({position:"relative",display:"block",height:"0",width:"0","text-decoration":"none"}).find("span").css({display:"block",position:"absolute",
top:"-24px",left:"12px",padding:"4px 8px",width:"100px",background:"rgba(255, 255, 0, 0.7)"})}},Base=Sammy(function(b){b.debug=false;b.use(Sammy.JSON);b.use(Sammy.Mustache);b.use(Sammy.NestedParams);b.template_engine="mustache";b.helpers({setup:function(a,e){jQuery.each(a,function(g,i){var l=new e({id:i.id});l.merge(i);e.add(l)})},update:function(){var a=this;window.ninja=true;jQuery.ajax({url:"/admin/activity.json",type:"POST",data:{updated:window.timestamp},success:function(e){jQuery.each(e.models,
function(g,i){i.class_name=="Page"&&a.trigger("update-page",i);i.class_name=="Layout"&&a.trigger("update-layout",i);Utilities.setTimestamp()});window.ninja=false}})}});b.bind("run",function(){var a=this;jQuery("#sidebar .opener").live("click",function(){a.trigger("toggle-children",this);return false});a.trigger("toggle-sidebar")});b.bind("set-active-tab",function(a,e){var g=jQuery("div#tabs a.tab"),i=e.split("?")[0].split("/")[2];i=jQuery("#"+i+"-tab");g.removeClass("active");i.addClass("active")});
b.bind("update-page",function(a,e){var g=Page.find(e.id);g.merge(e);g.save();jQuery("li#page-"+e.id).find("span.title a").text(g.attr("title"));jQuery("h1#page-title-"+g.id()).text(g.attr("title"));jQuery("#page-iframe-"+g.id()).length&&Utilities.notice('<a class="page-reload" href="'+g.attr("admin_path")+'">This page has been changed. Click here to reload.</span>',{persist:true,"class":"warning"})});b.bind("update-layout",function(a,e){var g=Layout.find(e.id);g.merge(e);g.save();jQuery("li#layout-"+
e.id).find("span.title a").text(g.attr("name"))});b.bind("sanskrit",function(a,e){if(e.length){var g=e.find("textarea.sanskrit");g.each(function(i,l){new Sanskrit(l,{toolbar:{actions:{strong:"B",em:"I",ins:"ins",del:"del",link:"link",unlink:"unlink",textile:"Textile"}}})});setTimeout(function(){g.fadeIn("fast")},420)}});b.bind("toggle-sidebar",function(){var a=jQuery("div#sidebar"),e=jQuery("div#editor"),g=jQuery("p#toggle");g.find("a").click(function(i){i.preventDefault();if(a.hasClass("closed")){i=
"300";jQuery.cookie("sidebar_toggled","")}else{i="24";jQuery.cookie("sidebar_toggled","closed")}Modernizr.cssanimations&&console.log("Move.js goes here");a.animate({width:i},200).toggleClass("closed");e.animate({left:i},200).toggleClass("open");g.animate({left:i},200).toggleClass("open")})});b.before(function(){Galerie.close()});b.before(function(a){var e=a.path;a&&a.verb=="get"&&a.trigger("set-active-tab",e)})}),Sites=Sammy(function(){var b=this;this.helpers({loadSites:function(a){if(Site.all().length==
0)Site.load(function(){a&&a.call(this)});else a&&a.call(this)},renderSiteIndex:function(a){this.load(jQuery("script#admin-sites-index")).interpolate(Site.toMustache(),"mustache").replace("#editor").then(function(){a&&a.call(this)})}});this.bind("run",function(){b.modal=false;b.refresh_pages=true});this.get("#/preview",function(){alert(document.location.hostname);window.open("/","preview")});this.get("/admin/sites",function(a){b.refresh_pages=true;jQuery("#editor").html('<h1 class="section">Sites</div>');
...
Site.find(a.params.id);if(jQuery("#sites").length){jQuery(".site-form").html("");a.load(jQuery("script#admin-sites-edit")).interpolate({site:site.asJSON()},"mustache").replace("#site-form-"+site.id())}else a.renderSiteIndex(function(){jQuery(".site-form").html("");a.load(jQuery("script#admin-sites-edit")).interpolate({site:site.asJSON()},"mustache").replace("#site-form-"+site.id())})})});this.put("/admin/sites/:id",function(a){var e=Site.find(a.params.id);e.attr(a.params.site);e.save(function(g,i){var l=
JSON.parse(i.responseText);l.errors?alert(JSON.stringify(l)):a.redirect("#/sites")})});this.get("/admin/sites/:id/remove",function(a){Galerie.open();a.loadSites(function(){site=Site.find(a.params.id);a.load(jQuery("script#admin-sites-remove")).interpolate({site:site.asJSON()},"mustache").replace("#modal");a.renderSiteIndex(Site.all())})});this.del("/admin/sites/:id",function(a){site.destroy(function(e){e?a.redirect("#/sites"):alert("there were issues")})})}),Layouts=Sammy(function(){var b=this;this.helpers({loadLayouts:function(a){a&&
a.call(this)},renderLayoutIndex:function(){this.load(jQuery("script#admin-templates-index")).interpolate({layouts:Layout.find_all_by_class("Layout").map(function(a){return a.attributes}),partials:Layout.find_all_by_class("Partial").map(function(a){return a.attributes}),javascripts:Layout.find_all_by_class("Javascript").map(function(a){return a.attributes}),stylesheets:Layout.find_all_by_class("Stylesheet").map(function(a){return a.attributes})},"mustache").replace("#sidebar")},renderLayout:function(a){this.load(jQuery("script#admin-templates-edit")).interpolate({layout:a.asJSON(),
filters:[{name:"none",value:"none",selected:a.attr("filter")=="css"?'selected="selected"':""},{name:"Sass",value:"sass",selected:a.attr("filter")=="sass"?'selected="selected"':""},{name:"Scss",value:"scss",selected:a.attr("filter")=="scss"?'selected="selected"':""}]},"mustache").replace("#editor").then(function(){var e=a.attr("mode");e=aceModes[e];window.editor=ace.edit("layout_content");window.editor.setTheme("ace/theme/textmate");window.editor.getSession().setMode(new e);window.editor.session.setUseWrapMode(true);
window.editor.getSession().setValue(a.attr("content"))})}});this.bind("run",function(){b.refresh_templates=true;b.refresh_pages=true;b.modal=false});this.get("/admin/templates",function(a){b.refresh_templates=false;jQuery("#editor").html('<h1 class="section">Templates</div>');a.renderLayoutIndex(Layout.all())});this.get("/admin/templates/new/:klass",function(a){var e=$("<div />").attr({id:"new-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(e);a.load(jQuery("#admin-templates-new")).interpolate({klass:a.params.klass},
"mustache").replace("#new-page-container");a.renderLayoutIndex(Layout.all())});this.post("/admin/templates",function(a){(new Layout(a.params.template)).save(function(e,g){var i=JSON.parse(g.responseText);if(i.errors)Utilities.notice(JSON.stringify(i));else{a.redirect("/admin/templates");Utilities.notice("Successfully saved template")}})});this.get("/admin/templates/:id/edit",function(a){b.refresh_pages=true;var e=Layout.find(a.params.id);a.renderLayout(e);a.renderLayoutIndex(Layout.all())});this.put("/admin/templates/:id",
function(a){var e=Layout.find(a.params.id);a.params.layout.content=window.editor.getSession().getValue();e.attr(a.params.layout);e.save(function(g){if(g){a.renderLayoutIndex();Utilities.notice("Successfully saved template");g=aceModes[e.attr("mode")];window.editor.getSession().setUseSoftTabs(true);window.editor.getSession().setTabSize(2);window.editor.getSession().setMode(new g)}})});this.get("/admin/templates/:id/remove",function(a){var e=Layout.find(a.params.id);Galerie.open();a.load(jQuery("#admin-templates-remove")).interpolate({layout:e.asJSON()},
"mustache").replace("#modal");a.renderLayoutIndex(Layout.all())});this.del("/templates/:id",function(a){Layout.find(a.params.id).destroy(function(e){if(e){a.renderLayoutIndex();Utilities.notice("Successfully deleted template");a.redirect("/admin/templates")}})});this.post("/layouts/:id/parts",function(a){this.loadLayouts(function(){var e=a.params.id;Part.create(a.params.part,function(){a.redirect("/admin/templates/"+e+"/edit")})})})}),Assets=Sammy(function(b){var a=this;b.helpers({renderFolderTree:function(e){if(jQuery("ul#folders").length)e&&
e.call(this);else this.load(jQuery("script#admin-folders-index")).interpolate({folders:Folder.asJSON()},"mustache").replace("#sidebar").then(function(){e&&e.call(this)})},send_files:function(e,g,i){for(var l=0,n=0;n<e.length;n++){var w=e[n];Asset.create(w,{before:function(C){jQuery(".progress").append('<p id="progress-'+C+'">'+w.name+'<span class="percentage"></span></p>')},progress:function(C,B){jQuery("#progress-"+C+" .percentage").text(" "+B+"%")},success:function(){l+=1;l==e.length&&Asset.searchAdmin(g,
function(){i&&i.call(this)})}})}}});b.bind("render-asset",function(e,g,i){this.load(jQuery("script#admin-assets-edit")).interpolate(g.toMustacheWithNeighbors(i),"mustache").appendTo("body").then(function(){Utilities.loadModal("div#asset-editor",function(){jQuery("div#asset-editor").fadeIn("fast")})})});b.bind("render-index",function(e,g){var i=this;i.load(jQuery("script#admin-assets-index")).interpolate(Asset.toMustache(g),"mustache").replace("#editor").then(function(){jQuery("a.info-icon").click(function(l){l.preventDefault();
current_asset_id=this.id.split("-")[2];i.trigger("show_info",{current_asset_id:current_asset_id})});jQuery("#ajax_uploader").attr("multiple","multiple").change(function(){jQuery(this).parents("form:first").submit()});jQuery("a.remove-icon").click(function(l){l.preventDefault();current_asset_id=this.id.split("-")[2];i.trigger("show_remove_dialog",{current_asset_id:current_asset_id})})})});b.bind("show_info",function(e,g){var i=Asset.find(g.current_asset_id),l=jQuery("#asset-"+i.id()),n=l.find(".info");
jQuery(".modal-strip").remove();n.length||this.load(jQuery("#admin-assets-info")).interpolate({asset:i.toMustache()},"mustache").appendTo(l).then(function(){jQuery(".modal-strip").fadeIn("fast")})});b.bind("show_remove_dialog",function(e,g){var i=Asset.find(g.current_asset_id),l=jQuery("#asset-"+i.id()),n=l.find(".remove");jQuery(".modal-strip").remove();n.length||this.load(jQuery("script#admin-assets-remove")).interpolate({asset:i.toMustache()},"mustache").appendTo(l).then(function(){jQuery(".modal-strip").fadeIn("fast")})});
b.get("/admin/assets",function(e){jQuery("#overlay").remove();var g=e.params.query?e.params.query:"",i={query:g};i.limit=e.params.limit||48;i.page=e.params.page||1;e.renderFolderTree();Asset.searchAdmin(i,function(){e.trigger("render-index",g)})});b.get("/admin/folders/:id",function(e){jQuery("#overlay").remove();var g=e.params.id,i=Folder.find(g);e.renderFolderTree(function(){jQuery("ul#folders li").removeClass("active");jQuery("#folder-"+g).addClass("active")});i.loadAssets(function(){e.trigger("render-index",
"")})});b.get("/admin/assets/new",function(e){e.load(jQuery("script#admin-assets-new")).interpolate({},"mustache").replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple")});a.first_run=false});b.post("/admin/assets",function(e){var g=document.getElementById("ajax_uploader").files,i=e.params.query?e.params.query:null;jQuery("form#new_asset");var l=i?{query:i}:{};l.limit=e.params.limit||48;l.page=e.params.page||1;this.send_files(g,l,function(){e.trigger("render-index",
i)});return false});b.get("/admin/assets/:id/edit",function(e){jQuery("#overlay").remove();var g=e.params.query?e.params.query:"",i=g?{query:e.params.query}:{},l=Asset.find(e.params.id);l?e.trigger("render-asset",l,g):Asset.searchAdmin(i,function(){var n=Asset.find(e.params.id);e.trigger("render-index",g);e.renderFolderTree();e.trigger("render-asset",n,g)})});b.put("/admin/assets/:id",function(e){var g=Asset.find(e.params.id);if(g){g.attr(e.params.asset);g.save(function(i){i&&Utilities.notice("Successfully saved asset")})}else{g=
new Asset({id:e.params.id});g.load(function(){g.attr(e.params.asset);g.save(function(i){i&&Utilities.notice("Successfully saved asset")})})}});b.del("/admin/assets/:id",function(e){var g=e.params.query?e.params.query:null,i=g?"?"+decodeURIComponent(jQuery.param({query:g})):"";Asset.find(e.params.id).destroy(function(l){if(l){Utilities.notice("Successfully saved asset");e.redirect("/admin/assets"+i)}})})}),Pages=Sammy(function(b){b.helpers({renderPagePreview:function(a,e){var g=this;jQuery("div#preview-"+
...
"mustache").replace("#editor").then(function(){jQuery("li.node").removeClass("active");jQuery("#page-"+a.id()).addClass("active");e&&e.call(this);g.trigger("set-active-page",a)})},renderPageProperties:function(a,e){this.load(jQuery("script#admin-pages-form")).interpolate({page:a.asJSON(),layouts:Layout.asLayoutJSON(a.attr("layout_id")),base_page_id:a.id(),timestamp:window.timestamp},"mustache").replace("#page-tabs-"+a.id()).then(function(){e&&e.call(this)})},renderPageMenu:function(a){this.load(jQuery("script#admin-pages-index")).interpolate({pages:[a.asJSON()],
children:a.childrenAsJSON()},"mustache").replace("#sidebar")}});b.bind("show-page-menu",function(a,e){var g=this;if(e.children().count()==0&&e.attr("child_count")!=0){console.log("ajax");e.getChildren(function(){g.renderPageMenu(e)})}else{if(e.attr("child_count")==0)e=e.parent();g.renderPageMenu(e)}});b.bind("set-active-page",function(a,e){jQuery("li.node").removeClass("active");jQuery("li#page-"+e.id()).addClass("active")});b.bind("reload-page",function(a,e){var g=e.id(),i=jQuery("iframe#page-iframe-"+
...
else{e=new Page({id:a.params.id});e.load(function(){a.renderPagePreview(e)})}a.trigger("show-page-menu",e)});this.get("/admin/pages/:id/edit/?",function(a){var e=Page.find(a.params.id);a.renderPageEditor(e,function(){a.renderPageProperties(e)});a.trigger("show-page-menu",e)});this.put("/admin/pages/:page_id",function(a){var e=Page.find(a.params.page_id);e.attr(a.params.page);jQuery("span.timestamp").text()<window.timestamp?alert("Someone else has edited this page, please reload and try again"):e.save(function(g){if(g){Utilities.setTimestamp();
Utilities.notice("Successfully saved page");a.trigger("reload-page",e);a.redirect(e.attr("admin_path"))}})});this.get("/admin/pages/:id/remove",function(a){var e=Page.find(a.params.id),g=$("<div />").attr({id:"remove-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(g);a.trigger("page-index");a.load(jQuery("#admin-pages-remove")).interpolate({page:e.asJSON()},"mustache").replace("#remove-page-container")});this.del("/admin/pages/:id",function(a){var e=Page.find(a.params.id);
e.destroy(function(g){if(g){jQuery("#page-"+e.id()).remove();a.redirect("/admin/pages")}})})}),Parts=Sammy(function(b){b.helpers({renderPart:function(a,e,g){var i=this;i.load(jQuery("script#admin-"+g+"-edit")).interpolate({part:a.asJSON(),page:e.asJSON(),assets:Asset.asJSON(),timestamp:window.timestamp},"mustache").replace("#page-tabs-"+e.id()).then(function(){var l=aceModes.textile;window.editor=ace.edit("part-"+a.id()+"-content");window.editor.setTheme("ace/theme/textmate");window.editor.getSession().setMode(new l);
window.editor.session.setUseWrapMode(true);window.editor.getSession().setValue(a.attr("content"));i.trigger("set_asset_links");jQuery("#ajax_uploader").attr("multiple","multiple").change(function(){var n=jQuery(this).parents("form:first");jQuery(".progress").slideDown("slow",function(){n.submit()})});i.trigger("page-index")})}});this.bind("set_asset_links",function(){jQuery("#search-results-container li.asset").each(function(a,e){var g=jQuery(e).find("a"),i=jQuery(e).attr("id").split("-")[1],l=g.find("img").attr("src");
g.click(function(n){n.preventDefault();jQuery("input#part-asset-id").attr("value",i);jQuery("form#edit-part img.preview").attr("src",l.replace("icon","thumbnail"))})})});this.get("/admin/pages/:page_id/parts/:id/edit",function(a){jQuery(".modal-editor").remove();$("iframe");var e=Page.find(a.params.page_id),g=e.parts().find(a.params.id);a.trigger("show-page-menu",e);a.renderPageEditor(e,function(){a.renderPart(g,e,"parts")})});this.get("/admin/pages/:page_id/image_parts/:id/edit",function(a){var e=
Page.find(a.params.page_id),g=e.parts().find(a.params.id);$("iframe");Asset.searchAdmin({limit:"24"},function(){a.trigger("show-page-menu",e);a.renderPageEditor(e,function(){a.renderPart(g,e,"image_parts")})})});this.get("/admin/pages/:page_id/parts/:id/results",function(a){Page.find(a.params.page_id);Part.find(a.params.id);var e=a.params.query?a.params.query:null;Asset.searchAdmin(e?{query:e}:{},function(){a.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),"mustache").replace("#search-results-container").then(function(){a.trigger("set_asset_links")})})});
this.put("#/pages/:page_id/parts/:id",function(a){var e=Page.find(a.params.page_id),g=e.parts().find(a.params.id),i=e.attr("contents"),l=i.length;a.params.part.content=window.editor.getSession().getValue();for(var n=0;n<l;++n){g=i[n];if(a.params.id==g.id){g.content=a.params.part.content;g.asset_id=a.params.part.asset_id;var w=Part.find(g.id);w.attr({asset_id:a.params.part.asset_id,content:a.params.part.content});w.save()}}e.save(function(C){if(C){Utilities.setTimestamp();Utilities.notice(w.attr("name")+
" saved");window.editor.getSession().setUseSoftTabs(true);window.editor.getSession().setTabSize(2);window.editor.getSession().setMode(new editorMode)}})});this.post("#/pages/:page_id/image_parts/:id/assets",function(a){Page.find(a.params.page_id).parts().find(a.params.id);var e=document.getElementById("ajax_uploader").files,g=a.params.query?a.params.query:null;jQuery("form#new_asset");g=g?{query:g}:{};g.limit=a.params.limit||48;g.page=a.params.page||1;this.send_files(e,g,function(){a.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),
"mustache").replace("#search-results-container").then(function(){jQuery("#ajax_uploader").attr("files",null);jQuery(".progress").slideUp("slow",function(){jQuery(this).html("")});a.trigger("set_asset_links")})})})}),Users=Sammy(function(){var b=this;this.helpers({loadUsers:function(a){if(User.all().length==0)User.load(function(){a&&a.call(this)});else a&&a.call(this)},renderUserIndex:function(a){this.load(jQuery("script#admin-users-index")).interpolate(User.toMustache(),"mustache").replace("#editor").then(function(){a&&
a.call(this)})}});this.bind("run",function(){b.modal=false;b.refresh_pages=true});this.get("/admin/users",function(a){jQuery("#editor").html('<h1 class="section">Users</div>');a.loadUsers(function(){a.renderUserIndex()})});this.get("/admin/users/new",function(a){a.loadUsers(function(){jQuery("#modal").length||Galerie.open();a.load(jQuery("script#admin-users-new")).interpolate({},"mustache").replace("#modal");a.renderUserIndex()})});this.post("/admin/users",function(a){(new User(a.params.user)).save(function(e,
