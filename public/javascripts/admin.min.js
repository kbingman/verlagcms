...
children:function(){var c;jQuery.each(this.attr("child_pages"),function(a,f){c=Page.find(f.id);if(!c){c=new Page(f);Page.add(c)}c.children()});return Page.find_all_by_parent_id(this.id())},set_children:function(c){var a=this,f=Page.select(function(){return this.attr("parent_id")==a.id()}).all(),e=[];jQuery.each(f,function(h,l){l.set_children(function(){e.push(l.asJSON())})});a.merge({"children?":e.length?true:false,child_count:e.length,child_pages:e});c&&c.call(this)},setPartAttributes:function(c,
a){var f=this.attr("contents"),e;jQuery.each(f,function(h,l){if(c==l.id){if(a.content)l.content=a.content;if(a.asset_id)l.asset_id=a.asset_id;if(a.path)l.path=a.path;e=l}});return e},findPartByName:function(c){var a=this.attr("contents"),f;jQuery.each(a,function(e,h){if(c==h.name){f=h;return false}});return f},findPartById:function(c){var a=this.attr("contents"),f;jQuery.each(a,function(e,h){if(c==h.id){f=h;return false}});return f},getChildren:function(c){var a=this,f="/admin/pages/"+a.id()+"/children.json";
jQuery.ajax({type:"GET",url:f,dataType:"json",success:function(e){$.each(e,function(h,l){var n=new Page({id:l.id});n.merge(l);Page.add(n)});a.attr({"children?":true,children:a.childrenAsJSON()});c&&c.call(this,e)}})},has_children:function(){if(this.children().all().length>0)return true},parent:function(){return Page.find(this.attr("parent_id"))},parts:function(){var c=this.attr("contents"),a=c.length;Part.each(function(){Part.remove(this)});for(var f=0;f<a;++f){var e=c[f],h=new Part({id:e.id});h.merge(e);
h.attr("page_id",this.id());Part.add(h)}return Part},load:function(c){var a=this,f="/admin/pages/"+a.id()+".json";jQuery.ajax({type:"GET",url:f,dataType:"json",success:function(e){a.merge(e);Page.add(a);c.call(this)}})}});this.extend({toMustache:function(){return{pages:this.map(function(c){return c.attr()})}},root:function(){return this.detect(function(){return this.attr("parent_id")==null})},site_map:function(c){var a=Page.root();Page.node(a,c)},node:function(c,a){var f=Page.select(function(){return this.attr("parent_id")==
c.id()}).all();jQuery.each(f,function(e,h){h.parent().set_children();Page.node(h,a)});f.length==0&&a&&a.call(this)},find_by_parent_id:function(c){return this.detect(function(){return this.attr("parent_id")==c})},find_all_by_parent_id:function(c){return this.select(function(){return this.attr("parent_id")==c})},find_by_path:function(c){return this.detect(function(){return this.attr("path")==c})},load_by_id:function(c,a){jQuery.ajax({type:"GET",url:"/admin/pages/"+c+".json",dataType:"json",success:function(f){var e=
Page.find(f.id);e||(e=new Page({id:f.id}));e.merge(f);Page.add(e);a&&a.call(this,f)}})},create:function(c,a){jQuery.ajax({type:"post",url:"/admin/pages.json",dataType:"json",data:{page:c},success:function(f){var e=new Page({id:f.id});e.merge(f);Page.add(e);a.call(this,f)}})}})}),Layout=Model("layout",function(){this.persistence(Model.SinatraREST,"/admin/templates");this.include({load:function(c){var a=this,f="/admin/templates/"+a.id()+".json";jQuery.ajax({type:"GET",url:f,dataType:"json",success:function(e){a.merge(e);
...
jQuery("a").click(function(c){c.preventDefault();if((c=jQuery(this).attr("href"))&&!c.match("http://")&&!c.match(/^#/)){console.log("#"+c);document.location.hash=$(this).attr("#href")}})},loadModal:function(c,a){var f=this,e=jQuery(c);if(e.length){Loader.start();var h=e.find("img");if(h.length)setTimeout(function(){e.height()<20?h.load(function(){f.resizeModal(h,e,a)}):f.resizeModal(h,e,a)},13);else{jQuery(window).width();var l=jQuery(window).height();e.css({width:"640px",height:"480px","margin-top":(l-
540)/2+"px"});Loader.stop();a&&a.call(this)}}},resizeModal:function(c,a,f){var e=c.clone();e=e[0].width/e[0].height;var h=jQuery(window).width(),l=jQuery(window).height();if(a.height()>l-40){c.height(l-40).width((l-40)*e);a.height(l-40).width((l-40)*e)}if(a.width()>h-40){c.height((h-40)/e).width(h-40);a.height((h-40)/e).width(h-40);a.css({"margin-top":(l-(h-40)/e)/2})}a.width()<h-40&&a.height()<l-40&&a.css({width:a.height()*e+"px","margin-top":(l-a.height())/2});Loader.stop();f&&f.call(this)},notice:function(c,
a){a=a||{};var f=jQuery(".notice"),e=a["class"]||"message";f.html(c).addClass(e).fadeIn("fast",function(){setTimeout(function(){a.persist||f.fadeOut("slow")},1800)})},hideNotice:function(){jQuery(".notice").fadeOut("slow")},setTimestamp:function(){window.timestamp=(new Date).getTime()},keyboard_nav:function(){jQuery("body").keydown(function(c){switch(c.keyCode){case 37:$("a.previous").click();break;case 39:$("a.next").click();break}})},check_browser_version:function(){},formObserver:function(){},
unique:function(c){tmp=[];for(i=0;i<c.length;i++)if(!Utilities.contains(tmp,c[i])){tmp.length+=1;tmp[tmp.length-1]=c[i]}return tmp},contains:function(c,a){for(j=0;j<c.length;j++)if(c[j]==a)return true;return false}},Loader={start:function(c){c=jQuery("#loader");if(c.length){c.show();Loader.timer=setInterval(function(){var a=c.css("background-position-y").replace("px","");c.css({"background-position-y":a-40+"px"})},67)}},stop:function(c){c=jQuery("#loader");if(c.length){c.hide();clearInterval(Loader.timer)}}},
Updater={setup:function(c,a){jQuery.each(c,function(f,e){var h=new a({id:e.id});h.merge(e);a.add(h)})},update:function(){window.ninja=true;jQuery.ajax({url:"/admin/activity.json",type:"POST",data:{updated:window.timestamp},success:function(c){jQuery.each(c.models,function(a,f){Page.find(f.id).merge(f);Utilities.setTimestamp()});window.ninja=false}})}},aceModes={javascript:require("ace/mode/javascript").Mode,html:require("ace/mode/html").Mode,css:require("ace/mode/css").Mode,scss:require("ace/mode/scss").Mode,
sass:require("ace/mode/scss").Mode,textile:require("ace/mode/textile").Mode,none:require("ace/mode/scss").Mode},delay=function(){var c=0;return function(a,f){clearTimeout(c);c=setTimeout(a,f)}}(),Editor={initialize:function(){var c=jQuery("#editor-buttons input[type=button]"),a=jQuery("#editor-buttons input#save");c.click(function(){var f=jQuery(this).data("cmd"),e=jQuery(this).data("value");if(e=="promptUser")e=prompt(jQuery(this).data("text"));var h=jQuery("iframe")[0];f&&h.contentDocument.execCommand(f,
false,e)});a.click(function(){var f=document.location.pathname.split("/"),e=Page.find(f[f.length-1]),h={};jQuery("iframe").contents().find("div.editable").each(function(){var l=jQuery(this),n=l.attr("id").split("-")[1];h[n]=l.html()});jQuery.each(h,function(l,n){n=Editor.textilize(n);e.setPartAttributes(l,{content:n})});e.save()})},textilize:function(c,a){c=c.replace(/\s*<p>((.|[\r\n])*?)<\/p>\s*/gi,"\n\n$1\n\n\n");c=c.replace(/\s*<h1>((.|[\r\n])*?)<\/h1>\s*/gi,"h1. $1\n\n");c=c.replace(/\s*<h2>((.|[\r\n])*?)<\/h2>\s*/gi,
"h2. $1\n\n");c=c.replace(/\s*<h3>((.|[\r\n])*?)<\/h3>\s*/gi,"h3. $1\n\n");c=c.replace(/\s*<h4>((.|[\r\n])*?)<\/h4>\s*/gi,"h4. $1\n\n");c=c.replace(/\s*<h5>((.|[\r\n])*?)<\/h5>\s*/gi,"h5. $1\n\n");c=c.replace(/\s*<h6>((.|[\r\n])*?)<\/h6>\s*/gi,"h6. $1\n\n");c=c.replace(/<(?:b|strong)>((.|[\r\n])*?)<\/(?:b|strong)>/gi,"*$1*");c=c.replace(/<(?:i|em)>((.|[\r\n])*?)<\/(?:i|em)>/gi,"_$1_");c=c.replace(/<(?:strike|del)>((.|[\r\n])*?)<\/(?:strike|del)>/gi,"-$1-");c=c.replace(/<(?:u|ins)>((.|[\r\n])*?)<\/(?:u|ins)>/gi,
"+$1+");c=c.replace(/<a href="(.*?)">((.|[\r\n])*?)<\/a>/gi,'"$2":$1');c=c.replace(/<code.*?>((.|[\r\n])*?)<\/code>/gi,"@$1@");if(a)c=c.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");c=c.replace(/(\r\n|\n){3,}/g,"\n\n");return c=c.replace(/^[\r\n]+|[\r\n]+$/g,"")}};jQuery("span.opener").live("click",function(){var c=jQuery(this).parents("li.node:first").attr("id").split("-")[1];Pages.trigger("open-page-children",c)});
...
"multiple").bind("change",function(){jQuery(this).parents("form:first").submit()});n.trigger("set_draggable_assets");n.trigger("set_droppable_folders")});window.modal=false});c.bind("set_draggable_assets",function(){jQuery("ul#assets").sortable({stop:function(f,e){console.log(e.serialize())}})});c.bind("set_droppable_folders",function(){jQuery("li.folder").droppable({hoverClass:"active",drop:function(f,e){var h=f.target.id.split("-")[1],l=Folder.find(h),n=e.draggable[0].id.split("-")[1];n=Asset.find(n);
n.attr("folder_id",h);e.draggable.fadeOut("fast");n.save(function(v){v?Utilities.notice("Asset added to "+l.attr("name")):e.draggable.fadeIn("fast")})}})});c.bind("show_info",function(f,e){var h=Asset.find(e.current_asset_id),l=jQuery("#asset-"+h.id()),n=l.find(".info");jQuery(".modal-strip").remove();n.length||this.load(jQuery("#admin-assets-info")).interpolate({asset:h.toMustache()},"mustache").appendTo(l).then(function(){jQuery(".modal-strip").fadeIn("fast")})});c.bind("show_remove_dialog",function(f,
e){var h=Asset.find(e.current_asset_id),l=jQuery("#asset-"+h.id()),n=l.find(".remove");jQuery(".modal-strip").remove();n.length||this.load(jQuery("script#admin-assets-remove")).interpolate({asset:h.toMustache()},"mustache").appendTo(jQuery("body")).then(function(){var v=jQuery(".modal-strip");v.css({top:l.offset().top+l.height()/2,left:l.offset().left+l.width()/2});v.fadeIn("fast")})});c.get("/admin/assets",function(f){jQuery("#overlay").remove();var e=f.params.query,h={query:e};h.limit=f.params.limit||
96;h.page=f.params.page||1;if(e){f.renderFolderTree();Asset.searchAdmin(h,function(){f.trigger("render-index",{query:e})})}else f.redirect(Folder.first().attr("admin_path"))});c.get("/admin/assets/new",function(f){f.load(jQuery("script#admin-assets-new")).interpolate({},"mustache").replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple")});a.first_run=false});c.post("/admin/assets",function(f){var e=document.getElementById("ajax_uploader").files,h=f.params.asset.folder_id,
l=f.params.query?f.params.query:null;jQuery("form#new_asset");l=l?{query:l}:{};l.limit=f.params.limit||48;l.page=f.params.page||1;l.folder_id=h;this.send_files(e,l,{before:function(n){f.load(jQuery("script#admin-assets-loading")).interpolate({uuid:n.uuid,title:n.file_name.split(".")[0]},"mustache").appendTo("#assets")},progress:function(){},success:function(n){var v=Mustache.to_html(jQuery("script#admin-assets-asset").html(),n.attr());jQuery("<img />").attr("src","/images/"+n.id()+"/"+n.attr("file_name")).load(function(){jQuery("li#asset-"+
n.attr("uuid")).replaceWith(v)})}});return false});c.get("/admin/assets/:id/edit",function(f){jQuery("#overlay").remove();var e=f.params.query?f.params.query:"",h=e?{query:f.params.query}:{},l=Asset.find(f.params.id);window.modal=true;if(l)f.trigger("render-asset",l,e);else{alert("hey");Asset.searchAdmin(h,function(){var n=Asset.find(f.params.id);f.trigger("render-index",{folder_id:n.attr("folder_id")});f.renderFolderTree();f.trigger("render-asset",n,e)})}});c.put("/admin/assets/:id",function(f){var e=
Asset.find(f.params.id);if(e){e.attr(f.params.asset);e.save(function(h){h&&Utilities.notice("Successfully saved asset")})}else{e=new Asset({id:f.params.id});e.load(function(){e.attr(f.params.asset);e.save(function(h){h&&Utilities.notice("Successfully saved asset")})})}});c.del("/admin/assets/:id",function(f){f=f.params.id;var e=Asset.find(f);e.attr("folder_id");jQuery("li#asset-"+f).fadeOut("fast");jQuery(".modal-strip").remove();e.destroy(function(h){h&&Utilities.notice("Asset removed")})})}),Folders=
Sammy(function(c){this.helpers({renderFolderTree:function(a){if(jQuery("ul#folders").length)a&&a.call(this);else this.load(jQuery("script#admin-folders-index")).interpolate({folders:Folder.asJSON()},"mustache").replace("#sidebar").then(function(){a&&a.call(this)})}});this.get("/admin/folders/new/?",function(a){$("#modal").length==0&&Galerie.open();a.renderFolderTree(function(){jQuery("ul#folders li").removeClass("active")});a.load(jQuery("#admin-folders-new")).interpolate({},"mustache").replace("#modal")});
c.get("/admin/folders/:id",function(a){jQuery("#overlay").remove();var f=a.params.id,e=Folder.find(f);a.renderFolderTree(function(){jQuery("ul#folders li").removeClass("active");jQuery("#folder-"+f).addClass("active")});window.modal||e.loadAssets(function(){a.trigger("render-index",{folder_id:e.id()})});window.modal=false});c.get("/admin/folders/:folder_id/assets/:id/edit",function(a){jQuery("#overlay").remove();var f=Folder.find(a.params.folder_id),e=Asset.find(a.params.id);window.modal=true;e?a.trigger("render-asset",
e):f.loadAssets(function(){e=Asset.find(a.params.id);a.renderFolderTree();a.trigger("render-index",{folder_id:f.id()});a.trigger("render-asset",e)})});this.post("/admin/folders",function(a){var f=new Folder(a.params.folder);f.save(function(e,h){var l=JSON.parse(h.responseText);if(l.errors)alert(JSON.stringify(l));else{alert(f.attr("admin_path"));a.load(jQuery("script#admin-folders-index")).interpolate({folders:Folder.asJSON()},"mustache").replace("#sidebar");a.redirect(f.attr("admin_path"))}})})}),
Pages=Sammy(function(c){c.helpers({renderPagePreview:function(a,f){var e=this;e.load(jQuery("script#admin-pages-show")).interpolate({page:a.asJSON(),layouts:Layout.asLayoutJSON(a.attr("layout_id")),base_page_id:a.id()},"mustache").replace("#editor").then(function(){iFramer.initialize(".preview iframe",function(){f&&f.call(this)});e.trigger("set-active-page",a);Editor.initialize()})},renderPageEditor:function(a,f){var e=this;if(jQuery("#page-editor").length){f&&f.call(this);e.trigger("set-active-page",
a)}else e.load(jQuery("script#admin-pages-edit")).interpolate({page:a.asJSON()},"mustache").replace("#editor").then(function(){jQuery("li.node").removeClass("active");jQuery("#page-"+a.id()).addClass("active");f&&f.call(this);e.trigger("set-active-page",a)})},renderPageProperties:function(a,f){this.load(jQuery("script#admin-pages-form")).interpolate({page:a.asJSON(),layouts:Layout.asLayoutJSON(a.attr("layout_id")),base_page_id:a.id(),timestamp:window.timestamp},"mustache").replace("#page-tabs-"+a.id()).then(function(){f&&
f.call(this)})}});c.bind("page-index",function(a,f){var e=Page.root(),h;jQuery.cookie("active_page_ids")&&jQuery.cookie("active_page_ids").split(",");Page.site_map();this.load(jQuery("script#admin-pages-index")).interpolate({root:e.asJSON(),partials:{node:jQuery("script#admin-pages-node").html()}},"mustache").replace("#sidebar").then(function(){h=jQuery("li#page-"+f.id()).parents("ul:first");console.log(h);h.show()})});c.bind("open-page-children",function(a,f){var e=Page.find(f);e=jQuery("li#page-"+
e.id()).find("ul:first");jQuery.cookie("active_page_ids")&&jQuery.cookie("active_page_ids").split(",");console.log(e.is_visible);e.toggle().toggleClass("open")});c.bind("set-active-page",function(a,f){jQuery("li.node").removeClass("active");jQuery("li#page-"+f.id()).addClass("active")});c.bind("reload-page",function(a,f){var e=f.id(),h=jQuery("iframe#page-iframe-"+e);jQuery("li#page-"+e).find("span.title a").text(f.attr("title"));jQuery("h1#page-title-"+e).text(f.attr("title"));h.attr("src",h.attr("src"))});
this.get("/admin/pages/?",function(a){var f=Page.first();a.redirect(f.attr("admin_path"))});this.get("/admin/pages/:id/new/?",function(a){var f=Page.find(a.params.id),e=$("<div />").attr({id:"new-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(e);a.trigger("page-index");a.load(jQuery("#admin-pages-new")).interpolate({parent:f.asJSON(),layouts:Layout.asLayoutJSON(f.attr("layout_id"))},"mustache").replace("#new-page-container")});this.post("/admin/pages/:page_id/?",function(a){Page.find(a.params.page_id);
(new Page(a.params.page)).save(function(f,e){var h=JSON.parse(e.responseText);h.errors?alert(JSON.stringify(h)):a.redirect(h.admin_path)})});this.get("/admin/pages/:id/?",function(a){jQuery("#overlay").remove();jQuery(".modal-editor").remove();var f=Page.find(a.params.id);if(f)a.renderPagePreview(f);else{f=new Page({id:a.params.id});f.load(function(){a.renderPagePreview(f)})}a.trigger("page-index",f)});this.get("/admin/pages/:id/edit/?",function(a){var f=Page.find(a.params.id);a.renderPageEditor(f,
function(){a.renderPageProperties(f)});a.trigger("page-index",f)});this.put("/admin/pages/:page_id",function(a){var f=Page.find(a.params.page_id),e=jQuery("span.timestamp").text();f.attr(a.params.page);e<window.timestamp?alert("Someone else has edited this page, please reload and try again"):f.save(function(h){if(h){Utilities.setTimestamp();Utilities.notice("Successfully saved page");a.trigger("reload-page",f);a.redirect(f.attr("admin_path"))}})});this.get("/admin/pages/:id/remove",function(a){var f=
Page.find(a.params.id),e=$("<div />").attr({id:"remove-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(e);a.trigger("page-index");a.load(jQuery("#admin-pages-remove")).interpolate({page:f.asJSON()},"mustache").replace("#remove-page-container")});this.del("/admin/pages/:id",function(a){var f=Page.find(a.params.id);console.log("HEY");console.log(f);Page.remove(f);f.destroy(function(e){e&&a.redirect("/admin/pages")})})}),Parts=Sammy(function(c){c.helpers({renderPart:function(a,
f,e){var h=this;h.load(jQuery("script#admin-"+e+"-edit")).interpolate({part:a.asJSON(),page:f.asJSON(),assets:Asset.asJSON(),timestamp:window.timestamp},"mustache").replace("#page-tabs-"+f.id()).then(function(){var l=aceModes.textile;window.editor=ace.edit("part-"+a.id()+"-content");window.editor.setTheme("ace/theme/textmate");window.editor.getSession().setMode(new l);window.editor.session.setUseWrapMode(true);window.editor.getSession().setValue(a.attr("content"));jQuery(".toolbar a.tab").removeClass("active");
jQuery(".toolbar a."+a.attr("name")+"-tab").addClass("active");h.trigger("set_asset_links");jQuery("#ajax_uploader").attr("multiple","multiple").change(function(){var n=jQuery(this).parents("form:first");jQuery(".progress").slideDown("slow",function(){n.submit()})});h.trigger("page-index")})}});this.bind("set_asset_links",function(){jQuery("#search-results-container li.asset").each(function(a,f){var e=jQuery(f).find("a"),h=jQuery(f).attr("id").split("-")[1],l=e.find("img").attr("src");e.click(function(n){n.preventDefault();
jQuery("input#part-asset-id").attr("value",h);jQuery("form#edit-part img.preview").attr("src",l.replace("icon","thumbnail"))})})});this.get("/admin/pages/:page_id/parts/:id/edit",function(a){jQuery(".modal-editor").remove();$("iframe");var f=Page.find(a.params.page_id),e=f.parts().find(a.params.id);a.trigger("show-page-menu",f);a.renderPageEditor(f,function(){a.renderPart(e,f,"parts")})});this.get("/admin/pages/:page_id/image_parts/:id/edit",function(a){var f=Page.find(a.params.page_id),e=f.parts().find(a.params.id);
$("iframe");Asset.searchAdmin({limit:"24"},function(){a.trigger("show-page-menu",f);a.renderPageEditor(f,function(){a.renderPart(e,f,"image_parts")})})});this.get("/admin/pages/:page_id/parts/:id/results",function(a){Page.find(a.params.page_id);Part.find(a.params.id);var f=a.params.query?a.params.query:null;Asset.searchAdmin(f?{query:f}:{},function(){a.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),"mustache").replace("#search-results-container").then(function(){a.trigger("set_asset_links")})})});
this.put("#/pages/:page_id/parts/:id",function(a){var f=Page.find(a.params.page_id),e=f.parts().find(a.params.id),h=f.attr("contents"),l=h.length;a.params.part.content=window.editor.getSession().getValue();for(var n=0;n<l;++n){e=h[n];if(a.params.id==e.id){e.content=a.params.part.content;e.asset_id=a.params.part.asset_id;var v=Part.find(e.id);v.attr({asset_id:a.params.part.asset_id,content:a.params.part.content});v.save()}}f.save(function(D){if(D){Utilities.setTimestamp();Utilities.notice(v.attr("name")+
" saved");window.editor.getSession().setUseSoftTabs(true);window.editor.getSession().setTabSize(2);window.editor.getSession().setMode(new editorMode)}})});this.post("#/pages/:page_id/image_parts/:id/assets",function(a){Page.find(a.params.page_id).parts().find(a.params.id);var f=document.getElementById("ajax_uploader").files,e=a.params.query?a.params.query:null;jQuery("form#new_asset");e=e?{query:e}:{};e.limit=a.params.limit||48;e.page=a.params.page||1;this.send_files(f,e,function(){a.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),
"mustache").replace("#search-results-container").then(function(){jQuery("#ajax_uploader").attr("files",null);jQuery(".progress").slideUp("slow",function(){jQuery(this).html("")});a.trigger("set_asset_links")})})})}),Users=Sammy(function(){this.helpers({loadUsers:function(c){if(User.all().length==0)User.load(function(){c&&c.call(this)});else c&&c.call(this)},renderUserIndex:function(c){this.load(jQuery("script#admin-users-index")).interpolate(User.toMustache(),"mustache").replace("#editor").then(function(){c&&
c.call(this)})}});this.get("/admin/users",function(c){jQuery("#editor").html('<h1 class="section">Users</div>');c.loadUsers(function(){c.renderUserIndex()})});this.get("/admin/users/new",function(c){c.loadUsers(function(){jQuery("#modal").length||Galerie.open();c.load(jQuery("script#admin-users-new")).interpolate({},"mustache").replace("#modal");c.renderUserIndex()})});this.post("/admin/users",function(c){(new User(c.params.user)).save(function(a,f){var e=JSON.parse(f.responseText);e.errors?alert(JSON.stringify(e)):
c.redirect("/admin/users")})});this.get("/admin/users/:id/edit",function(c){c.loadUsers(function(){user=User.find(c.params.id);if(jQuery("#users").length){jQuery(".user-form").html("");c.load(jQuery("script#admin-users-edit")).interpolate({user:user.asJSON()},"mustache").replace("#user-form-"+user.id())}else c.renderUserIndex(function(){jQuery(".user-form").html("");c.load(jQuery("script#admin-users-edit")).interpolate({user:user.asJSON()},"mustache").replace("#user-form-"+user.id())})})});this.put("/admin/users/:id",
function(c){var a=User.find(c.params.id);a.attr(c.params.user);a.save(function(f,e){var h=JSON.parse(e.responseText);h.errors?alert(JSON.stringify(h)):c.redirect("/admin/users")})})});
jQuery(document).ready(function(){var c=jQuery("#loader");jQuery("<img />",{src:"/images/loader.png"});jQuery("body").ajaxStart(function(){window.ninja||Loader.start(c)}).ajaxSuccess(function(){Loader.stop(c)});if(!jQuery("#login").length){jQuery.ajax({url:"/templates",success:function(a){jQuery("head").append(a)}});jQuery.ajax({url:"/admin/sites/current.json",success:function(a){var f=new Page({id:a.root.id});f.merge(a.root);Page.add(f);f.children();Updater.setup(a.templates,Layout);Updater.setup(a.folders,
Folder);Base.run();window.timestamp=a.now}})}require("pilot/canon").addCommand({name:"save",bindKey:{win:"Ctrl-S",mac:"Command-S",sender:"editor"},exec:function(){jQuery("form.command-save").submit()}})});function children(c){var a=new Page({id:c.id});a.merge(c);Page.add(a);a.children()};
