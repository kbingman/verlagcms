...
function(l,q){var s=new Asset({id:q.id});s.merge(q);Asset.add(s)});g.call(this)}})},keys:{},create:function(h,g){Asset.callback=g;var j=Asset.generate_uuid();jQuery(".progress").append('<p id="progress-'+j+'">'+h.name+'<span class="percentage"></span></p>');this.xhr=new XMLHttpRequest;this.xhr.upload.addEventListener("loadstart",Asset.onloadstartHandler,false);this.xhr.upload.addEventListener("progress",Asset.onprogressHandler);this.xhr.upload.addEventListener("load",Asset.onloadHandler,false);this.xhr.addEventListener("readystatechange",
Asset.onreadystatechangeHandler,false);this.xhr.upload.uuid=j;this.xhr.upload.filename=h.name;this.xhr.open("POST","/admin/assets.json",true);this.xhr.setRequestHeader("Content-Type","application/octet-stream");this.xhr.setRequestHeader("X-File-Name",h.name);this.xhr.setRequestHeader("X-File-Upload","true");this.xhr.send(h)},onloadstartHandler:function(){console.log("started")},onloadHandler:function(){console.log("success")},onprogressHandler:function(h){var g=h.loaded/h.total*100;jQuery("#progress-"+
h.target.uuid+" .percentage").text(" "+g+"%")},onreadystatechangeHandler:function(h){var g=null;try{g=h.target.status}catch(j){return}if(g=="200"&&h.target.readyState==4&&h.target.responseText){h=JSON.parse(h.target.responseText);g=new Asset({id:h.id});g.merge(h);Asset.add(g);Asset.callback.success&&Asset.callback.success.call(this)}},generate_uuid:function(){for(var h=[],g=0;g<32;g++)h[g]="0123456789ABCDEF".substr(Math.floor(Math.random()*16),1);h[12]="4";h[16]="0123456789ABCDEF".substr(h[16]&3|
8,1);return h.join("")}})}),Page=Model("page",function(){this.persistence(Model.SinatraREST,"/admin/pages");this.include({children:function(){return Page.find_all_by_parent_id(this.id())},has_children:function(){if(this.children().all().length>0)return true},assets:function(){for(var h=this.attr("assets"),g=0,j=h.length;g<j;++g){var l=h[g],q=new Asset({id:l.id});q.merge(l);Asset.add(q)}return Asset},parts:function(){for(var h=this.attr("parts"),g=0,j=h.length;g<j;++g){var l=h[g],q=new Part({id:l.id});
q.merge(l);q.attr("page_id",this.id());Part.add(q)}return Part},load:function(h){var g=this,j="/admin/pages/"+g.id()+".json";jQuery.ajax({type:"GET",url:j,dataType:"json",success:function(l){g.merge(l);h.call(this)}})},saveRemote:function(h,g){var j=this,l="/admin/pages/"+this.id()+".json";jQuery.ajax({type:"PUT",url:l,data:h,dataType:"json",success:function(q){if(q.errors){alert(JSON.stringify(q.errors));g.error&&g.error.call(this)}else{j.merge(q);g.success&&g.success.call(this)}}})}});this.extend({toMustache:function(){return{pages:this.map(function(h){return h.attr()})}},
...
Assets=Sammy(function(){var h=this;this.debug=false;this.disable_push_state=true;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);this.swap=function(g){jQuery("#editor").html(g)};this.helpers({loadAssets:function(g,j){if(Asset.all().length==0)Asset.searchAdmin(g,function(){j&&j.call(this)});else j&&j.call(this)},send_files:function(g,j,l){for(var q=0,s=0;s<g.length;s++)Asset.create(g[s],{progress:function(){},success:function(){q+=1;q==g.length&&Asset.searchAdmin(j,function(){l&&
l.call(this)})}})}});this.bind("run",function(){h.modal=false;h.first_run=true});this.get("#/assets",function(g){var j=g.params.query,l=j?{query:j}:{};l.limit=g.params.limit||48;l.page=g.params.page||1;Galerie.close();h.modal||Asset.searchAdmin(l,function(){g.render("/templates/admin/assets/index.mustache",Asset.toMustache(j)).replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple")})});h.modal=false;h.first_run=false});this.get("#/assets/new",function(g){g.render("/templates/admin/assets/new.mustache").replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple",
"multiple")});h.first_run=false});this.post("/admin/assets",function(g){var j=this,l=document.getElementById("ajax_uploader").files,q=g.params.query?g.params.query:null;jQuery("form#new_asset");var s=q?{query:q}:{};s.limit=g.params.limit||48;s.page=g.params.page||1;this.send_files(l,s,function(){j.render("/templates/admin/assets/index.mustache",Asset.toMustache(s.query)).replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple")})});return false});this.get("#/assets/:id/edit",
function(g){var j=g.params.query?g.params.query:null;this.loadAssets(j?{query:g.params.query}:{},function(){var l=Asset.find(g.params.id);g.render("/templates/admin/assets/edit.mustache",l.toMustacheWithNeighbors(j)).replace("#editor").then(function(){setTimeout(function(){$("img.fade-in").fadeIn("slow")},100);Utilities.formObserver(".image-info input[type=text], .image-info textarea")})});h.modal=false});this.put("#/assets/:id",function(g){var j=Asset.find(g.params.id);j.attr(g.params.asset);j.save(function(l){l&&
Utilities.notice("Successfully saved asset")})});this.get("#/assets/:id/remove",function(g){var j=g.params.query?g.params.query:null,l=j?{query:g.params.query}:null;Galerie.close();Galerie.open(jQuery("<div />").attr({id:"remove-asset-container","class":"wide-modal"}));this.loadAssets(l,function(){var q=Asset.find(g.params.id);g.render("/templates/admin/assets/remove.mustache",{asset:q.toMustache(j)}).replace("#remove-asset-container");h.first_run&&g.render("/templates/admin/assets/index.mustache",
Asset.toMustache(j)).replace("#editor")})});this.del("#/assets/:id",function(g){var j=g.params.query?g.params.query:null,l=j?"?"+decodeURIComponent(jQuery.param({query:j})):"";Asset.find(g.params.id).destroy(function(q){if(q){Galerie.close();Utilities.notice("Successfully saved asset");g.redirect("#/assets"+l)}})})});
