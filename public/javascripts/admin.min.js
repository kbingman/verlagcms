...
j=j.height()+42;j=(t-j)/2;$("#overlay").css({height:t}).hide().fadeIn("fast");o.hide().fadeIn("fast").css({top:j});h&&h.call(this)},close:function(){$("#modal_wrapper").fadeOut("fast",function(){$(this).remove();$("#overlay").fadeOut("fast",function(){$(this).remove()})})}},Utilities={notice:function(i){var h=jQuery(".notice");h.text(i);h.slideDown("fast");setTimeout(function(){h.slideUp("fast")},"2000")},keyboard_nav:function(){jQuery("body").keydown(function(i){switch(i.keyCode){case 83:i=jQuery("form.command-save");
i.submit();return false;case 83:i=jQuery("form.command-save");i.submit();return false;case 37:$("a.previous").click();break;case 39:$("a.next").click();break}})},check_browser_version:function(){if(!Modernizr.history){jQuery("a").live("click",function(h){h.preventDefault();document.location.hash=$(this).attr("href")});var i=jQuery("form#search-form");i.attr("action","#"+i.attr("action"))}},formObserver:function(i){jQuery(i).keyup(function(){delay(function(){jQuery(i).parents("form:first").submit()},
800)})}},iFramer={initialize:function(i){i=jQuery(i);var h=this;i&&i.load(function(){var j=$(this).contents().find("a.verlag-editor");h.setEditFlags(j);j.click(function(){window.top.trigger=$(this);window.top.location.hash=$(this).attr("href");return false})})},setEditFlags:function(i){i.css({position:"relative",display:"block",height:"0",width:"0","text-decoration":"none"}).find("span").css({display:"block",position:"absolute",top:"-24px",left:"12px",padding:"4px 8px",width:"100px",background:"rgba(255, 255, 0, 0.7)"})}},
TabControl={initialize:function(i){if(i=jQuery(i)){i.each(function(h,j){$(j).hide()});i.first().show();this.tabControl(jQuery(".tab-control"))}},tabControl:function(i){i.click(function(){var h="tab-"+$(this).find("label").attr("for").split("-")[1];console.log(h);jQuery(".tab").hide();jQuery("#"+h).show()})}},delay=function(){var i=0;return function(h,j){clearTimeout(i);i=setTimeout(h,j)}}();
Sites=Sammy(function(){var i=this;this.debug=false;this.disable_push_state=true;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);this.helpers({loadSites:function(h){if(Site.all().length==0)Site.load(function(){h&&h.call(this)});else h&&h.call(this)},renderSiteIndex:function(){this.render("/templates/admin/sites/index.mustache",Site.toMustache()).replace("#sidebar")}});this.bind("run",function(){i.modal=false;i.refresh_pages=true});this.get("#/preview",function(h){window.open("/",
"preview");h.redirect("admin/#/")});this.get("#/sites",function(h){i.refresh_pages=true;Galerie.close();jQuery("#editor").html('<h1 class="section">Sites</div>');h.loadSites(function(){h.renderSiteIndex()})});this.get("#/sites/new",function(h){h.loadSites(function(){$("#modal").length==0&&Galerie.open();h.render("/templates/admin/sites/new.mustache").replace("#modal");h.renderSiteIndex(Site.all())})});this.post("#/sites",function(h){(new Site(h.params.site)).save(function(j,o){var t=JSON.parse(o.responseText);
...
this.post("#/templates",function(h){Layout.create(h.params.template,{success:function(){h.redirect("#/templates");Utilities.notice("Successfully saved template")},error:function(){Utilities.notice("errors creating template");alert("hey")}})});this.get("#/templates/:id/edit",function(h){Galerie.close();i.refresh_pages=true;this.loadLayouts(function(){var j=Layout.find(h.params.id);h.renderLayout(j);h.renderLayoutIndex(Layout.all())})});this.put("#/templates/:id",function(h){var j=Layout.find(h.params.id);
j.attr(h.params.layout);j.save(function(o){if(o){h.renderLayoutIndex();Utilities.notice("Successfully saved template")}})});this.get("#/templates/:id/remove",function(h){this.loadLayouts(function(){var j=Layout.find(h.params.id);Galerie.open();h.render("/templates/admin/templates/remove.mustache",{layout:j.asJSON()}).replace("#modal");h.renderLayoutIndex(Layout.all())})});this.del("/templates/:id",function(h){Layout.find(h.params.id).deleteRemote(function(){h.redirect("#/templates")})});this.post("/layouts/:id/parts",
function(h){this.loadLayouts(function(){var j=h.params.id,o=h.params.part;alert(JSON.stringify(h.params.part));Part.create(o,function(){h.redirect("#/templates/"+j+"/edit")})})})});
...
{parent:o.asJSON()}).replace("#new-page-container")})});this.post("#/pages/:page_id",function(j){Page.find(j.params.page_id);Page.create(j.params.page,function(o){h.refresh_pages=true;Utilities.notice("Successfully saved page");j.redirect("#/pages/"+o.id+"/edit")})});this.get("#/pages/:id",function(j){Galerie.close();this.loadPages(function(){var o=j.params.id,t=Page.find(o);if(!h.do_not_refresh){if(t)j.renderPagePreview(t);else{t=new Page({id:o});t.load(function(){j.renderPagePreview(t)})}h.modal=
false;h.do_not_refresh=false;if(h.refresh_pages){j.renderTree(Page.root());h.refresh_pages=false}}});h.do_not_refresh=false});this.get("#/pages/:id/edit",function(j){Galerie.close();h.modal=true;this.loadPages(function(){var o=j.params.id,t=Page.find(o);if(t)j.renderPage(t);else{t=new Page({id:o});t.load(function(){j.renderPage(t)})}if(h.refresh_pages){j.renderTree(Page.root());h.refresh_pages=false}})});this.put("#/pages/:page_id",function(j){var o=j.params.page_id,t=Page.find(o),u=$("#edit-page");
t.attr(j.params.page);t.saveRemote(u.serialize(),{success:function(){h.modal=false;Utilities.notice("Successfully saved page");j.redirect("#/pages/"+o+"/edit")}})});this.get("#/pages/:id/remove",function(j){this.loadPages(function(){var o=Page.find(j.params.id),t=$("<div />").attr({id:"remove-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(t);h.refresh_pages&&j.renderTree(Page.root());j.render("/templates/admin/pages/remove.mustache",{page:o.asJSON()}).replace("#remove-page-container")})});
this.del("/pages/:id",function(j){var o=j.params.id;Page.find(o).deleteRemote(function(){jQuery("#page-"+o).remove();h.refresh_pages=false;j.redirect("#/pages")})});this.get("/pages/:id/results",function(j){this.loadPages(function(){var o=Page.find(j.params.id);Asset.searchAdmin(j.params.query,function(){Asset.each(function(t){t.attr("current_page_id",o.id());t.save()});j.render("/templates/admin/pages/search_results.mustache",Asset.toMustache()).replace("#search-results-container")})})});this.get("#/pages/:page_id/assets/:id/add",
function(j){this.loadPages(function(){var o=j.params.page_id,t=Page.find(o),u=Asset.find(j.params.id),x=jQuery("#page-asset-ids");x.attr("value");var G=x.attr("value")+","+u.id();x.attr("value",G);t.saveRemote({page:{assets_list:G}},{success:function(){var C=jQuery('<div class="asset" />').attr("id","page-asset-"+u.id()),z=jQuery('<a href="#/assets/'+u.id()+'/edit"></a>'),M=jQuery('<br /><a href="#/pages/'+t.id()+"/assets/"+u.id()+'/remove">Remove</a>');z.append('<img src="/images/icons/'+u.id()+
"/"+u.attr("file_name")+'" alt="'+u.attr("title")+'" /></a>');C.html(z);C.append(M);jQuery("#page-assets").append(C);C=jQuery("#preview iframe");C.attr("src",C.attr("src"));h.do_not_refresh=true;j.redirect("#/pages/"+o+"/edit")}})})});this.get("#/pages/:page_id/assets/:id/remove",function(j){this.loadPages(function(){var o=j.params.id,t=j.params.page_id,u=Page.find(t),x=jQuery("#page-asset-ids"),G=x.attr("value").split(","),C=G.indexOf(o);C!=-1&&G.splice(C,1);x.attr("value",G);u.saveRemote({page:{assets_list:G.join(",")}},
{success:function(){jQuery("#page-asset-"+o).fadeOut("fast",function(){$(this).remove()});var z=jQuery("#preview iframe");z.attr("src",z.attr("src"));h.do_not_refresh=true;j.redirect("#/pages/"+t+"/edit")}})})})});
jQuery(document).ready(function(){jQuery("#login").length||Pages.run("#/pages");jQuery("#ajax_uploader").attr("multiple","multiple");jQuery(".js-only").show();Utilities.keyboard_nav();Utilities.check_browser_version();var i=jQuery("#tabs a");i.live("click",function(){i.removeClass("active");$(this).addClass("active")})});
