...
setTimestamp:function(){Verlag.timestamp=(new Date).getTime()},keyboard_nav:function(){jQuery("body").keydown(function(c){switch(c.keyCode){case 37:$("a.previous").click();break;case 39:$("a.next").click();break}})},check_browser_version:function(){},formObserver:function(){},unique:function(c){tmp=[];for(i=0;i<c.length;i++)if(!Utilities.contains(tmp,c[i])){tmp.length+=1;tmp[tmp.length-1]=c[i]}return tmp},contains:function(c,a){for(j=0;j<c.length;j++)if(c[j]==a)return true;return false}},Loader={start:function(c){c=
jQuery("#loader");if(c.length){c.show();Loader.timer=setInterval(function(){var a=c.css("background-position-y").replace("px","");c.css({"background-position-y":a-40+"px"})},67)}},stop:function(c){c=jQuery("#loader");if(c.length){c.hide();clearInterval(Loader.timer)}}},Updater={setup:function(c,a){jQuery.each(c,function(f,d){var h=new a({id:d.id});h.merge(d);a.add(h)})},update:function(){window.ninja=true;jQuery.ajax({url:"/admin/activity.json",type:"POST",data:{updated:Verlag.timestamp},success:function(c){jQuery.each(c.models,
function(a,f){Page.find(f.id).merge(f);Utilities.setTimestamp()});window.ninja=false}})}},ace_modes={javascript:require("ace/mode/javascript").Mode,html:require("ace/mode/html").Mode,css:require("ace/mode/css").Mode,scss:require("ace/mode/scss").Mode,sass:require("ace/mode/scss").Mode,textile:require("ace/mode/textile").Mode,none:require("ace/mode/scss").Mode},delay=function(){var c=0;return function(a,f){clearTimeout(c);c=setTimeout(a,f)}}(),Editor={initialize:function(){var c=jQuery("#editor-buttons input[type=button]"),
...
selected:a.attr("filter")=="css"?'selected="selected"':""},{name:"Sass",value:"sass",selected:a.attr("filter")=="sass"?'selected="selected"':""},{name:"SCSS",value:"scss",selected:a.attr("filter")=="scss"?'selected="selected"':""},{name:"Partial",value:"none",selected:a.attr("filter")=="none"?'selected="selected"':""}]},"mustache").replace("#editor").then(function(){var f=a.attr("mode");f=ace_modes[f];window.editor=ace.edit("layout_content");window.editor.setTheme("ace/theme/textmate");window.editor.getSession().setMode(new f);
...
f.load(function(){a.renderPagePreview(f)})}a.trigger("page-index",f)});this.get("/admin/pages/:id/edit/?",function(a){var f=Page.find(a.params.id);a.renderPageEditor(f,function(){a.renderPageProperties(f)});a.trigger("page-index",f)});this.put("/admin/pages/:page_id",function(a){var f=Page.find(a.params.page_id),d=jQuery("span.timestamp").text();f.attr(a.params.page);d<Verlag.timestamp?alert("Someone else has edited this page, please reload and try again"):f.save(function(h){if(h){Utilities.setTimestamp();
...
console.log("HEY");console.log(f);Page.remove(f);f.destroy(function(d){d&&a.redirect("/admin/pages")})})}),Parts=Sammy(function(c){c.helpers({renderPart:function(a,f,d){var h=this;h.load(jQuery("script#admin-"+d+"-edit")).interpolate({part:a.asJSON(),page:f.asJSON(),assets:Asset.asJSON(),timestamp:Verlag.timestamp},"mustache").replace("#page-tabs-"+f.id()).then(function(){var l=ace_modes.textile;window.editor=ace.edit("part-"+a.id()+"-content");window.editor.setTheme("ace/theme/textmate");window.editor.getSession().setMode(new l);
...
Verlag.timestamp=a.now}})}require("pilot/canon").addCommand({name:"save",bindKey:{win:"Ctrl-S",mac:"Command-S",sender:"editor"},exec:function(){jQuery("form.command-save").submit()}})});function children(c){var a=new Page({id:c.id});a.merge(c);Page.add(a);a.children()};
