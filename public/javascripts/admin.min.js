...
Assets=Sammy(function(){var h=this;this.debug=false;this.disable_push_state=true;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);this.swap=function(g){jQuery("#editor").html(g)};this.helpers({loadAssets:function(g,j){if(Asset.all().length==0)Asset.searchAdmin(g,function(){j&&j.call(this)});else j&&j.call(this)},send_files:function(g,j,l){for(var q=0,s=0;s<g.length;s++)Asset.create(file,{progress:function(){},success:function(){q+=1;Asset.searchAdmin(j,function(){l&&l.call(this)})}})}});
