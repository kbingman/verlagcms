...
value:"scss",selected:a.attr("filter")=="scss"?'selected="selected"':""}]},"mustache").replace("#editor").then(function(){var e=a.attr("mode");e=aceModes[e];window.editor=ace.edit("layout_content");window.editor.setTheme("ace/theme/textmate");window.editor.getSession().setMode(new e);window.editor.session.setUseWrapMode(true);window.editor.getSession().setValue(a.attr("content"))})}});this.bind("run",function(){b.refresh_templates=true;b.refresh_pages=true;b.modal=false});this.get("/admin/templates",
function(a){b.refresh_templates=false;jQuery("#editor").html('<h1 class="section">Templates</div>');a.renderLayoutIndex(Layout.all())});this.get("/admin/templates/new/:klass",function(a){var e=$("<div />").attr({id:"new-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(e);a.load(jQuery("#admin-templates-new")).interpolate({klass:a.params.klass},"mustache").replace("#new-page-container");a.renderLayoutIndex(Layout.all())});this.post("/admin/templates",function(a){(new Layout(a.params.template)).save(function(e,
g){var i=JSON.parse(g.responseText);if(i.errors)Utilities.notice(JSON.stringify(i));else{a.redirect("/admin/templates");Utilities.notice("Successfully saved template")}})});this.get("/admin/templates/:id/edit",function(a){b.refresh_pages=true;var e=Layout.find(a.params.id);a.renderLayout(e);a.renderLayoutIndex(Layout.all())});this.put("/admin/templates/:id",function(a){var e=Layout.find(a.params.id);a.params.layout.content=window.editor.getSession().getValue();e.attr(a.params.layout);e.save(function(g){if(g){a.renderLayoutIndex();
Utilities.notice("Successfully saved template");g=aceModes[e.attr("mode")];window.editor.getSession().setUseSoftTabs(true);window.editor.getSession().setTabSize(2);window.editor.getSession().setMode(new g)}})});this.get("/admin/templates/:id/remove",function(a){var e=Layout.find(a.params.id);Galerie.open();a.load(jQuery("#admin-templates-remove")).interpolate({layout:e.asJSON()},"mustache").replace("#modal");a.renderLayoutIndex(Layout.all())});this.del("/templates/:id",function(a){Layout.find(a.params.id).destroy(function(e){if(e){a.renderLayoutIndex();
Utilities.notice("Successfully deleted template");a.redirect("/admin/templates")}})});this.post("/layouts/:id/parts",function(a){this.loadLayouts(function(){var e=a.params.id;Part.create(a.params.part,function(){a.redirect("/admin/templates/"+e+"/edit")})})})}),Assets=Sammy(function(b){var a=this;b.helpers({renderFolderTree:function(e){if(jQuery("ul#folders").length)e&&e.call(this);else this.load(jQuery("script#admin-folders-index")).interpolate({folders:Folder.asJSON()},"mustache").replace("#sidebar").then(function(){e&&
e.call(this)})},send_files:function(e,g,i){for(var l=0,o=0;o<e.length;o++){var w=e[o];Asset.create(w,{before:function(C){jQuery(".progress").append('<p id="progress-'+C+'">'+w.name+'<span class="percentage"></span></p>')},progress:function(C,B){jQuery("#progress-"+C+" .percentage").text(" "+B+"%")},success:function(){l+=1;l==e.length&&Asset.searchAdmin(g,function(){i&&i.call(this)})}})}}});b.bind("render-asset",function(e,g,i){this.load(jQuery("script#admin-assets-edit")).interpolate(g.toMustacheWithNeighbors(i),
"mustache").appendTo("body").then(function(){Utilities.loadModal("div#asset-editor",function(){jQuery("div#asset-editor").fadeIn("fast")})})});b.bind("render-index",function(e,g){var i=this;i.load(jQuery("script#admin-assets-index")).interpolate(Asset.toMustache(g),"mustache").replace("#editor").then(function(){jQuery("a.info-icon").click(function(l){l.preventDefault();current_asset_id=this.id.split("-")[2];i.trigger("show_info",{current_asset_id:current_asset_id})});jQuery("#ajax_uploader").attr("multiple",
"multiple").change(function(){jQuery(this).parents("form:first").submit()});jQuery("a.remove-icon").click(function(l){l.preventDefault();current_asset_id=this.id.split("-")[2];i.trigger("show_remove_dialog",{current_asset_id:current_asset_id})})})});b.bind("show_info",function(e,g){var i=Asset.find(g.current_asset_id),l=jQuery("#asset-"+i.id()),o=l.find(".info");jQuery(".modal-strip").remove();o.length||this.load(jQuery("#admin-assets-info")).interpolate({asset:i.toMustache()},"mustache").appendTo(l).then(function(){jQuery(".modal-strip").fadeIn("fast")})});
b.bind("show_remove_dialog",function(e,g){var i=Asset.find(g.current_asset_id),l=jQuery("#asset-"+i.id()),o=l.find(".remove");jQuery(".modal-strip").remove();o.length||this.load(jQuery("script#admin-assets-remove")).interpolate({asset:i.toMustache()},"mustache").appendTo(l).then(function(){jQuery(".modal-strip").fadeIn("fast")})});b.get("/admin/assets",function(e){jQuery("#overlay").remove();var g=e.params.query?e.params.query:"",i={query:g};i.limit=e.params.limit||48;i.page=e.params.page||1;e.renderFolderTree();
Asset.searchAdmin(i,function(){e.trigger("render-index",g)})});b.get("/admin/folders/:id",function(e){jQuery("#overlay").remove();var g=e.params.id,i=Folder.find(g);e.renderFolderTree(function(){jQuery("ul#folders li").removeClass("active");jQuery("#folder-"+g).addClass("active")});i.loadAssets(function(){e.trigger("render-index","")})});b.get("/admin/assets/new",function(e){e.load(jQuery("script#admin-assets-new")).interpolate({},"mustache").replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple",
