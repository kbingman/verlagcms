...
function(l,q){var s=new Asset({id:q.id});s.merge(q);Asset.add(s)});g.call(this)}})},keys:{},create:function(h,g){Asset.callback=g;var j=Asset.generate_uuid();jQuery(".progress").append('<p id="progress-'+j+'">'+h.name+'<span class="percentage"></span></p>');xhr.upload.uuid=j;xhr.upload.filename=h.name;xhr=new XMLHttpRequest;xhr.upload.addEventListener("loadstart",Asset.onloadstartHandler,false);xhr.upload.addEventListener("progress",Asset.onprogressHandler);xhr.upload.addEventListener("load",Asset.onloadHandler,
...
g+"%")},onreadystatechangeHandler:function(h){var g=null;try{g=h.target.status}catch(j){return}if(g=="200"&&h.target.readyState==4&&h.target.responseText){h=JSON.parse(h.target.responseText);g=new Asset({id:h.id});g.merge(h);Asset.add(g);Asset.callback.success&&Asset.callback.success.call(this)}},generate_uuid:function(){for(var h=[],g=0;g<32;g++)h[g]="0123456789ABCDEF".substr(Math.floor(Math.random()*16),1);h[12]="4";h[16]="0123456789ABCDEF".substr(h[16]&3|8,1);return h.join("")}})}),Page=Model("page",
