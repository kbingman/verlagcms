...
this.extend({asJSON:function(){return Folder.map(function(a){return a.attr()})}})}),Page=Model("page",function(){this.persistence(Model.SinatraREST,"/admin/pages");this.include({initialize:function(){},children:function(){return Page.find_all_by_parent_id(this.id())},childrenAsJSON:function(){return this.children().map(function(a){return a.attr()})},setPartContent:function(a,b){var d=this.attr("contents"),f;jQuery.each(d,function(h,j){if(a==j.name){j.content=b;f=j}});return f},findPartByName:function(a){var b=
this.attr("contents"),d;jQuery.each(b,function(f,h){if(a==h.name){d=h;return false}});return d},findPartById:function(a){var b=this.attr("contents"),d;jQuery.each(b,function(f,h){if(a==h.id){d=h;return false}});return d},getChildren:function(a){var b="/admin/pages/"+this.id()+"/children.json";jQuery.ajax({type:"GET",url:b,dataType:"json",success:function(d){$.each(d,function(f,h){var j=new Page({id:h.id});j.merge(h);Page.add(j)});a&&a.call(this,d)}})},has_children:function(){if(this.children().all().length>
0)return true},parent:function(){return Page.find(this.attr("parent_id"))},assets:function(){for(var a=this.attr("assets"),b=0,d=a.length;b<d;++b){var f=a[b],h=new Asset({id:f.id});h.merge(f);Asset.add(h)}return Asset},parts:function(){var a=this.attr("contents"),b=a.length;Part.each(function(){Part.remove(this)});for(var d=0;d<b;++d){var f=a[d],h=new Part({id:f.id});h.merge(f);h.attr("page_id",this.id());Part.add(h)}return Part},load:function(a){var b=this,d="/admin/pages/"+b.id()+".json";jQuery.ajax({type:"GET",
url:d,dataType:"json",success:function(f){b.merge(f);a.call(this)}})}});this.extend({toMustache:function(){return{pages:this.map(function(a){return a.attr()})}},root:function(){return this.detect(function(){return this.attr("parent_id")==null})},find_by_parent_id:function(a){return this.detect(function(){return this.attr("parent_id")==a})},find_all_by_parent_id:function(a){return this.select(function(){return this.attr("parent_id")==a})},find_by_path:function(a){return this.detect(function(){return this.attr("path")==
a})},load_by_id:function(a,b){jQuery.ajax({type:"GET",url:"/admin/pages/"+a+".json",dataType:"json",success:function(d){var f=Page.find(d.id);f||(f=new Page({id:d.id}));f.merge(d);Page.add(f);b&&b.call(this,d)}})},create:function(a,b){jQuery.ajax({type:"post",url:"/admin/pages.json",dataType:"json",data:{page:a},success:function(d){var f=new Page({id:d.id});f.merge(d);Page.add(f);b.call(this,d)}})}})}),Layout=Model("layout",function(){this.persistence(Model.SinatraREST,"/admin/templates");this.include({load:function(a){var b=
this,d="/admin/templates/"+b.id()+".json";jQuery.ajax({type:"GET",url:d,dataType:"json",success:function(f){b.merge(f);a.call(this,f)}})}});this.extend({toMustache:function(){return{layouts:this.map(function(a){return a.attr()})}},asLayoutJSON:function(a){return Layout.find_all_by_class("Layout").map(function(b){a==b.id()?b.attr("selected",'selected="selected"'):b.attr("selected","");return b.attr()})},find_by_class:function(a){return this.detect(function(){return this.attr("klass")==a})},find_all_by_class:function(a){return this.select(function(){return this.attr("klass")==
a})},asJSON_by_class:function(a){return this.find_all_by_class(function(){return a.map(function(b){return b.attributes})})}})}),Part=Model("part",function(){this.include({deleteRemote:function(a,b){var d="/admin/pages/"+a.id()+"/parts/"+this.id()+".json";jQuery.ajax({type:"DELETE",url:d,dataType:"json",success:function(f){a.merge(f);b.call(this)}})}});this.extend({toMustache:function(){return{parts:this.map(function(a){return a.attr()})}},create:function(a,b){var d="/admin/templates/"+a.template_id+
"/parts.json",f=Layout.find(a.template_id);jQuery.ajax({type:"post",url:d,dataType:"json",data:{part:a},success:function(h){f.merge(h);b.call(this)}})}})}),Site=Model("site",function(){this.persistence(Model.SinatraREST,"/admin/sites");this.include({});this.extend({toMustache:function(){return{sites:this.map(function(a){return a.attr()})}}})}),User=Model("user",function(){this.persistence(Model.SinatraREST,"/admin/users");this.include({});this.extend({toMustache:function(){return{users:this.map(function(a){return a.attr()})}}})}),
Galerie={open:function(a,b){$("body").append('<div id="overlay"></div>').append('<div id="modal_wrapper"><div id="modal"></div></div>');var d=$("#modal");d.html(a);var f=$("#modal_wrapper"),h=$(window).height();d=d.height()+42;d=(h-d)/2;$("#overlay").css({height:h}).hide().fadeIn("fast");f.hide().fadeIn("fast").css({top:d});b&&b.call(this)},close:function(){$("#modal_wrapper").fadeOut("fast",function(){$(this).remove();$("#overlay").fadeOut("fast",function(){$(this).remove()})})}},logger={info:function(a){window.console&&
console.log(a)},debug:function(a){window.console&&console.log(a)}},Utilities={setNonHistoryLinks:function(){Modernizr.history||jQuery("a").click(function(a){a.preventDefault();if((a=jQuery(this).attr("href"))&&!a.match("http://")&&!a.match(/^#/)){console.log("#"+a);document.location.hash=$(this).attr("#href")}})},loadModal:function(a,b){var d=this,f=jQuery(a);if(f.length){Loader.start();var h=f.find("img");setTimeout(function(){f.height()<20?h.load(function(){d.resizeModal(f,b)}):d.resizeModal(f,
b)},13)}},resizeModal:function(a,b){var d=a.width(),f=a.height();d=d/f;jQuery(window).width();var h=jQuery(window).height();f>h-40?a.height(h-40).width((h-40)*d):a.css({"margin-top":(h-f)/2});Loader.stop();b&&b.call(this)},notice:function(a,b){b=b||{};var d=jQuery(".notice"),f=b["class"]||"message";d.html(a).addClass(f).fadeIn("fast",function(){setTimeout(function(){b.persist||d.fadeOut("slow")},1800)})},hideNotice:function(){jQuery(".notice").fadeOut("slow")},setTimestamp:function(){window.timestamp=
(new Date).getTime()},keyboard_nav:function(){jQuery("body").keydown(function(a){switch(a.keyCode){case 37:$("a.previous").click();break;case 39:$("a.next").click();break}})},check_browser_version:function(){},formObserver:function(){}},Loader={start:function(a){a=jQuery("#loader");if(a.length){a.show();Loader.timer=setInterval(function(){var b=a.css("background-position-y").replace("px","");a.css({"background-position-y":b-40+"px"})},67)}},stop:function(a){a=jQuery("#loader");if(a.length){a.hide();
clearInterval(Loader.timer)}}},Updater={setup:function(a,b){jQuery.each(a,function(d,f){var h=new b({id:f.id});h.merge(f);b.add(h)})},update:function(){window.ninja=true;jQuery.ajax({url:"/admin/activity.json",type:"POST",data:{updated:window.timestamp},success:function(a){jQuery.each(a.models,function(b,d){Page.find(d.id).merge(d);Utilities.setTimestamp()});window.ninja=false}})}},aceModes={javascript:require("ace/mode/javascript").Mode,html:require("ace/mode/html").Mode,css:require("ace/mode/css").Mode,
scss:require("ace/mode/scss").Mode,sass:require("ace/mode/scss").Mode,textile:require("ace/mode/textile").Mode,none:require("ace/mode/text").Mode},delay=function(){var a=0;return function(b,d){clearTimeout(a);a=setTimeout(b,d)}}(),Editor={initialize:function(){var a=jQuery("#testForm input[type=button]"),b=jQuery("#testForm input#save");a.click(function(){var d=jQuery(this).data("cmd"),f=jQuery(this).data("value");if(f=="promptUser")f=prompt(jQuery(this).data("text"));var h=jQuery("iframe")[0];d&&
h.contentDocument.execCommand(d,false,f)});b.click(function(){var d=document.location.pathname.split("/"),f=Page.find(d[d.length-1]),h={};jQuery("iframe").contents().find("div.editable").each(function(){var j=jQuery(this),m=j.attr("id").split("_")[0];h[m]=j.html()});jQuery.each(h,function(j,m){m=Editor.textilize(m);f.setPartContent(j,m);console.log(m)});f.save()})},textilize:function(a,b){a=a.replace(/\s*<p>((.|[\r\n])*?)<\/p>\s*/gi,"\n\n$1\n\n\n");a=a.replace(/\s*<h1>((.|[\r\n])*?)<\/h1>\s*/gi,"h1. $1\n\n");
a=a.replace(/\s*<h2>((.|[\r\n])*?)<\/h2>\s*/gi,"h2. $1\n\n");a=a.replace(/\s*<h3>((.|[\r\n])*?)<\/h3>\s*/gi,"h3. $1\n\n");a=a.replace(/\s*<h4>((.|[\r\n])*?)<\/h4>\s*/gi,"h4. $1\n\n");a=a.replace(/\s*<h5>((.|[\r\n])*?)<\/h5>\s*/gi,"h5. $1\n\n");a=a.replace(/\s*<h6>((.|[\r\n])*?)<\/h6>\s*/gi,"h6. $1\n\n");a=a.replace(/<(?:b|strong)>((.|[\r\n])*?)<\/(?:b|strong)>/gi,"*$1*");a=a.replace(/<(?:i|em)>((.|[\r\n])*?)<\/(?:i|em)>/gi,"_$1_");a=a.replace(/<(?:strike|del)>((.|[\r\n])*?)<\/(?:strike|del)>/gi,"-$1-");
a=a.replace(/<(?:u|ins)>((.|[\r\n])*?)<\/(?:u|ins)>/gi,"+$1+");a=a.replace(/<a href="(.*?)">((.|[\r\n])*?)<\/a>/gi,'"$2":$1');a=a.replace(/<code.*?>((.|[\r\n])*?)<\/code>/gi,"@$1@");if(b)a=a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");a=a.replace(/(\r\n|\n){3,}/g,"\n\n");return a=a.replace(/^[\r\n]+|[\r\n]+$/g,"")}},iFramer={initialize:function(a,b){var d=jQuery(a);jQuery("#loader");if(d.length){Loader.start();d.load(function(){var f=$(this),h=f.contents();h.find("div.editable").attr("contenteditable",
"true").css({background:"hsla(30, 17.7%, 61%, 0.3)"});Loader.stop();f.fadeIn("fast");h.find("a").click(function(j){var m=jQuery(this).attr("href").split("?")[0].replace("/preview","");if(m=Page.find_by_path(m)){j.preventDefault();history.pushState({path:m.attr("title")},m.attr("title"),m.attr("admin_path"))}});b&&b.call(this)})}},setEditFlags:function(a){a.css({position:"relative",display:"block",height:"0",width:"0","text-decoration":"none"}).find("span").css({display:"block",position:"absolute",
top:"-24px",left:"12px",padding:"4px 8px",width:"100px",background:"rgba(255, 255, 0, 0.7)"})}},Base=Sammy(function(a){a.debug=false;a.use(Sammy.JSON);a.use(Sammy.Mustache);a.use(Sammy.NestedParams);a.template_engine="mustache";a.helpers({setup:function(b,d){jQuery.each(b,function(f,h){var j=new d({id:h.id});j.merge(h);d.add(j)})},update:function(){var b=this;window.ninja=true;jQuery.ajax({url:"/admin/activity.json",type:"POST",data:{updated:window.timestamp},success:function(d){jQuery.each(d.models,
function(f,h){h.class_name=="Page"&&b.trigger("update-page",h);h.class_name=="Layout"&&b.trigger("update-layout",h);Utilities.setTimestamp()});window.ninja=false}})}});a.bind("run",function(){var b=this;jQuery("#sidebar .opener").live("click",function(){b.trigger("toggle-children",this);return false});b.trigger("toggle-sidebar")});a.bind("set-active-tab",function(b,d){var f=jQuery("div#tabs a.tab"),h=d.split("?")[0].split("/")[2];h=jQuery("#"+h+"-tab");f.removeClass("active");h.addClass("active")});
a.bind("update-page",function(b,d){var f=Page.find(d.id);f.merge(d);f.save();jQuery("li#page-"+d.id).find("span.title a").text(f.attr("title"));jQuery("h1#page-title-"+f.id()).text(f.attr("title"));jQuery("#page-iframe-"+f.id()).length&&Utilities.notice('<a class="page-reload" href="'+f.attr("admin_path")+'">This page has been changed. Click here to reload.</span>',{persist:true,"class":"warning"})});a.bind("update-layout",function(b,d){var f=Layout.find(d.id);f.merge(d);f.save();jQuery("li#layout-"+
d.id).find("span.title a").text(f.attr("name"))});a.bind("sanskrit",function(b,d){if(d.length){var f=d.find("textarea.sanskrit");f.each(function(h,j){new Sanskrit(j,{toolbar:{actions:{strong:"B",em:"I",ins:"ins",del:"del",link:"link",unlink:"unlink",textile:"Textile"}}})});setTimeout(function(){f.fadeIn("fast")},420)}});a.bind("toggle-sidebar",function(){var b=jQuery("div#sidebar"),d=jQuery("div#editor"),f=jQuery("p#toggle");f.find("a").click(function(h){h.preventDefault();if(b.hasClass("closed")){h=
"300";jQuery.cookie("sidebar_toggled","")}else{h="24";jQuery.cookie("sidebar_toggled","closed")}Modernizr.cssanimations&&console.log("Move.js goes here");b.animate({width:h},200).toggleClass("closed");d.animate({left:h},200).toggleClass("open");f.animate({left:h},200).toggleClass("open")})});a.before(function(){Galerie.close()});a.before(function(b){var d=b.path;b&&b.verb=="get"&&b.trigger("set-active-tab",d)})}),Sites=Sammy(function(){var a=this;this.helpers({loadSites:function(b){if(Site.all().length==
0)Site.load(function(){b&&b.call(this)});else b&&b.call(this)},renderSiteIndex:function(b){this.load(jQuery("script#admin-sites-index")).interpolate(Site.toMustache(),"mustache").replace("#editor").then(function(){b&&b.call(this)})}});this.bind("run",function(){a.modal=false;a.refresh_pages=true});this.get("#/preview",function(){alert(document.location.hostname);window.open("/","preview")});this.get("/admin/sites",function(b){a.refresh_pages=true;jQuery("#editor").html('<h1 class="section">Sites</div>');
b.loadSites(function(){b.renderSiteIndex()})});this.get("/admin/sites/new",function(b){b.loadSites(function(){$("#modal").length==0&&Galerie.open();b.load(jQuery("script#admin-sites-new")).interpolate({},"mustache").replace("#modal");b.renderSiteIndex(Site.all())})});this.post("/admin/sites",function(b){(new Site(b.params.site)).save(function(d,f){var h=JSON.parse(f.responseText);h.errors?alert(JSON.stringify(h)):b.redirect("#/sites")})});this.get("/admin/sites/:id/edit",function(b){b.loadSites(function(){site=
Site.find(b.params.id);if(jQuery("#sites").length){jQuery(".site-form").html("");b.load(jQuery("script#admin-sites-edit")).interpolate({site:site.asJSON()},"mustache").replace("#site-form-"+site.id())}else b.renderSiteIndex(function(){jQuery(".site-form").html("");b.load(jQuery("script#admin-sites-edit")).interpolate({site:site.asJSON()},"mustache").replace("#site-form-"+site.id())})})});this.put("/admin/sites/:id",function(b){var d=Site.find(b.params.id);d.attr(b.params.site);d.save(function(f,h){var j=
JSON.parse(h.responseText);j.errors?alert(JSON.stringify(j)):b.redirect("#/sites")})});this.get("/admin/sites/:id/remove",function(b){Galerie.open();b.loadSites(function(){site=Site.find(b.params.id);b.load(jQuery("script#admin-sites-remove")).interpolate({site:site.asJSON()},"mustache").replace("#modal");b.renderSiteIndex(Site.all())})});this.del("/admin/sites/:id",function(b){site.destroy(function(d){d?b.redirect("#/sites"):alert("there were issues")})})}),Layouts=Sammy(function(){var a=this;this.helpers({loadLayouts:function(b){b&&
b.call(this)},renderLayoutIndex:function(){this.load(jQuery("script#admin-templates-index")).interpolate({layouts:Layout.find_all_by_class("Layout").map(function(b){return b.attributes}),partials:Layout.find_all_by_class("Partial").map(function(b){return b.attributes}),javascripts:Layout.find_all_by_class("Javascript").map(function(b){return b.attributes}),stylesheets:Layout.find_all_by_class("Stylesheet").map(function(b){return b.attributes})},"mustache").replace("#sidebar")},renderLayout:function(b){this.load(jQuery("script#admin-templates-edit")).interpolate({layout:b.asJSON(),
filters:[{name:"none",value:"none",selected:b.attr("filter")=="css"?'selected="selected"':""},{name:"Sass",value:"sass",selected:b.attr("filter")=="sass"?'selected="selected"':""},{name:"Scss",value:"scss",selected:b.attr("filter")=="scss"?'selected="selected"':""}]},"mustache").replace("#editor").then(function(){var d=b.attr("mode");d=aceModes[d];window.editor=ace.edit("layout_content");window.editor.setTheme("ace/theme/textmate");window.editor.getSession().setMode(new d);window.editor.session.setUseWrapMode(true);
window.editor.getSession().setValue(b.attr("content"))})}});this.bind("run",function(){a.refresh_templates=true;a.refresh_pages=true;a.modal=false});this.get("/admin/templates",function(b){a.refresh_templates=false;jQuery("#editor").html('<h1 class="section">Templates</div>');b.renderLayoutIndex(Layout.all())});this.get("/admin/templates/new/:klass",function(b){var d=$("<div />").attr({id:"new-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(d);b.load(jQuery("#admin-templates-new")).interpolate({klass:b.params.klass},
"mustache").replace("#new-page-container");b.renderLayoutIndex(Layout.all())});this.post("/admin/templates",function(b){(new Layout(b.params.template)).save(function(d,f){var h=JSON.parse(f.responseText);if(h.errors)Utilities.notice(JSON.stringify(h));else{b.redirect("/admin/templates");Utilities.notice("Successfully saved template")}})});this.get("/admin/templates/:id/edit",function(b){a.refresh_pages=true;var d=Layout.find(b.params.id);b.renderLayout(d);b.renderLayoutIndex(Layout.all())});this.put("/admin/templates/:id",
function(b){var d=Layout.find(b.params.id);b.params.layout.content=window.editor.getSession().getValue();d.attr(b.params.layout);d.save(function(f){if(f){b.renderLayoutIndex();Utilities.notice("Successfully saved template");f=aceModes[d.attr("mode")];window.editor.getSession().setUseSoftTabs(true);window.editor.getSession().setTabSize(2);window.editor.getSession().setMode(new f)}})});this.get("/admin/templates/:id/remove",function(b){var d=Layout.find(b.params.id);Galerie.open();b.load(jQuery("#admin-templates-remove")).interpolate({layout:d.asJSON()},
"mustache").replace("#modal");b.renderLayoutIndex(Layout.all())});this.del("/templates/:id",function(b){Layout.find(b.params.id).destroy(function(d){if(d){b.renderLayoutIndex();Utilities.notice("Successfully deleted template");b.redirect("/admin/templates")}})});this.post("/layouts/:id/parts",function(b){this.loadLayouts(function(){var d=b.params.id;Part.create(b.params.part,function(){b.redirect("/admin/templates/"+d+"/edit")})})})}),Assets=Sammy(function(a){var b=this;a.helpers({renderFolderTree:function(d){if(jQuery("ul#folders").length)d&&
d.call(this);else this.load(jQuery("script#admin-folders-index")).interpolate({folders:Folder.asJSON()},"mustache").replace("#sidebar").then(function(){d&&d.call(this)})},send_files:function(d,f,h){for(var j=0,m=0;m<d.length;m++){var p=d[m];Asset.create(p,{before:function(v){jQuery(".progress").append('<p id="progress-'+v+'">'+p.name+'<span class="percentage"></span></p>')},progress:function(v,s){jQuery("#progress-"+v+" .percentage").text(" "+s+"%")},success:function(){j+=1;j==d.length&&Asset.searchAdmin(f,
function(){h&&h.call(this)})}})}}});a.bind("render-asset",function(d,f,h){this.load(jQuery("script#admin-assets-edit")).interpolate(f.toMustacheWithNeighbors(h),"mustache").appendTo("body").then(function(){Utilities.loadModal("div#asset-editor",function(){jQuery("div#asset-editor").fadeIn("fast")})})});a.bind("render-index",function(d,f){var h=this;h.load(jQuery("script#admin-assets-index")).interpolate(Asset.toMustache(f),"mustache").replace("#editor").then(function(){jQuery("a.info-icon").click(function(j){j.preventDefault();
current_asset_id=this.id.split("-")[2];h.trigger("show_info",{current_asset_id:current_asset_id})});jQuery("#ajax_uploader").attr("multiple","multiple").change(function(){jQuery(this).parents("form:first").submit()});jQuery("a.remove-icon").click(function(j){j.preventDefault();current_asset_id=this.id.split("-")[2];h.trigger("show_remove_dialog",{current_asset_id:current_asset_id})})})});a.bind("show_info",function(d,f){var h=Asset.find(f.current_asset_id),j=jQuery("#asset-"+h.id()),m=j.find(".info");
jQuery(".modal-strip").remove();m.length||this.load(jQuery("#admin-assets-info")).interpolate({asset:h.toMustache()},"mustache").appendTo(j).then(function(){jQuery(".modal-strip").fadeIn("fast")})});a.bind("show_remove_dialog",function(d,f){var h=Asset.find(f.current_asset_id),j=jQuery("#asset-"+h.id()),m=j.find(".remove");jQuery(".modal-strip").remove();m.length||this.load(jQuery("script#admin-assets-remove")).interpolate({asset:h.toMustache()},"mustache").appendTo(j).then(function(){jQuery(".modal-strip").fadeIn("fast")})});
a.get("/admin/assets",function(d){jQuery("#overlay").remove();var f=d.params.query?d.params.query:"",h={query:f};h.limit=d.params.limit||48;h.page=d.params.page||1;d.renderFolderTree();Asset.searchAdmin(h,function(){d.trigger("render-index",f)})});a.get("/admin/folders/:id",function(d){jQuery("#overlay").remove();var f=d.params.id,h=Folder.find(f);d.renderFolderTree(function(){jQuery("ul#folders li").removeClass("active");jQuery("#folder-"+f).addClass("active")});h.loadAssets(function(){d.trigger("render-index",
"")})});a.get("/admin/assets/new",function(d){d.load(jQuery("script#admin-assets-new")).interpolate({},"mustache").replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple")});b.first_run=false});a.post("/admin/assets",function(d){var f=document.getElementById("ajax_uploader").files,h=d.params.query?d.params.query:null;jQuery("form#new_asset");var j=h?{query:h}:{};j.limit=d.params.limit||48;j.page=d.params.page||1;this.send_files(f,j,function(){d.trigger("render-index",
h)});return false});a.get("/admin/assets/:id/edit",function(d){jQuery("#overlay").remove();var f=d.params.query?d.params.query:"",h=f?{query:d.params.query}:{},j=Asset.find(d.params.id);j?d.trigger("render-asset",j,f):Asset.searchAdmin(h,function(){var m=Asset.find(d.params.id);d.trigger("render-index",f);d.renderFolderTree();d.trigger("render-asset",m,f)})});a.put("/admin/assets/:id",function(d){var f=Asset.find(d.params.id);if(f){f.attr(d.params.asset);f.save(function(h){h&&Utilities.notice("Successfully saved asset")})}else{f=
new Asset({id:d.params.id});f.load(function(){f.attr(d.params.asset);f.save(function(h){h&&Utilities.notice("Successfully saved asset")})})}});a.del("/admin/assets/:id",function(d){var f=d.params.query?d.params.query:null,h=f?"?"+decodeURIComponent(jQuery.param({query:f})):"";Asset.find(d.params.id).destroy(function(j){if(j){Utilities.notice("Successfully saved asset");d.redirect("/admin/assets"+h)}})})}),Pages=Sammy(function(a){a.helpers({renderPagePreview:function(b,d){var f=this;jQuery("div#preview-"+
b.id()).length?f.trigger("set-active-page",b):f.load(jQuery("script#admin-pages-show")).interpolate({page:b.asJSON(),layouts:Layout.asLayoutJSON(b.attr("layout_id")),base_page_id:b.id()},"mustache").replace("#editor").then(function(){iFramer.initialize(".preview iframe",function(){d&&d.call(this)});f.trigger("set-active-page",b);Editor.initialize()})},renderPageEditor:function(b,d){var f=this;if(jQuery("#page-editor").length){d&&d.call(this);f.trigger("set-active-page",b)}else f.load(jQuery("script#admin-pages-edit")).interpolate({page:b.asJSON()},
"mustache").replace("#editor").then(function(){jQuery("li.node").removeClass("active");jQuery("#page-"+b.id()).addClass("active");d&&d.call(this);f.trigger("set-active-page",b)})},renderPageProperties:function(b,d){this.load(jQuery("script#admin-pages-form")).interpolate({page:b.asJSON(),layouts:Layout.asLayoutJSON(b.attr("layout_id")),base_page_id:b.id(),timestamp:window.timestamp},"mustache").replace("#page-tabs-"+b.id()).then(function(){d&&d.call(this)})},renderPageMenu:function(b){this.load(jQuery("script#admin-pages-index")).interpolate({pages:[b.asJSON()],
children:b.childrenAsJSON()},"mustache").replace("#sidebar")}});a.bind("show-page-menu",function(b,d){var f=this;if(d.children().count()==0&&d.attr("child_count")!=0){console.log("ajax");d.getChildren(function(){f.renderPageMenu(d)})}else{if(d.attr("child_count")==0)d=d.parent();f.renderPageMenu(d)}});a.bind("set-active-page",function(b,d){jQuery("li.node").removeClass("active");jQuery("li#page-"+d.id()).addClass("active")});a.bind("reload-page",function(b,d){var f=d.id(),h=jQuery("iframe#page-iframe-"+
f);jQuery("li#page-"+f).find("span.title a").text(d.attr("title"));jQuery("h1#page-title-"+f).text(d.attr("title"));h.attr("src",h.attr("src"))});this.get("/admin/pages/?",function(b){var d=Page.first();b.redirect(d.attr("admin_path"))});this.get("/admin/pages/:id/new/?",function(b){var d=Page.find(b.params.id),f=$("<div />").attr({id:"new-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(f);b.trigger("page-index");b.load(jQuery("#admin-pages-new")).interpolate({parent:d.asJSON(),
layouts:Layout.asLayoutJSON(d.attr("layout_id"))},"mustache").replace("#new-page-container")});this.post("/admin/pages/:page_id/?",function(b){Page.find(b.params.page_id);(new Page(b.params.page)).save(function(d,f){var h=JSON.parse(f.responseText);if(h.errors)alert(JSON.stringify(h));else{Utilities.notice("Successfully created page");b.redirect(h.admin_path)}})});this.get("/admin/pages/:id/?",function(b){jQuery("#overlay").remove();jQuery(".modal-editor").remove();var d=Page.find(b.params.id);if(d)b.renderPagePreview(d);
else{d=new Page({id:b.params.id});d.load(function(){b.renderPagePreview(d)})}b.trigger("show-page-menu",d)});this.get("/admin/pages/:id/edit/?",function(b){var d=Page.find(b.params.id);b.renderPageEditor(d,function(){b.renderPageProperties(d)});b.trigger("show-page-menu",d)});this.put("/admin/pages/:page_id",function(b){var d=Page.find(b.params.page_id);d.attr(b.params.page);jQuery("span.timestamp").text()<window.timestamp?alert("Someone else has edited this page, please reload and try again"):d.save(function(f){if(f){Utilities.setTimestamp();
Utilities.notice("Successfully saved page");b.trigger("reload-page",d);b.redirect(d.attr("admin_path"))}})});this.get("/admin/pages/:id/remove",function(b){var d=Page.find(b.params.id),f=$("<div />").attr({id:"remove-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(f);b.trigger("page-index");b.load(jQuery("#admin-pages-remove")).interpolate({page:d.asJSON()},"mustache").replace("#remove-page-container")});this.del("/admin/pages/:id",function(b){var d=Page.find(b.params.id);
d.destroy(function(f){if(f){jQuery("#page-"+d.id()).remove();b.redirect("/admin/pages")}})})}),Parts=Sammy(function(a){a.helpers({renderPart:function(b,d,f){var h=this;h.load(jQuery("script#admin-"+f+"-edit")).interpolate({part:b.asJSON(),page:d.asJSON(),assets:Asset.asJSON(),timestamp:window.timestamp},"mustache").replace("#page-tabs-"+d.id()).then(function(){var j=aceModes.textile;window.editor=ace.edit("part-"+b.id()+"-content");window.editor.setTheme("ace/theme/textmate");window.editor.getSession().setMode(new j);
window.editor.session.setUseWrapMode(true);window.editor.getSession().setValue(b.attr("content"));h.trigger("set_asset_links");jQuery("#ajax_uploader").attr("multiple","multiple").change(function(){var m=jQuery(this).parents("form:first");jQuery(".progress").slideDown("slow",function(){m.submit()})});h.trigger("page-index")})}});this.bind("set_asset_links",function(){jQuery("#search-results-container li.asset").each(function(b,d){var f=jQuery(d).find("a"),h=jQuery(d).attr("id").split("-")[1],j=f.find("img").attr("src");
f.click(function(m){m.preventDefault();jQuery("input#part-asset-id").attr("value",h);jQuery("form#edit-part img.preview").attr("src",j.replace("icon","thumbnail"))})})});this.get("/admin/pages/:page_id/parts/:id/edit",function(b){jQuery(".modal-editor").remove();$("iframe");var d=Page.find(b.params.page_id),f=d.parts().find(b.params.id);b.trigger("show-page-menu",d);b.renderPageEditor(d,function(){b.renderPart(f,d,"parts")})});this.get("/admin/pages/:page_id/image_parts/:id/edit",function(b){var d=
Page.find(b.params.page_id),f=d.parts().find(b.params.id);$("iframe");Asset.searchAdmin({limit:"24"},function(){b.trigger("show-page-menu",d);b.renderPageEditor(d,function(){b.renderPart(f,d,"image_parts")})})});this.get("/admin/pages/:page_id/parts/:id/results",function(b){Page.find(b.params.page_id);Part.find(b.params.id);var d=b.params.query?b.params.query:null;Asset.searchAdmin(d?{query:d}:{},function(){b.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),"mustache").replace("#search-results-container").then(function(){b.trigger("set_asset_links")})})});
this.put("#/pages/:page_id/parts/:id",function(b){var d=Page.find(b.params.page_id),f=d.parts().find(b.params.id),h=d.attr("contents"),j=h.length;b.params.part.content=window.editor.getSession().getValue();for(var m=0;m<j;++m){f=h[m];if(b.params.id==f.id){f.content=b.params.part.content;f.asset_id=b.params.part.asset_id;var p=Part.find(f.id);p.attr({asset_id:b.params.part.asset_id,content:b.params.part.content});p.save()}}d.save(function(v){if(v){Utilities.setTimestamp();Utilities.notice(p.attr("name")+
" saved");window.editor.getSession().setUseSoftTabs(true);window.editor.getSession().setTabSize(2);window.editor.getSession().setMode(new editorMode)}})});this.post("#/pages/:page_id/image_parts/:id/assets",function(b){Page.find(b.params.page_id).parts().find(b.params.id);var d=document.getElementById("ajax_uploader").files,f=b.params.query?b.params.query:null;jQuery("form#new_asset");f=f?{query:f}:{};f.limit=b.params.limit||48;f.page=b.params.page||1;this.send_files(d,f,function(){b.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),
"mustache").replace("#search-results-container").then(function(){jQuery("#ajax_uploader").attr("files",null);jQuery(".progress").slideUp("slow",function(){jQuery(this).html("")});b.trigger("set_asset_links")})})})}),Users=Sammy(function(){var a=this;this.helpers({loadUsers:function(b){if(User.all().length==0)User.load(function(){b&&b.call(this)});else b&&b.call(this)},renderUserIndex:function(b){this.load(jQuery("script#admin-users-index")).interpolate(User.toMustache(),"mustache").replace("#editor").then(function(){b&&
b.call(this)})}});this.bind("run",function(){a.modal=false;a.refresh_pages=true});this.get("/admin/users",function(b){jQuery("#editor").html('<h1 class="section">Users</div>');b.loadUsers(function(){b.renderUserIndex()})});this.get("/admin/users/new",function(b){b.loadUsers(function(){jQuery("#modal").length||Galerie.open();b.load(jQuery("script#admin-users-new")).interpolate({},"mustache").replace("#modal");b.renderUserIndex()})});this.post("/admin/users",function(b){(new User(b.params.user)).save(function(d,
f){var h=JSON.parse(f.responseText);h.errors?alert(JSON.stringify(h)):b.redirect("/admin/users")})});this.get("/admin/users/:id/edit",function(b){b.loadUsers(function(){user=User.find(b.params.id);if(jQuery("#users").length){jQuery(".user-form").html("");b.load(jQuery("script#admin-users-edit")).interpolate({user:user.asJSON()},"mustache").replace("#user-form-"+user.id())}else b.renderUserIndex(function(){jQuery(".user-form").html("");b.load(jQuery("script#admin-users-edit")).interpolate({user:user.asJSON()},
"mustache").replace("#user-form-"+user.id())})})});this.put("/admin/users/:id",function(b){var d=User.find(b.params.id);d.attr(b.params.user);d.save(function(f,h){var j=JSON.parse(h.responseText);j.errors?alert(JSON.stringify(j)):b.redirect("/admin/users")})})});
