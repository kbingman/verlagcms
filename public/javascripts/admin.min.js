...
console.log(c)},debug:function(c){window.console&&console.log(c)}},Utilities={setNonHistoryLinks:function(){Modernizr.history||jQuery("a").click(function(c){c.preventDefault();if((c=jQuery(this).attr("href"))&&!c.match("http://")&&!c.match(/^#/)){console.log("#"+c);document.location.hash=$(this).attr("#href")}})},loadModal:function(c,a){var f=this,e=jQuery(c);if(e.length){Loader.start();var h=e.find("img");setTimeout(function(){e.height()<20?h.load(function(){f.resizeModal(h,e,a)}):f.resizeModal(h,
e,a)},13)}},resizeModal:function(c,a,f){var e=c.clone();e=e[0].width/e[0].height;var h=jQuery(window).width(),l=jQuery(window).height();if(a.height()>l-40){c.height(l-40).width((l-40)*e);a.height(l-40).width((l-40)*e)}if(a.width()>h-40){c.height((h-40)/e).width(h-40);a.height((h-40)/e).width(h-40);a.css({"margin-top":(l-(h-40)/e)/2})}a.width()<h-40&&a.height()<l-40&&a.css({width:a.height()*e+"px","margin-top":(l-a.height())/2});Loader.stop();f&&f.call(this)},notice:function(c,a){a=a||{};var f=jQuery(".notice"),
e=a["class"]||"message";f.html(c).addClass(e).fadeIn("fast",function(){setTimeout(function(){a.persist||f.fadeOut("slow")},1800)})},hideNotice:function(){jQuery(".notice").fadeOut("slow")},setTimestamp:function(){window.timestamp=(new Date).getTime()},keyboard_nav:function(){jQuery("body").keydown(function(c){switch(c.keyCode){case 37:$("a.previous").click();break;case 39:$("a.next").click();break}})},check_browser_version:function(){},formObserver:function(){},unique:function(c){tmp=[];for(i=0;i<
c.length;i++)if(!Utilities.contains(tmp,c[i])){tmp.length+=1;tmp[tmp.length-1]=c[i]}return tmp},contains:function(c,a){for(j=0;j<c.length;j++)if(c[j]==a)return true;return false}},Loader={start:function(c){c=jQuery("#loader");if(c.length){c.show();Loader.timer=setInterval(function(){var a=c.css("background-position-y").replace("px","");c.css({"background-position-y":a-40+"px"})},67)}},stop:function(c){c=jQuery("#loader");if(c.length){c.hide();clearInterval(Loader.timer)}}},Updater={setup:function(c,
a){jQuery.each(c,function(f,e){var h=new a({id:e.id});h.merge(e);a.add(h)})},update:function(){window.ninja=true;jQuery.ajax({url:"/admin/activity.json",type:"POST",data:{updated:window.timestamp},success:function(c){jQuery.each(c.models,function(a,f){Page.find(f.id).merge(f);Utilities.setTimestamp()});window.ninja=false}})}},aceModes={javascript:require("ace/mode/javascript").Mode,html:require("ace/mode/html").Mode,css:require("ace/mode/css").Mode,scss:require("ace/mode/scss").Mode,sass:require("ace/mode/scss").Mode,
textile:require("ace/mode/textile").Mode,none:require("ace/mode/scss").Mode},delay=function(){var c=0;return function(a,f){clearTimeout(c);c=setTimeout(a,f)}}(),Editor={initialize:function(){var c=jQuery("#editor-buttons input[type=button]"),a=jQuery("#editor-buttons input#save");c.click(function(){var f=jQuery(this).data("cmd"),e=jQuery(this).data("value");if(e=="promptUser")e=prompt(jQuery(this).data("text"));var h=jQuery("iframe")[0];f&&h.contentDocument.execCommand(f,false,e)});a.click(function(){var f=
document.location.pathname.split("/"),e=Page.find(f[f.length-1]),h={};jQuery("iframe").contents().find("div.editable").each(function(){var l=jQuery(this),n=l.attr("id").split("-")[1];h[n]=l.html()});jQuery.each(h,function(l,n){n=Editor.textilize(n);e.setPartAttributes(l,{content:n})});e.save()})},textilize:function(c,a){c=c.replace(/\s*<p>((.|[\r\n])*?)<\/p>\s*/gi,"\n\n$1\n\n\n");c=c.replace(/\s*<h1>((.|[\r\n])*?)<\/h1>\s*/gi,"h1. $1\n\n");c=c.replace(/\s*<h2>((.|[\r\n])*?)<\/h2>\s*/gi,"h2. $1\n\n");
...
l=l?{query:l}:{};l.limit=f.params.limit||48;l.page=f.params.page||1;l.folder_id=h;this.send_files(e,l,{before:function(n){f.load(jQuery("script#admin-assets-loading")).interpolate({uuid:n.uuid,title:n.file_name.split(".")[0]},"mustache").appendTo("#assets")},progress:function(){},success:function(n){jQuery("li#asset-"+n.attr("uuid")).remove();f.load(jQuery("script#admin-assets-asset")).interpolate(n.attr(),"mustache").appendTo("ul#assets")}});return false});c.get("/admin/assets/:id/edit",function(f){jQuery("#overlay").remove();
var e=f.params.query?f.params.query:"",h=e?{query:f.params.query}:{},l=Asset.find(f.params.id);l?f.trigger("render-asset",l,e):Asset.searchAdmin(h,function(){var n=Asset.find(f.params.id);f.trigger("render-index",{folder_id:n.attr("folder_id")});f.renderFolderTree();f.trigger("render-asset",n,e)})});c.put("/admin/assets/:id",function(f){var e=Asset.find(f.params.id);if(e){e.attr(f.params.asset);e.save(function(h){h&&Utilities.notice("Successfully saved asset")})}else{e=new Asset({id:f.params.id});
e.load(function(){e.attr(f.params.asset);e.save(function(h){h&&Utilities.notice("Successfully saved asset")})})}});c.del("/admin/assets/:id",function(f){var e=f.params.query?f.params.query:null;e&&decodeURIComponent(jQuery.param({query:e}));e=Asset.find(f.params.id);var h=e.attr("folder_id");e.destroy(function(l){if(l){Utilities.notice("Asset removed");jQuery(".modal-strip").remove();f.redirect("/admin/folders/"+h)}})})}),Folders=Sammy(function(c){this.helpers({renderFolderTree:function(a){if(jQuery("ul#folders").length)a&&
...
e=Folder.find(f);a.renderFolderTree(function(){jQuery("ul#folders li").removeClass("active");jQuery("#folder-"+f).addClass("active")});e.loadAssets(function(){a.trigger("render-index",{folder_id:e.id()})})});this.post("/admin/folders",function(a){var f=new Folder(a.params.folder);f.save(function(e,h){var l=JSON.parse(h.responseText);if(l.errors)alert(JSON.stringify(l));else{alert(f.attr("admin_path"));a.load(jQuery("script#admin-folders-index")).interpolate({folders:Folder.asJSON()},"mustache").replace("#sidebar");
a.redirect(f.attr("admin_path"))}})})}),Pages=Sammy(function(c){c.helpers({renderPagePreview:function(a,f){var e=this;e.load(jQuery("script#admin-pages-show")).interpolate({page:a.asJSON(),layouts:Layout.asLayoutJSON(a.attr("layout_id")),base_page_id:a.id()},"mustache").replace("#editor").then(function(){iFramer.initialize(".preview iframe",function(){f&&f.call(this)});e.trigger("set-active-page",a);Editor.initialize()})},renderPageEditor:function(a,f){var e=this;if(jQuery("#page-editor").length){f&&
