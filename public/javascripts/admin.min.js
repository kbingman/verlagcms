...
null;try{a=c.target.status}catch(f){return}if(a=="200"&&c.target.readyState==4&&c.target.responseText){c=JSON.parse(c.target.responseText);Asset.callback.success&&Asset.callback.success.call(this,c)}},generate_uuid:function(){for(var c=[],a=0;a<32;a++)c[a]="0123456789ABCDEF".substr(Math.floor(Math.random()*16),1);c[12]="4";c[16]="0123456789ABCDEF".substr(c[16]&3|8,1);return c.join("")}})}),Folder=Model("folder",function(){this.persistence(Model.SinatraREST,"/admin/folders");this.include({loadAssets:function(c){Asset.each(function(){Asset.remove(this)});
var a="/admin/folders/"+this.attr("id")+"/assets.json";jQuery.ajax({type:"get",url:a,contentType:"application/json",dataType:"json",success:function(f){jQuery.each(f,function(e,h){var l=new Asset({id:h.id});l.merge(h);Asset.add(l)});c&&c.call(this)}})}});this.extend({asJSON:function(){return Folder.map(function(c){return c.attr()})}})}),Page=Model("page",function(){this.persistence(Model.SinatraREST,"/admin/pages");this.include({initialize:function(){},children:function(){return Page.find_all_by_parent_id(this.id())},
childrenAsJSON:function(){return this.children().map(function(c){return c.attr()})},setPartAttributes:function(c,a){var f=this.attr("contents"),e;jQuery.each(f,function(h,l){if(c==l.id){if(a.content)l.content=a.content;if(a.asset_id)l.asset_id=a.asset_id;if(a.path)l.path=a.path;e=l}});return e},findPartByName:function(c){var a=this.attr("contents"),f;jQuery.each(a,function(e,h){if(c==h.name){f=h;return false}});return f},findPartById:function(c){var a=this.attr("contents"),f;jQuery.each(a,function(e,
h){if(c==h.id){f=h;return false}});return f},getChildren:function(c){var a=this,f="/admin/pages/"+a.id()+"/children.json";jQuery.ajax({type:"GET",url:f,dataType:"json",success:function(e){$.each(e,function(h,l){var n=new Page({id:l.id});n.merge(l);Page.add(n)});a.attr({"children?":true,children:a.childrenAsJSON()});c&&c.call(this,e)}})},has_children:function(){if(this.children().all().length>0)return true},parent:function(){return Page.find(this.attr("parent_id"))},assets:function(){for(var c=this.attr("assets"),
a=0,f=c.length;a<f;++a){var e=c[a],h=new Asset({id:e.id});h.merge(e);Asset.add(h)}return Asset},parts:function(){var c=this.attr("contents"),a=c.length;Part.each(function(){Part.remove(this)});for(var f=0;f<a;++f){var e=c[f],h=new Part({id:e.id});h.merge(e);h.attr("page_id",this.id());Part.add(h)}return Part},load:function(c){var a=this,f="/admin/pages/"+a.id()+".json";jQuery.ajax({type:"GET",url:f,dataType:"json",success:function(e){a.merge(e);c.call(this)}})}});this.extend({toMustache:function(){return{pages:this.map(function(c){return c.attr()})}},
root:function(){return this.detect(function(){return this.attr("parent_id")==null})},find_by_parent_id:function(c){return this.detect(function(){return this.attr("parent_id")==c})},find_all_by_parent_id:function(c){return this.select(function(){return this.attr("parent_id")==c})},find_by_path:function(c){return this.detect(function(){return this.attr("path")==c})},load_by_id:function(c,a){jQuery.ajax({type:"GET",url:"/admin/pages/"+c+".json",dataType:"json",success:function(f){var e=Page.find(f.id);
e||(e=new Page({id:f.id}));e.merge(f);Page.add(e);a&&a.call(this,f)}})},create:function(c,a){jQuery.ajax({type:"post",url:"/admin/pages.json",dataType:"json",data:{page:c},success:function(f){var e=new Page({id:f.id});e.merge(f);Page.add(e);a.call(this,f)}})}})}),Layout=Model("layout",function(){this.persistence(Model.SinatraREST,"/admin/templates");this.include({load:function(c){var a=this,f="/admin/templates/"+a.id()+".json";jQuery.ajax({type:"GET",url:f,dataType:"json",success:function(e){a.merge(e);
c.call(this,e)}})}});this.extend({toMustache:function(){return{layouts:this.map(function(c){return c.attr()})}},asLayoutJSON:function(c){return Layout.find_all_by_class("Layout").map(function(a){c==a.id()?a.attr("selected",'selected="selected"'):a.attr("selected","");return a.attr()})},find_by_class:function(c){return this.detect(function(){return this.attr("klass")==c})},find_all_by_class:function(c){return this.select(function(){return this.attr("klass")==c})},asJSON_by_class:function(c){return this.find_all_by_class(function(){return c.map(function(a){return a.attributes})})}})}),
Part=Model("part",function(){this.include({deleteRemote:function(c,a){var f="/admin/pages/"+c.id()+"/parts/"+this.id()+".json";jQuery.ajax({type:"DELETE",url:f,dataType:"json",success:function(e){c.merge(e);a.call(this)}})}});this.extend({toMustache:function(){return{parts:this.map(function(c){return c.attr()})}},create:function(c,a){var f="/admin/templates/"+c.template_id+"/parts.json",e=Layout.find(c.template_id);jQuery.ajax({type:"post",url:f,dataType:"json",data:{part:c},success:function(h){e.merge(h);
a.call(this)}})}})}),Site=Model("site",function(){this.persistence(Model.SinatraREST,"/admin/sites");this.include({});this.extend({toMustache:function(){return{sites:this.map(function(c){return c.attr()})}}})}),User=Model("user",function(){this.persistence(Model.SinatraREST,"/admin/users");this.include({});this.extend({toMustache:function(){return{users:this.map(function(c){return c.attr()})}}})}),Galerie={open:function(c,a){$("body").append('<div id="overlay"></div>').append('<div id="modal_wrapper"><div id="modal"></div></div>');
var f=$("#modal");f.html(c);var e=$("#modal_wrapper"),h=$(window).height();f=f.height()+42;f=(h-f)/2;$("#overlay").css({height:h}).hide().fadeIn("fast");e.hide().fadeIn("fast").css({top:f});a&&a.call(this)},close:function(){$("#modal_wrapper").fadeOut("fast",function(){$(this).remove();$("#overlay").fadeOut("fast",function(){$(this).remove()})})}},logger={info:function(c){window.console&&console.log(c)},debug:function(c){window.console&&console.log(c)}},Utilities={setNonHistoryLinks:function(){Modernizr.history||
jQuery("a").click(function(c){c.preventDefault();if((c=jQuery(this).attr("href"))&&!c.match("http://")&&!c.match(/^#/)){console.log("#"+c);document.location.hash=$(this).attr("#href")}})},loadModal:function(c,a){var f=this,e=jQuery(c);if(e.length){Loader.start();var h=e.find("img");setTimeout(function(){e.height()<20?h.load(function(){f.resizeModal(h,e,a)}):f.resizeModal(h,e,a)},13)}},resizeModal:function(c,a,f){var e=c.clone();e=e[0].width/e[0].height;var h=jQuery(window).width(),l=jQuery(window).height();
if(a.height()>l-40){c.height(l-40).width((l-40)*e);a.height(l-40).width((l-40)*e)}if(a.width()>h-40){c.height((h-40)/e).width(h-40);a.height((h-40)/e).width(h-40);a.css({"margin-top":(l-(h-40)/e)/2})}a.width()<h-40&&a.height()<l-40&&a.css({width:a.height()*e+"px","margin-top":(l-a.height())/2});Loader.stop();f&&f.call(this)},notice:function(c,a){a=a||{};var f=jQuery(".notice"),e=a["class"]||"message";f.html(c).addClass(e).fadeIn("fast",function(){setTimeout(function(){a.persist||f.fadeOut("slow")},
1800)})},hideNotice:function(){jQuery(".notice").fadeOut("slow")},setTimestamp:function(){window.timestamp=(new Date).getTime()},keyboard_nav:function(){jQuery("body").keydown(function(c){switch(c.keyCode){case 37:$("a.previous").click();break;case 39:$("a.next").click();break}})},check_browser_version:function(){},formObserver:function(){},unique:function(c){tmp=[];for(i=0;i<c.length;i++)if(!Utilities.contains(tmp,c[i])){tmp.length+=1;tmp[tmp.length-1]=c[i]}return tmp},contains:function(c,a){for(j=
0;j<c.length;j++)if(c[j]==a)return true;return false}},Loader={start:function(c){c=jQuery("#loader");if(c.length){c.show();Loader.timer=setInterval(function(){var a=c.css("background-position-y").replace("px","");c.css({"background-position-y":a-40+"px"})},67)}},stop:function(c){c=jQuery("#loader");if(c.length){c.hide();clearInterval(Loader.timer)}}},Updater={setup:function(c,a){jQuery.each(c,function(f,e){var h=new a({id:e.id});h.merge(e);a.add(h)})},update:function(){window.ninja=true;jQuery.ajax({url:"/admin/activity.json",
type:"POST",data:{updated:window.timestamp},success:function(c){jQuery.each(c.models,function(a,f){Page.find(f.id).merge(f);Utilities.setTimestamp()});window.ninja=false}})}},aceModes={javascript:require("ace/mode/javascript").Mode,html:require("ace/mode/html").Mode,css:require("ace/mode/css").Mode,scss:require("ace/mode/scss").Mode,sass:require("ace/mode/scss").Mode,textile:require("ace/mode/textile").Mode,none:require("ace/mode/scss").Mode},delay=function(){var c=0;return function(a,f){clearTimeout(c);
c=setTimeout(a,f)}}(),Editor={initialize:function(){var c=jQuery("#editor-buttons input[type=button]"),a=jQuery("#editor-buttons input#save");c.click(function(){var f=jQuery(this).data("cmd"),e=jQuery(this).data("value");if(e=="promptUser")e=prompt(jQuery(this).data("text"));var h=jQuery("iframe")[0];f&&h.contentDocument.execCommand(f,false,e)});a.click(function(){var f=document.location.pathname.split("/"),e=Page.find(f[f.length-1]),h={};jQuery("iframe").contents().find("div.editable").each(function(){var l=
jQuery(this),n=l.attr("id").split("-")[1];h[n]=l.html()});jQuery.each(h,function(l,n){n=Editor.textilize(n);e.setPartAttributes(l,{content:n})});e.save()})},textilize:function(c,a){c=c.replace(/\s*<p>((.|[\r\n])*?)<\/p>\s*/gi,"\n\n$1\n\n\n");c=c.replace(/\s*<h1>((.|[\r\n])*?)<\/h1>\s*/gi,"h1. $1\n\n");c=c.replace(/\s*<h2>((.|[\r\n])*?)<\/h2>\s*/gi,"h2. $1\n\n");c=c.replace(/\s*<h3>((.|[\r\n])*?)<\/h3>\s*/gi,"h3. $1\n\n");c=c.replace(/\s*<h4>((.|[\r\n])*?)<\/h4>\s*/gi,"h4. $1\n\n");c=c.replace(/\s*<h5>((.|[\r\n])*?)<\/h5>\s*/gi,
...
"&lt;").replace(/>/g,"&gt;");c=c.replace(/(\r\n|\n){3,}/g,"\n\n");return c=c.replace(/^[\r\n]+|[\r\n]+$/g,"")}};jQuery("span.opener").live("click",function(){var c=jQuery(this),a=c.parents("li.node:first").attr("id").split("-")[1];if(c.hasClass("open")){c.removeClass("open");Pages.trigger("close-page-children",a)}else{c.addClass("open");Pages.trigger("open-page-children",a)}});
jQuery("a.page-reload").live("click",function(){var c=jQuery(this).attr("href").split("/")[3];c=jQuery("iframe#page-iframe-"+c);c.attr("src",c.attr("src"));Utilities.hideNotice()});jQuery("div#sidebar a").live("click",function(){Utilities.hideNotice()});jQuery(window).resize(function(){var c=jQuery("div#asset-editor");Utilities.loadModal(c)});jQuery("a.insert-file").live("click",function(c){c.preventDefault();window.editor.insert($(this).attr("href"))});
jQuery("a.info-icon").live("click",function(c){c.preventDefault();current_asset_id=this.id.split("-")[2];Assets.trigger("show_info",{current_asset_id:current_asset_id})});jQuery("a.close-modal-strip").live("click",function(c){c.preventDefault();jQuery("div.modal-strip").fadeOut("fast",function(){jQuery(this).remove()})});jQuery("a.insert-image").live("click",function(c){c.preventDefault();c=jQuery(this).attr("href");Base.trigger("image-browser",c)});
...
"multiple").bind("change",function(){jQuery(this).parents("form:first").submit()});n.trigger("set_draggable_assets");n.trigger("set_droppable_folders")})});c.bind("set_draggable_assets",function(){jQuery("li.asset").draggable({revert:true,stack:".asset"})});c.bind("set_droppable_folders",function(){jQuery("li.folder").droppable({hoverClass:"active",drop:function(f,e){var h=f.target.id.split("-")[1],l=Folder.find(h),n=e.draggable[0].id.split("-")[1];n=Asset.find(n);n.attr("folder_id",h);e.draggable.fadeOut("fast");
n.save(function(v){v?Utilities.notice("Asset added to "+l.attr("name")):e.draggable.fadeIn("fast")})}})});c.bind("show_info",function(f,e){var h=Asset.find(e.current_asset_id),l=jQuery("#asset-"+h.id()),n=l.find(".info");jQuery(".modal-strip").remove();n.length||this.load(jQuery("#admin-assets-info")).interpolate({asset:h.toMustache()},"mustache").appendTo(l).then(function(){jQuery(".modal-strip").fadeIn("fast")})});c.bind("show_remove_dialog",function(f,e){var h=Asset.find(e.current_asset_id),l=
jQuery("#asset-"+h.id()),n=l.find(".remove");jQuery(".modal-strip").remove();n.length||this.load(jQuery("script#admin-assets-remove")).interpolate({asset:h.toMustache()},"mustache").appendTo(jQuery("body")).then(function(){var v=jQuery(".modal-strip");v.css({top:l.offset().top+l.height()/2,left:l.offset().left+l.width()/2});v.fadeIn("fast")})});c.get("/admin/assets",function(f){jQuery("#overlay").remove();var e=f.params.query,h={query:e};h.limit=f.params.limit||96;h.page=f.params.page||1;if(e){f.renderFolderTree();
...
l=l?{query:l}:{};l.limit=f.params.limit||48;l.page=f.params.page||1;l.folder_id=h;this.send_files(e,l,{before:function(n){f.load(jQuery("script#admin-assets-loading")).interpolate({uuid:n.uuid,title:n.file_name.split(".")[0]},"mustache").appendTo("#assets")},progress:function(n,v){console.log(n);console.log(v);v==100&&setTimeout(function(){jQuery("li#asset-"+n).remove()},420)},success:function(n){f.load(jQuery("script#admin-assets-asset")).interpolate(n.attr(),"mustache").appendTo("ul#assets")}});
return false});c.get("/admin/assets/:id/edit",function(f){jQuery("#overlay").remove();var e=f.params.query?f.params.query:"",h=e?{query:f.params.query}:{},l=Asset.find(f.params.id);l?f.trigger("render-asset",l,e):Asset.searchAdmin(h,function(){var n=Asset.find(f.params.id);f.trigger("render-index",{folder_id:n.attr("folder_id")});f.renderFolderTree();f.trigger("render-asset",n,e)})});c.put("/admin/assets/:id",function(f){var e=Asset.find(f.params.id);if(e){e.attr(f.params.asset);e.save(function(h){h&&
Utilities.notice("Successfully saved asset")})}else{e=new Asset({id:f.params.id});e.load(function(){e.attr(f.params.asset);e.save(function(h){h&&Utilities.notice("Successfully saved asset")})})}});c.del("/admin/assets/:id",function(f){var e=f.params.query?f.params.query:null;e&&decodeURIComponent(jQuery.param({query:e}));e=Asset.find(f.params.id);var h=e.attr("folder_id");e.destroy(function(l){if(l){Utilities.notice("Asset removed");jQuery(".modal-strip").remove();f.redirect("/admin/folders/"+h)}})})}),
Folders=Sammy(function(c){this.helpers({renderFolderTree:function(a){if(jQuery("ul#folders").length)a&&a.call(this);else this.load(jQuery("script#admin-folders-index")).interpolate({folders:Folder.asJSON()},"mustache").replace("#sidebar").then(function(){a&&a.call(this)})}});this.get("/admin/folders/new/?",function(a){$("#modal").length==0&&Galerie.open();a.renderFolderTree(function(){jQuery("ul#folders li").removeClass("active")});a.load(jQuery("#admin-folders-new")).interpolate({},"mustache").replace("#modal")});
c.get("/admin/folders/:id",function(a){jQuery("#overlay").remove();var f=a.params.id,e=Folder.find(f);a.renderFolderTree(function(){jQuery("ul#folders li").removeClass("active");jQuery("#folder-"+f).addClass("active")});e.loadAssets(function(){a.trigger("render-index",{folder_id:e.id()})})});this.post("/admin/folders",function(a){var f=new Folder(a.params.folder);f.save(function(e,h){var l=JSON.parse(h.responseText);if(l.errors)alert(JSON.stringify(l));else{alert(f.attr("admin_path"));a.load(jQuery("script#admin-folders-index")).interpolate({folders:Folder.asJSON()},
"mustache").replace("#sidebar");a.redirect(f.attr("admin_path"))}})})}),Pages=Sammy(function(c){c.helpers({renderPagePreview:function(a,f){var e=this;e.load(jQuery("script#admin-pages-show")).interpolate({page:a.asJSON(),layouts:Layout.asLayoutJSON(a.attr("layout_id")),base_page_id:a.id()},"mustache").replace("#editor").then(function(){iFramer.initialize(".preview iframe",function(){f&&f.call(this)});e.trigger("set-active-page",a);Editor.initialize()})},renderPageEditor:function(a,f){var e=this;if(jQuery("#page-editor").length){f&&
f.call(this);e.trigger("set-active-page",a)}else e.load(jQuery("script#admin-pages-edit")).interpolate({page:a.asJSON()},"mustache").replace("#editor").then(function(){jQuery("li.node").removeClass("active");jQuery("#page-"+a.id()).addClass("active");f&&f.call(this);e.trigger("set-active-page",a)})},renderPageProperties:function(a,f){this.load(jQuery("script#admin-pages-form")).interpolate({page:a.asJSON(),layouts:Layout.asLayoutJSON(a.attr("layout_id")),base_page_id:a.id(),timestamp:window.timestamp},
"mustache").replace("#page-tabs-"+a.id()).then(function(){f&&f.call(this)})},renderPageMenu:function(){var a=Page.root(),f=Page.all(),e=jQuery.cookie("active_page_ids")?jQuery.cookie("active_page_ids").split(","):[];jQuery.each(f,function(h,l){var n=Page.root()==l||_.include(e,l.id())?true:false;l.attr({children:l.childrenAsJSON(),"open?":n})});this.load(jQuery("script#admin-pages-index")).interpolate({root:a.asJSON(),partials:{node:jQuery("script#admin-pages-node").html()}},"mustache").replace("#sidebar")}});
c.bind("show-page-menu",function(a,f){var e=this;if(f.children().count()==0&&f.attr("child_count")!=0)f.getChildren(function(){f.attr("open?",true);e.renderPageMenu(f)});else{if(f.attr("child_count")==0)f=f.parent();e.renderPageMenu(f)}});c.bind("open-page-children",function(a,f){var e=this,h=Page.find(f),l=jQuery("li#page-"+h.id()),n=jQuery.cookie("active_page_ids")?jQuery.cookie("active_page_ids").split(","):[];h.getChildren(function(){h.attr("open?",true);h.attr("children",h.childrenAsJSON());
n.push(h.id());jQuery.cookie("active_page_ids",Utilities.unique(n).join(","),{path:"/admin"});e.load(jQuery("script#admin-pages-children")).interpolate({page:h.asJSON(),partials:{node:jQuery("script#admin-pages-node").html()}},"mustache").appendTo(l)})});c.bind("close-page-children",function(a,f){var e=Page.find(f);jQuery("li#page-"+e.id()).find("ul.page-children").remove();e.children().each(function(h){Page.remove(h)})});c.bind("set-active-page",function(a,f){jQuery("li.node").removeClass("active");
jQuery("li#page-"+f.id()).addClass("active")});c.bind("reload-page",function(a,f){var e=f.id(),h=jQuery("iframe#page-iframe-"+e);jQuery("li#page-"+e).find("span.title a").text(f.attr("title"));jQuery("h1#page-title-"+e).text(f.attr("title"));h.attr("src",h.attr("src"))});this.get("/admin/pages/?",function(a){var f=Page.first();a.redirect(f.attr("admin_path"))});this.get("/admin/pages/:id/new/?",function(a){var f=Page.find(a.params.id),e=$("<div />").attr({id:"new-page-container","class":"small-modal"});
$("#modal").length==0&&Galerie.open(e);a.trigger("page-index");a.load(jQuery("#admin-pages-new")).interpolate({parent:f.asJSON(),layouts:Layout.asLayoutJSON(f.attr("layout_id"))},"mustache").replace("#new-page-container")});this.post("/admin/pages/:page_id/?",function(a){Page.find(a.params.page_id);(new Page(a.params.page)).save(function(f,e){var h=JSON.parse(e.responseText);if(h.errors)alert(JSON.stringify(h));else{Utilities.notice("Successfully created page");a.redirect(h.admin_path)}})});this.get("/admin/pages/:id/?",
function(a){jQuery("#overlay").remove();jQuery(".modal-editor").remove();var f=Page.find(a.params.id);if(f)a.renderPagePreview(f);else{f=new Page({id:a.params.id});f.load(function(){a.renderPagePreview(f)})}a.trigger("show-page-menu",f)});this.get("/admin/pages/:id/edit/?",function(a){var f=Page.find(a.params.id);a.renderPageEditor(f,function(){a.renderPageProperties(f)});a.trigger("show-page-menu",f)});this.put("/admin/pages/:page_id",function(a){var f=Page.find(a.params.page_id),e=jQuery("span.timestamp").text();
f.attr(a.params.page);e<window.timestamp?alert("Someone else has edited this page, please reload and try again"):f.save(function(h){if(h){Utilities.setTimestamp();Utilities.notice("Successfully saved page");a.trigger("reload-page",f);a.redirect(f.attr("admin_path"))}})});this.get("/admin/pages/:id/remove",function(a){var f=Page.find(a.params.id),e=$("<div />").attr({id:"remove-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(e);a.trigger("page-index");a.load(jQuery("#admin-pages-remove")).interpolate({page:f.asJSON()},
"mustache").replace("#remove-page-container")});this.del("/admin/pages/:id",function(a){var f=Page.find(a.params.id);f.destroy(function(e){if(e){jQuery("#page-"+f.id()).remove();a.redirect("/admin/pages")}})})}),Parts=Sammy(function(c){c.helpers({renderPart:function(a,f,e){var h=this;h.load(jQuery("script#admin-"+e+"-edit")).interpolate({part:a.asJSON(),page:f.asJSON(),assets:Asset.asJSON(),timestamp:window.timestamp},"mustache").replace("#page-tabs-"+f.id()).then(function(){var l=aceModes.textile;
window.editor=ace.edit("part-"+a.id()+"-content");window.editor.setTheme("ace/theme/textmate");window.editor.getSession().setMode(new l);window.editor.session.setUseWrapMode(true);window.editor.getSession().setValue(a.attr("content"));jQuery(".toolbar a.tab").removeClass("active");jQuery(".toolbar a."+a.attr("name")+"-tab").addClass("active");h.trigger("set_asset_links");jQuery("#ajax_uploader").attr("multiple","multiple").change(function(){var n=jQuery(this).parents("form:first");jQuery(".progress").slideDown("slow",
function(){n.submit()})});h.trigger("page-index")})}});this.bind("set_asset_links",function(){jQuery("#search-results-container li.asset").each(function(a,f){var e=jQuery(f).find("a"),h=jQuery(f).attr("id").split("-")[1],l=e.find("img").attr("src");e.click(function(n){n.preventDefault();jQuery("input#part-asset-id").attr("value",h);jQuery("form#edit-part img.preview").attr("src",l.replace("icon","thumbnail"))})})});this.get("/admin/pages/:page_id/parts/:id/edit",function(a){jQuery(".modal-editor").remove();
$("iframe");var f=Page.find(a.params.page_id),e=f.parts().find(a.params.id);a.trigger("show-page-menu",f);a.renderPageEditor(f,function(){a.renderPart(e,f,"parts")})});this.get("/admin/pages/:page_id/image_parts/:id/edit",function(a){var f=Page.find(a.params.page_id),e=f.parts().find(a.params.id);$("iframe");Asset.searchAdmin({limit:"24"},function(){a.trigger("show-page-menu",f);a.renderPageEditor(f,function(){a.renderPart(e,f,"image_parts")})})});this.get("/admin/pages/:page_id/parts/:id/results",
function(a){Page.find(a.params.page_id);Part.find(a.params.id);var f=a.params.query?a.params.query:null;Asset.searchAdmin(f?{query:f}:{},function(){a.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),"mustache").replace("#search-results-container").then(function(){a.trigger("set_asset_links")})})});this.put("#/pages/:page_id/parts/:id",function(a){var f=Page.find(a.params.page_id),e=f.parts().find(a.params.id),h=f.attr("contents"),l=h.length;a.params.part.content=window.editor.getSession().getValue();
for(var n=0;n<l;++n){e=h[n];if(a.params.id==e.id){e.content=a.params.part.content;e.asset_id=a.params.part.asset_id;var v=Part.find(e.id);v.attr({asset_id:a.params.part.asset_id,content:a.params.part.content});v.save()}}f.save(function(D){if(D){Utilities.setTimestamp();Utilities.notice(v.attr("name")+" saved");window.editor.getSession().setUseSoftTabs(true);window.editor.getSession().setTabSize(2);window.editor.getSession().setMode(new editorMode)}})});this.post("#/pages/:page_id/image_parts/:id/assets",
function(a){Page.find(a.params.page_id).parts().find(a.params.id);var f=document.getElementById("ajax_uploader").files,e=a.params.query?a.params.query:null;jQuery("form#new_asset");e=e?{query:e}:{};e.limit=a.params.limit||48;e.page=a.params.page||1;this.send_files(f,e,function(){a.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),"mustache").replace("#search-results-container").then(function(){jQuery("#ajax_uploader").attr("files",null);jQuery(".progress").slideUp("slow",
function(){jQuery(this).html("")});a.trigger("set_asset_links")})})})}),Users=Sammy(function(){this.helpers({loadUsers:function(c){if(User.all().length==0)User.load(function(){c&&c.call(this)});else c&&c.call(this)},renderUserIndex:function(c){this.load(jQuery("script#admin-users-index")).interpolate(User.toMustache(),"mustache").replace("#editor").then(function(){c&&c.call(this)})}});this.get("/admin/users",function(c){jQuery("#editor").html('<h1 class="section">Users</div>');c.loadUsers(function(){c.renderUserIndex()})});
this.get("/admin/users/new",function(c){c.loadUsers(function(){jQuery("#modal").length||Galerie.open();c.load(jQuery("script#admin-users-new")).interpolate({},"mustache").replace("#modal");c.renderUserIndex()})});this.post("/admin/users",function(c){(new User(c.params.user)).save(function(a,f){var e=JSON.parse(f.responseText);e.errors?alert(JSON.stringify(e)):c.redirect("/admin/users")})});this.get("/admin/users/:id/edit",function(c){c.loadUsers(function(){user=User.find(c.params.id);if(jQuery("#users").length){jQuery(".user-form").html("");
c.load(jQuery("script#admin-users-edit")).interpolate({user:user.asJSON()},"mustache").replace("#user-form-"+user.id())}else c.renderUserIndex(function(){jQuery(".user-form").html("");c.load(jQuery("script#admin-users-edit")).interpolate({user:user.asJSON()},"mustache").replace("#user-form-"+user.id())})})});this.put("/admin/users/:id",function(c){var a=User.find(c.params.id);a.attr(c.params.user);a.save(function(f,e){var h=JSON.parse(e.responseText);h.errors?alert(JSON.stringify(h)):c.redirect("/admin/users")})})});
