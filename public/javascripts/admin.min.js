...
break;case 39:$("a.next").click();break}})},check_browser_version:function(){if(!Modernizr.history){jQuery("a").live("click",function(a){a.preventDefault();document.location.hash=$(this).attr("href")});var c=jQuery("form#search-form");c.attr("action","#"+c.attr("action"))}},formObserver:function(c){jQuery(c).keyup(function(){delay(function(){jQuery(c).parents("form:first").submit()},800)})}},iFramer={initialize:function(c,a){var e=jQuery(c),g=this;e.length&&e.load(function(){var i=$(this),l=i.contents(),
n=l.find("span.part-editor"),v=n.find("a");g.setEditFlags(n);i.fadeIn("fast");l.find('a[href^="/preview"]').click(function(x){var s=$(this).attr("href").split("?")[0].replace("/preview","");if(s=Page.find_by_path(s)){x.preventDefault();s.id();document.location.hash=s.attr("admin_path")}});v.click(function(){window.top.trigger=$(this);window.top.location.hash=$(this).attr("href");return false});a&&a.call(this)})},setEditFlags:function(c){c.css({position:"relative",display:"block",height:"0",width:"0",
"text-decoration":"none"}).find("span").css({display:"block",position:"absolute",top:"-24px",left:"12px",padding:"4px 8px",width:"100px",background:"rgba(255, 255, 0, 0.7)"})}},TabControl={initialize:function(c){if(c=jQuery(c)){c.each(function(a,e){$(e).hide()});c.first().show();this.tabControl(jQuery(".tab-control"))}},tabControl:function(c){c.click(function(){var a="tab-"+$(this).find("label").attr("for").split("-")[1];jQuery(".tab").hide();jQuery("#"+a).show()})}},delay=function(){var c=0;return function(a,
e){clearTimeout(c);c=setTimeout(a,e)}}(),Sites=Sammy(function(){var c=this;this.debug=false;this.disable_push_state=true;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);this.helpers({loadSites:function(a){if(Site.all().length==0)Site.load(function(){a&&a.call(this)});else a&&a.call(this)},renderSiteIndex:function(){this.load(jQuery("#admin-sites-index")).interpolate(Site.toMustache(),"mustache").replace("#sidebar")}});this.bind("run",function(){c.modal=false;c.refresh_pages=
true});this.get("#/preview",function(){alert(document.location.hostname);window.open("/","preview")});this.get("#/sites",function(a){c.refresh_pages=true;Galerie.close();jQuery("#editor").html('<h1 class="section">Sites</div>');a.loadSites(function(){a.renderSiteIndex()})});this.get("#/sites/new",function(a){a.loadSites(function(){$("#modal").length==0&&Galerie.open();a.load(jQuery("#admin-sites-new")).interpolate({},"mustache").replace("#modal");a.renderSiteIndex(Site.all())})});this.post("#/sites",
function(a){(new Site(a.params.site)).save(function(e,g){var i=JSON.parse(g.responseText);i.errors?alert(JSON.stringify(i)):a.redirect("#/sites")})});this.get("#/sites/:id/edit",function(a){Galerie.close();a.loadSites(function(){site=Site.find(a.params.id);a.load(jQuery("#admin-sites-edit")).interpolate({site:site.asJSON()},"mustache").replace("#editor");a.renderSiteIndex(Site.all())})});this.put("#/sites/:id",function(a){var e=Site.find(a.params.id);e.attr(a.params.site);e.save(function(g,i){var l=
JSON.parse(i.responseText);l.errors?alert(JSON.stringify(l)):a.redirect("#/sites")})});this.get("#/sites/:id/remove",function(a){Galerie.open();a.loadSites(function(){site=Site.find(a.params.id);a.load(jQuery("#admin-sites-remove")).interpolate({site:site.asJSON()},"mustache").replace("#modal");a.renderSiteIndex(Site.all())})});this.del("#/sites/:id",function(a){site.destroy(function(e){e?a.redirect("#/sites"):alert("there were issues")})})}),Layouts=Sammy(function(){var c=this;this.debug=false;this.disable_push_state=
true;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);this.helpers({loadLayouts:function(a){if(Layout.all().length==0)Layout.load(function(){a&&a.call(this)});else a&&a.call(this)},renderLayoutIndex:function(){this.load(jQuery("#admin-templates-index")).interpolate({layouts:Layout.find_all_by_class("Layout").map(function(a){return a.attributes}),partials:Layout.find_all_by_class("Partial").map(function(a){return a.attributes}),javascripts:Layout.find_all_by_class("Javascript").map(function(a){return a.attributes}),
stylesheets:Layout.find_all_by_class("Stylesheet").map(function(a){return a.attributes})},"mustache").replace("#sidebar")},renderLayout:function(a){this.load(jQuery("#admin-templates-edit")).interpolate({layout:a.asJSON(),filters:[{name:"none",value:"none",selected:a.attr("filter")=="css"?'selected="selected"':""},{name:"Sass",value:"sass",selected:a.attr("filter")=="sass"?'selected="selected"':""},{name:"Scss",value:"scss",selected:a.attr("filter")=="scss"?'selected="selected"':""}]},"mustache").replace("#editor").then(function(){var e=
jQuery("#layout_content");e.attr("value",a.attr("content"));e=e.attr("class");jQuery("#layout_content").length>0&&CodeMirror.fromTextArea(document.getElementById("layout_content"),{mode:e,lineNumbers:true})})}});this.bind("run",function(){c.refresh_templates=true;c.refresh_pages=true;c.modal=false});this.get("#/templates",function(a){Galerie.close();c.refresh_templates=false;c.refresh_pages=true;jQuery("#editor").html('<h1 class="section">Templates</div>');a.loadLayouts(function(){a.renderLayoutIndex(Layout.all())})});
this.get("#/templates/new/:klass",function(a){this.loadLayouts(function(){var e=$("<div />").attr({id:"new-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(e);a.load(jQuery("#admin-templates-new")).interpolate({klass:a.params.klass},"mustache").replace("#new-page-container");a.renderLayoutIndex(Layout.all())})});this.post("#/templates",function(a){(new Layout(a.params.template)).save(function(e,g){var i=JSON.parse(g.responseText);if(i.errors)Utilities.notice(JSON.stringify(i));
else{a.redirect("#/templates");Utilities.notice("Successfully saved template")}})});this.get("#/templates/:id/edit",function(a){Galerie.close();c.refresh_pages=true;this.loadLayouts(function(){var e=Layout.find(a.params.id);a.renderLayout(e);a.renderLayoutIndex(Layout.all())})});this.put("#/templates/:id",function(a){var e=Layout.find(a.params.id);e.attr(a.params.layout);e.save(function(g){if(g){a.renderLayoutIndex();Utilities.notice("Successfully saved template")}})});this.get("#/templates/:id/remove",
function(a){this.loadLayouts(function(){var e=Layout.find(a.params.id);Galerie.open();a.load(jQuery("#admin-templates-remove")).interpolate({layout:e.asJSON()},"mustache").replace("#modal");a.renderLayoutIndex(Layout.all())})});this.del("/templates/:id",function(a){Layout.find(a.params.id).destroy(function(e){if(e){a.renderLayoutIndex();Utilities.notice("Successfully deleted template");a.redirect("#/templates")}})});this.post("/layouts/:id/parts",function(a){this.loadLayouts(function(){var e=a.params.id;
Part.create(a.params.part,function(){a.redirect("#/templates/"+e+"/edit")})})})}),Assets=Sammy(function(){var c=this;this.debug=false;this.disable_push_state=true;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);this.swap=function(a){jQuery("#editor").html(a)};this.helpers({loadAssets:function(a,e){if(Asset.all().length==0)Asset.searchAdmin(a,function(){e&&e.call(this)});else e&&e.call(this)},send_files:function(a,e,g){for(var i=0,l=0;l<a.length;l++){var n=a[l];Asset.create(n,
{before:function(v){jQuery(".progress").append('<p id="progress-'+v+'">'+n.name+'<span class="percentage"></span></p>')},progress:function(v,x){jQuery("#progress-"+v+" .percentage").text(" "+x+"%")},success:function(){i+=1;i==a.length&&Asset.searchAdmin(e,function(){g&&g.call(this)})}})}}});this.bind("run",function(){c.modal=false;c.first_run=true});this.get("#/assets",function(a){var e=a.params.query,g=e?{query:e}:{};g.limit=a.params.limit||48;g.page=a.params.page||1;Galerie.close();c.modal||Asset.searchAdmin(g,
function(){a.load(jQuery("#admin-assets-index")).interpolate(Asset.toMustache(e),"mustache").replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple")})});c.modal=false;c.first_run=false});this.get("#/assets/new",function(a){a.load(jQuery("#admin-assets-new")).interpolate({},"mustache").replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple")});c.first_run=false});this.post("/admin/assets",function(a){var e=this,g=document.getElementById("ajax_uploader").files,
i=a.params.query?a.params.query:null;jQuery("form#new_asset");var l=i?{query:i}:{};l.limit=a.params.limit||48;l.page=a.params.page||1;this.send_files(g,l,function(){e.render("/templates/admin/assets/index.mustache",Asset.toMustache(l.query)).replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple")})});return false});this.get("#/assets/:id/edit",function(a){var e=a.params.query?a.params.query:null;this.loadAssets(e?{query:a.params.query}:{},function(){var g=Asset.find(a.params.id);
a.load(jQuery("#admin-assets-edit")).interpolate(g.toMustacheWithNeighbors(e),"mustache").replace("#editor").then(function(){setTimeout(function(){$("img.fade-in").fadeIn("slow")},100);Utilities.formObserver(".image-info input[type=text], .image-info textarea")})});c.modal=false});this.put("#/assets/:id",function(a){var e=Asset.find(a.params.id);e.attr(a.params.asset);e.save(function(g){g&&Utilities.notice("Successfully saved asset")})});this.get("#/assets/:id/remove",function(a){var e=a.params.query?
a.params.query:null,g=e?{query:a.params.query}:null;jQuery(".modal-strip").remove();this.loadAssets(g,function(){var i=Asset.find(a.params.id);if(c.first_run){var l=a.load(jQuery("#admin-assets-edit")).interpolate(Asset.toMustache(e),"mustache"),n=a.load(jQuery("#admin-assets-remove")).interpolate({asset:i.toMustache(e)},"mustache");l.replace("#editor").then(function(){var v=jQuery("#asset-"+i.id());n.appendTo(v).then(function(){jQuery(".modal-strip").fadeIn("fast").css({})})})}else{l=jQuery("#asset-"+
i.id());n=a.load(jQuery("#admin-assets-remove")).interpolate({asset:i.toMustache(e)},"mustache");n.appendTo(l).then(function(){jQuery(".modal-strip").fadeIn("fast").css({})})}})});this.del("#/assets/:id",function(a){var e=a.params.query?a.params.query:null,g=e?"?"+decodeURIComponent(jQuery.param({query:e})):"";Asset.find(a.params.id).destroy(function(i){if(i){Galerie.close();Utilities.notice("Successfully saved asset");a.redirect("#/assets"+g)}})})}),Pages=Sammy(function(c){var a=this;this.debug=
false;this.disable_push_state=true;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);c.helpers({loadPages:function(e){if(Page.all().length==0)Page.load(function(){Layout.load(function(){e&&e.call(this)})});else e&&e.call(this)},renderTree:function(e,g){var i=this;i.load(jQuery("#admin-pages-node")).interpolate({pages:[e.asJSON()]},"mustache").replace("#sidebar").then(function(){jQuery("ul.page-children:first").attr("id","pages");i.renderNode(e,g);e.id()==g.id()&&jQuery("li#page-"+
e.id()).addClass("active")})},renderNode:function(e,g){var i=this;i.load(jQuery("#admin-pages-node")).interpolate(e.children().toMustache(),"mustache").appendTo("#page-"+e.id()).then(function(){logger.info(e.id());$("ul#pages ul.page-children").sortable({items:"li"});e.children().each(function(l){l.id()==g.id()&&jQuery("li#page-"+l.id()).addClass("active");if(l.has_children()==true){jQuery("#page-"+l.id()).addClass("open");i.renderNode(l,g)}})})},renderPagePreview:function(e,g){a.modal||this.load(jQuery("#admin-pages-show")).interpolate({page:e.asJSON(),
...
e.removeClass("open").animate({height:"0"},300);g.attr("disabled","disabled")}});c.bind("page-index",function(){var e=this;jQuery(".page-children").length||e.loadPages(function(){e.renderTree(Page.root(),Page.root())})});c.bind("set-active-tab",function(){});c.before(function(e){var g=jQuery("div#tabs a.tab");e=e.path.split("?")[0].split("#/")[1].split("/")[0];e=jQuery("#"+e);g.removeClass("active");e.addClass("active")});c.bind("run",function(){a.application=this;a.refresh_pages=true;a.modal=false;
jQuery(".add-asset").live("click",function(){a.application.trigger("add-asset");return false});jQuery("#sidebar .opener").live("click",function(){var e=(new Date).getTime();e=$(this).parents("li:first");console.log(e);var g=this.id.split("-")[2],i=Page.find(g),l=jQuery.cookie("active_page_ids");l=l?l.split(","):[];if(e.hasClass("open")){e.toggleClass("open");l=jQuery.grep(l,function(n){return n!=g});jQuery.cookie("active_page_ids",l.join(","),{path:"/admin"});i.children().each(function(n){Page.remove(n)});
jQuery("#page-"+g+" ul").remove()}else{l.push(g);e.toggleClass("open");e=(new Date).getTime();jQuery.cookie("active_page_ids",l.join(","),{path:"/admin"});Page.load(function(){a.application.renderNode(i,i)})}a.refresh_pages=false;return a.modal=false})});this.get("#/pages/?",function(e){Galerie.close();e.trigger("page-index");jQuery("#editor").html('<h1 class="section">Pages</div>');a.do_not_refresh=false});this.get("#/pages/:id/new/?",function(e){this.loadPages(function(){var g=Page.find(e.params.id),
i=$("<div />").attr({id:"new-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(i);e.trigger("page-index");e.load(jQuery("#admin-pages-new")).interpolate({parent:g.asJSON(),layouts:Layout.asLayoutJSON(g.attr("layout_id"))},"mustache").replace("#new-page-container")})});this.post("#/pages/:page_id",function(e){Page.find(e.params.page_id);(new Page(e.params.page)).save(function(g,i){var l=JSON.parse(i.responseText);if(l.errors)alert(JSON.stringify(l));else{a.refresh_pages=true;
Utilities.notice("Successfully saved page");e.redirect("#/pages/"+l.id)}})});this.get("#/pages/:id/?",function(e){var g=this;Galerie.close();jQuery(".modal-editor").remove();this.loadPages(function(){var i=e.params.id,l=Page.find(i);if(a.do_not_refresh)g.close_page_editor();else{if(l)e.renderPagePreview(l);else{l=new Page({id:i});l.load(function(){e.renderPagePreview(l)})}a.modal=false;a.do_not_refresh=false;e.trigger("page-index")}});a.do_not_refresh=false});this.get("#/pages/:id/edit/?",function(e){var g=
this;Galerie.close();a.modal=false;this.loadPages(function(){var i=Page.find(e.params.id);if(jQuery("#preview-"+i.id()).length)g.open_page_editor();else{e.renderPagePreview(i,function(){g.open_page_editor()});e.trigger("page-index")}});a.do_not_refresh=true});this.put("#/pages/:page_id",function(e){var g=Page.find(e.params.page_id);g.attr(e.params.page);g.save(function(i){if(i){a.modal=false;Utilities.notice("Successfully saved page");e.renderTree(Page.root(),g)}})});this.get("#/pages/:id/remove",
function(e){this.loadPages(function(){var g=Page.find(e.params.id),i=$("<div />").attr({id:"remove-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(i);e.trigger("page-index");e.load(jQuery("#admin-pages-remove")).interpolate({page:g.asJSON()},"mustache").replace("#remove-page-container")})});this.del("/pages/:id",function(e){var g=e.params.id;Page.find(g).destroy(function(i){if(i){jQuery("#page-"+g).remove();a.refresh_pages=false;e.redirect("#/pages")}})})}),Parts=Sammy(function(c){var a=
this;this.debug=false;this.disable_push_state=true;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);c.helpers({render_part:function(e,g,i){var l=this;l.load(jQuery("#admin-"+i+"-edit")).interpolate({part:e.asJSON(),page:g.asJSON(),assets:Asset.asJSON()},"mustache").appendTo(jQuery("body")).then(function(){var n=jQuery(".modal-editor"),v=$("iframe").contents(),x=v.find("#editor-"+e.id());n.fadeIn("fast").css({top:x.offset().top-v.find("body").scrollTop()+"px",left:x.offset().left+
400+"px"});l.set_asset_links(e,g);jQuery("#ajax_uploader").attr("multiple","multiple").change(function(){var s=jQuery(this).parents("form:first");jQuery(".progress").slideDown("slow",function(){s.submit()})});l.trigger("page-index");a.modal=true})},set_asset_links:function(e,g){jQuery("#search-results-container li.asset").each(function(i,l){var n=jQuery(l).find("a"),v=jQuery(l).attr("id").split("-")[1];n.click(function(x){x.preventDefault();x=g.attr("parts");for(var s=0,D=x.length;s<D;++s){var K=
x[s];if(e.id()==K.id){K.asset_id=v;e.attr("asset_id",v);e.save()}}g.save(function(){jQuery(".modal-editor").remove();a.modal=false;document.location.hash="#"+g.attr("admin_path")})})})}});this.get("#/pages/:page_id/parts/:id/edit",function(e){jQuery(".modal-editor").remove();var g=$("iframe");this.loadPages(function(){Page.load_by_id(e.params.page_id,function(){var i=Page.find(e.params.page_id),l=i.parts().find(e.params.id),n=jQuery("#page-updated_at").attr("value");if(g.length)if(n!=i.attr("updated_at")){var v=
jQuery(".preview iframe");v.hide().attr("src",v.attr("src"));v.load(function(){v.fadeIn("fast");e.render_part(l,i,"parts")})}else e.render_part(l,i,"parts");else e.renderPagePreview(i,function(){e.render_part(l,i,"parts")})})})});this.get("#/pages/:page_id/image_parts/:id/edit",function(e){var g=this;jQuery(".modal-editor").remove();g.loadPages(function(){var i=Page.find(e.params.page_id),l=i.parts().find(e.params.id),n=$("iframe");Asset.searchAdmin({limit:"8"},function(){n.length?g.render_part(l,
i,"image_parts"):e.renderPagePreview(i,function(){g.render_part(l,i,"image_parts")})})})});this.get("/pages/:page_id/parts/:id/results",function(e){var g=this;this.loadPages(function(){var i=Page.find(e.params.page_id),l=Part.find(e.params.id),n=e.params.query?e.params.query:null;Asset.searchAdmin(n?{query:n}:{},function(){e.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),"mustache").replace("#search-results-container").then(function(){g.set_asset_links(l,i)})})})});this.put("#/pages/:page_id/parts/:id",
function(e){var g=e.params.id,i=e.params.page_id,l=Page.find(i),n=l.parts().find(g);logger.info(e.params.part);for(var v=l.attr("parts"),x=0,s=v.length;x<s;++x){n=v[x];if(g==n.id){var D=e.params.part.content;n.content=D;n=Part.find(n.id);n.attr("content",D);n.save()}}l.save(function(K){if(K){a.modal=false;e.redirect("#/pages/"+i)}})});this.post("#/pages/:page_id/image_parts/:id/assets",function(e){var g=this,i=Page.find(e.params.page_id),l=i.parts().find(e.params.id),n=document.getElementById("ajax_uploader").files,
v=e.params.query?e.params.query:null;jQuery("form#new_asset");v=v?{query:v}:{};v.limit=e.params.limit||48;v.page=e.params.page||1;this.send_files(n,v,function(){e.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),"mustache").replace("#search-results-container").then(function(){jQuery("#ajax_uploader").attr("files",null);jQuery(".progress").slideUp("slow",function(){jQuery(this).html("")});g.set_asset_links(l,i)})})})}),Users=Sammy(function(){var c=this;this.debug=false;this.disable_push_state=
true;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);this.helpers({loadUsers:function(a){if(User.all().length==0)User.load(function(){a&&a.call(this)});else a&&a.call(this)},renderUserIndex:function(a){this.load(jQuery("#admin-users-index")).interpolate(User.toMustache(),"mustache").replace("#editor").then(function(){a&&a.call(this)})}});this.bind("run",function(){c.modal=false;c.refresh_pages=true});this.get("#/users",function(a){Galerie.close();jQuery("#editor").html('<h1 class="section">Users</div>');
a.loadUsers(function(){a.renderUserIndex()})});this.get("#/users/new",function(a){a.loadUsers(function(){jQuery("#modal").length||Galerie.open();a.load(jQuery("#admin-users-new")).interpolate({},"mustache").replace("#modal");a.renderUserIndex()})});this.post("#/users",function(a){(new User(a.params.user)).save(function(e,g){var i=JSON.parse(g.responseText);i.errors?alert(JSON.stringify(i)):a.redirect("#/users")})});this.get("#/users/:id/edit",function(a){a.loadUsers(function(){user=User.find(a.params.id);
if(jQuery("#users").length){jQuery(".user-form").html("");a.load(jQuery("#admin-users-edit")).interpolate({user:user.asJSON()},"mustache").replace("#user-form-"+user.id())}else a.renderUserIndex(function(){jQuery(".user-form").html("");a.load(jQuery("#admin-users-edit")).interpolate({user:user.asJSON()},"mustache").replace("#user-form-"+user.id())})})});this.put("#/users/:id",function(a){var e=User.find(a.params.id);e.attr(a.params.user);e.save(function(g,i){var l=JSON.parse(i.responseText);l.errors?
alert(JSON.stringify(l)):a.redirect("#/users")})})});jQuery(document).ready(function(){var c=jQuery("#loader");jQuery("body").ajaxStart(function(){c.show()});jQuery("body").ajaxSuccess(function(){c.hide()});jQuery("#login").length||jQuery.ajax({url:"/templates",success:function(a){jQuery("head").append(a);Pages.run("#/pages")}});Utilities.keyboard_nav();Utilities.check_browser_version()});
