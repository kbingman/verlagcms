...
f.call(this)})}});c.bind("page-index",function(){var a=Page.root();jQuery.cookie("active_page_ids")&&jQuery.cookie("active_page_ids").split(",");Page.site_map();this.load(jQuery("script#admin-pages-index")).interpolate({root:a.asJSON(),partials:{node:jQuery("script#admin-pages-node").html()}},"mustache").replace("#sidebar")});c.bind("set-active-page",function(a,f){jQuery("li.node").removeClass("active");jQuery("li#page-"+f.id()).addClass("active")});c.bind("reload-page",function(a,f){var e=f.id(),
h=jQuery("iframe#page-iframe-"+e);jQuery("li#page-"+e).find("span.title a").text(f.attr("title"));jQuery("h1#page-title-"+e).text(f.attr("title"));h.attr("src",h.attr("src"))});this.get("/admin/pages/?",function(a){var f=Page.first();a.redirect(f.attr("admin_path"))});this.get("/admin/pages/:id/new/?",function(a){var f=Page.find(a.params.id),e=$("<div />").attr({id:"new-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(e);a.trigger("page-index");a.load(jQuery("#admin-pages-new")).interpolate({parent:f.asJSON(),
layouts:Layout.asLayoutJSON(f.attr("layout_id"))},"mustache").replace("#new-page-container")});this.post("/admin/pages/:page_id/?",function(a){Page.find(a.params.page_id);(new Page(a.params.page)).save(function(f,e){var h=JSON.parse(e.responseText);h.errors?alert(JSON.stringify(h)):a.redirect(h.admin_path)})});this.get("/admin/pages/:id/?",function(a){jQuery("#overlay").remove();jQuery(".modal-editor").remove();var f=Page.find(a.params.id);if(f)a.renderPagePreview(f);else{f=new Page({id:a.params.id});
f.load(function(){a.renderPagePreview(f)})}a.trigger("page-index",f)});this.get("/admin/pages/:id/edit/?",function(a){var f=Page.find(a.params.id);a.renderPageEditor(f,function(){a.renderPageProperties(f)});a.trigger("page-index",f)});this.put("/admin/pages/:page_id",function(a){var f=Page.find(a.params.page_id),e=jQuery("span.timestamp").text();f.attr(a.params.page);e<window.timestamp?alert("Someone else has edited this page, please reload and try again"):f.save(function(h){if(h){Utilities.setTimestamp();
Utilities.notice("Successfully saved page");a.trigger("reload-page",f);a.redirect(f.attr("admin_path"))}})});this.get("/admin/pages/:id/remove",function(a){var f=Page.find(a.params.id),e=$("<div />").attr({id:"remove-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(e);a.trigger("page-index");a.load(jQuery("#admin-pages-remove")).interpolate({page:f.asJSON()},"mustache").replace("#remove-page-container")});this.del("/admin/pages/:id",function(a){var f=Page.find(a.params.id);
console.log("HEY");console.log(f);Page.remove(f);f.destroy(function(e){e&&a.redirect("/admin/pages")})})}),Parts=Sammy(function(c){c.helpers({renderPart:function(a,f,e){var h=this;h.load(jQuery("script#admin-"+e+"-edit")).interpolate({part:a.asJSON(),page:f.asJSON(),assets:Asset.asJSON(),timestamp:window.timestamp},"mustache").replace("#page-tabs-"+f.id()).then(function(){var l=aceModes.textile;window.editor=ace.edit("part-"+a.id()+"-content");window.editor.setTheme("ace/theme/textmate");window.editor.getSession().setMode(new l);
