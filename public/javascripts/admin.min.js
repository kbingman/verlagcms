...
j=j.height()+42;j=(t-j)/2;$("#overlay").css({height:t}).hide().fadeIn("fast");o.hide().fadeIn("fast").css({top:j});h&&h.call(this)},close:function(){$("#modal_wrapper").fadeOut("fast",function(){$(this).remove();$("#overlay").fadeOut("fast",function(){$(this).remove()})})}},Utilities={notice:function(i){var h=jQuery(".notice");h.text(i);h.slideDown("fast");setTimeout(function(){h.slideUp("fast")},"2000")},keyboard_nav:function(){jQuery("body").keydown(function(i){switch(i.keyCode){case 37:$("a.previous").click();
break;case 39:$("a.next").click();break}})},check_browser_version:function(){if(!Modernizr.history){jQuery("a").live("click",function(h){h.preventDefault();document.location.hash=$(this).attr("href")});var i=jQuery("form#search-form");i.attr("action","#"+i.attr("action"))}},formObserver:function(i){jQuery(i).keyup(function(){delay(function(){jQuery(i).parents("form:first").submit()},800)})}},iFramer={initialize:function(i){i=jQuery(i);var h=this;i&&i.load(function(){var j=$(this).contents().find("a.verlag-editor");
h.setEditFlags(j);j.click(function(){window.top.trigger=$(this);window.top.location.hash=$(this).attr("href");return false})})},setEditFlags:function(i){i.css({position:"relative",display:"block",height:"0",width:"0","text-decoration":"none"}).find("span").css({display:"block",position:"absolute",top:"-24px",left:"12px",padding:"4px 8px",width:"100px",background:"rgba(255, 255, 0, 0.7)"})}},TabControl={initialize:function(i){if(i=jQuery(i)){i.each(function(h,j){$(j).hide()});i.first().show();this.tabControl(jQuery(".tab-control"))}},
tabControl:function(i){i.click(function(){var h="tab-"+$(this).find("label").attr("for").split("-")[1];console.log(h);jQuery(".tab").hide();jQuery("#"+h).show()})}},delay=function(){var i=0;return function(h,j){clearTimeout(i);i=setTimeout(h,j)}}();
