...
d)}function z(a,d,b,e,f,g){f=f||d.dataTypes[0];g=g||{};g[f]=true;f=a[f];for(var i=0,n=f?f.length:0,p=a===cb,r;i<n&&(p||!r);i++){r=f[i](d,b,e);typeof r=="string"&&(!p||g[r]?(r=h):(d.dataTypes.unshift(r),r=z(a,d,b,e,r,g)))}(p||!r)&&!g["*"]&&(r=z(a,d,b,e,"*",g));return r}function N(a){return function(d,b){typeof d!="string"&&(b=d,d="*");if(c.isFunction(b))for(var e=d.toLowerCase().split(cc),f=0,g=e.length,i,n;f<g;f++){i=e[f];(n=/^\+/.test(i))&&(i=i.substr(1)||"*");i=a[i]=a[i]||[];i[n?"unshift":"push"](b)}}}
...
function(f){return f.nodeType===1});if(Ka.test(d))return c.filter(d,e,!b);d=c.filter(d,e)}return c.grep(a,function(f){return c.inArray(f,d)>=0===b})}function ca(a,d){return(a&&a!=="*"?a+".":"")+d.replace(La,"`").replace(gb,"&")}function oa(a){var d,b,e,f,g,i,n,p,r,x,C,R=[];f=[];g=c._data(this,"events");if(!(a.liveFired===this||!g||!g.live||a.target.disabled||a.button&&a.type==="click")){a.namespace&&(C=new RegExp("(^|\\.)"+a.namespace.split(".").join("\\.(?:.*\\.)?")+"(\\.|$)"));a.liveFired=this;
var ba=g.live.slice(0);for(n=0;n<ba.length;n++){g=ba[n];g.origType.replace(sa,"")===a.type?f.push(g.selector):ba.splice(n--,1)}f=c(a.target).closest(f,a.currentTarget);p=0;for(r=f.length;p<r;p++){x=f[p];for(n=0;n<ba.length;n++){g=ba[n];if(x.selector===g.selector&&(!C||C.test(g.namespace))&&!x.elem.disabled){i=x.elem;e=null;if(g.preType==="mouseenter"||g.preType==="mouseleave"){a.type=g.preType;(e=c(a.relatedTarget).closest(g.selector)[0])&&c.contains(i,e)&&(e=i)}(!e||e!==i)&&R.push({elem:i,handleObj:g,
...
p,"mouseleave"+p);else{n=g;rb[g]?(b.push(rb[g]+p),g+=p):(g=(rb[g]||g)+p);if(d==="live"){p=0;for(var C=x.length;p<C;p++)c.event.add(x[p],"live."+ca(g,r),{data:e,selector:r,handler:f,origType:g,origHandler:f,preType:n})}else x.unbind("live."+ca(g,r),f)}}return this}});c.each("blur focus focusin focusout load resize scroll unload click dblclick mousedown mouseup mousemove mouseover mouseout mouseenter mouseleave change select submit keydown keypress keyup error".split(" "),function(a,d){c.fn[d]=function(b,
e){e==null&&(e=b,b=null);return arguments.length>0?this.bind(d,b,e):this.trigger(d)};c.attrFn&&(c.attrFn[d]=true)});(function(){function a(o,u,G,I,D,J){D=0;for(var U=I.length;D<U;D++){var Z=I[D];if(Z){var ra=false;for(Z=Z[o];Z;){if(Z.sizcache===G){ra=I[Z.sizset];break}if(Z.nodeType===1){J||(Z.sizcache=G,Z.sizset=D);if(typeof u!="string"){if(Z===u){ra=true;break}}else if(r.filter(u,[Z]).length>0){ra=Z;break}}Z=Z[o]}I[D]=ra}}}function d(o,u,G,I,D,J){D=0;for(var U=I.length;D<U;D++){var Z=I[D];if(Z){var ra=
false;for(Z=Z[o];Z;){if(Z.sizcache===G){ra=I[Z.sizset];break}Z.nodeType===1&&!J&&(Z.sizcache=G,Z.sizset=D);if(Z.nodeName.toLowerCase()===u){ra=Z;break}Z=Z[o]}I[D]=ra}}}var b=/((?:\((?:\([^()]+\)|[^()]+)+\)|\[(?:\[[^\[\]]*\]|['"][^'"]*['"]|[^\[\]'"]+)+\]|\\.|[^ >+~,(\[\\]+)+|[>+~])(\s*,\s*)?((?:.|\r|\n)*)/g,e=0,f=Object.prototype.toString,g=false,i=true,n=/\\/g,p=/\W/;[0,0].sort(function(){i=false;return 0});var r=function(o,u,G,I){G=G||[];var D=u=u||X;if(u.nodeType!==1&&u.nodeType!==9)return[];if(!o||
typeof o!="string")return G;var J,U,Z,ra,Ma,Ja,Sa=true,v=r.isXML(u),K=[],da=o;do{b.exec("");if(J=b.exec(da)){da=J[3];K.push(J[1]);if(J[2]){ra=J[3];break}}}while(J);if(K.length>1&&C.exec(o))if(K.length===2&&x.relative[K[0]])U=Ca(K[0]+K[1],u);else for(U=x.relative[K[0]]?[u]:r(K.shift(),u);K.length;){o=K.shift();x.relative[o]&&(o+=K.shift());U=Ca(o,U)}else{!I&&K.length>1&&u.nodeType===9&&!v&&x.match.ID.test(K[0])&&!x.match.ID.test(K[K.length-1])&&(Ma=r.find(K.shift(),u,v),u=Ma.expr?r.filter(Ma.expr,
Ma.set)[0]:Ma.set[0]);if(u){Ma=I?{expr:K.pop(),set:ka(I)}:r.find(K.pop(),K.length===1&&(K[0]==="~"||K[0]==="+")&&u.parentNode?u.parentNode:u,v);U=Ma.expr?r.filter(Ma.expr,Ma.set):Ma.set;for(K.length>0?(Z=ka(U)):(Sa=false);K.length;){J=Ja=K.pop();x.relative[Ja]?(J=K.pop()):(Ja="");J==null&&(J=u);x.relative[Ja](Z,J,v)}}else Z=[]}Z||(Z=U);Z||r.error(Ja||o);if(f.call(Z)==="[object Array]")if(Sa)if(u&&u.nodeType===1)for(o=0;Z[o]!=null;o++)Z[o]&&(Z[o]===true||Z[o].nodeType===1&&r.contains(u,Z[o]))&&G.push(U[o]);
else for(o=0;Z[o]!=null;o++)Z[o]&&Z[o].nodeType===1&&G.push(U[o]);else G.push.apply(G,Z);else ka(Z,G);ra&&(r(ra,D,G,I),r.uniqueSort(G));return G};r.uniqueSort=function(o){if(za){g=i;o.sort(za);if(g)for(var u=1;u<o.length;u++)o[u]===o[u-1]&&o.splice(u--,1)}return o};r.matches=function(o,u){return r(o,null,null,u)};r.matchesSelector=function(o,u){return r(u,null,null,[o]).length>0};r.find=function(o,u,G){var I;if(!o)return[];for(var D=0,J=x.order.length;D<J;D++){var U,Z=x.order[D];if(U=x.leftMatch[Z].exec(o)){var ra=
U[1];U.splice(1,1);if(ra.substr(ra.length-1)!=="\\"){U[1]=(U[1]||"").replace(n,"");I=x.find[Z](U,u,G);if(I!=null){o=o.replace(x.match[Z],"");break}}}}I||(I=typeof u.getElementsByTagName!="undefined"?u.getElementsByTagName("*"):[]);return{set:I,expr:o}};r.filter=function(o,u,G,I){for(var D,J,U=o,Z=[],ra=u,Ma=u&&u[0]&&r.isXML(u[0]);o&&u.length;){for(var Ja in x.filter)if((D=x.leftMatch[Ja].exec(o))!=null&&D[2]){var Sa,v,K=x.filter[Ja];v=D[1];J=false;D.splice(1,1);if(v.substr(v.length-1)!=="\\"){ra===
Z&&(Z=[]);if(x.preFilter[Ja])if(D=x.preFilter[Ja](D,ra,G,Z,I,Ma)){if(D===true)continue}else J=Sa=true;if(D)for(var da=0;(v=ra[da])!=null;da++)if(v){Sa=K(v,D,da,ra);var Y=I^!!Sa;G&&Sa!=null?Y?(J=true):(ra[da]=false):Y&&(Z.push(v),J=true)}if(Sa!==h){G||(ra=Z);o=o.replace(x.match[Ja],"");if(!J)return[];break}}}if(o===U)if(J==null)r.error(o);else break;U=o}return ra};r.error=function(o){throw"Syntax error, unrecognized expression: "+o;};var x=r.selectors={order:["ID","NAME","TAG"],match:{ID:/#((?:[\w\u00c0-\uFFFF\-]|\\.)+)/,
CLASS:/\.((?:[\w\u00c0-\uFFFF\-]|\\.)+)/,NAME:/\[name=['"]*((?:[\w\u00c0-\uFFFF\-]|\\.)+)['"]*\]/,ATTR:/\[\s*((?:[\w\u00c0-\uFFFF\-]|\\.)+)\s*(?:(\S?=)\s*(?:(['"])(.*?)\3|(#?(?:[\w\u00c0-\uFFFF\-]|\\.)*)|)|)\s*\]/,TAG:/^((?:[\w\u00c0-\uFFFF\*\-]|\\.)+)/,CHILD:/:(only|nth|last|first)-child(?:\(\s*(even|odd|(?:[+\-]?\d+|(?:[+\-]?\d*)?n\s*(?:[+\-]\s*\d+)?))\s*\))?/,POS:/:(nth|eq|gt|lt|first|last|even|odd)(?:\((\d*)\))?(?=[^\-]|$)/,PSEUDO:/:((?:[\w\u00c0-\uFFFF\-]|\\.)+)(?:\((['"]?)((?:\([^\)]+\)|[^\(\)]*)+)\2\))?/},
leftMatch:{},attrMap:{"class":"className","for":"htmlFor"},attrHandle:{href:function(o){return o.getAttribute("href")},type:function(o){return o.getAttribute("type")}},relative:{"+":function(o,u){var G=typeof u=="string",I=G&&!p.test(u);G=G&&!I;I&&(u=u.toLowerCase());I=0;for(var D=o.length,J;I<D;I++)if(J=o[I]){for(;(J=J.previousSibling)&&J.nodeType!==1;);o[I]=G||J&&J.nodeName.toLowerCase()===u?J||false:J===u}G&&r.filter(u,o,true)},">":function(o,u){var G,I=typeof u=="string",D=0,J=o.length;if(I&&
!p.test(u))for(u=u.toLowerCase();D<J;D++){if(G=o[D]){G=G.parentNode;o[D]=G.nodeName.toLowerCase()===u?G:false}}else{for(;D<J;D++)(G=o[D])&&(o[D]=I?G.parentNode:G.parentNode===u);I&&r.filter(u,o,true)}},"":function(o,u,G){var I,D=e++,J=a;typeof u=="string"&&!p.test(u)&&(u=u.toLowerCase(),I=u,J=d);J("parentNode",u,D,o,I,G)},"~":function(o,u,G){var I,D=e++,J=a;typeof u=="string"&&!p.test(u)&&(u=u.toLowerCase(),I=u,J=d);J("previousSibling",u,D,o,I,G)}},find:{ID:function(o,u,G){if(typeof u.getElementById!=
...
(" "+U.className+" ").replace(/[\t\n\r]/g," ").indexOf(o)>=0)?G||I.push(U):G&&(u[J]=false));return false},ID:function(o){return o[1].replace(n,"")},TAG:function(o){return o[1].replace(n,"").toLowerCase()},CHILD:function(o){if(o[1]==="nth"){o[2]||r.error(o[0]);o[2]=o[2].replace(/^\+|\s*/g,"");var u=/(-?)(\d*)(?:n([+\-]?\d*))?/.exec(o[2]==="even"&&"2n"||o[2]==="odd"&&"2n+1"||!/\D/.test(o[2])&&"0n+"+o[2]||o[2]);o[2]=u[1]+(u[2]||1)-0;o[3]=u[3]-0}else o[2]&&r.error(o[0]);o[0]=e++;return o},ATTR:function(o,
u,G,I,D,J){u=o[1]=o[1].replace(n,"");!J&&x.attrMap[u]&&(o[1]=x.attrMap[u]);o[4]=(o[4]||o[5]||"").replace(n,"");o[2]==="~="&&(o[4]=" "+o[4]+" ");return o},PSEUDO:function(o,u,G,I,D){if(o[1]==="not")if((b.exec(o[3])||"").length>1||/^\w/.test(o[3]))o[3]=r(o[3],null,null,u);else{o=r.filter(o[3],u,G,true^D);G||I.push.apply(I,o);return false}else if(x.match.POS.test(o[0])||x.match.CHILD.test(o[0]))return true;return o},POS:function(o){o.unshift(true);return o}},filters:{enabled:function(o){return o.disabled===
false&&o.type!=="hidden"},disabled:function(o){return o.disabled===true},checked:function(o){return o.checked===true},selected:function(o){return o.selected===true},parent:function(o){return!!o.firstChild},empty:function(o){return!o.firstChild},has:function(o,u,G){return!!r(G[3],o).length},header:function(o){return/h\d/i.test(o.nodeName)},text:function(o){var u=o.getAttribute("type"),G=o.type;return o.nodeName.toLowerCase()==="input"&&"text"===G&&(u===G||u===null)},radio:function(o){return o.nodeName.toLowerCase()===
...
2)});o=null})();X.querySelectorAll&&function(){var o=r,u=X.createElement("div");u.innerHTML="<p class='TEST'></p>";if(!u.querySelectorAll||u.querySelectorAll(".TEST").length!==0){r=function(I,D,J,U){D=D||X;if(!U&&!r.isXML(D)){var Z=/^(\w+$)|^\.([\w\-]+$)|^#([\w\-]+$)/.exec(I);if(Z&&(D.nodeType===1||D.nodeType===9)){if(Z[1])return ka(D.getElementsByTagName(I),J);if(Z[2]&&x.find.CLASS&&D.getElementsByClassName)return ka(D.getElementsByClassName(Z[2]),J)}if(D.nodeType===9){if(I==="body"&&D.body)return ka([D.body],
...
300||D===304){if(e.ifModified){if(Y=u.getResponseHeader("Last-Modified"))c.lastModified[r]=Y;if(Y=u.getResponseHeader("Etag"))c.etag[r]=Y}if(D===304){J="notmodified";ra=true}else try{Y=e;U=U;Y.dataFilter&&(U=Y.dataFilter(U,Y.dataType));var Fa=Y.dataTypes;qa={};var vb,wb,oc=Fa.length,jc,Bb=Fa[0],Nb,nc,Cb,Ib,Ob;for(vb=1;vb<oc;vb++){if(vb===1)for(wb in Y.converters)typeof wb=="string"&&(qa[wb.toLowerCase()]=Y.converters[wb]);Nb=Bb;Bb=Fa[vb];if(Bb==="*")Bb=Nb;else if(Nb!=="*"&&Nb!==Bb){nc=Nb+" "+Bb;Cb=
...
ub++}});c.ajaxPrefilter("json jsonp",function(a,d,b){d=a.contentType==="application/x-www-form-urlencoded"&&typeof a.data=="string";if(a.dataTypes[0]==="jsonp"||a.jsonp!==false&&(Db.test(a.url)||d&&Db.test(a.data))){var e,f=a.jsonpCallback=c.isFunction(a.jsonpCallback)?a.jsonpCallback():a.jsonpCallback,g=k[f],i=a.url,n=a.data,p="$1"+f+"$2";a.jsonp!==false&&(i=i.replace(Db,p),a.url===i&&(d&&(n=n.replace(Db,p)),a.data===n&&(i+=(/\?/.test(i)?"&":"?")+a.jsonp+"="+f)));a.url=i;a.data=n;k[f]=function(r){e=
[r]};b.always(function(){k[f]=g;e&&c.isFunction(g)&&k[f](e[0])});a.converters["script json"]=function(){e||c.error(f+" was not called");return e[0]};a.dataTypes[0]="json";return"script"}});c.ajaxSetup({accepts:{script:"text/javascript, application/javascript, application/ecmascript, application/x-ecmascript"},contents:{script:/javascript|ecmascript/},converters:{"text script":function(a){c.globalEval(a);return a}}});c.ajaxPrefilter("script",function(a){a.cache===h&&(a.cache=false);a.crossDomain&&
...
g,i;a.username?f.open(a.type,a.url,a.async,a.username,a.password):f.open(a.type,a.url,a.async);if(a.xhrFields)for(i in a.xhrFields)f[i]=a.xhrFields[i];a.mimeType&&f.overrideMimeType&&f.overrideMimeType(a.mimeType);!a.crossDomain&&!b["X-Requested-With"]&&(b["X-Requested-With"]="XMLHttpRequest");try{for(i in b)f.setRequestHeader(i,b[i])}catch(n){}f.send(a.hasContent&&a.data||null);d=function(p,r){var x,C,R,ba,ka;try{if(d&&(r||f.readyState===4)){d=h;g&&(f.onreadystatechange=c.noop,bb&&delete pb[g]);
...
jb,Pa=[["height","marginTop","marginBottom","paddingTop","paddingBottom"],["width","marginLeft","marginRight","paddingLeft","paddingRight"],["opacity"]],qb,Eb=k.webkitRequestAnimationFrame||k.mozRequestAnimationFrame||k.oRequestAnimationFrame;c.fn.extend({show:function(a,d,b){var e;if(a||a===0)return this.animate(s("show",3),a,d,b);d=0;for(b=this.length;d<b;d++){a=this[d];a.style&&(e=a.style.display,!c._data(a,"olddisplay")&&e==="none"&&(e=a.style.display=""),e===""&&c.css(a,"display")==="none"&&
...
(d+=parseFloat(c.css(a,"marginTop"))||0,b+=parseFloat(c.css(a,"marginLeft"))||0);return{top:d,left:b}},setOffset:function(a,d,b){var e=c.css(a,"position");e==="static"&&(a.style.position="relative");var f=c(a),g=f.offset(),i=c.css(a,"top"),n=c.css(a,"left"),p={},r={},x,C;(e==="absolute"||e==="fixed")&&c.inArray("auto",[i,n])>-1?(r=f.position(),x=r.top,C=r.left):(x=parseFloat(i)||0,C=parseFloat(n)||0);c.isFunction(d)&&(d=d.call(a,b,g));d.top!=null&&(p.top=d.top-g.top+x);d.left!=null&&(p.left=d.left-
...
function(E,ca){l._unlisten(q,ca)})});this._running=false;return this},bindToAllEvents:function(l){var q=this;k.each(this.APP_EVENTS,function(y,E){q.bind(E,l)});k.each(this.listeners.keys(true),function(y,E){k.inArray(E,q.APP_EVENTS)==-1&&q.bind(E,l)});return this},routablePath:function(l){return l.replace(s,"")},lookupRoute:function(l,q){var y=false,E=0,ca,oa;if(typeof this.routes[l]!="undefined")for(ca=this.routes[l].length;E<ca;E++){oa=this.routes[l][E];if(this.routablePath(q).match(oa.path)){y=
...
(function(k){function h(m){m=unescape(m);return m==="true"?true:m==="false"?false:m}function j(m,s,t){var w;if(s.match(/^[^\[]+$/))m[s]=h(t);else if(w=s.match(/^([^\[]+)\[\](.*)$/)){s=w[1];w=w[2];if(m[s]&&!k.isArray(m[s]))throw"400 Bad Request";if(w){w=w.match(/^\[([^\]]+)\](.*)$/);if(!w)throw"400 Bad Request";if(m[s])m[s][m[s].length-1][w[1]]?m[s].push(j({},w[1]+w[2],t)):k.extend(true,m[s][m[s].length-1],j({},w[1]+w[2],t));else m[s]=[j({},w[1]+w[2],t)]}else if(m[s])m[s].push(h(t));else m[s]=[h(t)]}else if(w=
s.match(/^([^\[]+)\[([^\[]+)\](.*)$/)){s=w[1];w=w[2]+w[3];if(m[s]&&k.isArray(m[s]))throw"400 Bad Request";if(m[s])k.extend(true,m[s],j(m[s],w,t));else m[s]=j({},w,t)}return m}Sammy=Sammy||{};Sammy.NestedParams=function(m){m._parseParamPair=j}})(jQuery);
(function(){if(!window.JSON)window.JSON={};(function(){function k(z){return z<10?"0"+z:z}function h(z){s.lastIndex=0;return s.test(z)?'"'+z.replace(s,function(N){var P=F[N];return typeof P==="string"?P:"\\u"+("0000"+N.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+z+'"'}function j(z,N){var P,la,pa,na,l=t,q,y=N[z];if(y&&typeof y==="object"&&typeof y.toJSON==="function")y=y.toJSON(z);if(typeof B==="function")y=B.call(N,z,y);switch(typeof y){case "string":return h(y);case "number":return isFinite(y)?
...
(function(k){if(!window.Mustache)var h=function(){var j=function(){};j.prototype={otag:"{{",ctag:"}}",pragmas:{},buffer:[],pragmas_implemented:{"IMPLICIT-ITERATOR":true},context:{},render:function(m,s,t,w){if(!w){this.context=s;this.buffer=[]}if(!this.includes("",m))if(w)return m;else{this.send(m);return}m=this.render_pragmas(m);m=this.render_section(m,s,t);if(w)return this.render_tags(m,s,t,w);this.render_tags(m,s,t,w)},send:function(m){m!=""&&this.buffer.push(m)},render_pragmas:function(m){if(!this.includes("%",
m))return m;var s=this;return m.replace(new RegExp(this.otag+"%([\\w-]+) ?([\\w]+=[\\w]+)?"+this.ctag),function(t,w,F){if(!s.pragmas_implemented[w])throw{message:"This implementation of mustache doesn't understand the '"+w+"' pragma"};s.pragmas[w]={};if(F){t=F.split("=");s.pragmas[w][t[0]]=t[1]}return""})},render_partial:function(m,s,t){m=this.trim(m);if(!t||t[m]===undefined)throw{message:"unknown_partial '"+m+"'"};if(typeof s[m]!="object")return this.render(t[m],s,t,true);return this.render(t[m],
s[m],t,true)},render_section:function(m,s,t){if(!this.includes("#",m)&&!this.includes("^",m))return m;var w=this;return m.replace(new RegExp(this.otag+"(\\^|\\#)\\s*(.+)\\s*"+this.ctag+"\n*([\\s\\S]+?)"+this.otag+"\\/\\s*\\2\\s*"+this.ctag+"\\s*","mg"),function(F,B,z,N){F=w.find(z,s);if(B=="^")return!F||w.is_array(F)&&F.length===0?w.render(N,s,t,true):"";else if(B=="#")return w.is_array(F)?w.map(F,function(P){return w.render(N,w.create_context(P),t,true)}).join(""):w.is_object(F)?w.render(N,w.create_context(F),
t,true):typeof F==="function"?F.call(s,N,function(P){return w.render(P,s,t,true)}):F?w.render(N,s,t,true):""})},render_tags:function(m,s,t,w){var F=this,B=function(){return new RegExp(F.otag+"(=|!|>|\\{|%)?([^\\/#\\^]+?)\\1?"+F.ctag+"+","g")},z=B(),N=function(la,pa,na){switch(pa){case "!":return"";case "=":F.set_delimiters(na);z=B();return"";case ">":return F.render_partial(na,s,t);case "{":return F.find(na,s);default:return F.escape(F.find(na,s))}};m=m.split("\n");for(var P=0;P<m.length;P++){m[P]=
m[P].replace(z,N,this);w||this.send(m[P])}if(w)return m.join("\n")},set_delimiters:function(m){m=m.split(" ");this.otag=this.escape_regex(m[0]);this.ctag=this.escape_regex(m[1])},escape_regex:function(m){if(!arguments.callee.sRE)arguments.callee.sRE=new RegExp("(\\/|\\.|\\*|\\+|\\?|\\||\\(|\\)|\\[|\\]|\\{|\\}|\\\\)","g");return m.replace(arguments.callee.sRE,"\\$1")},find:function(m,s){m=this.trim(m);var t;if(s[m]===false||s[m]===0||s[m])t=s[m];else if(this.context[m]===false||this.context[m]===0||
this.context[m])t=this.context[m];if(typeof t==="function")return t.apply(s);if(t!==undefined)return t;return""},includes:function(m,s){return s.indexOf(this.otag+m)!=-1},escape:function(m){return String(m===null?"":m).replace(/&(?!\w+;)|["<>\\]/g,function(s){switch(s){case "&":return"&amp;";case "\\":return"\\\\";case '"':return'"';case "<":return"&lt;";case ">":return"&gt;";default:return s}})},create_context:function(m){if(this.is_object(m))return m;else{var s=".";if(this.pragmas["IMPLICIT-ITERATOR"])s=
this.pragmas["IMPLICIT-ITERATOR"].iterator;var t={};t[s]=m;return t}},is_object:function(m){return m&&typeof m=="object"},is_array:function(m){return Object.prototype.toString.call(m)==="[object Array]"},trim:function(m){return m.replace(/^\s*|\s*$/g,"")},map:function(m,s){if(typeof m.map=="function")return m.map(s);else{for(var t=[],w=m.length,F=0;F<w;F++)t.push(s(m[F]));return t}}};return{name:"mustache.js",version:"0.3.1-dev",to_html:function(m,s,t,w){var F=new j;if(w)F.send=w;F.render(m,s,t);
if(!w)return F.buffer.join("\n")}}}();Sammy=Sammy||{};Sammy.Mustache=function(j,m){m||(m="mustache");j.helper(m,function(s,t,w){t=k.extend({},this,t);w=k.extend({},t.partials,w);return h.to_html(s,t,w)})}})(jQuery);
...
L.getComputedStyle(la,null).WebkitAppearance!=="textfield"&&la.offsetHeight!==0,B.removeChild(la)):/^(search|tel)$/.test(A)||(/^(url|email)$/.test(A)?(c=la.checkValidity&&la.checkValidity()===false):/^color$/.test(A)?(B.appendChild(la),c=la.value!=pa,B.removeChild(la)):(c=la.value!=pa)));ca[Q[V]]=!!c}return ca}("search tel url email datetime date month week time datetime-local number range color".split(" "))}function s(Q,V){var c=Q.charAt(0).toUpperCase()+Q.substr(1);c=(Q+" "+q.join(c+" ")+c).split(" ");
...
l.join("transform-3d),(")+"modernizr)"));return Q};E.csstransitions=function(){return s("transitionProperty")};E.fontface=function(){var Q,V,c=z||B,A=h.createElement("style");V=h.implementation||{hasFeature:function(){return false}};A.type="text/css";c.insertBefore(A,c.firstChild);Q=A.sheet||A.styleSheet;V=(V.hasFeature("CSS2","")?function(L){if(!Q||!L)return false;var M=false;try{Q.insertRule(L,0);M=/src/i.test(Q.cssRules[0].cssText);Q.deleteRule(Q.cssRules.length-1)}catch(S){}return M}:function(L){if(!Q||
...
Model.ClassMethods={add:function(k){var h=k.id();if(jQuery.inArray(k,this.collection)===-1&&!(h&&this.find(h))){this.collection.push(k);this.trigger("add",[k])}return this},all:function(){return this.collection.slice()},chain:function(k){return jQuery.extend({},this,{collection:k})},count:function(){return this.all().length},detect:function(k){for(var h=this.all(),j,m=0,s=h.length;m<s;m++){j=h[m];if(k.call(j,j,m))return j}},each:function(k,h){for(var j=this.all(),m=0,s=j.length;m<s;m++)k.call(h||
j[m],j[m],m,j);return this},find:function(k){return this.detect(function(){return this.id()==k})},first:function(){return this.all()[0]},load:function(k){if(this._persistence){var h=this;this._persistence.read(function(j){for(var m=0,s=j.length;m<s;m++)h.add(j[m]);k&&k.call(h,j)})}return this},last:function(){var k=this.all();return k[k.length-1]},map:function(k,h){for(var j=this.all(),m=[],s=0,t=j.length;s<t;s++)m.push(k.call(h||j[s],j[s],s,j));return m},persistence:function(k){if(arguments.length==
0)return this._persistence;else{var h=Array.prototype.slice.call(arguments,1);h.unshift(this);this._persistence=k.apply(k,h);return this}},pluck:function(k){for(var h=this.all(),j=[],m=0,s=h.length;m<s;m++)j.push(h[m].attr(k));return j},remove:function(k){for(var h,j=0,m=this.collection.length;j<m;j++)if(this.collection[j]===k){h=j;break}if(h!=undefined){this.collection.splice(h,1);this.trigger("remove",[k]);return true}else return false},reverse:function(){return this.chain(this.all().reverse())},
select:function(k,h){for(var j=this.all(),m=[],s,t=0,w=j.length;t<w;t++){s=j[t];if(k.call(h||s,s,t,j))m.push(s)}return this.chain(m)},sort:function(k){return this.chain(this.all().sort(k))},sortBy:function(k){var h=jQuery.isFunction(k);return this.sort(function(j,m){var s=h?k.call(j):j.attr(k),t=h?k.call(m):m.attr(k);return s<t?-1:s>t?1:0})},use:function(k){var h=Array.prototype.slice.call(arguments,1);h.unshift(this);k.apply(this,h);return this}};
...
Model.localStorage=function(k){if(!window.localStorage)return{create:function(s,t){t(true)},destroy:function(s,t){t(true)},read:function(s){s([])},update:function(s,t){t(true)}};var h=[k._name,"collection"].join("-"),j=function(){var s=localStorage[h];return s?JSON.parse(s):[]},m=function(s){localStorage.setItem(s.uid,JSON.stringify(s.asJSON()));s=s.uid;var t=j();if(jQuery.inArray(s,t)===-1){t.push(s);localStorage.setItem(h,JSON.stringify(t))}};return{create:function(s,t){m(s);t(true)},destroy:function(s,
t){localStorage.removeItem(s.uid);var w=s.uid,F=j();w=jQuery.inArray(w,F);if(w>-1){F.splice(w,1);localStorage.setItem(h,JSON.stringify(F))}t(true)},read:function(s){if(!s)return false;for(var t=k.map(function(){return this.uid}),w=j(),F=[],B,z,N=0,P=w.length;N<P;N++){z=w[N];if(jQuery.inArray(z,t)==-1){B=JSON.parse(localStorage[z]);B=new k(B);B.uid=z;F.push(B)}}s(F)},update:function(s,t){m(s);t(true)}}};Model.Log=function(){window.console&&window.console.log.apply(window.console,arguments)};
...
n=Ka({line:n,ch:g});ua({line:f.start,ch:0},{line:i-1,ch:W[i-1].text.length},f.old,n,n)}}function wa(){ia(Za.done,Za.undone)}function Ga(){ia(Za.undone,Za.done)}function ua(b,e,f,g,i){for(var n=false,p=Eb.length,r=b.line;r<e.line;++r)if(W[r].text.length==p){n=true;break}var x=e.line-b.line;r=W[b.line];var C=W[e.line];if(r==C)if(f.length==1)r.replace(b.ch,e.ch,f[0]);else{C=r.split(e.ch,f[f.length-1]);var R=[b.line+1,x];r.replace(b.ch,r.text.length,f[0]);r=1;for(var ba=f.length-1;r<ba;++r)R.push(new s(f[r]));
R.push(C);W.splice.apply(W,R)}else if(f.length==1){r.replace(b.ch,r.text.length,f[0]+C.text.slice(e.ch));W.splice(b.line+1,x)}else{R=[b.line+1,x-1];r.replace(b.ch,r.text.length,f[0]);C.replace(0,e.ch,f[f.length-1]);r=1;for(ba=f.length-1;r<ba;++r)R.push(new s(f[r]));W.splice.apply(W,R)}r=b.line;for(ba=r+f.length;r<ba;++r){C=W[r].text;if(C.length>p){Eb=C;p=C.length;n=false}}if(n){r=p=0;for(ba=W.length;r<ba;++r){C=W[r].text;if(C.length>p){p=C.length;Eb=C}}}n=[];x=f.length-x-1;r=0;for(C=cb.length;r<C;++r){p=
...
b))}}function lb(b){if(va.clientWidth){for(var e=b===true?[]:[{from:Ha,to:ib,domStart:0}],f=0,g=b.length||0;f<g;++f){for(var i=b[f],n=[],p=i.diff||0,r=0,x=e.length;r<x;++r){var C=e[r];if(i.to<=C.from)n.push({from:C.from+p,to:C.to+p,domStart:C.domStart});else if(C.to<=i.from)n.push(C);else{i.from>C.from&&n.push({from:C.from,to:i.from,domStart:C.domStart});i.to<C.to&&n.push({from:i.to+p,to:C.to+p,domStart:C.domStart+(i.to-C.from)})}}e=n}p=Sb();i=Math.min(Ha,Math.max(p.from-3,0));n=Math.min(W.length,
Math.max(ib,p.to+3));b=[];r=0;x=ib-Ha;var R=i,ba=0;f=0;for(g=e.length;f<g;++f){C=e[f];if(!(C.to<=i)){if(C.from>=n)break;if(C.domStart>r||C.from>R){b.push({from:R,to:C.from,domSize:C.domStart-r,domStart:r});ba+=C.from-R}R=C.to;r=C.domStart+(C.to-C.from)}}if(r!=x||R!=n){ba+=Math.abs(n-R);b.push({from:R,to:n,domSize:x-r,domStart:r})}if(b.length){Qa.style.display="none";if(ba>(p.to-p.from)*0.3){C=i=Math.max(p.from-10,0);e=n=Math.min(p.to+7,W.length);f=[];g={line:C,ch:0};g=l(O.from,g)&&!l(O.to,g);for(C=
C;C<e;++C){r=p=null;if(g){p=0;if(O.to.line==C){g=false;r=O.to.ch}}else if(O.from.line==C)if(O.to.line==C){p=O.from.ch;r=O.to.ch}else{g=true;p=O.from.ch}f.push(W[C].getHTML(p,r,true))}Qa.innerHTML=f.join("")}else{e=O.from.line;f=O.to.line;g=0;C=oa&&mb.createElement("div");p=0;for(r=b.length;p<r;++p){x=b[p];R=x.to-x.from-x.domSize;ba=Qa.childNodes[x.domStart+x.domSize+g]||null;if(oa)for(var ka=Math.max(-R,x.domSize);ka>0;--ka)Qa.removeChild(ba?ba.previousSibling:Qa.lastChild);else if(R){for(ka=Math.max(0,
...
var i=Ea.value=ha();kb();V(Ea,0,Ea.value.length);Xa=true;if(T){b.stop();var n=N(window,"mouseup",function(){n();setTimeout(e,20)},true)}else setTimeout(e,50)}}function dc(){clearInterval(bc);var b=true;ob.style.visibility="";bc=setInterval(function(){ob.style.visibility=(b=!b)?"":"hidden"},650)}function Va(b){function e(Ca,o,u){if(Ca.text){var G=Ca.styles;Ca=p?0:Ca.text.length-1;for(var I,D=p?0:G.length-2,J=p?G.length:-2;D!=J;D+=2*r){var U=G[D];if(G[D+1]!=null&&G[D+1]!=ba)Ca+=r*U.length;else for(var Z=
p?0:U.length-1,ra=p?U.length:-1;Z!=ra;Z+=r,Ca+=r)if(Ca>=o&&Ca<u&&Ia.test(I=U.charAt(Z))){var Ma=a[I];if(Ma.charAt(1)==">"==p)ka.push(I);else if(ka.pop()!=Ma.charAt(0))return{pos:Ca,match:false};else if(!ka.length)return{pos:Ca,match:true}}}}}var f=O.inverted?O.from:O.to,g=W[f.line],i=f.ch-1,n=i>=0&&a[g.text.charAt(i)]||a[g.text.charAt(++i)];if(n){n.charAt(0);var p=n.charAt(1)==">",r=p?1:-1,x=g.styles,C=i+1;n=0;for(var R=x.length;n<R;n+=2)if((C-=x[n].length)<=0){var ba=x[n+1];break}var ka=[g.text.charAt(i)],
...
if(f==null||e>i){f=g;e=i}}return f}function hc(b){var e=Xb(b),f=e&&W[e-1].stateAfter;f=f?h(Ta,f):j(Ta);for(e=e;e<b;++e){var g=W[e];g.highlight(Ta,f);g.stateAfter=h(Ta,f)}W[b].stateAfter||cb.push(b);return f}function mc(){for(var b=+new Date+ma.workTime,e=false;cb.length;){var f=W[Ha].stateAfter?cb.pop():Ha;if(!(f>=W.length)){e=true;var g=Xb(f),i=g&&W[g-1].stateAfter;i=i?h(Ta,i):j(Ta);g=g;for(var n=W.length;g<n;++g){var p=W[g],r=p.stateAfter;if(+new Date>b){cb.push(g);Kb(ma.workDelay);Ua.push({from:f,
...
b&&ma.onChange&&$a&&ma.onChange($a,b)}function ya(b){return function(){d++||Yb();try{var e=b.apply(this,arguments)}finally{--d||Zb()}return e}}function ic(b,e,f){this.atOccurrence=false;if(f==null)f=typeof b=="string"&&b==b.toLowerCase();e=e&&typeof e=="object"?Ka(e):{line:0,ch:0};this.pos={from:e,to:e};if(typeof b!="string")this.matches=function(n,p){if(n)for(var r=W[p.line].text.slice(0,p.ch),x=r.match(b),C=0;x;){var R=r.indexOf(x[0]);C+=R;r=r.slice(R+1);if(R=r.match(b))x=R;else break;C++}else{r=
W[p.line].text.slice(p.ch);C=(x=r.match(b))&&p.ch+r.indexOf(x[0])}if(x)return{from:{line:p.line,ch:C},to:{line:p.line,ch:C+x[0].length},match:x}};else{if(f)b=b.toLowerCase();var g=f?function(n){return n.toLowerCase()}:function(n){return n},i=b.split("\n");this.matches=i.length==1?function(n,p){var r=g(W[p.line].text),x=b.length,C;if(n?p.ch>=x&&(C=r.lastIndexOf(b,p.ch-x))!=-1:(C=r.indexOf(b,p.ch))!=-1)return{from:{line:p.line,ch:C},to:{line:p.line,ch:C+x}}}:function(n,p){var r=p.line,x=n?i.length-
1:0,C=i[x],R=g(W[r].text),ba=n?R.indexOf(C)+C.length:R.lastIndexOf(C);if(!(n?ba>=p.ch||ba!=C.length:ba<=p.ch||ba!=R.length-C.length))for(;;){if(n?!r:r==W.length-1)return;R=g(W[r+=n?-1:1].text);C=i[n?--x:++x];if(x>0&&x<i.length-1)if(R!=C)return;else continue;x=n?R.lastIndexOf(C):R.indexOf(C)+C.length;if(n?x!=R.length-C.length:x!=C.length)return;C={line:p.line,ch:ba};r={line:r,ch:x};return{from:n?r:C,to:n?C:r}}}}}var ma={},Qb=k.defaults;for(var fb in Qb)if(Qb.hasOwnProperty(fb))ma[fb]=(A&&A.hasOwnProperty(fb)?
...
+new Date-f<400)return lc(i.line);yb(i.line,i.ch,true);var r=N(mb,"mousemove",ya(function(C){clearTimeout(p);C.stop();e(C)}),true),x=N(mb,"mouseup",ya(function(C){clearTimeout(p);var R=tb(C);R&&rb(i,R);C.stop();kb();bb=true;r();x()}),true)}else b.target()==va&&b.stop()}}));T||N(va,"contextmenu",ya(gc));N(Ab,"dblclick",ya(function(b){var e=tb(b);if(e){for(var f=W[e.line].text,g=e.ch,i=e.ch;g>0&&/\w/.test(f.charAt(g-1));)--g;for(;i<f.length&&/\w/.test(f.charAt(i));)++i;rb({line:e.line,ch:g},{line:e.line,
...
L[M]=S}return L}function j(c,A,L){return c.startState?c.startState(A,L):true}function m(c){this.pos=this.start=0;this.string=c}function s(c,A){this.styles=A||[c,null];this.stateAfter=null;this.text=c;this.marked=this.gutterMarker=this.className=null}function t(c,A,L,M){for(var S=0,ja=0,ia=0;ja<A;S+=2){var wa=L[S],Ga=ja+wa.length;if(ia==0){Ga>c&&M.push(wa.slice(c-ja,Math.min(wa.length,A-ja)),L[S+1]);if(Ga>=c)ia=1}else if(ia==1)Ga>A?M.push(wa.slice(0,A-ja),L[S+1]):M.push(wa,L[S+1]);ja=Ga}}function w(){this.time=
...
s.prototype={replace:function(c,A,L){var M=[],S=this.marked;t(0,c,this.styles,M);L&&M.push(L,null);t(A,this.text.length,this.styles,M);this.styles=M;this.text=this.text.slice(0,c)+L+this.text.slice(A);this.stateAfter=null;if(S){c=L.length-(A-c);L=this.text.length;for(M=0;M<S.length;++M){var ja=S[M],ia=false;if(ja.from>=L)ia=true;else{ja.from=ja.from<=Math.min(A,A+c)?ja.from:ja.from+c;if(ja.to!=null)ja.to=ja.to<=Math.min(A,A+c)?ja.to:ja.to+c}if(ia||ja.from>=ja.to){S.splice(M,1);M--}}}},split:function(c,
A){var L=[A,null];t(c,this.text.length,this.styles,L);return new s(A+this.text.slice(c),L)},addMark:function(c,A,L){c={from:c,to:A,style:L};if(this.marked==null)this.marked=[];this.marked.push(c);this.marked.sort(function(M,S){return M.from-S.from});return c},removeMark:function(c){var A=this.marked;if(A)for(var L=0;L<A.length;++L)if(A[L]==c){A.splice(L,1);break}},highlight:function(c,A){var L=new m(this.text),M=this.styles,S=0,ja=false,ia=M[0],wa;for(this.text==""&&c.blankLine&&c.blankLine(A);!L.eol();){var Ga=
...
jQuery.cookie=function(k,h,j){if(typeof h!="undefined"){j=j||{};if(h===null){h="";j.expires=-1}var m="";if(j.expires&&(typeof j.expires=="number"||j.expires.toUTCString)){if(typeof j.expires=="number"){m=new Date;m.setTime(m.getTime()+j.expires*24*60*60*1E3)}else m=j.expires;m="; expires="+m.toUTCString()}var s=j.path?"; path="+j.path:"",t=j.domain?"; domain="+j.domain:"";j=j.secure?"; secure":"";document.cookie=[k,"=",encodeURIComponent(h),m,s,t,j].join("")}else{h=null;if(document.cookie&&document.cookie!=
""){j=document.cookie.split(";");for(m=0;m<j.length;m++){s=jQuery.trim(j[m]);if(s.substring(0,k.length+1)==k+"="){h=decodeURIComponent(s.substring(k.length+1));break}}}return h}};
CodeMirror.defineMode("javascript",function(k,h){function j(H,ea){for(var aa=false,ha;(ha=H.next())!=null;){if(ha==ea&&!aa)return false;aa=!aa&&ha=="\\"}return aa}function m(H,ea,aa){ia=H;wa=aa;return ea}function s(H,ea){var aa=H.next();if(aa=='"'||aa=="'"){aa=t(aa);ea.tokenize=aa;return aa(H,ea)}else if(/[\[\]{}\(\),;\:\.]/.test(aa))return m(aa);else if(aa=="0"&&H.eat(/x/i)){H.eatWhile(/[\da-f]/i);return m("number","js-atom")}else if(/\d/.test(aa)){H.match(/^\d*(?:\.\d*)?(?:e[+\-]?\d+)?/);return m("number",
...
aa)}}function t(H){return function(ea,aa){if(!j(ea,H))aa.tokenize=s;return m("string","js-string")}}function w(H,ea){for(var aa=false,ha;ha=H.next();){if(ha=="/"&&aa){ea.tokenize=s;break}aa=ha=="*"}return m("comment","js-comment")}function F(H,ea,aa,ha,sa,ta){this.indented=H;this.column=ea;this.type=aa;this.prev=sa;this.info=ta;if(ha!=null)this.align=ha}function B(){for(var H=arguments.length-1;H>=0;H--)ua.cc.push(arguments[H])}function z(){B.apply(null,arguments);return true}function N(H){var ea=
...
"openTag";T.eatSpace();for(y="";xa=T.eat(/[^\s\u00a0=<>\"\'\/?]/);)y+=xa;fa.tokenize=m;return"xml-tag"}else if(xa=="&"){T.eatWhile(/[^;]/);T.eat(";");return"xml-entity"}else{T.eatWhile(/[^&<]/);return null}}function m(T,fa){var ga=T.next();if(ga==">"||ga=="/"&&T.eat(">")){fa.tokenize=j;E=ga==">"?"endTag":"selfcloseTag";return"xml-tag"}else if(ga=="="){E="equals";return null}else if(/[\'\"]/.test(ga)){fa.tokenize=s(ga);return fa.tokenize(T,fa)}else{T.eatWhile(/[^\s\u00a0=<>\"\'\/?]/);return"xml-word"}}
function s(T){return function(fa,ga){for(;!fa.eol();)if(fa.next()==T){ga.tokenize=m;break}return"xml-attribute"}}function t(T,fa){return function(ga,xa){for(;!ga.eol();){if(ga.match(fa)){xa.tokenize=j;break}ga.next()}return T}}function w(){for(var T=arguments.length-1;T>=0;T--)ca.cc.push(arguments[T])}function F(){w.apply(null,arguments);return true}function B(T,fa){var ga=l.doNotIndent.hasOwnProperty(T)||ca.context&&ca.context.noIndent;ca.context={prev:ca.context,tagName:T,indent:ca.indented,startOfLine:fa,
...
CodeMirror.defineMode("css",function(k){function h(B,z){F=z;return B}function j(B,z){var N=B.next();if(N=="@"){B.eatWhile(/\w/);return h("css-at",B.current())}else if(N=="/"&&B.eat("*")){z.tokenize=m;return m(B,z)}else if(N=="<"&&B.eat("!")){z.tokenize=s;return s(B,z)}else if(N=="=")h(null,"compare");else if((N=="~"||N=="|")&&B.eat("="))return h(null,"compare");else if(N=='"'||N=="'"){z.tokenize=t(N);return z.tokenize(B,z)}else if(N=="#"){B.eatWhile(/\w/);return h("css-selector","hash")}else if(N==
"!"){B.match(/^\s*\w*/);return h("css-important","important")}else if(/\d/.test(N)){B.eatWhile(/[\w.%]/);return h("css-unit","unit")}else if(/[,.+>*\/]/.test(N))return h(null,"select-op");else if(/[;{}:\[\]]/.test(N))return h(null,N);else{B.eatWhile(/[\w\\\-_]/);return h("css-identifier","identifier")}}function m(B,z){for(var N=false,P;(P=B.next())!=null;){if(N&&P=="/"){z.tokenize=j;break}N=P=="*"}return h("css-comment","comment")}function s(B,z){for(var N=0,P;(P=B.next())!=null;){if(N>=2&&P==">"){z.tokenize=
...
CodeMirror.defineMode("liquid",function(k){function h(B,z){var N=t.token(B,z.htmlState);if(N=="xml-tag"&&B.current()==">"&&z.htmlState.context)if(/^script$/i.test(z.htmlState.context.tagName)){z.token=m;z.localState=w.startState(t.indent(z.htmlState,""))}else if(/^style$/i.test(z.htmlState.context.tagName)){z.token=s;z.localState=F.startState(t.indent(z.htmlState,""))}return N}function j(B,z,N){var P=B.current();z=P.search(z);z>-1&&B.backUp(P.length-z);return N}function m(B,z){if(B.match(/^<\/\s*script\s*>/i,
false)){z.token=h;z.curState=null;return h(B,z)}return j(B,/<\/\s*script\s*>/,w.token(B,z.localState))}function s(B,z){if(B.match(/^<\/\s*style\s*>/i,false)){z.token=h;z.localState=null;return h(B,z)}return j(B,/<\/\s*style\s*>/,F.token(B,z.localState))}var t=CodeMirror.getMode(k,{name:"xml",htmlMode:true}),w=CodeMirror.getMode(k,"javascript"),F=CodeMirror.getMode(k,"css");return{startState:function(){var B=t.startState();return{token:h,localState:null,htmlState:B}},copyState:function(B){if(B.localState)var z=
CodeMirror.copyState(B.token==s?F:w,B.localState);return{token:B.token,localState:z,htmlState:CodeMirror.copyState(t,B.htmlState)}},token:function(B,z){return z.token(B,z)},indent:function(B,z){return B.token==h||/^\s*<\//.test(z)?t.indent(B.htmlState,z):B.token==m?w.indent(B.localState,z):F.indent(B.localState,z)},electricChars:"/{}:"}});CodeMirror.defineMIME("text/html","htmlmixed");
var Asset=Model("asset",function(){this.persistence(Model.REST,"/admin/assets");this.include({toMustache:function(k){this.merge({query_path:k?"?"+decodeURIComponent(jQuery.param({query:k})):"",query:k});return this.attr()},toMustacheWithNeighbors:function(k){var h=Asset.all(),j=h.indexOf(this),m=this.toMustache(k),s=h[j+1]?Asset.all()[j+1].toMustache(k):null;k=h[j-1]?Asset.all()[j-1].toMustache(k):null;jQuery("<img />")[0].src="/images/large/"+m.id+"/"+m.file_name;if(s)jQuery("<img />")[0].src="/images/large/"+
s.id+"/"+s.file_name;if(k)jQuery("<img />")[0].src="/images/large/"+k.id+"/"+k.file_name;return{asset:m,next:s,previous:k}}});this.extend({toMustache:function(k){var h=k?"?"+decodeURIComponent(jQuery.param({query:k})):"";return{assets:this.map(function(j){j.merge({query_path:h,query:k});return j.attr()}),query:k}},asJSON:function(){return Asset.map(function(k){return k.attr()})},tags:function(){tags=[];Asset.map(function(){tags=tags.concat(this.attr("tags"))});return tags},searchRemote:function(k,
h){var j=k!=null?decodeURIComponent(jQuery.param({query:k})):"";Asset.each(function(){Asset.remove(this)});jQuery.ajax({type:"get",url:"/search.json",contentType:"application/json",dataType:"json",data:j,success:function(m){jQuery.each(m,function(s,t){var w=new Asset({id:t.id});w.merge(t);Asset.add(w)});h.call(this)}})},searchAdmin:function(k,h){Asset.each(function(){Asset.remove(this)});jQuery.ajax({type:"get",url:"/admin/assets.json",contentType:"application/json",dataType:"json",data:k,success:function(j){$.each(j,
function(m,s){var t=new Asset({id:s.id});t.merge(s);Asset.add(t)});h.call(this)}})},create:function(k,h){Asset.callback=h;var j=new XMLHttpRequest;j.upload.addEventListener("loadstart",Asset.onloadstartHandler,false);j.upload.addEventListener("progress",Asset.onprogressHandler,false);j.upload.addEventListener("load",Asset.onloadHandler,false);j.addEventListener("readystatechange",Asset.onreadystatechangeHandler,false);j.open("POST","/admin/assets.json",true);j.setRequestHeader("Content-Type","application/octet-stream");
j.setRequestHeader("X-File-Name",k.name);j.send(k)},onloadstartHandler:function(){},onloadHandler:function(){},onprogressHandler:function(){},onreadystatechangeHandler:function(k){var h=null;try{h=k.target.status}catch(j){return}if(h=="200"&&k.target.readyState==4&&k.target.responseText){k=JSON.parse(k.target.responseText);h=new Asset({id:k.id});h.merge(k);Asset.add(h);Asset.callback&&Asset.callback.call(this)}}})}),Page=Model("page",function(){this.persistence(Model.SinatraREST,"/admin/pages");this.include({children:function(){return Page.find_all_by_parent_id(this.id())},
has_children:function(){if(this.children().all().length>0)return true},assets:function(){for(var k=this.attr("assets"),h=0,j=k.length;h<j;++h){var m=k[h],s=new Asset({id:m.id});s.merge(m);Asset.add(s)}return Asset},parts:function(){for(var k=this.attr("parts"),h=0,j=k.length;h<j;++h){var m=k[h],s=new Part({id:m.id});s.merge(m);s.attr("page_id",this.id());Part.add(s)}return Part},load:function(k){var h=this,j="/admin/pages/"+h.id()+".json";jQuery.ajax({type:"GET",url:j,dataType:"json",success:function(m){h.merge(m);
k.call(this)}})},saveRemote:function(k,h){var j=this,m="/admin/pages/"+this.id()+".json";jQuery.ajax({type:"PUT",url:m,data:k,dataType:"json",success:function(s){if(s.errors){alert(JSON.stringify(s.errors));h.error&&h.error.call(this)}else{j.merge(s);h.success&&h.success.call(this)}}})}});this.extend({toMustache:function(){return{pages:this.map(function(k){return k.attr()})}},asJSON:function(){return Page.map(function(k){return k.attr()})},root:function(){return this.detect(function(){return this.attr("parent_id")==
null})},find_by_parent_id:function(k){return this.detect(function(){return this.attr("parent_id")==k})},find_all_by_parent_id:function(k){return this.select(function(){return this.attr("parent_id")==k})},create:function(k,h){jQuery.ajax({type:"post",url:"/admin/pages.json",dataType:"json",data:{page:k},success:function(j){var m=new Page({id:j.id});m.merge(j);Page.add(m);h.call(this,j)}})}})}),Layout=Model("template",function(){this.persistence(Model.SinatraREST,"/admin/templates");this.include({});
this.extend({toMustache:function(){return{layouts:this.map(function(k){return k.attr()})}},asLayoutJSON:function(k){return Layout.find_all_by_class("Layout").map(function(h){k==h.id()?h.attr("selected",'selected="selected"'):h.attr("selected","");return h.attr()})},find_by_class:function(k){return this.detect(function(){return this.attr("klass")==k})},find_all_by_class:function(k){return this.select(function(){return this.attr("klass")==k})},asJSON_by_class:function(k){return this.find_all_by_class(function(){return k.map(function(h){return h.attributes})})}})}),
Part=Model("part",function(){this.include({deleteRemote:function(k,h){var j="/admin/pages/"+k.id()+"/parts/"+this.id()+".json";jQuery.ajax({type:"DELETE",url:j,dataType:"json",success:function(m){k.merge(m);h.call(this)}})}});this.extend({toMustache:function(){return{parts:this.map(function(k){return k.attr()})}},create:function(k,h){var j="/admin/templates/"+k.template_id+"/parts.json",m=Layout.find(k.template_id);jQuery.ajax({type:"post",url:j,dataType:"json",data:{part:k},success:function(s){m.merge(s);
h.call(this)}})}})}),Site=Model("site",function(){this.persistence(Model.SinatraREST,"/admin/sites");this.include({});this.extend({toMustache:function(){return{sites:this.map(function(k){return k.attr()})}}})}),User=Model("user",function(){this.persistence(Model.SinatraREST,"/admin/users");this.include({});this.extend({toMustache:function(){return{users:this.map(function(k){return k.attr()})}}})}),Galerie={open:function(k,h){$("body").append('<div id="overlay"></div>').append('<div id="modal_wrapper"><div id="modal"></div></div>');
var j=$("#modal");j.html(k);var m=$("#modal_wrapper"),s=$(window).height();j=j.height()+42;j=(s-j)/2;$("#overlay").css({height:s}).hide().fadeIn("fast");m.hide().fadeIn("fast").css({top:j});h&&h.call(this)},close:function(){$("#modal_wrapper").fadeOut("fast",function(){$(this).remove();$("#overlay").fadeOut("fast",function(){$(this).remove()})})}},logger={info:function(k){window.console&&console.log(k)},debug:function(k){window.console&&console.log(k)}},Utilities={notice:function(k){var h=jQuery(".notice");
h.text(k);h.slideDown("slow",function(){setTimeout(function(){h.slideUp("slow")},1E3)})},keyboard_nav:function(){jQuery("body").keydown(function(k){switch(k.keyCode){case 37:$("a.previous").click();break;case 39:$("a.next").click();break}})},check_browser_version:function(){if(!Modernizr.history){jQuery("a").live("click",function(h){h.preventDefault();document.location.hash=$(this).attr("href")});var k=jQuery("form#search-form");k.attr("action","#"+k.attr("action"))}},formObserver:function(k){jQuery(k).keyup(function(){delay(function(){jQuery(k).parents("form:first").submit()},
800)})}},iFramer={initialize:function(k){k=jQuery(k);var h=this;k.length&&k.load(function(){var j=$(this).contents().find("div.part-editor"),m=j.find("a");logger.info("load");$(this).fadeIn("fast");h.setEditFlags(j);m.click(function(){window.top.trigger=$(this);window.top.location.hash=$(this).attr("href");return false})})},setEditFlags:function(k){k.css({position:"relative",display:"block",height:"0",width:"0","text-decoration":"none"}).find("span").css({display:"block",position:"absolute",top:"-24px",
left:"12px",padding:"4px 8px",width:"100px",background:"rgba(255, 255, 0, 0.7)"})}},TabControl={initialize:function(k){if(k=jQuery(k)){k.each(function(h,j){$(j).hide()});k.first().show();this.tabControl(jQuery(".tab-control"))}},tabControl:function(k){k.click(function(){var h="tab-"+$(this).find("label").attr("for").split("-")[1];console.log(h);jQuery(".tab").hide();jQuery("#"+h).show()})}},delay=function(){var k=0;return function(h,j){clearTimeout(k);k=setTimeout(h,j)}}();
...
this.post("#/templates",function(h){(new Layout(h.params.template)).save(function(j,m){var s=JSON.parse(m.responseText);if(s.errors)Utilities.notice(JSON.stringify(s));else{h.redirect("#/templates");Utilities.notice("Successfully saved template")}})});this.get("#/templates/:id/edit",function(h){Galerie.close();k.refresh_pages=true;this.loadLayouts(function(){var j=Layout.find(h.params.id);h.renderLayout(j);h.renderLayoutIndex(Layout.all())})});this.put("#/templates/:id",function(h){var j=Layout.find(h.params.id);
...
Assets=Sammy(function(k){var h=this;this.debug=false;this.disable_push_state=true;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);this.swap=function(j){jQuery("#editor").html(j)};this.helpers({loadAssets:function(j,m){if(Asset.all().length==0)Asset.searchAdmin(j,function(){m&&m.call(this)});else m&&m.call(this)}});this.bind("run",function(){h.modal=false;h.first_run=true});this.get("#/assets",function(j){var m=j.params.query,s=m?{query:m}:{};s.limit=48;s.page=j.params.page||
1;Galerie.close();h.modal||Asset.searchAdmin(s,function(){j.render("/templates/admin/assets/index.mustache",Asset.toMustache(m)).replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple")})});h.modal=false;h.first_run=false});this.get("#/assets/new",function(j){j.render("/templates/admin/assets/new.mustache").replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple")});h.first_run=false});this.post("/admin/assets",function(j){var m=document.getElementById("ajax_uploader").files,
s=j.params.query?j.params.query:null;jQuery("form#new_asset");var t=s?{query:s}:{};t.limit=48;t.page=j.params.page||1;for(var w=0,F=0;F<m.length;F++)Asset.create(m[F],function(){w+=1;logger.info("asset "+w/m.length*100+"%");jQuery(".progress").text(w/m.length*100+"%");w==m.length&&Asset.searchAdmin(t,function(){j.render("/templates/admin/assets/index.mustache",Asset.toMustache(s)).replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple")})})});return false});this.get("#/assets/:id/edit",
function(j){var m=j.params.query?j.params.query:null;this.loadAssets(m?{query:j.params.query}:{},function(){var s=Asset.find(j.params.id);j.render("/templates/admin/assets/edit.mustache",s.toMustacheWithNeighbors(m)).replace("#editor").then(function(){setTimeout(function(){$("img.fade-in").fadeIn("slow")},100);Utilities.formObserver(".image-info input[type=text], .image-info textarea")})});h.modal=false});this.put("#/assets/:id",function(j){var m=Asset.find(j.params.id);m.attr(j.params.asset);m.save(function(s){s&&
Utilities.notice("Successfully saved asset")})});this.get("#/assets/:id/remove",function(j){var m=j.params.query?j.params.query:null,s=m?{query:j.params.query}:null;Galerie.close();Galerie.open(jQuery("<div />").attr({id:"remove-asset-container","class":"wide-modal"}));this.loadAssets(s,function(){var t=Asset.find(j.params.id);j.render("/templates/admin/assets/remove.mustache",{asset:t.toMustache(m)}).replace("#remove-asset-container");h.first_run&&j.render("/templates/admin/assets/index.mustache",
Asset.toMustache(m)).replace("#editor")})});this.del("#/assets/:id",function(j){var m=j.params.query?j.params.query:null,s=m?"?"+decodeURIComponent(jQuery.param({query:m})):"";Asset.find(j.params.id).destroy(function(t){if(t){Galerie.close();Utilities.notice("Successfully saved asset");j.redirect("#/assets"+s)}})});k.get("/",function(){})});
...
m.render("/templates/admin/pages/node.mustache",j.children().toMustache());pageNode.appendTo("#page-"+j.id()).then(function(){j.children().each(function(s){if(s.has_children()==true){jQuery("#page-"+s.id()).addClass("open");m.renderNode(s)}})})},renderPagePreview:function(j){h.modal||this.render("/templates/admin/pages/show.mustache",{page:j.asJSON(),base_page_id:j.id()}).replace("#editor").then(function(){iFramer.initialize("#preview iframe")})},renderPage:function(j){this.render("/templates/admin/pages/edit.mustache",
{page:j.asJSON(),layouts:Layout.asLayoutJSON(j.attr("layout_id"))}).replace("#editor").then(function(){})}});this.bind("run",function(){h.application=this;h.refresh_pages=true;h.modal=false;jQuery(".add-asset").live("click",function(){h.application.trigger("add-asset");return false});jQuery("#sidebar .opener").live("click",function(){var j=(new Date).getTime(),m=$(this).parents("li:first"),s=this.id.split("-")[2],t=Page.find(s),w=jQuery.cookie("active_page_ids");w=w?w.split(","):[];if(m.hasClass("open")){m.toggleClass("open");
w=jQuery.grep(w,function(F){return F!=s});jQuery.cookie("active_page_ids",w.join(","),{path:"/admin"});t.children().each(function(F){Page.remove(F)});jQuery("#page-"+s+" ul").remove()}else{w.push(s);m.toggleClass("open");j=(new Date).getTime();jQuery.cookie("active_page_ids",w.join(","),{path:"/admin"});Page.load(function(){h.application.renderNode(t);var F=(new Date).getTime()-j;console.log(F)})}h.refresh_pages=false;return h.modal=false})});this.get("#/pages/?",function(j){Galerie.close();h.refresh_pages&&
j.loadPages(function(){j.renderTree(Page.root());logger.info("pages")});jQuery("#editor").html('<h1 class="section">Pages</div>');h.refresh_pages=true;h.do_not_refresh=false});this.get("#/pages/:id/new/?",function(j){this.loadPages(function(){var m=Page.find(j.params.id),s=$("<div />").attr({id:"new-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(s);if(h.refresh_pages){j.renderTree(Page.root());h.refresh_pages=true}j.render("/templates/admin/pages/new.mustache",{parent:m.asJSON()}).replace("#new-page-container")})});
this.post("#/pages/:page_id",function(j){Page.find(j.params.page_id);Page.create(j.params.page,function(m){h.refresh_pages=true;Utilities.notice("Successfully saved page");j.redirect("#/pages/"+m.id+"/edit")})});this.get("#/pages/:id/?",function(j){logger.info("pages show");Galerie.close();jQuery(".modal-editor").remove();this.loadPages(function(){var m=j.params.id,s=Page.find(m);if(!h.do_not_refresh){if(s)j.renderPagePreview(s);else{s=new Page({id:m});s.load(function(){j.renderPagePreview(s)})}h.modal=
false;h.do_not_refresh=false;if(h.refresh_pages){j.renderTree(Page.root());h.refresh_pages=false}}});h.do_not_refresh=false});this.get("#/pages/:id/edit/?",function(j){Galerie.close();h.modal=true;this.loadPages(function(){var m=j.params.id,s=Page.find(m);if(!h.do_not_refresh){if(s)j.renderPage(s);else{s=new Page({id:m});s.load(function(){j.renderPage(s)})}if(h.refresh_pages){j.renderTree(Page.root());h.refresh_pages=false}}});h.do_not_refresh=false});this.put("#/pages/:page_id",function(j){var m=
Page.find(j.params.page_id);m.attr(j.params.page);m.save(function(s){if(s){h.modal=false;Utilities.notice("Successfully saved page")}})});this.get("#/pages/:id/remove",function(j){this.loadPages(function(){var m=Page.find(j.params.id),s=$("<div />").attr({id:"remove-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(s);h.refresh_pages&&j.renderTree(Page.root());j.render("/templates/admin/pages/remove.mustache",{page:m.asJSON()}).replace("#remove-page-container")})});this.del("/pages/:id",
function(j){var m=j.params.id;Page.find(m).destroy(function(s){if(s){jQuery("#page-"+m).remove();h.refresh_pages=false;j.redirect("#/pages")}})});this.get("/pages/:id/results",function(j){this.loadPages(function(){var m=Page.find(j.params.id),s=j.params.query?j.params.query:null;Asset.searchAdmin(s?{query:s}:{},function(){Asset.each(function(t){t.attr("current_page_id",m.id());t.save()});j.render("/templates/admin/pages/search_results.mustache",Asset.toMustache()).replace("#search-results-container")})})});
this.get("#/pages/:page_id/assets/:id/add",function(j){this.loadPages(function(){var m=j.params.page_id,s=Page.find(m),t=Asset.find(j.params.id),w=jQuery("#page-asset-ids");w.attr("value");var F=w.attr("value")+","+t.id();w.attr("value",F);s.saveRemote({page:{assets_list:F}},{success:function(){var B=jQuery('<div class="asset" />').attr("id","page-asset-"+t.id()),z=jQuery('<a href="#/assets/'+t.id()+'/edit"></a>'),N=jQuery('<br /><a href="#/pages/'+s.id()+"/assets/"+t.id()+'/remove">Remove</a>');
z.append('<img src="/images/icons/'+t.id()+"/"+t.attr("file_name")+'" alt="'+t.attr("title")+'" /></a>');B.html(z);B.append(N);jQuery("#page-assets").append(B);var P=jQuery("#preview iframe");P.hide().attr("src",P.attr("src"));P.load(function(){P.fadeIn("fast")});h.do_not_refresh=true;j.redirect("#/pages/"+m)}})})});this.get("#/pages/:page_id/assets/:id/remove",function(j){this.loadPages(function(){var m=j.params.id,s=j.params.page_id,t=Page.find(s),w=jQuery("#page-asset-ids"),F=w.attr("value").split(","),
B=F.indexOf(m);B!=-1&&F.splice(B,1);w.attr("value",F);t.attr("assets_list",F.join(","));t.save(function(){jQuery("#page-asset-"+m).fadeOut("fast",function(){$(this).remove()});var z=jQuery("#preview iframe");z.hide().attr("src",z.attr("src"));z.load(function(){z.fadeIn("fast")});h.do_not_refresh=true;j.redirect("#/pages/"+s)})})})});
Pages=Sammy(function(k){var h=this;this.debug=false;this.disable_push_state=true;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);k.helpers({set_asset_links:function(j,m){jQuery("#search-results-container li.asset").each(function(s,t){var w=jQuery(t).find("a"),F=jQuery(t).attr("id").split("-")[1];w.click(function(B){B.preventDefault();B=j.attr("parts");for(var z=0,N=B.length;z<N;++z){var P=B[z];if(m.id()==P.id){P.asset_id=F;m.attr("asset_id",F);m.save()}}j.save(function(){jQuery(".modal-editor").remove();
document.location.hash="#/pages/"+j.id()})})})}});this.get("#/pages/:page_id/parts/:id/edit",function(j){this.loadPages(function(){jQuery(".modal-editor").remove();var m=j.params.id,s=Page.find(j.params.page_id).parts().find(m),t=$("iframe").contents(),w=t.find("#editor-"+m);m=this.render("/templates/admin/parts/edit.mustache",{part:s.asJSON()});logger.info(w.offset());logger.info(t.find("body").scrollTop());m.appendTo(jQuery("body")).then(function(){jQuery(".modal-editor").fadeIn("fast").css({top:w.offset().top-
t.find("body").scrollTop()+"px",left:w.offset().left+400+"px"})})})});this.get("#/pages/:page_id/image_parts/:id/edit",function(j){this.loadPages(function(){var m=this;jQuery(".modal-editor").remove();var s=j.params.id,t=Page.find(j.params.page_id),w=t.parts().find(s),F=$("iframe").contents(),B=F.find("#editor-"+s);Asset.searchAdmin({limit:"12"},function(){m.render("/templates/admin/image_parts/edit.mustache",{part:w.asJSON(),page:t.asJSON(),assets:Asset.asJSON()}).appendTo(jQuery("body")).then(function(){jQuery(".modal-editor").fadeIn("fast").css({top:B.offset().top-
F.find("body").scrollTop()+"px",left:B.offset().left+400+"px"});m.set_asset_links(t,w)})})})});this.get("/pages/:page_id/parts/:id/results",function(j){var m=this;this.loadPages(function(){var s=Page.find(j.params.page_id),t=Part.find(j.params.id),w=j.params.query?j.params.query:null;Asset.searchAdmin(w?{query:w}:{},function(){j.render("/templates/admin/pages/search_results.mustache",Asset.toMustache()).replace("#search-results-container").then(function(){m.set_asset_links(s,t)})})})});this.put("#/pages/:page_id/parts/:id",
function(j){var m=j.params.id,s=j.params.page_id,t=Page.find(s),w=t.parts().find(m);logger.info(j.params.part);for(var F=t.attr("parts"),B=0,z=F.length;B<z;++B){w=F[B];if(m==w.id){var N=j.params.part.content;w.content=N;w=Part.find(w.id);w.attr("content",N);w.save()}}t.save(function(P){if(P){h.modal=false;j.redirect("#/pages/"+s)}})})});
...
s){var t=JSON.parse(s.responseText);t.errors?alert(JSON.stringify(t)):h.redirect("#/users")})})});jQuery(document).ready(function(){jQuery("#login").length||Pages.run("#/pages");Utilities.keyboard_nav();Utilities.check_browser_version()});
