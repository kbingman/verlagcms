...
j=f.css(d,"display");h.removeChild(Xa)}db[b]=j}return db[b]}function i(b,h){var d={};f.each(Pa.concat.apply([],Pa.slice(0,h)),function(){d[this]=b});return d}function l(){qb=a}function n(){setTimeout(l,0);return qb=f.now()}function u(){try{return new c.XMLHttpRequest}catch(b){}}function x(b,h,d,j){if(f.isArray(h))f.each(h,function(m,o){d||eb.test(b)?j(b,o):x(b+"["+(typeof o=="object"||f.isArray(o)?m:"")+"]",o,d,j)});else if(!d&&h!=null&&typeof h=="object")for(var k in h)x(b+"["+k+"]",h[k],d,j);else j(b,
h)}function s(b,h,d,j,k,m){k=k||h.dataTypes[0];m=m||{};m[k]=true;k=b[k];for(var o=0,q=k?k.length:0,v=b===cb,w;o<q&&(v||!w);o++){w=k[o](h,d,j);typeof w=="string"&&(!v||m[w]?(w=a):(h.dataTypes.unshift(w),w=s(b,h,d,j,w,m)))}(v||!w)&&!m["*"]&&(w=s(b,h,d,j,"*",m));return w}function C(b){return function(h,d){typeof h!="string"&&(d=h,h="*");if(f.isFunction(d))for(var j=h.toLowerCase().split(cc),k=0,m=j.length,o,q;k<m;k++){o=j[k];(q=/^\+/.test(o))&&(o=o.substr(1)||"*");o=b[o]=b[o]||[];o[q?"unshift":"push"](d)}}}
...
function(k){return k.nodeType===1});if(Ka.test(h))return f.filter(h,j,!d);h=f.filter(h,j)}return f.grep(b,function(k){return f.inArray(k,h)>=0===d})}function Y(b,h){return(b&&b!=="*"?b+".":"")+h.replace(La,"`").replace(gb,"&")}function pa(b){var h,d,j,k,m,o,q,v,w,A,F,R=[];k=[];m=f._data(this,"events");if(!(b.liveFired===this||!m||!m.live||b.target.disabled||b.button&&b.type==="click")){b.namespace&&(F=new RegExp("(^|\\.)"+b.namespace.split(".").join("\\.(?:.*\\.)?")+"(\\.|$)"));b.liveFired=this;var da=
m.live.slice(0);for(q=0;q<da.length;q++){m=da[q];m.origType.replace(sa,"")===b.type?k.push(m.selector):da.splice(q--,1)}k=f(b.target).closest(k,b.currentTarget);v=0;for(w=k.length;v<w;v++){A=k[v];for(q=0;q<da.length;q++){m=da[q];if(A.selector===m.selector&&(!F||F.test(m.namespace))&&!A.elem.disabled){o=A.elem;j=null;if(m.preType==="mouseenter"||m.preType==="mouseleave"){b.type=m.preType;(j=f(b.relatedTarget).closest(m.selector)[0])&&f.contains(o,j)&&(j=o)}(!j||j!==o)&&R.push({elem:o,handleObj:m,level:A.level})}}}v=
0;for(w=R.length;v<w;v++){k=R[v];if(d&&k.level>d)break;b.currentTarget=k.elem;b.data=k.handleObj.data;b.handleObj=k.handleObj;F=k.handleObj.origHandler.apply(k.elem,arguments);if(F===false||b.isPropagationStopped()){d=k.level;F===false&&(h=false);if(b.isImmediatePropagationStopped())break}}return h}}function T(b,h,d){var j=f.extend({},d[0]);j.type=b;j.originalEvent={};j.liveFired=a;f.event.handle.call(h,j);j.isDefaultPrevented()&&d[0].preventDefault()}function ga(){return true}function ha(){return false}
...
M){return z.nodeName&&z.nodeName.toUpperCase()===M.toUpperCase()},each:function(z,M,ea){var Z,qa=0,Aa=z.length,Da=Aa===a||h.isFunction(z);if(ea)if(Da)for(Z in z){if(M.apply(z[Z],ea)===false)break}else for(;qa<Aa;){if(M.apply(z[qa++],ea)===false)break}else if(Da)for(Z in z){if(M.call(z[Z],Z,z[Z])===false)break}else for(;qa<Aa;)if(M.call(z[qa],qa,z[qa++])===false)break;return z},trim:Ma?function(z){return z==null?"":Ma.call(z)}:function(z){return z==null?"":(z+"").replace(q,"").replace(v,"")},makeArray:function(z,
...
h.uaMatch(H);H.browser&&(h.browser[H.browser]=true,h.browser.version=H.version);h.browser.webkit&&(h.browser.safari=true);o.test("\u00a0")&&(q=/^[\s\xA0]+/,v=/[\s\xA0]+$/);k=h(X);X.addEventListener?(G=function(){X.removeEventListener("DOMContentLoaded",G,false);h.ready()}):X.attachEvent&&(G=function(){X.readyState==="complete"&&(X.detachEvent("onreadystatechange",G),h.ready())});return h}(),D="done fail isResolved isRejected promise then always pipe".split(" "),N=[].slice;f.extend({_Deferred:function(){var b=
[],h,d,j,k={done:function(){if(!j){var m=arguments,o,q,v,w,A;h&&(A=h,h=0);o=0;for(q=m.length;o<q;o++){v=m[o];w=f.type(v);w==="array"?k.done.apply(k,v):w==="function"&&b.push(v)}A&&k.resolveWith(A[0],A[1])}return this},resolveWith:function(m,o){if(!j&&!h&&!d){o=o||[];d=1;try{for(;b[0];)b.shift().apply(m,o)}finally{h=[m,o];d=0}}return this},resolve:function(){k.resolveWith(this,arguments);return this},isResolved:function(){return!!d||!!h},cancel:function(){j=1;b=[];return this}};return k},Deferred:function(b){var h=
f._Deferred(),d=f._Deferred(),j;f.extend(h,{then:function(k,m){h.done(k).fail(m);return this},always:function(){return h.done.apply(h,arguments).fail.apply(this,arguments)},fail:d.done,rejectWith:d.resolveWith,reject:d.resolve,isRejected:d.isResolved,pipe:function(k,m){return f.Deferred(function(o){f.each({done:[k,"resolve"],fail:[m,"reject"]},function(q,v){var w=v[0],A=v[1],F;f.isFunction(w)?h[q](function(){(F=w.apply(this,arguments))&&f.isFunction(F.promise)?F.promise().then(o.resolve,o.reject):
o[A](F)}):h[q](o[A])})}).promise()},promise:function(k){if(k==null){if(j)return j;j=k={}}for(var m=D.length;m--;)k[D[m]]=h[D[m]];return k}});h.done(d.cancel).fail(h.cancel);delete h.cancel;b&&b.call(h,h);return h},when:function(b){function h(q){return function(v){d[q]=arguments.length>1?N.call(arguments,0):v;--m||o.resolveWith(o,N.call(d,0))}}var d=arguments,j=0,k=d.length,m=k,o=k<=1&&b&&f.isFunction(b.promise)?b:f.Deferred();if(k>1){for(;j<k;j++)d[j]&&f.isFunction(d[j].promise)?d[j].promise().then(h(j),
o.reject):--m;m||o.resolveWith(o,d)}else o!==b&&o.resolveWith(o,k?[b]:[]);return o.promise()}});f.support=function(){var b=X.createElement("div"),h=X.documentElement,d,j,k,m,o,q,v;b.setAttribute("className","t");b.innerHTML="   <link/><table></table><a href='/a' style='top:1px;float:left;opacity:.55;'>a</a><input type='checkbox'/>";d=b.getElementsByTagName("*");j=b.getElementsByTagName("a")[0];if(!d||!d.length||!j)return{};k=X.createElement("select");m=k.appendChild(X.createElement("option"));d=b.getElementsByTagName("input")[0];
...
1)for(var j=this[0].attributes,k,m=0,o=j.length;m<o;m++){k=j[m].name;k.indexOf("data-")===0&&(k=f.camelCase(k.substring(5)),Ra(this[0],k,d[k]))}}return d}if(typeof b=="object")return this.each(function(){f.data(this,b)});var q=b.split(".");q[1]=q[1]?"."+q[1]:"";if(h===a){d=this.triggerHandler("getData"+q[1]+"!",[q[0]]);d===a&&this.length&&(d=f.data(this[0],b),d=Ra(this[0],b,d));return d===a&&q[1]?this.data(q[0]):d}return this.each(function(){var v=f(this),w=[q[0],h];v.triggerHandler("setData"+q[1]+
"!",w);f.data(this,b,h);v.triggerHandler("changeData"+q[1]+"!",w)})},removeData:function(b){return this.each(function(){f.removeData(this,b)})}});f.extend({_mark:function(b,h){b&&(h=(h||"fx")+"mark",f.data(b,h,(f.data(b,h,a,true)||0)+1,true))},_unmark:function(b,h,d){b!==true&&(d=h,h=b,b=false);if(h){d=d||"fx";var j=d+"mark";(b=b?0:(f.data(h,j,a,true)||1)-1)?f.data(h,j,b,true):(f.removeData(h,j,true),xa(h,d,"mark"))}},queue:function(b,h,d){if(b){h=(h||"fx")+"queue";var j=f.data(b,h,a,true);d&&(!j||
...
b)})},dequeue:function(b){return this.each(function(){f.dequeue(this,b)})},delay:function(b,h){b=f.fx?f.fx.speeds[b]||b:b;h=h||"fx";return this.queue(h,function(){var d=this;setTimeout(function(){f.dequeue(d,h)},b)})},clearQueue:function(b){return this.queue(b||"fx",[])},promise:function(b,h){function d(){--o||j.resolveWith(k,[k])}typeof b!="string"&&(h=b,b=a);b=b||"fx";for(var j=f.Deferred(),k=this,m=k.length,o=1,q=b+"defer",v=b+"queue",w=b+"mark",A;m--;)if(A=f.data(k[m],q,a,true)||(f.data(k[m],
v,a,true)||f.data(k[m],w,a,true))&&f.data(k[m],q,f._Deferred(),true)){o++;A.done(d)}d();return j.promise()}});var la=/[\n\t\r]/g,ja=/\s+/,wa=/\r/g,Ga=/^(?:button|input)$/i,ua=/^(?:button|input|object|select|textarea)$/i,Na=/^a(?:rea)?$/i,I=/^(?:autofocus|autoplay|async|checked|controls|defer|disabled|hidden|loop|multiple|open|readonly|required|scoped|selected)$/i,fa=/\:|^on/,ca,ia;f.fn.extend({attr:function(b,h){return f.access(this,b,h,true,f.attr)},removeAttr:function(b){return this.each(function(){f.removeAttr(this,
b)})},prop:function(b,h){return f.access(this,b,h,true,f.prop)},removeProp:function(b){b=f.propFix[b]||b;return this.each(function(){try{this[b]=a;delete this[b]}catch(h){}})},addClass:function(b){var h,d,j,k,m,o,q;if(f.isFunction(b))return this.each(function(v){f(this).addClass(b.call(this,v,this.className))});if(b&&typeof b=="string"){h=b.split(ja);d=0;for(j=this.length;d<j;d++){k=this[d];if(k.nodeType===1)if(!k.className&&h.length===1)k.className=b;else{m=" "+k.className+" ";o=0;for(q=h.length;o<
q;o++)~m.indexOf(" "+h[o]+" ")||(m+=h[o]+" ");k.className=f.trim(m)}}}return this},removeClass:function(b){var h,d,j,k,m,o,q;if(f.isFunction(b))return this.each(function(v){f(this).removeClass(b.call(this,v,this.className))});if(b&&typeof b=="string"||b===a){h=(b||"").split(ja);d=0;for(j=this.length;d<j;d++){k=this[d];if(k.nodeType===1&&k.className)if(b){m=(" "+k.className+" ").replace(la," ");o=0;for(q=h.length;o<q;o++)m=m.replace(" "+h[o]+" "," ");k.className=f.trim(m)}else k.className=""}}return this},
toggleClass:function(b,h){var d=typeof b,j=typeof h=="boolean";if(f.isFunction(b))return this.each(function(k){f(this).toggleClass(b.call(this,k,this.className,h),h)});return this.each(function(){if(d==="string")for(var k,m=0,o=f(this),q=h,v=b.split(ja);k=v[m++];){q=j?q:!o.hasClass(k);o[q?"addClass":"removeClass"](k)}else if(d==="undefined"||d==="boolean"){this.className&&f._data(this,"__className__",this.className);this.className=this.className||b===false?"":f._data(this,"__className__")||""}})},
...
m,o.val())):(q=b);q==null?(q=""):typeof q=="number"?(q+=""):f.isArray(q)&&(q=f.map(q,function(v){return v==null?"":v+""}));h=f.valHooks[this.nodeName.toLowerCase()]||f.valHooks[this.type];if(!h||!("set"in h)||h.set(this,q,"value")===a)this.value=q}})}});f.extend({valHooks:{option:{get:function(b){var h=b.attributes.value;return!h||h.specified?b.value:b.text}},select:{get:function(b){var h,d=b.selectedIndex,j=[],k=b.options;b=b.type==="select-one";if(d<0)return null;for(var m=b?d:0,o=b?d+1:k.length;m<
...
m.events,q=m.handle;o||(m.events=o={});q||(m.handle=q=function(da){return typeof f!="undefined"&&(!da||f.event.triggered!==da.type)?f.event.handle.apply(q.elem,arguments):a});q.elem=b;h=h.split(" ");for(var v,w=0,A;v=h[w++];){m=k?f.extend({},k):{handler:d,data:j};v.indexOf(".")>-1?(A=v.split("."),v=A.shift(),m.namespace=A.slice(0).sort().join(".")):(A=[],m.namespace="");m.type=v;m.guid||(m.guid=d.guid);var F=o[v],R=f.event.special[v]||{};if(!F){F=o[v]=[];if(!R.setup||R.setup.call(b,j,A,q)===false)b.addEventListener?
b.addEventListener(v,q,false):b.attachEvent&&b.attachEvent("on"+v,q)}R.add&&(R.add.call(b,m),m.handler.guid||(m.handler.guid=d.guid));F.push(m);f.event.global[v]=true}b=null}}},global:{},remove:function(b,h,d,j){if(b.nodeType!==3&&b.nodeType!==8){d===false&&(d=ha);var k,m,o=0,q,v,w,A,F,R,da=f.hasData(b)&&f._data(b),na=da&&da.events;if(da&&na){h&&h.type&&(d=h.handler,h=h.type);if(!h||typeof h=="string"&&h.charAt(0)===".")for(k in na)f.event.remove(b,k+(h||""));else{for(h=h.split(" ");k=h[o++];){A=
k;q=k.indexOf(".")<0;v=[];q||(v=k.split("."),k=v.shift(),w=new RegExp("(^|\\.)"+f.map(v.slice(0).sort(),Fb).join("\\.(?:.*\\.)?")+"(\\.|$)"));if(F=na[k])if(d){A=f.event.special[k]||{};for(m=j||0;m<F.length;m++){R=F[m];if(d.guid===R.guid){if(q||w.test(R.namespace)){j==null&&F.splice(m--,1);A.remove&&A.remove.call(b,R)}if(j!=null)break}}if(F.length===0||j!=null&&F.length===1){(!A.teardown||A.teardown.call(b,v)===false)&&f.removeEvent(b,k,da.handle);delete na[k]}}else for(m=0;m<F.length;m++){R=F[m];
...
m.ownerDocument||m===b.target.ownerDocument&&c}while(m&&!b.isPropagationStopped());if(!b.isDefaultPrevented()){var q;m=f.event.special[k]||{};if((!m._default||m._default.call(d.ownerDocument,b)===false)&&(k!=="click"||!f.nodeName(d,"a"))&&f.acceptData(d)){try{j&&d[k]&&(q=d[j],q&&(d[j]=null),f.event.triggered=k,d[k]())}catch(v){}q&&(d[j]=q);f.event.triggered=a}}return b.result}}else f.each(f.cache,function(){var w=this[f.expando];w&&w.events&&w.events[k]&&f.event.trigger(b,h,w.handle.elem)})}},handle:function(b){b=
...
h){function d(k){var m=f.event.fix(k);m.type=h;m.originalEvent={};f.event.trigger(m,null,m.target);m.isDefaultPrevented()&&k.preventDefault()}var j=0;f.event.special[h]={setup:function(){j++===0&&X.addEventListener(b,d,true)},teardown:function(){--j===0&&X.removeEventListener(b,d,true)}}});f.each(["bind","one"],function(b,h){f.fn[h]=function(d,j,k){var m;if(typeof d=="object"){for(var o in d)this[h](o,j,d[o],k);return this}if(arguments.length===2||j===false){k=j;j=a}h==="one"?(m=function(v){f(this).unbind(v,
...
b)}});var rb={focus:"focusin",blur:"focusout",mouseenter:"mouseover",mouseleave:"mouseout"};f.each(["live","die"],function(b,h){f.fn[h]=function(d,j,k,m){var o=0,q,v,w=m||this.selector,A=m?this:f(this.context);if(typeof d=="object"&&!d.preventDefault){for(q in d)A[h](q,j,d[q],w);return this}if(h==="die"&&!d&&m&&m.charAt(0)==="."){A.unbind(m);return this}if(j===false||f.isFunction(j)){k=j||ha;j=a}for(d=(d||"").split(" ");(m=d[o++])!=null;){q=sa.exec(m);v="";q&&(v=q[0],m=m.replace(sa,""));if(m==="hover")d.push("mouseenter"+
v,"mouseleave"+v);else{q=m;rb[m]?(d.push(rb[m]+v),m+=v):(m=(rb[m]||m)+v);if(h==="live"){v=0;for(var F=A.length;v<F;v++)f.event.add(A[v],"live."+Y(m,w),{data:j,selector:w,handler:k,origType:m,origHandler:k,preType:q})}else A.unbind("live."+Y(m,w),k)}}return this}});f.each("blur focus focusin focusout load resize scroll unload click dblclick mousedown mouseup mousemove mouseover mouseout mouseenter mouseleave change select submit keydown keypress keyup error".split(" "),function(b,h){f.fn[h]=function(d,
...
PSEUDO:/:((?:[\w\u00c0-\uFFFF\-]|\\.)+)(?:\((['"]?)((?:\([^\)]+\)|[^\(\)]*)+)\2\))?/},leftMatch:{},attrMap:{"class":"className","for":"htmlFor"},attrHandle:{href:function(r){return r.getAttribute("href")},type:function(r){return r.getAttribute("type")}},relative:{"+":function(r,y){var H=typeof y=="string",J=H&&!v.test(y);H=H&&!J;J&&(y=y.toLowerCase());J=0;for(var G=r.length,L;J<G;J++)if(L=r[J]){for(;(L=L.previousSibling)&&L.nodeType!==1;);r[J]=H||L&&L.nodeName.toLowerCase()===y?L||false:L===y}H&&
w.filter(y,r,true)},">":function(r,y){var H,J=typeof y=="string",G=0,L=r.length;if(J&&!v.test(y))for(y=y.toLowerCase();G<L;G++){if(H=r[G]){H=H.parentNode;r[G]=H.nodeName.toLowerCase()===y?H:false}}else{for(;G<L;G++)(H=r[G])&&(r[G]=J?H.parentNode:H.parentNode===y);J&&w.filter(y,r,true)}},"":function(r,y,H){var J,G=j++,L=b;typeof y=="string"&&!v.test(y)&&(y=y.toLowerCase(),J=y,L=h);L("parentNode",y,G,r,J,H)},"~":function(r,y,H){var J,G=j++,L=b;typeof y=="string"&&!v.test(y)&&(y=y.toLowerCase(),J=y,
...
"not",b)},filter:function(b){return this.pushStack(E(this,b,true),"filter",b)},is:function(b){return!!b&&(typeof b=="string"?f.filter(b,this).length>0:this.filter(b).length>0)},closest:function(b,h){var d=[],j,k,m=this[0];if(f.isArray(b)){var o,q={},v=1;if(m&&b.length){j=0;for(k=b.length;j<k;j++){o=b[j];q[o]||(q[o]=sb.test(o)?f(o,h||this.context):o)}for(;m&&m.ownerDocument&&m!==h;){for(o in q){j=q[o];(j.jquery?j.index(m)>-1:f(m).is(j))&&d.push({selector:o,elem:m,level:v})}m=m.parentNode;v++}}return d}o=
...
q));m=j.fragment;m.childNodes.length===1?(k=m=m.firstChild):(k=m.firstChild);if(k){h=h&&f.nodeName(k,"tr");k=0;for(var v=this.length,w=v-1;k<v;k++)d.call(h?f.nodeName(this[k],"table")?this[k].getElementsByTagName("tbody")[0]||this[k].appendChild(this[k].ownerDocument.createElement("tbody")):this[k]:this[k],j.cacheable||v>1&&k<w?f.clone(m,true,true):m)}q.length&&f.each(q,ba)}return this}});f.buildFragment=function(b,h,d){var j,k,m,o;h&&h[0]&&(o=h[0].ownerDocument||h[0]);o.createDocumentFragment||(o=
...
for(o=0;k[o];++o)B(k[o],m[o])}}return j},clean:function(b,h,d,j){h=h||X;typeof h.createElement=="undefined"&&(h=h.ownerDocument||h[0]&&h[0].ownerDocument||X);for(var k=[],m,o=0,q;(q=b[o])!=null;o++){typeof q=="number"&&(q+="");if(q){if(typeof q=="string")if(fc.test(q)){q=q.replace(Ub,"<$1></$2>");m=(Vb.exec(q)||["",""])[1].toLowerCase();var v=Va[m]||Va._default,w=v[0],A=h.createElement("div");for(A.innerHTML=v[1]+q+v[2];w--;)A=A.lastChild;if(!f.support.tbody){w=Wb.test(q);v=m==="table"&&!w?A.firstChild&&
A.firstChild.childNodes:v[1]==="<table>"&&!w?A.childNodes:[];for(m=v.length-1;m>=0;--m)f.nodeName(v[m],"tbody")&&!v[m].childNodes.length&&v[m].parentNode.removeChild(v[m])}!f.support.leadingWhitespace&&Jb.test(q)&&A.insertBefore(h.createTextNode(Jb.exec(q)[0]),A.firstChild);q=A.childNodes}else q=h.createTextNode(q);var F;if(!f.support.appendChecked)if(q[0]&&typeof(F=q.length)=="number")for(m=0;m<F;m++)ka(q[m]);else ka(q);q.nodeType?k.push(q):(k=f.merge(k,q))}}if(d){b=function(R){return!R.type||gc.test(R.type)};
for(o=0;k[o];o++)if(j&&f.nodeName(k[o],"script")&&(!k[o].type||k[o].type.toLowerCase()==="text/javascript"))j.push(k[o].parentNode?k[o].parentNode.removeChild(k[o]):k[o]);else{if(k[o].nodeType===1){h=f.grep(k[o].getElementsByTagName("script"),b);k.splice.apply(k,[o+1,0].concat(h))}d.appendChild(k[o])}}return k},cleanData:function(b){for(var h,d,j=f.cache,k=f.expando,m=f.event.special,o=f.support.deleteExpando,q=0,v;(v=b[q])!=null;q++)if(!(v.nodeName&&f.noData[v.nodeName.toLowerCase()]))if(d=v[f.expando]){if((h=
j[d]&&j[d][k])&&h.events){for(var w in h.events)m[w]?f.event.remove(v,w):f.removeEvent(v,w,h.handle);h.handle&&(h.handle.elem=null)}o?delete v[f.expando]:v.removeAttribute&&v.removeAttribute(f.expando);delete j[d]}}});var Xb=/alpha\([^)]*\)/i,hc=/opacity=([^)]*)/,mc=/([A-Z]|^ms)/g,Kb=/^-?\d+(?:px)?$/i,Yb=/^-?\d/,Zb=/^[+\-]=/,ya=/[^+\-\.\de]+/g,ic={position:"absolute",visibility:"hidden",display:"block"},oa=["Left","Right"],Qb=["Top","Bottom"],fb,mb,va;f.fn.css=function(b,h){if(arguments.length===
...
f.camelCase(h),o=b.style,q=f.cssHooks[m];h=f.cssProps[m]||m;if(d===a){if(q&&"get"in q&&(k=q.get(b,false,j))!==a)return k;return o[h]}j=typeof d;if(!(j==="number"&&isNaN(d)||d==null)){j==="string"&&Zb.test(d)&&(d=+d.replace(ya,"")+parseFloat(f.css(b,h)),j="number");j==="number"&&!f.cssNumber[m]&&(d+="px");if(!q||!("set"in q)||(d=q.set(b,d))!==a)try{o[h]=d}catch(v){}}}},css:function(b,h,d){var j,k;h=f.camelCase(h);k=f.cssHooks[h];h=f.cssProps[h]||h;h==="cssFloat"&&(h="float");if(k&&"get"in k&&(j=k.get(b,
...
b=b.slice(0,j)}j="GET";h&&(f.isFunction(h)?(d=h,h=a):typeof h=="object"&&(h=f.param(h,f.ajaxSettings.traditional),j="POST"));var m=this;f.ajax({url:b,type:j,dataType:"html",data:h,complete:function(o,q,v){v=o.responseText;o.isResolved()&&(o.done(function(w){v=w}),m.html(k?f("<div>").append(v.replace(Qa,"")).find(k):v));d&&m.each(d,[v,q,o])}});return this},serialize:function(){return f.param(this.serializeArray())},serializeArray:function(){return this.map(function(){return this.elements?f.makeArray(this.elements):
...
global:true,type:"GET",contentType:"application/x-www-form-urlencoded",processData:true,async:true,accepts:{xml:"application/xml, text/xml",html:"text/html",text:"text/plain",json:"application/json, text/javascript","*":"*/*"},contents:{xml:/xml/,html:/html/,json:/json/},responseFields:{xml:"responseXML",text:"responseText"},converters:{"* text":c.String,"text html":true,"text json":f.parseJSON,"text xml":f.parseXML}},ajaxPrefilter:C(cb),ajaxTransport:C(Za),ajax:function(b,h){function d(G,L,U,aa){if(Ba!==
...
"each":"queue"](function(){k.queue===false&&f._mark(this);var m=f.extend({},k),o=this.nodeType===1,q=o&&f(this).is(":hidden"),v,w,A,F,R,da,na,Ia;m.animatedProperties={};for(A in b){v=f.camelCase(A);A!==v&&(b[v]=b[A],delete b[A]);w=b[v];f.isArray(w)?(m.animatedProperties[v]=w[1],w=b[v]=w[0]):(m.animatedProperties[v]=m.specialEasing&&m.specialEasing[v]||m.easing||"swing");if(w==="hide"&&q||w==="show"&&!q)return m.complete.call(this);o&&(v==="height"||v==="width")&&(m.overflow=[this.style.overflow,this.style.overflowX,
...
null&&!f.support.shrinkWrapBlocks&&f.each(["","X","Y"],function(v,w){j.style["overflow"+w]=k.overflow[v]});k.hide&&f(j).hide();if(k.hide||k.show)for(var q in k.animatedProperties)f.style(j,q,k.orig[q]);k.complete.call(j)}return false}k.duration==Infinity?(this.now=h):(o=h-this.startTime,this.state=o/k.duration,this.pos=f.easing[k.animatedProperties[this.prop]](this.state,o,0,1,k.duration),this.now=this.start+(this.end-this.start)*this.pos);this.update();return true}};f.extend(f.fx,{tick:function(){for(var b=
...
m.getComputedStyle(h,null):h.currentStyle;for(var v=h.offsetTop,w=h.offsetLeft;(h=h.parentNode)&&h!==q&&h!==o;){if(f.offset.supportsFixedPosition&&d.position==="fixed")break;d=m?m.getComputedStyle(h,null):h.currentStyle;v-=h.scrollTop;w-=h.scrollLeft;h===j&&(v+=h.offsetTop,w+=h.offsetLeft,f.offset.doesNotAddBorder&&(!f.offset.doesAddBorderForTableAndCells||!$a.test(h.nodeName))&&(v+=parseFloat(d.borderTopWidth)||0,w+=parseFloat(d.borderLeftWidth)||0),k=j,j=h.offsetParent);f.offset.subtractsBorderForOverflowNotVisible&&
d.overflow!=="visible"&&(v+=parseFloat(d.borderTopWidth)||0,w+=parseFloat(d.borderLeftWidth)||0);d=d}if(d.position==="relative"||d.position==="static"){v+=q.offsetTop;w+=q.offsetLeft}f.offset.supportsFixedPosition&&d.position==="fixed"&&(v+=Math.max(o.scrollTop,q.scrollTop),w+=Math.max(o.scrollLeft,q.scrollLeft));return{top:v,left:w}});f.offset={initialize:function(){var b=X.body,h=X.createElement("div"),d,j,k,m=parseFloat(f.css(b,"marginTop"))||0;f.extend(h.style,{position:"absolute",top:0,left:0,
...
"mousedown")+".ui-disableSelection",function(i){i.preventDefault()})},enableSelection:function(){return this.unbind(".ui-disableSelection")}});c.each(["Width","Height"],function(i,l){function n(C,K,ba,ka){c.each(u,function(){K-=parseFloat(c.curCSS(C,"padding"+this,true))||0;if(ba)K-=parseFloat(c.curCSS(C,"border"+this+"Width",true))||0;if(ka)K-=parseFloat(c.curCSS(C,"margin"+this,true))||0});return K}var u=l==="Width"?["Left","Right"]:["Top","Bottom"],x=l.toLowerCase(),s={innerWidth:c.fn.innerWidth,
innerHeight:c.fn.innerHeight,outerWidth:c.fn.outerWidth,outerHeight:c.fn.outerHeight};c.fn["inner"+l]=function(C){if(C===a)return s["inner"+l].call(this);return this.each(function(){c(this).css(x,n(this,C)+"px")})};c.fn["outer"+l]=function(C,K){if(typeof C!=="number")return s["outer"+l].call(this,C);return this.each(function(){c(this).css(x,n(this,C,true,K)+"px")})}});c.extend(c.expr[":"],{data:function(i,l,n){return!!c.data(i,n[3])},focusable:function(i){return e(i,!isNaN(c.attr(i,"tabindex")))},
tabbable:function(i){var l=c.attr(i,"tabindex"),n=isNaN(l);return(n||l>=0)&&e(i,!n)}});c(function(){var i=document.body,l=i.appendChild(l=document.createElement("div"));c.extend(l.style,{minHeight:"100px",height:"auto",padding:0,borderWidth:0});c.support.minHeight=l.offsetHeight===100;c.support.selectstart="onselectstart"in l;i.removeChild(l).style.display="none"});c.extend(c.ui,{plugin:{add:function(i,l,n){i=c.ui[i].prototype;for(var u in n){i.plugins[u]=i.plugins[u]||[];i.plugins[u].push([l,n[u]])}},
call:function(i,l,n){if((l=i.plugins[l])&&i.element[0].parentNode)for(var u=0;u<l.length;u++)i.options[l[u][0]]&&l[u][1].apply(i.element,n)}},contains:function(i,l){return document.compareDocumentPosition?i.compareDocumentPosition(l)&16:i!==l&&i.contains(l)},hasScroll:function(i,l){if(c(i).css("overflow")==="hidden")return false;var n=l&&l==="left"?"scrollLeft":"scrollTop",u=false;if(i[n]>0)return true;i[n]=1;u=i[n]>0;i[n]=0;return u},isOverAxis:function(i,l,n){return i>l&&i<l+n},isOver:function(i,
l,n,u,x,s){return c.ui.isOverAxis(i,n,x)&&c.ui.isOverAxis(l,u,s)}})}})(jQuery);
(function(c,a){if(c.cleanData){var e=c.cleanData;c.cleanData=function(i){for(var l=0,n;(n=i[l])!=null;l++)try{c(n).triggerHandler("remove")}catch(u){}e(i)}}else{var g=c.fn.remove;c.fn.remove=function(i,l){return this.each(function(){if(!l)if(!i||c.filter(i,[this]).length)c("*",this).add([this]).each(function(){try{c(this).triggerHandler("remove")}catch(n){}});return g.call(c(this),i,l)})}}c.widget=function(i,l,n){var u=i.split(".")[0],x;i=i.split(".")[1];x=u+"-"+i;if(!n){n=l;l=c.Widget}c.expr[":"][x]=
function(s){return!!c.data(s,i)};c[u]=c[u]||{};c[u][i]=function(s,C){arguments.length&&this._createWidget(s,C)};l=new l;l.options=c.extend(true,{},l.options);c[u][i].prototype=c.extend(true,l,{namespace:u,widgetName:i,widgetEventPrefix:c[u][i].prototype.widgetEventPrefix||i,widgetBaseClass:x},n);c.widget.bridge(i,c[u][i])};c.widget.bridge=function(i,l){c.fn[i]=function(n){var u=typeof n==="string",x=Array.prototype.slice.call(arguments,1),s=this;n=!u&&x.length?c.extend.apply(null,[true,n].concat(x)):
n;if(u&&n.charAt(0)==="_")return s;u?this.each(function(){var C=c.data(this,i),K=C&&c.isFunction(C[n])?C[n].apply(C,x):C;if(K!==C&&K!==a){s=K;return false}}):this.each(function(){var C=c.data(this,i);C?C.option(n||{})._init():c.data(this,i,new l(n,this))});return s}};c.Widget=function(i,l){arguments.length&&this._createWidget(i,l)};c.Widget.prototype={widgetName:"widget",widgetEventPrefix:"",options:{disabled:false},_createWidget:function(i,l){c.data(l,this.widgetName,this);this.element=c(l);this.options=
...
"-disabled ui-state-disabled")},widget:function(){return this.element},option:function(i,l){var n=i;if(arguments.length===0)return c.extend({},this.options);if(typeof i==="string"){if(l===a)return this.options[i];n={};n[i]=l}this._setOptions(n);return this},_setOptions:function(i){var l=this;c.each(i,function(n,u){l._setOption(n,u)});return this},_setOption:function(i,l){this.options[i]=l;if(i==="disabled")this.widget()[l?"addClass":"removeClass"](this.widgetBaseClass+"-disabled ui-state-disabled").attr("aria-disabled",
l);return this},enable:function(){return this._setOption("disabled",false)},disable:function(){return this._setOption("disabled",true)},_trigger:function(i,l,n){var u=this.options[i];l=c.Event(l);l.type=(i===this.widgetEventPrefix?i:this.widgetEventPrefix+i).toLowerCase();n=n||{};if(l.originalEvent){i=c.event.props.length;for(var x;i;){x=c.event.props[--i];l[x]=l.originalEvent[x]}}this.element.trigger(l,n);return!(c.isFunction(u)&&u.call(this.element[0],l,n)===false||l.isDefaultPrevented())}}})(jQuery);
...
!(this.scrollParent[0]!=document&&c.ui.contains(this.scrollParent[0],this.offsetParent[0]))?this.offsetParent:this.scrollParent,i=/(html|body)/i.test(g[0].tagName),l=a.pageX,n=a.pageY;if(this.originalPosition){var u;if(this.containment){if(this.relative_container){u=this.relative_container.offset();u=[this.containment[0]+u.left,this.containment[1]+u.top,this.containment[2]+u.left,this.containment[3]+u.top]}else u=this.containment;if(a.pageX-this.offset.click.left<u[0])l=u[0]+this.offset.click.left;
if(a.pageY-this.offset.click.top<u[1])n=u[1]+this.offset.click.top;if(a.pageX-this.offset.click.left>u[2])l=u[2]+this.offset.click.left;if(a.pageY-this.offset.click.top>u[3])n=u[3]+this.offset.click.top}if(e.grid){n=e.grid[1]?this.originalPageY+Math.round((n-this.originalPageY)/e.grid[1])*e.grid[1]:this.originalPageY;n=u?!(n-this.offset.click.top<u[1]||n-this.offset.click.top>u[3])?n:!(n-this.offset.click.top<u[1])?n-e.grid[1]:n+e.grid[1]:n;l=e.grid[0]?this.originalPageX+Math.round((l-this.originalPageX)/
e.grid[0])*e.grid[0]:this.originalPageX;l=u?!(l-this.offset.click.left<u[0]||l-this.offset.click.left>u[2])?l:!(l-this.offset.click.left<u[0])?l-e.grid[0]:l+e.grid[0]:l}}return{top:n-this.offset.click.top-this.offset.relative.top-this.offset.parent.top+(c.browser.safari&&c.browser.version<526&&this.cssPosition=="fixed"?0:this.cssPosition=="fixed"?-this.scrollParent.scrollTop():i?0:g.scrollTop()),left:l-this.offset.click.left-this.offset.relative.left-this.offset.parent.left+(c.browser.safari&&c.browser.version<
...
width:g.outerWidth(),height:g.outerHeight(),top:i.top,left:i.left})})},drag:function(a,e){for(var g=c(this).data("draggable"),i=g.options,l=i.snapTolerance,n=e.offset.left,u=n+g.helperProportions.width,x=e.offset.top,s=x+g.helperProportions.height,C=g.snapElements.length-1;C>=0;C--){var K=g.snapElements[C].left,ba=K+g.snapElements[C].width,ka=g.snapElements[C].top,ma=ka+g.snapElements[C].height;if(K-l<n&&n<ba+l&&ka-l<x&&x<ma+l||K-l<n&&n<ba+l&&ka-l<s&&s<ma+l||K-l<u&&u<ba+l&&ka-l<x&&x<ma+l||K-l<u&&
u<ba+l&&ka-l<s&&s<ma+l){if(i.snapMode!="inner"){var p=Math.abs(ka-s)<=l,t=Math.abs(ma-x)<=l,B=Math.abs(K-u)<=l,E=Math.abs(ba-n)<=l;if(p)e.position.top=g._convertPositionTo("relative",{top:ka-g.helperProportions.height,left:0}).top-g.margins.top;if(t)e.position.top=g._convertPositionTo("relative",{top:ma,left:0}).top-g.margins.top;if(B)e.position.left=g._convertPositionTo("relative",{top:0,left:K-g.helperProportions.width}).left-g.margins.left;if(E)e.position.left=g._convertPositionTo("relative",{top:0,
left:ba}).left-g.margins.left}var Y=p||t||B||E;if(i.snapMode!="outer"){p=Math.abs(ka-x)<=l;t=Math.abs(ma-s)<=l;B=Math.abs(K-n)<=l;E=Math.abs(ba-u)<=l;if(p)e.position.top=g._convertPositionTo("relative",{top:ka,left:0}).top-g.margins.top;if(t)e.position.top=g._convertPositionTo("relative",{top:ma-g.helperProportions.height,left:0}).top-g.margins.top;if(B)e.position.left=g._convertPositionTo("relative",{top:0,left:K}).left-g.margins.left;if(E)e.position.left=g._convertPositionTo("relative",{top:0,left:ba-
g.helperProportions.width}).left-g.margins.left}if(!g.snapElements[C].snapping&&(p||t||B||E||Y))g.options.snap.snap&&g.options.snap.snap.call(g.element,a,c.extend(g._uiHash(),{snapItem:g.snapElements[C].item}));g.snapElements[C].snapping=p||t||B||E||Y}else{g.snapElements[C].snapping&&g.options.snap.release&&g.options.snap.release.call(g.element,a,c.extend(g._uiHash(),{snapItem:g.snapElements[C].item}));g.snapElements[C].snapping=false}}}});c.ui.plugin.add("draggable","stack",{start:function(){var a=
...
a,this.ui(g));return this.element}return false},ui:function(a){return{draggable:a.currentItem||a.element,helper:a.helper,position:a.position,offset:a.positionAbs}}});c.extend(c.ui.droppable,{version:"1.8.16"});c.ui.intersect=function(a,e,g){if(!e.offset)return false;var i=(a.positionAbs||a.position.absolute).left,l=i+a.helperProportions.width,n=(a.positionAbs||a.position.absolute).top,u=n+a.helperProportions.height,x=e.offset.left,s=x+e.proportions.width,C=e.offset.top,K=C+e.proportions.height;switch(g){case "fit":return x<=
i&&l<=s&&C<=n&&u<=K;case "intersect":return x<i+a.helperProportions.width/2&&l-a.helperProportions.width/2<s&&C<n+a.helperProportions.height/2&&u-a.helperProportions.height/2<K;case "pointer":return c.ui.isOver((a.positionAbs||a.position.absolute).top+(a.clickOffset||a.offset.click).top,(a.positionAbs||a.position.absolute).left+(a.clickOffset||a.offset.click).left,C,x,e.proportions.height,e.proportions.width);case "touch":return(n>=C&&n<=K||u>=C&&u<=K||n<C&&u>K)&&(i>=x&&i<=s||l>=x&&l<=s||i<x&&l>s);
default:return false}};c.ui.ddmanager={current:null,droppables:{"default":[]},prepareOffsets:function(a,e){var g=c.ui.ddmanager.droppables[a.options.scope]||[],i=e?e.type:null,l=(a.currentItem||a.element).find(":data(droppable)").andSelf(),n=0;a:for(;n<g.length;n++)if(!(g[n].options.disabled||a&&!g[n].accept.call(g[n].element[0],a.currentItem||a.element))){for(var u=0;u<l.length;u++)if(l[u]==g[n].element[0]){g[n].proportions.height=0;continue a}g[n].visible=g[n].element.css("display")!="none";if(g[n].visible){i==
...
nw:".ui-resizable-nw"});if(this.handles.constructor==String){if(this.handles=="all")this.handles="n,e,s,w,se,sw,ne,nw";var l=this.handles.split(",");this.handles={};for(var n=0;n<l.length;n++){var u=c.trim(l[n]),x=c('<div class="ui-resizable-handle '+("ui-resizable-"+u)+'"></div>');/sw|se|ne|nw/.test(u)&&x.css({zIndex:++i.zIndex});"se"==u&&x.addClass("ui-icon ui-icon-gripsmall-diagonal-se");this.handles[u]=".ui-resizable-"+u;this.element.append(x)}}this._renderAxis=function(s){s=s||this.element;for(var C in this.handles){if(this.handles[C].constructor==
String)this.handles[C]=c(this.handles[C],this.element).show();if(this.elementIsWrapper&&this.originalElement[0].nodeName.match(/textarea|input|select|button/i)){var K=c(this.handles[C],this.element),ba=0;ba=/sw|ne|nw|se|n|s/.test(C)?K.outerHeight():K.outerWidth();K=["padding",/ne|nw|n/.test(C)?"Top":/se|sw|s/.test(C)?"Bottom":/^e$/.test(C)?"Right":"Left"].join("");s.css(K,ba);this._proportionallyResize()}c(this.handles[C])}};this._renderAxis(this.element);this._handles=c(".ui-resizable-handle",this.element).disableSelection();
...
this._renderProxy();l=a(this.helper.css("left"));var u=a(this.helper.css("top"));if(i.containment){l+=c(i.containment).scrollLeft()||0;u+=c(i.containment).scrollTop()||0}this.offset=this.helper.offset();this.position={left:l,top:u};this.size=this._helper?{width:n.outerWidth(),height:n.outerHeight()}:{width:n.width(),height:n.height()};this.originalSize=this._helper?{width:n.outerWidth(),height:n.outerHeight()}:{width:n.width(),height:n.height()};this.originalPosition={left:l,top:u};this.sizeDiff=
...
_mouseStop:function(g){this.resizing=false;var i=this.options;if(this._helper){var l=this._proportionallyResizeElements,n=l.length&&/textarea/i.test(l[0].nodeName);l=n&&c.ui.hasScroll(l[0],"left")?0:this.sizeDiff.height;n=n?0:this.sizeDiff.width;n={width:this.helper.width()-n,height:this.helper.height()-l};l=parseInt(this.element.css("left"),10)+(this.position.left-this.originalPosition.left)||null;var u=parseInt(this.element.css("top"),10)+(this.position.top-this.originalPosition.top)||null;i.animate||
this.element.css(c.extend(n,{top:u,left:l}));this.helper.height(this.size.height);this.helper.width(this.size.width);this._helper&&!i.animate&&this._proportionallyResize()}c("body").css("cursor","auto");this.element.removeClass("ui-resizable-resizing");this._propagate("stop",g);this._helper&&this.helper.remove();return false},_updateVirtualBoundaries:function(g){var i=this.options,l,n,u;i={minWidth:e(i.minWidth)?i.minWidth:0,maxWidth:e(i.maxWidth)?i.maxWidth:Infinity,minHeight:e(i.minHeight)?i.minHeight:
0,maxHeight:e(i.maxHeight)?i.maxHeight:Infinity};if(this._aspectRatio||g){g=i.minHeight*this.aspectRatio;n=i.minWidth/this.aspectRatio;l=i.maxHeight*this.aspectRatio;u=i.maxWidth/this.aspectRatio;if(g>i.minWidth)i.minWidth=g;if(n>i.minHeight)i.minHeight=n;if(l<i.maxWidth)i.maxWidth=l;if(u<i.maxHeight)i.maxHeight=u}this._vBoundaries=i},_updateCache:function(g){this.offset=this.helper.offset();if(e(g.left))this.position.left=g.left;if(e(g.top))this.position.top=g.top;if(e(g.height))this.size.height=
g.height;if(e(g.width))this.size.width=g.width},_updateRatio:function(g){var i=this.position,l=this.size,n=this.axis;if(e(g.height))g.width=g.height*this.aspectRatio;else if(e(g.width))g.height=g.width/this.aspectRatio;if(n=="sw"){g.left=i.left+(l.width-g.width);g.top=null}if(n=="nw"){g.top=i.top+(l.height-g.height);g.left=i.left+(l.width-g.width)}return g},_respectSize:function(g){var i=this._vBoundaries,l=this.axis,n=e(g.width)&&i.maxWidth&&i.maxWidth<g.width,u=e(g.height)&&i.maxHeight&&i.maxHeight<
g.height,x=e(g.width)&&i.minWidth&&i.minWidth>g.width,s=e(g.height)&&i.minHeight&&i.minHeight>g.height;if(x)g.width=i.minWidth;if(s)g.height=i.minHeight;if(n)g.width=i.maxWidth;if(u)g.height=i.maxHeight;var C=this.originalPosition.left+this.originalSize.width,K=this.position.top+this.size.height,ba=/sw|nw|w/.test(l);l=/nw|ne|n/.test(l);if(x&&ba)g.left=C-i.minWidth;if(n&&ba)g.left=C-i.maxWidth;if(s&&l)g.top=K-i.minHeight;if(u&&l)g.top=K-i.maxHeight;if((i=!g.width&&!g.height)&&!g.left&&g.top)g.top=
null;else if(i&&!g.top&&g.left)g.left=null;return g},_proportionallyResize:function(){if(this._proportionallyResizeElements.length)for(var g=this.helper||this.element,i=0;i<this._proportionallyResizeElements.length;i++){var l=this._proportionallyResizeElements[i];if(!this.borderDif){var n=[l.css("borderTopWidth"),l.css("borderRightWidth"),l.css("borderBottomWidth"),l.css("borderLeftWidth")],u=[l.css("paddingTop"),l.css("paddingRight"),l.css("paddingBottom"),l.css("paddingLeft")];this.borderDif=c.map(n,
function(x,s){var C=parseInt(x,10)||0,K=parseInt(u[s],10)||0;return C+K})}c.browser.msie&&(c(g).is(":hidden")||c(g).parents(":hidden").length)||l.css({height:g.height()-this.borderDif[0]-this.borderDif[2]||0,width:g.width()-this.borderDif[1]-this.borderDif[3]||0})}},_renderProxy:function(){var g=this.options;this.elementOffset=this.element.offset();if(this._helper){this.helper=this.helper||c('<div style="overflow:hidden;"></div>');var i=c.browser.msie&&c.browser.version<7,l=i?1:0;i=i?2:-1;this.helper.addClass(this._helper).css({width:this.element.outerWidth()+
...
10),height:parseInt(n.height(),10),left:parseInt(n.css("left"),10),top:parseInt(n.css("top"),10),position:n.css("position")})})};if(typeof g.alsoResize=="object"&&!g.alsoResize.parentNode)if(g.alsoResize.length){g.alsoResize=g.alsoResize[0];i(g.alsoResize)}else c.each(g.alsoResize,function(l){i(l)});else i(g.alsoResize)},resize:function(g,i){var l=c(this).data("resizable"),n=l.options,u=l.originalSize,x=l.originalPosition,s={height:l.size.height-u.height||0,width:l.size.width-u.width||0,top:l.position.top-
x.top||0,left:l.position.left-x.left||0},C=function(K,ba){c(K).each(function(){var ka=c(this),ma=c(this).data("resizable-alsoresize"),p={},t=ba&&ba.length?ba:ka.parents(i.originalElement[0]).length?["width","height"]:["width","height","top","left"];c.each(t,function(B,E){var Y=(ma[E]||0)+(s[E]||0);if(Y&&Y>=0)p[E]=Y||null});if(c.browser.opera&&/relative/.test(ka.css("position"))){l._revertToRelativePosition=true;ka.css({position:"absolute",top:"auto",left:"auto"})}ka.css(p)})};typeof n.alsoResize==
"object"&&!n.alsoResize.nodeType?c.each(n.alsoResize,function(K,ba){C(K,ba)}):C(n.alsoResize)},stop:function(){var g=c(this).data("resizable"),i=g.options,l=function(n){c(n).each(function(){var u=c(this);u.css({position:u.data("resizable-alsoresize").position})})};if(g._revertToRelativePosition){g._revertToRelativePosition=false;typeof i.alsoResize=="object"&&!i.alsoResize.nodeType?c.each(i.alsoResize,function(n){l(n)}):l(i.alsoResize)}c(this).removeData("resizable-alsoresize")}});c.ui.plugin.add("resizable",
"animate",{stop:function(g){var i=c(this).data("resizable"),l=i.options,n=i._proportionallyResizeElements,u=n.length&&/textarea/i.test(n[0].nodeName),x=u&&c.ui.hasScroll(n[0],"left")?0:i.sizeDiff.height;u={width:i.size.width-(u?0:i.sizeDiff.width),height:i.size.height-x};x=parseInt(i.element.css("left"),10)+(i.position.left-i.originalPosition.left)||null;var s=parseInt(i.element.css("top"),10)+(i.position.top-i.originalPosition.top)||null;i.element.animate(c.extend(u,s&&x?{top:s,left:x}:{}),{duration:l.animateDuration,
easing:l.animateEasing,step:function(){var C={width:parseInt(i.element.css("width"),10),height:parseInt(i.element.css("height"),10),top:parseInt(i.element.css("top"),10),left:parseInt(i.element.css("left"),10)};n&&n.length&&c(n[0]).css({width:C.width,height:C.height});i._updateCache(C);i._propagate("resize",g)}})}});c.ui.plugin.add("resizable","containment",{start:function(){var g=c(this).data("resizable"),i=g.element,l=g.options.containment;if(i=l instanceof c?l.get(0):/parent/.test(l)?i.parent().get(0):
l){g.containerElement=c(i);if(/document/.test(l)||l==document){g.containerOffset={left:0,top:0};g.containerPosition={left:0,top:0};g.parentData={element:c(document),left:0,top:0,width:c(document).width(),height:c(document).height()||document.body.parentNode.scrollHeight}}else{var n=c(i),u=[];c(["Top","Right","Left","Bottom"]).each(function(C,K){u[C]=a(n.css("padding"+K))});g.containerOffset=n.offset();g.containerPosition=n.position();g.containerSize={height:n.innerHeight()-u[3],width:n.innerWidth()-
u[1]};l=g.containerOffset;var x=g.containerSize.height,s=g.containerSize.width;s=c.ui.hasScroll(i,"left")?i.scrollWidth:s;x=c.ui.hasScroll(i)?i.scrollHeight:x;g.parentData={element:i,left:l.left,top:l.top,width:s,height:x}}}},resize:function(g){var i=c(this).data("resizable"),l=i.options,n=i.containerOffset,u=i.position;g=i._aspectRatio||g.shiftKey;var x={top:0,left:0},s=i.containerElement;if(s[0]!=document&&/static/.test(s.css("position")))x=n;if(u.left<(i._helper?n.left:0)){i.size.width+=i._helper?
i.position.left-n.left:i.position.left-x.left;if(g)i.size.height=i.size.width/l.aspectRatio;i.position.left=l.helper?n.left:0}if(u.top<(i._helper?n.top:0)){i.size.height+=i._helper?i.position.top-n.top:i.position.top;if(g)i.size.width=i.size.height*l.aspectRatio;i.position.top=i._helper?n.top:0}i.offset.left=i.parentData.left+i.position.left;i.offset.top=i.parentData.top+i.position.top;l=Math.abs((i._helper?i.offset.left-x.left:i.offset.left-x.left)+i.sizeDiff.width);n=Math.abs((i._helper?i.offset.top-
x.top:i.offset.top-n.top)+i.sizeDiff.height);u=i.containerElement.get(0)==i.element.parent().get(0);x=/relative|absolute/.test(i.containerElement.css("position"));if(u&&x)l-=i.parentData.left;if(l+i.size.width>=i.parentData.width){i.size.width=i.parentData.width-l;if(g)i.size.height=i.size.width/i.aspectRatio}if(n+i.size.height>=i.parentData.height){i.size.height=i.parentData.height-n;if(g)i.size.width=i.size.height*i.aspectRatio}},stop:function(){var g=c(this).data("resizable"),i=g.options,l=g.containerOffset,
n=g.containerPosition,u=g.containerElement,x=c(g.helper),s=x.offset(),C=x.outerWidth()-g.sizeDiff.width;x=x.outerHeight()-g.sizeDiff.height;g._helper&&!i.animate&&/relative/.test(u.css("position"))&&c(this).css({left:s.left-n.left-l.left,width:C,height:x});g._helper&&!i.animate&&/static/.test(u.css("position"))&&c(this).css({left:s.left-n.left-l.left,width:C,height:x})}});c.ui.plugin.add("resizable","ghost",{start:function(){var g=c(this).data("resizable"),i=g.options,l=g.size;g.ghost=g.originalElement.clone();
...
{resize:function(){var g=c(this).data("resizable"),i=g.options,l=g.size,n=g.originalSize,u=g.originalPosition,x=g.axis;i.grid=typeof i.grid=="number"?[i.grid,i.grid]:i.grid;var s=Math.round((l.width-n.width)/(i.grid[0]||1))*(i.grid[0]||1);i=Math.round((l.height-n.height)/(i.grid[1]||1))*(i.grid[1]||1);if(/^(se|s|e)$/.test(x)){g.size.width=n.width+s;g.size.height=n.height+i}else if(/^(ne)$/.test(x)){g.size.width=n.width+s;g.size.height=n.height+i;g.position.top=u.top-i}else{if(/^(sw)$/.test(x)){g.size.width=
n.width+s;g.size.height=n.height+i}else{g.size.width=n.width+s;g.size.height=n.height+i;g.position.top=u.top-i}g.position.left=u.left-s}}});var a=function(g){return parseInt(g,10)||0},e=function(g){return!isNaN(parseInt(g,10))}})(jQuery);
...
this.options,i=this.opos[0],l=this.opos[1],n=a.pageX,u=a.pageY;if(i>n){var x=n;n=i;i=x}if(l>u){x=u;u=l;l=x}this.helper.css({left:i,top:l,width:n-i,height:u-l});this.selectees.each(function(){var s=c.data(this,"selectable-item");if(!(!s||s.element==e.element[0])){var C=false;if(g.tolerance=="touch")C=!(s.left>n||s.right<i||s.top>u||s.bottom<l);else if(g.tolerance=="fit")C=s.left>i&&s.right<n&&s.top>l&&s.bottom<u;if(C){if(s.selected){s.$element.removeClass("ui-selected");s.selected=false}if(s.unselecting){s.$element.removeClass("ui-unselecting");
...
toArray:function(a){var e=this._getItemsAsjQuery(a&&a.connected),g=[];a=a||{};e.each(function(){g.push(c(a.item||this).attr(a.attribute||"id")||"")});return g},_intersectsWith:function(a){var e=this.positionAbs.left,g=e+this.helperProportions.width,i=this.positionAbs.top,l=i+this.helperProportions.height,n=a.left,u=n+a.width,x=a.top,s=x+a.height,C=this.offset.click.top,K=this.offset.click.left;return this.options.tolerance=="pointer"||this.options.forcePointerForContainers||this.options.tolerance!=
"pointer"&&this.helperProportions[this.floating?"width":"height"]>a[this.floating?"width":"height"]?i+C>x&&i+C<s&&e+K>n&&e+K<u:n<e+this.helperProportions.width/2&&g-this.helperProportions.width/2<u&&x<i+this.helperProportions.height/2&&l-this.helperProportions.height/2<s},_intersectsWithPointer:function(a){var e=c.ui.isOverAxis(this.positionAbs.top+this.offset.click.top,a.top,a.height);a=c.ui.isOverAxis(this.positionAbs.left+this.offset.click.left,a.left,a.width);e=e&&a;a=this._getDragVerticalDirection();
...
if(i&&a)for(a=i.length-1;a>=0;a--)for(var l=c(i[a]),n=l.length-1;n>=0;n--){var u=c.data(l[n],"sortable");if(u&&u!=this&&!u.options.disabled)g.push([c.isFunction(u.options.items)?u.options.items.call(u.element):c(u.options.items,u.element).not(".ui-sortable-helper").not(".ui-sortable-placeholder"),u])}g.push([c.isFunction(this.options.items)?this.options.items.call(this.element,null,{options:this.options,item:this.currentItem}):c(this.options.items,this.element).not(".ui-sortable-helper").not(".ui-sortable-placeholder"),
...
this]],i=this._connectWith();if(i)for(var l=i.length-1;l>=0;l--)for(var n=c(i[l]),u=n.length-1;u>=0;u--){var x=c.data(n[u],"sortable");if(x&&x!=this&&!x.options.disabled){g.push([c.isFunction(x.options.items)?x.options.items.call(x.element[0],a,{item:this.currentItem}):c(x.options.items,x.element),x]);this.containers.push(x)}}for(l=g.length-1;l>=0;l--){a=g[l][1];i=g[l][0];u=0;for(n=i.length;u<n;u++){x=c(i[u]);x.data("sortable-item",a);e.push({item:x,instance:a,width:0,height:0,left:0,top:0})}}},refreshPositions:function(a){if(this.offsetParent&&
...
(function(c,a){var e,g=/:([\w\d]+)/g,i=/\?([^#]*)$/,l=function(p){return Array.prototype.slice.call(p)},n=function(p){return Object.prototype.toString.call(p)==="[object Function]"},u=function(p){return Object.prototype.toString.call(p)==="[object Array]"},x=function(p){return decodeURIComponent((p||"").replace(/\+/g," "))},s=encodeURIComponent,C=function(p){return String(p).replace(/&(?!\w+;)/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")},K=function(p){return function(t,
...
l(arguments);p.unshift("["+Date()+"]");c.each(ma,function(t,B){B.apply(e,p)})};if(typeof a.console!="undefined")n(a.console.log.apply)?e.addLogger(function(){a.console.log.apply(a.console,arguments)}):e.addLogger(function(){a.console.log(arguments)});else typeof console!="undefined"&&e.addLogger(function(){console.log.apply(console,arguments)});c.extend(e,{makeArray:l,isFunction:n,isArray:u});e.Object=function(p){return c.extend(this,p||{})};c.extend(e.Object.prototype,{escapeHTML:C,h:C,toHash:function(){var p=
...
t).swap()},send:function(){var p=this,t=l(arguments),B=t.shift();if(u(t[0]))t=t[0];return this.then(function(){t.push(function(E){p.next(E)});p.wait();B.apply(B,t);return false})},collect:function(p,t,B){var E=this,Y=function(){if(n(p)){t=p;p=this.content}var pa=[],T=false;c.each(p,function(ga,ha){var xa=t.apply(E,[ga,ha]);if(xa.jquery&&xa.length==1){xa=xa[0];T=true}pa.push(xa);return xa});return T?pa:pa.join("")};return B?Y():this.then(Y)},renderEach:function(p,t,B,E){if(u(t)){E=B;B=t;t=null}return this.load(p).then(function(Y){var pa=
this;B||(B=u(this.previous_content)?this.previous_content:[]);if(E)c.each(B,function(T,ga){var ha={},xa=this.next_engine||p;t?(ha[t]=ga):(ha=ga);E(ga,pa.event_context.interpolate(Y,ha,xa))});else return this.collect(B,function(T,ga){var ha={},xa=this.next_engine||p;t?(ha[t]=ga):(ha=ga);return this.event_context.interpolate(Y,ha,xa)},true)})},interpolate:function(p,t,B){var E=this;return this.then(function(Y,pa){if(!p&&pa)p=pa;if(this.next_engine){t=this.next_engine;this.next_engine=false}var T=E.event_context.interpolate(Y,
...
(function(){if(!window.JSON)window.JSON={};(function(){function c(s){return s<10?"0"+s:s}function a(s){i.lastIndex=0;return i.test(s)?'"'+s.replace(i,function(C){var K=u[C];return typeof K==="string"?K:"\\u"+("0000"+C.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+s+'"'}function e(s,C){var K,ba,ka,ma,p=l,t,B=C[s];if(B&&typeof B==="object"&&typeof B.toJSON==="function")B=B.toJSON(s);if(typeof x==="function")B=x.call(C,s,B);switch(typeof B){case "string":return a(B);case "number":return isFinite(B)?
...
i=/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,l,n,u={"\u0008":"\\b","\t":"\\t","\n":"\\n","\u000c":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},x;if(typeof JSON.stringify!=="function")JSON.stringify=function(s,C,K){var ba;n=l="";if(typeof K==="number")for(ba=0;ba<K;ba+=1)n+=" ";else if(typeof K==="string")n=K;if((x=C)&&typeof C!=="function"&&(typeof C!=="object"||typeof C.length!=="number"))throw new Error("JSON.stringify");
return e("",{"":s})};if(typeof JSON.parse!=="function")JSON.parse=function(s,C){function K(ka,ma){var p,t,B=ka[ma];if(B&&typeof B==="object")for(p in B)if(Object.hasOwnProperty.call(B,p)){t=K(B,p);if(t!==undefined)B[p]=t;else delete B[p]}return C.call(ka,ma,B)}var ba;g.lastIndex=0;if(g.test(s))s=s.replace(g,function(ka){return"\\u"+("0000"+ka.charCodeAt(0).toString(16)).slice(-4)});if(/^[\],:{}\s]*$/.test(s.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,
"]").replace(/(?:^|:|,)(?:\s*\[)+/g,""))){ba=eval("("+s+")");return typeof C==="function"?K({"":ba},""):ba}throw new SyntaxError("JSON.parse");}})();Sammy=Sammy||{};Sammy.JSON=function(c){c.helpers({json:function(a){return typeof a=="string"?JSON.parse(a):JSON.stringify(a)}})}})(jQuery);
...
true)},render_section:function(a,e,g){if(!this.includes("#",a)&&!this.includes("^",a))return a;var i=this;return a.replace(new RegExp(this.otag+"(\\^|\\#)\\s*(.+)\\s*"+this.ctag+"\n*([\\s\\S]+?)"+this.otag+"\\/\\s*\\2\\s*"+this.ctag+"\\s*","mg"),function(l,n,u,x){l=i.find(u,e);if(n=="^")return!l||i.is_array(l)&&l.length===0?i.render(x,e,g,true):"";else if(n=="#")return i.is_array(l)?i.map(l,function(s){return i.render(x,i.create_context(s),g,true)}).join(""):i.is_object(l)?i.render(x,i.create_context(l),
g,true):typeof l==="function"?l.call(e,x,function(s){return i.render(s,e,g,true)}):l?i.render(x,e,g,true):""})},render_tags:function(a,e,g,i){var l=this,n=function(){return new RegExp(l.otag+"(=|!|>|\\{|%)?([^\\/#\\^]+?)\\1?"+l.ctag+"+","g")},u=n(),x=function(C,K,ba){switch(K){case "!":return"";case "=":l.set_delimiters(ba);u=n();return"";case ">":return l.render_partial(ba,e,g);case "{":return l.find(ba,e);default:return l.escape(l.find(ba,e))}};a=a.split("\n");for(var s=0;s<a.length;s++){a[s]=a[s].replace(u,
...
window.Modernizr=function(c,a,e){function g(){u.input=function(Q){for(var V=0,f=Q.length;V<f;V++)pa[Q[V]]=!!(Q[V]in ba);return pa}("autocomplete autofocus list placeholder max min multiple pattern required step".split(" "));u.inputtypes=function(Q){for(var V=0,f,D,N,O=Q.length;V<O;V++){ba.setAttribute("type",D=Q[V]);(f=ba.type!=="text")&&(ba.value=ka,ba.style.cssText="position:absolute;visibility:hidden;",/^range$/.test(D)&&ba.style.WebkitAppearance!==e?(x.appendChild(ba),N=a.defaultView,f=N.getComputedStyle&&
N.getComputedStyle(ba,null).WebkitAppearance!=="textfield"&&ba.offsetHeight!==0,x.removeChild(ba)):/^(search|tel)$/.test(D)||(/^(url|email)$/.test(D)?(f=ba.checkValidity&&ba.checkValidity()===false):/^color$/.test(D)?(x.appendChild(ba),f=ba.value!=ka,x.removeChild(ba)):(f=ba.value!=ka)));Y[Q[V]]=!!f}return Y}("search tel url email datetime date month week time datetime-local number range color".split(" "))}function i(Q,V){var f=Q.charAt(0).toUpperCase()+Q.substr(1);f=(Q+" "+t.join(f+" ")+f).split(" ");
return!!l(f,V)}function l(Q,V){for(var f in Q)if(K[Q[f]]!==e&&(!V||V(Q[f],C)))return true}function n(Q,V){return typeof Q===V}var u={},x=a.documentElement,s=a.head||a.getElementsByTagName("head")[0],C=a.createElement("modernizr"),K=C.style,ba=a.createElement("input"),ka=":)",ma=Object.prototype.toString,p=" -webkit- -moz- -o- -ms- -khtml- ".split(" "),t="Webkit Moz O ms Khtml".split(" "),B={svg:"http://www.w3.org/2000/svg"},E={},Y={},pa={},T=[],ga,ha=function(Q){var V=a.createElement("style"),f=a.createElement("div");
V.textContent=Q+"{#modernizr{height:3px}}";s.appendChild(V);f.id="modernizr";x.appendChild(f);Q=f.offsetHeight===3;V.parentNode.removeChild(V);f.parentNode.removeChild(f);return!!Q},xa=function(){var Q={select:"input",change:"input",submit:"form",reset:"form",error:"img",load:"img",abort:"img"};return function(V,f){f=f||a.createElement(Q[V]||"div");var D=(V="on"+V)in f;D||(f.setAttribute||(f=a.createElement("div")),f.setAttribute&&f.removeAttribute&&(f.setAttribute(V,""),D=n(f[V],"function"),n(f[V],
e)||(f[V]=e),f.removeAttribute(V)));return D}}(),Oa={}.hasOwnProperty,Ra;n(Oa,e)||n(Oa.call,e)?(Ra=function(Q,V){return V in Q&&n(Q.constructor.prototype[V],e)}):(Ra=function(Q,V){return Oa.call(Q,V)});E.flexbox=function(){var Q=a.createElement("div"),V=a.createElement("div");(function(D,N,O,S){N+=":";D.style.cssText=(N+p.join(O+";"+N)).slice(0,-N.length)+(S||"")})(Q,"display","box","width:42px;padding:0;");V.style.cssText=p.join("box-flex:1;")+"width:10px;";Q.appendChild(V);x.appendChild(Q);var f=
V.offsetWidth===42;Q.removeChild(V);x.removeChild(Q);return f};E.canvas=function(){var Q=a.createElement("canvas");return Q.getContext&&Q.getContext("2d")};E.canvastext=function(){return u.canvas&&n(a.createElement("canvas").getContext("2d").fillText,"function")};E.webgl=function(){return!!c.WebGLRenderingContext};E.touch=function(){return"ontouchstart"in c||ha("@media ("+p.join("touch-enabled),(")+"modernizr)")};E.geolocation=function(){return!!navigator.geolocation};E.postmessage=function(){return!!c.postMessage};
...
function(){return!!a.createElementNS&&!!a.createElementNS(B.svg,"svg").createSVGRect};E.inlinesvg=function(){var Q=a.createElement("div");Q.innerHTML="<svg/>";return(Q.firstChild&&Q.firstChild.namespaceURI)==B.svg};E.smil=function(){return!!a.createElementNS&&/SVG/.test(ma.call(a.createElementNS(B.svg,"animate")))};E.svgclippaths=function(){return!!a.createElementNS&&/SVG/.test(ma.call(a.createElementNS(B.svg,"clipPath")))};for(var X in E)Ra(E,X)&&(ga=X.toLowerCase(),u[ga]=E[X](),T.push((u[ga]?"":
"no-")+ga));u.input||g();u.crosswindowmessaging=u.postmessage;u.historymanagement=u.history;u.addTest=function(Q,V){Q=Q.toLowerCase();if(!u[Q]){V=!!V();x.className+=" "+(V?"":"no-")+Q;u[Q]=V;return u}};K.cssText="";C=ba=null;c.attachEvent&&function(){var Q=a.createElement("div");Q.innerHTML="<elem></elem>";return Q.childNodes.length!==1}()&&function(Q,V){function f(ca,ia){for(var sa=-1,ta=ca.length,La,gb=[];++sa<ta;){La=ca[sa];(ia=La.media||ia)!="screen"&&gb.push(f(La.imports,ia),La.cssText)}return gb.join("")}
function D(ca){for(var ia=-1;++ia<O;)ca.createElement(N[ia])}var N="abbr|article|aside|audio|canvas|details|figcaption|figure|footer|header|hgroup|mark|meter|nav|output|progress|section|summary|time|video".split("|"),O=N.length,S=new RegExp("(^|\\s)(abbr|article|aside|audio|canvas|details|figcaption|figure|footer|header|hgroup|mark|meter|nav|output|progress|section|summary|time|video)","gi"),la=new RegExp("<(/*)(abbr|article|aside|audio|canvas|details|figcaption|figure|footer|header|hgroup|mark|meter|nav|output|progress|section|summary|time|video)",
"gi"),ja=new RegExp("(^|[^\\n]*?\\s)(abbr|article|aside|audio|canvas|details|figcaption|figure|footer|header|hgroup|mark|meter|nav|output|progress|section|summary|time|video)([^\\n]*)({[\\n\\w\\W]*?})","gi"),wa=V.createDocumentFragment(),Ga=V.documentElement,ua=Ga.firstChild,Na=V.createElement("body"),I=V.createElement("style"),fa;D(V);D(wa);ua.insertBefore(I,ua.firstChild);I.media="print";Q.attachEvent("onbeforeprint",function(){var ca=-1,ia=f(V.styleSheets,"all"),sa=[],ta;for(fa=fa||V.body;(ta=
...
a);u._enableHTML5=true;u._version="1.7";x.className=x.className.replace(/\bno-js\b/,"")+" js "+T.join(" ");return u}(this,this.document);
...
l){localStorage.removeItem(i.uid);var n=i.uid,u=e();n=jQuery.inArray(n,u);if(n>-1){u.splice(n,1);localStorage.setItem(a,JSON.stringify(u))}l(true)},read:function(i){if(!i)return false;for(var l=c.map(function(){return this.uid}),n=e(),u=[],x,s,C=0,K=n.length;C<K;C++){s=n[C];if(jQuery.inArray(s,l)==-1){x=JSON.parse(localStorage[s]);x=new c(x);x.uid=s;u.push(x)}}i(u)},update:function(i,l){g(i);l(true)}}};Model.Log=function(){window.console&&window.console.log.apply(window.console,arguments)};
...
Model.REST=function(c,a,e){var g=/:([\w\d]+)/g,i=function(){for(var l=[],n;(n=g.exec(a))!==null;)l.push(n[1]);return l}();return jQuery.extend({path:function(l){var n=a;jQuery.each(i,function(u,x){n=n.replace(":"+x,l.attributes[x])});return n},create:function(l,n){return this.xhr("POST",this.create_path(l),l,n)},create_path:function(l){return this.path(l)},destroy:function(l,n){return this.xhr("DELETE",this.destroy_path(l),l,n)},destroy_path:function(l){return this.update_path(l)},params:function(l){var n;
if(l){var u=l.asJSON();delete u[l.constructor.unique_key];n={};n[l.constructor._name.toLowerCase()]=u}else n=null;if(jQuery.ajaxSettings.data)n=jQuery.extend({},jQuery.ajaxSettings.data,n);return n},read:function(l){return this.xhr("GET",this.read_path(),null,function(n,u,x){x=jQuery.makeArray(x);n=[];u=0;for(var s=x.length;u<s;u++)n.push(new c(x[u]));l(n)})},read_path:function(){return a},update:function(l,n){return this.xhr("PUT",this.update_path(l),l,n)},update_path:function(l){return[this.path(l),
l.id()].join("/")},xhr:function(l,n,u,x){var s=this,C=l=="GET"?undefined:this.params(u);return jQuery.ajax({type:l,url:n+".json",dataType:"json",data:C,dataFilter:function(K){return/\S/.test(K)?K:undefined},complete:function(K,ba){s.xhrComplete(K,ba,u,x)}})},xhrComplete:function(l,n,u,x){var s=Model.REST["handle"+l.status];s&&s.call(this,l,n,u);n=n==="success";s=Model.REST.parseResponseData(l);n&&u&&s&&u.attr(s);x&&x.call(u,n,l,s)}},e)};Model.RestPersistence=Model.REST;
...
if(l){var u=l.asJSON();delete u[l.constructor.unique_key];n={};n[l.constructor._name.toLowerCase()]=u}else n=null;if(jQuery.ajaxSettings.data)n=jQuery.extend({},jQuery.ajaxSettings.data,n);return n},read:function(l){return this.xhr("GET",this.read_path(),null,function(n,u,x){x=jQuery.makeArray(x);n=[];u=0;for(var s=x.length;u<s;u++)n.push(new c(x[u]));l(n)})},read_path:function(){return a},update:function(l,n){return this.xhr("PUT",this.update_path(l),l,n)},update_path:function(l){return[this.path(l),
l.id()].join("/")},xhr:function(l,n,u,x){var s=this,C=l=="GET"?undefined:this.params(u);return jQuery.ajax({type:l,url:n+".json",dataType:"json",data:C,complete:function(K,ba){s.xhrComplete(K,ba,u,x)}})},xhrComplete:function(l,n,u,x){var s=Model.REST["handle"+l.status];s&&s.call(this,l,n,u);n=n==="success";s=Model.REST.parseResponseData(l);n&&u&&s&&u.attr(s);x&&x.call(u,n,l,s)}},e)};
var CodeMirror=function(){function c(f,D){function N(d){Za=null;var j={line:0,ch:0};la(j,{line:W.length-1,ch:W[W.length-1].text.length},X(d),j,j);Za=new n}function O(){if(oa.readOnly!="nocursor"){!Wa&&oa.onFocus&&oa.onFocus($a);Wa=true;sa();if(va.className.search(/\bCodeMirror-focused\b/)==-1)va.className+=" CodeMirror-focused";dc()}}function S(){Wa&&oa.onBlur&&oa.onBlur($a);clearInterval(bc);hb=null;Wa=false;va.className=va.className.replace(" CodeMirror-focused","")}function la(d,j,k,m,o){if(Za){for(var q=
[],v=d.line,w=j.line+1;v<w;++v)q.push(W[v].text);for(Za.addChange(d.line,k.length,q);Za.done.length>oa.undoDepth;)Za.done.shift()}ua(d,j,k,m,o)}function ja(d,j){var k=d.pop();if(k){for(var m=[],o=k.start+k.added,q=k.start;q<o;++q)m.push(W[q].text);j.push({start:k.start,added:k.old.length,old:m});q=k.start+k.old.length-1;m=m[m.length-1];var v=k.old[k.old.length-1];if(v)if(m){for(var w=m.length,A=v.length;w>=0&&A>=0;--w,--A)if(m.charAt(w)!=v.charAt(A))break;m=A+1}else m=v.length;else m=m?m.length:0;
q=Ka({line:q,ch:m});ua({line:k.start,ch:0},{line:o-1,ch:W[o-1].text.length},k.old,q,q)}}function wa(){ja(Za.done,Za.undone)}function Ga(){ja(Za.undone,Za.done)}function ua(d,j,k,m,o){for(var q=false,v=Eb.length,w=d.line;w<j.line;++w)if(W[w].text.length==v){q=true;break}var A=j.line-d.line;w=W[d.line];var F=W[j.line];if(w==F)if(k.length==1)w.replace(d.ch,j.ch,k[0]);else{F=w.split(j.ch,k[k.length-1]);var R=[d.line+1,A];w.replace(d.ch,w.text.length,k[0]);w=1;for(var da=k.length-1;w<da;++w)R.push(new i(k[w]));
R.push(F);W.splice.apply(W,R)}else if(k.length==1){w.replace(d.ch,w.text.length,k[0]+F.text.slice(j.ch));W.splice(d.line+1,A)}else{R=[d.line+1,A-1];w.replace(d.ch,w.text.length,k[0]);F.replace(0,j.ch,k[k.length-1]);w=1;for(da=k.length-1;w<da;++w)R.push(new i(k[w]));W.splice.apply(W,R)}w=d.line;for(da=w+k.length;w<da;++w){F=W[w].text;if(F.length>v){Eb=F;v=F.length;q=false}}if(q){w=v=0;for(da=W.length;w<da;++w){F=W[w].text;if(F.length>v){v=F.length;Eb=F}}}q=[];A=k.length-A-1;w=0;for(F=cb.length;w<F;++w){v=
cb[w];if(v<d.line)q.push(v);else v>j.line&&q.push(v+A)}k.length&&q.push(d.line);cb=q;Kb(100);Ua.push({from:d.line,to:j.line+1,diff:A});pb={from:d,to:j,text:k};xb(m,o,P.from.line<=Math.min(j.line,j.line+A)?P.from.line:P.from.line+A,P.to.line<=Math.min(j.line,j.line+A)?P.to.line:P.to.line+A);Ab.style.height=W.length*ab()+2*Ya.offsetTop+"px"}function Na(d,j,k){function m(q){if(p(q,j))return q;if(!p(k,q))return o;var v=q.line+d.length-(k.line-j.line)-1,w=q.ch;if(q.line==k.line)w+=d[d.length-1].length-
(k.ch-(k.line==j.line?j.ch:0));return{line:v,ch:w}}j=Ka(j);k=k?Ka(k):j;d=X(d);var o;fa(d,j,k,function(q){o=q;return{from:m(P.from),to:m(P.to)}});return o}function I(d,j){fa(X(d),P.from,P.to,function(k){return j=="end"?{from:k,to:k}:j=="start"?{from:P.from,to:P.from}:{from:P.from,to:k}})}function fa(d,j,k,m){m=m({line:j.line+d.length-1,ch:d.length==1?d[0].length+j.ch:d[d.length-1].length});la(j,k,d,m.from,m.to)}function ca(d,j){var k=d.line,m=j.line;if(k==m)return W[k].text.slice(d.ch,j.ch);var o=
...
ch:Ba-r};++Ca;r=y+1}}if(!Xa){var j=false,k=Ea.value,m=Q(Ea);if(!m)return false;j=Pa.text!=k;var o=ub,q=j||m.start!=Pa.start||m.end!=(o?Pa.start:Pa.end);if(!q&&!o)return false;if(j){hb=ub=null;if(oa.readOnly){bb=true;return"changed"}}var v=d(m.start,Pa.from),w=d(m.end,Pa.from);if(o){v=m.start==o.anchor?w:v;w=hb?P.to:m.start==o.anchor?v:w;if(!p(v,w)){ub=null;P.inverted=false;o=v;v=w;w=o}}if(v.line==w.line&&v.line==P.from.line&&v.line==P.to.line&&!hb)bb=false;if(j){o=0;for(var A=k.length,F=Math.min(A,
Pa.text.length),R,da=Pa.from,na=-1;o<F&&(R=k.charAt(o))==Pa.text.charAt(o);){++o;if(R=="\n"){da++;na=o}}F=na>-1?o-na:o;for(var Ia=Pa.to-1,za=Pa.text.length;;){R=Pa.text.charAt(za);R=="\n"&&Ia--;if(k.charAt(A)!=R){++A;++za;break}if(za<=o||A<=o)break;--A;--za}na=Pa.text.lastIndexOf("\n",za-1);la({line:da,ch:F},{line:Ia,ch:na==-1?za:za-na-1},X(k.slice(o,A)),v,w);if(da!=Ia||v.line!=da)bb=true}else xb(v,w);Pa.text=k;Pa.start=m.start;Pa.end=m.end;return j?"changed":q?"moved":false}}function gb(){for(var d=
[],j=Math.max(0,P.from.line-1),k=Math.min(W.length,P.to.line+2),m=j;m<k;++m)d.push(W[m].text);d=Ea.value=d.join(ga);var o=P.from.ch,q=P.to.ch;for(m=j;m<P.from.line;++m)o+=ga.length+W[m].text.length;for(m=j;m<P.to.line;++m)q+=ga.length+W[m].text.length;Pa={text:d,from:j,to:k,start:o,end:q};V(Ea,o,ub?o:q)}function kb(){oa.readOnly!="nocursor"&&Ea.focus()}function Fb(){var d=Wb(P.inverted?P.from:P.to);return Rb(d.x,d.y,d.x,d.yBot)}function Rb(d,j,k,m){var o=Ya.offsetLeft,q=Ya.offsetTop,v=ab();j+=q;m+=
q;d+=o;k+=o;var w=va.clientHeight,A=va.scrollTop;o=false;q=true;if(j<A){va.scrollTop=Math.max(0,j-2*v);o=true}else if(m>A+w){va.scrollTop=m+v-w;o=true}j=va.clientWidth;m=va.scrollLeft;if(d<m){va.scrollLeft=Math.max(0,d-10);o=true}else if(k>j+m){va.scrollLeft=k+10-j;o=true;if(k>Ab.clientWidth)q=false}o&&oa.onScroll&&oa.onScroll($a);return q}function Sb(){var d=ab(),j=va.scrollTop-Ya.offsetTop;return{from:Math.min(W.length,Math.max(0,Math.floor(j/d))),to:Math.min(W.length,Math.ceil((j+va.clientHeight)/
d))}}function lb(d){if(va.clientWidth){for(var j=d===true?[]:[{from:Ha,to:ib,domStart:0}],k=0,m=d.length||0;k<m;++k){for(var o=d[k],q=[],v=o.diff||0,w=0,A=j.length;w<A;++w){var F=j[w];if(o.to<=F.from)q.push({from:F.from+v,to:F.to+v,domStart:F.domStart});else if(F.to<=o.from)q.push(F);else{o.from>F.from&&q.push({from:F.from,to:o.from,domStart:F.domStart});o.to<F.to&&q.push({from:o.to+v,to:F.to+v,domStart:F.domStart+(o.to-F.from)})}}j=q}v=Sb();o=Math.min(Ha,Math.max(v.from-3,0));q=Math.min(W.length,
Math.max(ib,v.to+3));d=[];w=0;A=ib-Ha;var R=o,da=0;k=0;for(m=j.length;k<m;++k){F=j[k];if(!(F.to<=o)){if(F.from>=q)break;if(F.domStart>w||F.from>R){d.push({from:R,to:F.from,domSize:F.domStart-w,domStart:w});da+=F.from-R}R=F.to;w=F.domStart+(F.to-F.from)}}if(w!=A||R!=q){da+=Math.abs(q-R);d.push({from:R,to:q,domSize:A-w,domStart:w})}if(d.length){Qa.style.display="none";if(da>(v.to-v.from)*0.3){F=o=Math.max(v.from-10,0);j=q=Math.min(v.to+7,W.length);k=[];m={line:F,ch:0};m=p(P.from,m)&&!p(P.to,m);for(F=
F;F<j;++F){w=v=null;if(m){v=0;if(P.to.line==F){m=false;w=P.to.ch}}else if(P.from.line==F)if(P.to.line==F){v=P.from.ch;w=P.to.ch}else{m=true;v=P.from.ch}k.push(W[F].getHTML(v,w,true))}Qa.innerHTML=k.join("")}else{j=P.from.line;k=P.to.line;m=0;F=pa&&mb.createElement("div");v=0;for(w=d.length;v<w;++v){A=d[v];R=A.to-A.from-A.domSize;da=Qa.childNodes[A.domStart+A.domSize+m]||null;if(pa)for(var na=Math.max(-R,A.domSize);na>0;--na)Qa.removeChild(da?da.previousSibling:Qa.lastChild);else if(R){for(na=Math.max(0,
...
Math.min(d,W.length-1))}function Ka(d){if(d.line<0)return{line:0,ch:0};if(d.line>=W.length)return{line:W.length-1,ch:W[W.length-1].text.length};var j=d.ch,k=W[d.line].text.length;return j==null||j>k?{line:d.line,ch:k}:j<0?{line:d.line,ch:0}:d}function lc(d){rb({line:d,ch:0},{line:d,ch:W[d].text.length})}function sb(d,j){if(j=="smart")if(Ta.indent)var k=hc(d);else j="prev";var m=W[d],o=m.indentation(),q=m.text.match(/^\s*/)[0],v;if(j=="prev")v=d?W[d-1].indentation():0;else if(j=="smart")v=Ta.indent(k,
m.text.slice(q.length));else if(j=="add")v=o+oa.indentUnit;else if(j=="subtract")v=o-oa.indentUnit;v=Math.max(0,v);if(v-o){o="";k=0;if(oa.indentWithTabs)for(m=Math.floor(v/ha);m;--m){k+=ha;o+="\t"}for(;k<v;){++k;o+=" "}}else{if(P.from.line!=d&&P.to.line!=d)return;o=q}Na(o,{line:d,ch:0},{line:d,ch:q.length})}function ec(){Ta=c.getMode(oa,oa.mode);for(var d=0,j=W.length;d<j;++d)W[d].stateAfter=null;cb=[0];Kb()}function Tb(d,j,k){function m(w,A,F,R){w=W[w];A=w.addMark(A,F,R);A.line=w;o.push(A)}d=Ka(d);
j=Ka(j);var o=[];if(d.line==j.line)m(d.line,d.ch,j.ch,k);else{m(d.line,d.ch,null,k);for(var q=d.line+1,v=j.line;q<v;++q)m(q,0,null,k);m(j.line,0,j.ch,k)}Ua.push({from:d.line,to:j.line+1});return function(){for(var w,A,F=0;F<o.length;++F){var R=o[F],da=B(W,R.line);R.line.removeMark(R);if(da>-1){if(w==null)w=da;A=da}}w!=null&&Ua.push({from:w,to:A+1})}}function Jb(d){eb.innerHTML="<pre><span>x</span></pre>";eb.firstChild.firstChild.firstChild.nodeValue=d;return eb.firstChild.firstChild.offsetWidth||
10}function Ub(d,j){if(j==0)return 0;eb.innerHTML="<pre><span>"+W[d].getHTML(null,null,false,j)+"</span></pre>";return eb.firstChild.firstChild.offsetWidth}function Vb(d,j){function k(R){eb.innerHTML="<pre><span>"+m.getHTML(null,null,false,R)+"</span></pre>";return eb.firstChild.firstChild.offsetWidth}if(j<=0)return 0;for(var m=W[d],o=0,q=0,v=m.text.length,w,A=Math.min(v,Math.ceil(j/Jb("x")));;){var F=k(A);if(F<=j&&A<v)A=Math.min(v,Math.ceil(A*1.2));else{w=F;v=A;break}}if(j>w)return v;A=Math.floor(v*
0.8);F=k(A);if(F<j){o=A;q=F}for(;;){if(v-o<=1)return w-j>j-q?o:v;A=Math.ceil((o+v)/2);F=k(A);if(F>j){v=A;w=F}else{o=A;q=F}}}function Wb(d,j){var k=ab(),m=d.line-(j?Ha:0);return{x:Ub(d.line,d.ch),y:m*k,yBot:(m+1)*k}}function fc(d){d=Wb(d,true);var j=ka(Ya);return{x:j.left+d.x,y:j.top+d.y,yBot:j.top+d.yBot}}function ab(){var d=Qa.childNodes.length;if(d)return Qa.offsetHeight/d||1;eb.innerHTML="<pre>x</pre>";return eb.firstChild.offsetHeight||1}function tb(d,j){var k=ka(va,true),m=d.e.clientX,o=d.e.clientY;
...
var o=Ea.value=ia();kb();V(Ea,0,Ea.value.length);Xa=true;if(T){d.stop();var q=C(window,"mouseup",function(){q();setTimeout(j,20)},true)}else setTimeout(j,50)}}function dc(){clearInterval(bc);var d=true;ob.style.visibility="";bc=setInterval(function(){ob.style.visibility=(d=!d)?"":"hidden"},650)}function Va(d){function j(Ca,r,y){if(Ca.text){var H=Ca.styles;Ca=v?0:Ca.text.length-1;for(var J,G=v?0:H.length-2,L=v?H.length:-2;G!=L;G+=2*w){var U=H[G];if(H[G+1]!=null&&H[G+1]!=da)Ca+=w*U.length;else for(var aa=
v?0:U.length-1,ra=v?U.length:-1;aa!=ra;aa+=w,Ca+=w)if(Ca>=r&&Ca<y&&Ia.test(J=U.charAt(aa))){var Ma=b[J];if(Ma.charAt(1)==">"==v)na.push(J);else if(na.pop()!=Ma.charAt(0))return{pos:Ca,match:false};else if(!na.length)return{pos:Ca,match:true}}}}}var k=P.inverted?P.from:P.to,m=W[k.line],o=k.ch-1,q=o>=0&&b[m.text.charAt(o)]||b[m.text.charAt(++o)];if(q){q.charAt(0);var v=q.charAt(1)==">",w=v?1:-1,A=m.styles,F=o+1;q=0;for(var R=A.length;q<R;q+=2)if((F-=A[q].length)<=0){var da=A[q+1];break}var na=[m.text.charAt(o)],
Ia=/[(){}[\]]/;q=k.line;for(R=v?Math.min(q+50,W.length):Math.max(-1,q-50);q!=R;q+=w){m=W[q];A=q==k.line;if(m=j(m,A&&v?o+1:0,A&&!v?o:m.text.length)){da=m.match?"CodeMirror-matchingbracket":"CodeMirror-nonmatchingbracket";var za=Tb({line:k.line,ch:o},{line:k.line,ch:o+1},da),Ba=Tb({line:q,ch:m.pos},{line:q,ch:m.pos+1},da);k=ya(function(){za();Ba()});if(d)setTimeout(k,800);else qb=k;break}}}}function Xb(d){for(var j,k,m=d;m>d-40;--m){if(m==0)return 0;var o=W[m-1];if(o.stateAfter)return m;o=o.indentation();
if(k==null||j>o){k=m;j=o}}return k}function hc(d){var j=Xb(d),k=j&&W[j-1].stateAfter;k=k?a(Ta,k):e(Ta);for(j=j;j<d;++j){var m=W[j];m.highlight(Ta,k);m.stateAfter=a(Ta,k)}W[d].stateAfter||cb.push(d);return k}function mc(){for(var d=+new Date+oa.workTime,j=false;cb.length;){var k=W[Ha].stateAfter?cb.pop():Ha;if(!(k>=W.length)){j=true;var m=Xb(k),o=m&&W[m-1].stateAfter;o=o?a(Ta,o):e(Ta);m=m;for(var q=W.length;m<q;++m){var v=W[m],w=v.stateAfter;if(+new Date>d){cb.push(m);Kb(oa.workDelay);Ua.push({from:k,
to:m});return}var A=v.highlight(Ta,o);v.stateAfter=a(Ta,o);if(w&&!A&&v.text)break}Ua.push({from:k,to:m})}}j&&oa.onHighlightComplete&&oa.onHighlightComplete($a)}function Kb(d){cb.length&&cc.set(d,ya(mc))}function Yb(){bb=null;Ua=[];pb=db=false}function Zb(){var d=false;if(db)d=!Fb();if(Ua.length)lb(Ua);else db&&Hb();d&&Fb();db&&dc();if(!Xa&&(bb===true||bb!==false&&db))gb();db&&oa.matchBrackets&&setTimeout(ya(function(){if(qb){qb();qb=null}Va(false)}),20);d=pb;db&&oa.onCursorActivity&&oa.onCursorActivity($a);
d&&oa.onChange&&$a&&oa.onChange($a,d)}function ya(d){return function(){h++||Yb();try{var j=d.apply(this,arguments)}finally{--h||Zb()}return j}}function ic(d,j,k){this.atOccurrence=false;if(k==null)k=typeof d=="string"&&d==d.toLowerCase();j=j&&typeof j=="object"?Ka(j):{line:0,ch:0};this.pos={from:j,to:j};if(typeof d!="string")this.matches=function(q,v){if(q)for(var w=W[v.line].text.slice(0,v.ch),A=w.match(d),F=0;A;){var R=w.indexOf(A[0]);F+=R;w=w.slice(R+1);if(R=w.match(d))A=R;else break;F++}else{w=
W[v.line].text.slice(v.ch);F=(A=w.match(d))&&v.ch+w.indexOf(A[0])}if(A)return{from:{line:v.line,ch:F},to:{line:v.line,ch:F+A[0].length},match:A}};else{if(k)d=d.toLowerCase();var m=k?function(q){return q.toLowerCase()}:function(q){return q},o=d.split("\n");this.matches=o.length==1?function(q,v){var w=m(W[v.line].text),A=d.length,F;if(q?v.ch>=A&&(F=w.lastIndexOf(d,v.ch-A))!=-1:(F=w.indexOf(d,v.ch))!=-1)return{from:{line:v.line,ch:F},to:{line:v.line,ch:F+A}}}:function(q,v){var w=v.line,A=q?o.length-
1:0,F=o[A],R=m(W[w].text),da=q?R.indexOf(F)+F.length:R.lastIndexOf(F);if(!(q?da>=v.ch||da!=F.length:da<=v.ch||da!=R.length-F.length))for(;;){if(q?!w:w==W.length-1)return;R=m(W[w+=q?-1:1].text);F=o[q?--A:++A];if(A>0&&A<o.length-1)if(R!=F)return;else continue;A=q?R.lastIndexOf(F):R.indexOf(F)+F.length;if(q?A!=R.length-F.length:A!=F.length)return;F={line:v.line,ch:da};w={line:w,ch:A};return{from:q?w:F,to:q?F:w}}}}}var oa={},Qb=c.defaults;for(var fb in Qb)if(Qb.hasOwnProperty(fb))oa[fb]=(D&&D.hasOwnProperty(fb)?
D:Qb)[fb];var mb=oa.document,va=mb.createElement("div");va.className="CodeMirror";if(window.ActiveXObject&&/MSIE [1-7]\b/.test(navigator.userAgent))va.style.position="relative";va.innerHTML='<div style="position: relative"><div style="position: absolute; height: 0; width: 0; overflow: hidden;"></div><div style="position: relative"><div class="CodeMirror-gutter"><div class="CodeMirror-gutter-text"></div></div><div style="overflow: hidden; position: absolute; width: 1px; height: 0; left: 0"><textarea style="position: absolute; width: 100000px;" wrap="off"></textarea></div><div class="CodeMirror-lines"><div style="position: relative"><pre class="CodeMirror-cursor">&#160;</pre><div></div></div></div></div></div>';
...
Eb="";ya(function(){N(oa.value||"");bb=false})();setTimeout(gb,20);C(va,"mousedown",ya(function(d){function j(F){var R=tb(F,true);if(R&&!ma(R,q)){Wa||O();q=R;rb(o,R);bb=false;var da=Sb();if(R.line>=da.to||R.line<da.from)v=setTimeout(ya(function(){j(F)}),150)}}var k=Db;Db=null;for(var m=d.target();m!=va;m=m.parentNode)if(m.parentNode==Mb){oa.onGutterClick&&oa.onGutterClick($a,B(Mb.childNodes,m)+Ha);return d.stop()}T&&d.button()==3&&gc(d);if(d.button()==1){var o=tb(d),q=o,v;if(o){Wa||O();d.stop();if(k&&
+new Date-k<400)return lc(o.line);yb(o.line,o.ch,true);var w=C(mb,"mousemove",ya(function(F){clearTimeout(v);F.stop();j(F)}),true),A=C(mb,"mouseup",ya(function(F){clearTimeout(v);var R=tb(F);R&&rb(o,R);F.stop();kb();bb=true;w();A()}),true)}else d.target()==va&&d.stop()}}));T||C(va,"contextmenu",ya(gc));C(Ab,"dblclick",ya(function(d){var j=tb(d);if(j){for(var k=W[j.line].text,m=j.ch,o=j.ch;m>0&&/\w/.test(k.charAt(m-1));)--m;for(;o<k.length&&/\w/.test(k.charAt(o));)++o;rb({line:j.line,ch:m},{line:j.line,
ch:o});d.stop();Db=+new Date}}));C(va,"scroll",function(){lb([]);oa.onScroll&&oa.onScroll($a)});C(window,"resize",function(){lb(true)});C(Ea,"keyup",ya(function(d){if(!(oa.onKeyEvent&&oa.onKeyEvent($a,x(d.e)))){if(ub){ub=null;bb=true}if(d.e.keyCode==16)hb=null}}));C(Ea,"keydown",ya(function(d){Wa||O();var j=d.e.keyCode,k=(xa?d.e.metaKey:d.e.ctrlKey)&&!d.e.altKey,m=d.e.ctrlKey||d.e.altKey||d.e.metaKey;hb=j==16||d.e.shiftKey?hb||(P.inverted?P.to:P.from):null;if(!(oa.onKeyEvent&&oa.onKeyEvent($a,x(d.e)))){if(j==
...
q;++o)sb(o,"smart");break;case "classic":if(ma(P.from,P.to)){m?sb(P.from.line,"smart"):I("\t","end");break}case "shift":o=P.from.line;for(q=P.to.line;o<=q;++o)sb(o,m?"subtract":"add");break}m=true}m=m}if(m)return d.stop();if(k&&j==90){wa();return d.stop()}if(k&&(d.e.shiftKey&&j==90||j==89)){Ga();return d.stop()}}jb=(k?"c":"")+j;if(P.inverted&&Oa.hasOwnProperty(jb))if(d=Q(Ea)){ub={anchor:d.start};V(Ea,d.start,d.start)}ta(jb)}}));C(Ea,"keypress",ya(function(d){if(!(oa.onKeyEvent&&oa.onKeyEvent($a,x(d.e)))){if(oa.electricChars&&
Ta.electricChars){var j=String.fromCharCode(d.e.charCode==null?d.e.keyCode:d.e.charCode);Ta.electricChars.indexOf(j)>-1&&setTimeout(ya(function(){sb(P.to.line,"smart")}),50)}j=d.e.keyCode;if(j==13){if(!oa.readOnly){I("\n","end");if(oa.enterMode!="flat")sb(P.from.line,oa.enterMode=="keep"?"prev":"smart")}d.stop()}else!d.e.ctrlKey&&!d.e.altKey&&!d.e.metaKey&&j==9&&oa.tabMode!="default"?d.stop():ta(jb)}}));C(Ea,"focus",O);C(Ea,"blur",S);C(va,"dragenter",function(d){d.stop()});C(va,"dragover",function(d){d.stop()});
C(va,"drop",ya(function(d){var j=tb(d,true),k=d.e.dataTransfer.files;if(!(!j||oa.readOnly))if(k&&k.length&&window.FileReader&&window.File){var m=k.length,o=Array(m),q=0;for(d=0;d<m;++d)v(k[d],d);var v=function(A,F){var R=new FileReader;R.onload=function(){o[F]=R.result;++q==m&&Na(o.join(""),Ka(j),Ka(j))};R.readAsText(A)}}else try{(o=d.e.dataTransfer.getData("Text"))&&Na(o,j,j)}catch(w){}}));C(va,"paste",function(){kb();ta()});C(Ea,"paste",function(){ta()});C(Ea,"cut",function(){ta()});mb.activeElement==
...
true;return this.pos.match||true}if(d){if(!m.line)return j(0);m={line:m.line-1,ch:W[m.line-1].text.length}}else{if(m.line==W.length-1)return j(W.length);m={line:m.line+1,ch:0}}}},from:function(){if(this.atOccurrence)return{line:this.pos.from.line,ch:this.pos.from.ch}},to:function(){if(this.atOccurrence)return{line:this.pos.to.line,ch:this.pos.to.ch}}};return $a}function a(f,D){if(D===true)return D;if(f.copyState)return f.copyState(D);var N={};for(var O in D){var S=D[O];if(S instanceof Array)S=S.concat([]);
N[O]=S}return N}function e(f,D,N){return f.startState?f.startState(D,N):true}function g(f){this.pos=this.start=0;this.string=f}function i(f,D){this.styles=D||[f,null];this.stateAfter=null;this.text=f;this.marked=this.gutterMarker=this.className=null}function l(f,D,N,O){for(var S=0,la=0,ja=0;la<D;S+=2){var wa=N[S],Ga=la+wa.length;if(ja==0){Ga>f&&O.push(wa.slice(f-la,Math.min(wa.length,D-la)),N[S+1]);if(Ga>=f)ja=1}else if(ja==1)Ga>D?O.push(wa.slice(0,D-la),N[S+1]):O.push(wa,N[S+1]);la=Ga}}function n(){this.time=
0;this.done=[];this.undone=[]}function u(){if(this.preventDefault){this.preventDefault();this.stopPropagation()}else{this.returnValue=false;this.cancelBubble=true}}function x(f){if(!f.stop)f.stop=u;return f}function s(f){this.e=f}function C(f,D,N,O){function S(la){N(new s(la||window.event))}if(typeof f.addEventListener=="function"){f.addEventListener(D,S,false);if(O)return function(){f.removeEventListener(D,S,false)}}else{f.attachEvent("on"+D,S);if(O)return function(){f.detachEvent("on"+D,S)}}}function K(){this.id=
null}function ba(f,D){if(D==null){D=f.search(/[^\s\u00a0]/);if(D==-1)D=f.length}for(var N=0,O=0;N<D;++N)if(f.charAt(N)=="\t")O+=ha-O%ha;else++O;return O}function ka(f,D){for(var N=f.ownerDocument.body,O=0,S=0,la=false,ja=f;ja;ja=ja.offsetParent){O+=ja.offsetLeft;S+=ja.offsetTop;if(ja==N)la=true}for(ja=f.parentNode;ja!=(D&&la?null:N);ja=ja.parentNode)if(ja.scrollLeft!=null){O-=ja.scrollLeft;S-=ja.scrollTop}return{left:O,top:S}}function ma(f,D){return f.line==D.line&&f.ch==D.ch}function p(f,D){return f.line<
D.line||f.line==D.line&&f.ch<D.ch}function t(f){return f.replace(/[<>&]/g,function(D){return D=="&"?"&amp;":D=="<"?"&lt;":"&gt;"})}function B(f,D){if(f.indexOf)return f.indexOf(D);for(var N=0,O=f.length;N<O;++N)if(f[N]==D)return N;return-1}c.defaults={value:"",mode:null,indentUnit:2,indentWithTabs:false,tabMode:"classic",enterMode:"indent",electricChars:true,onKeyEvent:null,lineNumbers:false,gutter:false,firstLineNumber:1,readOnly:false,onChange:null,onCursorActivity:null,onGutterClick:null,onHighlightComplete:null,
onFocus:null,onBlur:null,onScroll:null,matchBrackets:false,workTime:100,workDelay:200,undoDepth:40,tabindex:null,document:window.document};var E={},Y={};c.defineMode=function(f,D){if(!c.defaults.mode&&f!="null")c.defaults.mode=f;E[f]=D};c.defineMIME=function(f,D){Y[f]=D};c.getMode=function(f,D){if(typeof D=="string"&&Y.hasOwnProperty(D))D=Y[D];if(typeof D=="string")var N=D,O={};else if(D!=null){N=D.name;O=D}var S=E[N];if(!S){window.console&&console.warn("No mode "+N+" found, falling back to plain text.");
return c.getMode(f,"text/plain")}return S(f,O||{})};c.listModes=function(){var f=[];for(var D in E)E.propertyIsEnumerable(D)&&f.push(D);return f};c.listMIMEs=function(){var f=[];for(var D in Y)Y.propertyIsEnumerable(D)&&f.push(D);return f};c.fromTextArea=function(f,D){function N(){f.value=ja.getValue()}D||(D={});D.value=f.value;if(!D.tabindex&&f.tabindex)D.tabindex=f.tabindex;if(f.form){var O=C(f.form,"submit",N,true);if(typeof f.form.submit=="function"){var S=f.form.submit,la=function(){N();f.form.submit=
S;f.form.submit();f.form.submit=la};f.form.submit=la}}f.style.display="none";var ja=c(function(wa){f.parentNode.insertBefore(wa,f.nextSibling)},D);ja.save=N;ja.toTextArea=function(){N();f.parentNode.removeChild(ja.getWrapperElement());f.style.display="";if(f.form){O();if(typeof f.form.submit=="function")f.form.submit=S}};return ja};c.startState=e;c.copyState=a;g.prototype={eol:function(){return this.pos>=this.string.length},sol:function(){return this.pos==0},peek:function(){return this.string.charAt(this.pos)},
next:function(){if(this.pos<this.string.length)return this.string.charAt(this.pos++)},eat:function(f){var D=this.string.charAt(this.pos);if(typeof f=="string"?D==f:D&&(f.test?f.test(D):f(D))){++this.pos;return D}},eatWhile:function(f){for(var D=this.start;this.eat(f););return this.pos>D},eatSpace:function(){for(var f=this.pos;/[\s\u00a0]/.test(this.string.charAt(this.pos));)++this.pos;return this.pos>f},skipToEnd:function(){this.pos=this.string.length},skipTo:function(f){f=this.string.indexOf(f,this.pos);
if(f>-1){this.pos=f;return true}},backUp:function(f){this.pos-=f},column:function(){return ba(this.string,this.start)},indentation:function(){return ba(this.string)},match:function(f,D,N){if(typeof f=="string"){if((N?this.string.toLowerCase():this.string).indexOf(N?f.toLowerCase():f,this.pos)==this.pos){if(D!==false)this.pos+=f.length;return true}}else{if((f=this.string.slice(this.pos).match(f))&&D!==false)this.pos+=f[0].length;return f}},current:function(){return this.string.slice(this.start,this.pos)}};
i.prototype={replace:function(f,D,N){var O=[],S=this.marked;l(0,f,this.styles,O);N&&O.push(N,null);l(D,this.text.length,this.styles,O);this.styles=O;this.text=this.text.slice(0,f)+N+this.text.slice(D);this.stateAfter=null;if(S){f=N.length-(D-f);N=this.text.length;for(O=0;O<S.length;++O){var la=S[O],ja=false;if(la.from>=N)ja=true;else{la.from=la.from<=Math.min(D,D+f)?la.from:la.from+f;if(la.to!=null)la.to=la.to<=Math.min(D,D+f)?la.to:la.to+f}if(ja||la.from>=la.to){S.splice(O,1);O--}}}},split:function(f,
D){var N=[D,null];l(f,this.text.length,this.styles,N);return new i(D+this.text.slice(f),N)},addMark:function(f,D,N){f={from:f,to:D,style:N};if(this.marked==null)this.marked=[];this.marked.push(f);this.marked.sort(function(O,S){return O.from-S.from});return f},removeMark:function(f){var D=this.marked;if(D)for(var N=0;N<D.length;++N)if(D[N]==f){D.splice(N,1);break}},highlight:function(f,D){var N=new g(this.text),O=this.styles,S=0,la=false,ja=O[0],wa;for(this.text==""&&f.blankLine&&f.blankLine(D);!N.eol();){var Ga=
f.token(N,D),ua=this.text.slice(N.start,N.pos);N.start=N.pos;if(S&&O[S-1]==Ga)O[S-2]+=ua;else if(ua){if(!la&&(O[S+1]!=Ga||S&&O[S-2]!=wa))la=true;O[S++]=ua;O[S++]=Ga;wa=ja;ja=O[S]}if(N.pos>5E3){O[S++]=this.text.slice(N.pos);O[S++]=null;break}}if(O.length!=S){O.length=S;la=true}if(S&&O[S-2]!=wa)la=true;return la},getTokenAt:function(f,D,N){for(var O=new g(this.text);O.pos<N&&!O.eol();){O.start=O.pos;var S=f.token(O,D)}return{start:O.start,end:O.pos,string:O.current(),className:S||null,state:D}},indentation:function(){return ba(this.text)},
getHTML:function(f,D,N,O){function S(kb,Fb){if(kb)Fb?la.push('<span class="',Fb,'">',t(kb),"</span>"):la.push(t(kb))}var la=[];if(N)la.push(this.className?'<pre class="'+this.className+'">':"<pre>");var ja=this.styles,wa=this.text,Ga=this.marked;if(f==D)f=null;var ua=wa.length;if(O!=null)ua=Math.min(O,ua);if(!wa&&O==null)S(" ",f!=null&&D==null?"CodeMirror-selected":null);else if(!Ga&&f==null)for(f=O=0;f<ua;O+=2){D=ja[O];var Na=D.length;if(f+Na>ua)D=D.slice(0,ua-f);f+=Na;S(D,ja[O+1])}else{O=wa=0;var I=
"",fa=-1,ca=null,ia=function(){if(Ga){fa+=1;ca=fa<Ga.length?Ga[fa]:null}};for(ia();wa<ua;){var sa=ua,ta="";if(f!=null)if(f>wa)sa=f;else if(D==null||D>wa){ta=" CodeMirror-selected";if(D!=null)sa=Math.min(sa,D)}for(;ca&&ca.to!=null&&ca.to<=wa;)ia();if(ca)if(ca.from>wa)sa=Math.min(sa,ca.from);else{ta+=" "+ca.style;if(ca.to!=null)sa=Math.min(sa,ca.to)}for(;;){var La=wa+I.length,gb=Na;if(ta)gb=Na?Na+ta:ta;S(La>sa?I.slice(0,sa-wa):I,gb);if(La>=sa){I=I.slice(sa-wa);wa=sa;break}wa=La;I=ja[O++];Na=ja[O++]}}f!=
null&&D==null&&S(" ","CodeMirror-selected")}N&&la.push("</pre>");return la.join("")}};n.prototype={addChange:function(f,D,N){this.undone.length=0;var O=+new Date,S=this.done[this.done.length-1];if(O-this.time>400||!S||S.start>f+D||S.start+S.added<f-S.added+S.old.length)this.done.push({start:f,added:D,old:N});else{var la=0;if(f<S.start){for(var ja=S.start-f-1;ja>=0;--ja)S.old.unshift(N[ja]);S.added+=S.start-f;S.start=f}else if(S.start<f){la=f-S.start;D+=la}ja=S.added-la;for(f=N.length;ja<f;++ja)S.old.push(N[ja]);
if(S.added<D)S.added=D}this.time=O}};s.prototype={stop:function(){u.call(this.e)},target:function(){return this.e.target||this.e.srcElement},button:function(){if(this.e.which)return this.e.which;else if(this.e.button&1)return 1;else if(this.e.button&2)return 3;else if(this.e.button&4)return 2},pageX:function(){if(this.e.pageX!=null)return this.e.pageX;var f=this.target().ownerDocument;return this.e.clientX+f.body.scrollLeft+f.documentElement.scrollLeft},pageY:function(){if(this.e.pageY!=null)return this.e.pageY;
var f=this.target().ownerDocument;return this.e.clientY+f.body.scrollTop+f.documentElement.scrollTop}};K.prototype={set:function(f,D){clearTimeout(this.id);this.id=setTimeout(D,f)}};var pa=function(){var f=document.createElement("pre");f.innerHTML=" ";return!f.innerHTML}(),T=/gecko\/\d{7}/i.test(navigator.userAgent),ga="\n";(function(){var f=document.createElement("textarea");f.value="foo\nbar";if(f.value.indexOf("\r")>-1)ga="\r\n"})();for(var ha=8,xa=/Mac/.test(navigator.platform),Oa={},Ra=35;Ra<=
40;++Ra)Oa[Ra]=Oa["c"+Ra]=true;var X="\n\nb".split(/\n/).length!=3?function(f){for(var D=0,N,O=[];(N=f.indexOf("\n",D))>-1;){O.push(f.slice(D,f.charAt(N-1)=="\r"?N-1:N));D=N+1}O.push(f.slice(D));return O}:function(f){return f.split(/\r?\n/)};if(window.getSelection)var Q=function(f){try{return{start:f.selectionStart,end:f.selectionEnd}}catch(D){return null}},V=function(f,D,N){try{f.setSelectionRange(D,N)}catch(O){}};else{Q=function(f){try{var D=f.ownerDocument.selection.createRange()}catch(N){return null}if(!D||
D.parentElement()!=f)return null;var O=f.value,S=O.length,la=f.createTextRange();la.moveToBookmark(D.getBookmark());var ja=f.createTextRange();ja.collapse(false);if(la.compareEndPoints("StartToEnd",ja)>-1)return{start:S,end:S};f=-la.moveStart("character",-S);for(D=O.indexOf("\r");D>-1&&D<f;D=O.indexOf("\r",D+1),f++);if(la.compareEndPoints("EndToEnd",ja)>-1)return{start:f,end:S};S=-la.moveEnd("character",-S);for(D=O.indexOf("\r");D>-1&&D<S;D=O.indexOf("\r",D+1),S++);return{start:f,end:S}};V=function(f,
D,N){var O=f.createTextRange();O.collapse(true);var S=O.duplicate(),la=0;f=f.value;for(var ja=f.indexOf("\n");ja>-1&&ja<D;ja=f.indexOf("\n",ja+1))++la;for(O.move("character",D-la);ja>-1&&ja<N;ja=f.indexOf("\n",ja+1))++la;S.move("character",N-la);O.setEndPoint("EndToEnd",S);O.select()}}c.defineMode("null",function(){return{token:function(f){f.skipToEnd()}}});c.defineMIME("text/plain","null");return c}();
...
B)}function ga(I,fa){function ca(ia){if(ia==",")return s(I,ca);if(ia==fa)return s();return s(p(fa))}return function(ia){return ia==fa?s():x(I,ca)}}function ha(I){if(I=="}")return s();return x(t,ha)}function xa(I,fa){if(I=="variable"){C(fa);return s(Oa)}return s()}function Oa(I,fa){if(fa=="=")return s(B,Oa);if(I==",")return s(xa)}function Ra(I){if(I=="var")return s(xa,Q);if(I==";")return x(Q);if(I=="variable")return s(X);return x(Q)}function X(I,fa){if(fa=="in")return s(B);return s(E,Q)}function Q(I,
fa){if(I==";")return s(V);if(fa=="in")return s(B);return s(B,p(";"),V)}function V(I){I!=")"&&s(B)}function f(I,fa){if(I=="variable"){C(fa);return s(f)}if(I=="(")return s(ka(")"),K,ga(D,")"),ma,t,ba)}function D(I,fa){if(I=="variable"){C(fa);return s()}}var N=c.indentUnit,O=a.json,S=function(){function I(La){return{type:La,style:"js-keyword"}}var fa=I("keyword a"),ca=I("keyword b"),ia=I("keyword c"),sa=I("operator"),ta={type:"atom",style:"js-atom"};return{"if":fa,"while":fa,"with":fa,"else":ca,"do":ca,
...
function i(T){return function(ga,ha){for(;!ga.eol();)if(ga.next()==T){ha.tokenize=g;break}return"xml-attribute"}}function l(T,ga){return function(ha,xa){for(;!ha.eol();){if(ha.match(ga)){xa.tokenize=e;break}ha.next()}return T}}function n(){for(var T=arguments.length-1;T>=0;T--)Y.cc.push(arguments[T])}function u(){n.apply(null,arguments);return true}function x(T,ga){var ha=p.doNotIndent.hasOwnProperty(T)||Y.context&&Y.context.noIndent;Y.context={prev:Y.context,tagName:T,indent:Y.indented,startOfLine:ga,
noIndent:ha}}function s(T){if(T=="openTag"){Y.tagName=B;return u(ba,C(Y.startOfLine))}else if(T=="closeTag"){if(Y.context)Y.context=Y.context.prev;return u(K)}else{if(T=="xml-cdata"){if(!Y.context||Y.context.name!="!cdata")x("!cdata");if(Y.tokenize==e)if(Y.context)Y.context=Y.context.prev}return u()}}function C(T){return function(ga){if(ga=="selfcloseTag"||ga=="endTag"&&p.autoSelfClosers.hasOwnProperty(Y.tagName.toLowerCase()))return u();if(ga=="endTag"){x(Y.tagName,T);return u()}return u()}}function K(T){if(T==
"endTag")return u();return n()}function ba(T){if(T=="xml-word"){pa="xml-attname";return u(ba)}if(T=="equals")return u(ka,ba);return n()}function ka(T){if(T=="xml-word"&&p.allowUnquoted){pa="xml-attribute";return u()}if(T=="xml-attribute")return u();return n()}var ma=c.indentUnit,p=a.htmlMode?{autoSelfClosers:{br:true,img:true,hr:true,link:true,input:true,meta:true,col:true,frame:true,base:true,area:true},doNotIndent:{pre:true,"!cdata":true},allowUnquoted:true}:{autoSelfClosers:{},doNotIndent:{"!cdata":true},
...
CodeMirror.defineMode("css",function(c){function a(x,s){u=s;return x}function e(x,s){var C=x.next();if(C=="@"){x.eatWhile(/\w/);return a("css-at",x.current())}else if(C=="/"&&x.eat("*")){s.tokenize=g;return g(x,s)}else if(C=="<"&&x.eat("!")){s.tokenize=i;return i(x,s)}else if(C=="=")a(null,"compare");else if((C=="~"||C=="|")&&x.eat("="))return a(null,"compare");else if(C=='"'||C=="'"){s.tokenize=l(C);return s.tokenize(x,s)}else if(C=="#"){x.eatWhile(/\w/);return a("css-selector","hash")}else if(C==
"!"){x.match(/^\s*\w*/);return a("css-important","important")}else if(/\d/.test(C)){x.eatWhile(/[\w.%]/);return a("css-unit","unit")}else if(/[,.+>*\/]/.test(C))return a(null,"select-op");else if(/[;{}:\[\]]/.test(C))return a(null,C);else{x.eatWhile(/[\w\\\-_]/);return a("css-identifier","identifier")}}function g(x,s){for(var C=false,K;(K=x.next())!=null;){if(C&&K=="/"){s.tokenize=e;break}C=K=="*"}return a("css-comment","comment")}function i(x,s){for(var C=0,K;(K=x.next())!=null;){if(C>=2&&K==">"){s.tokenize=
e;break}C=K=="-"?C+1:0}return a("css-comment","comment")}function l(x){return function(s,C){for(var K=false,ba;(ba=s.next())!=null;){if(ba==x&&!K)break;K=!K&&ba=="\\"}if(!K)C.tokenize=e;return a("css-string","string")}}var n=c.indentUnit,u;return{startState:function(x){return{tokenize:e,baseIndent:x||0,stack:[]}},token:function(x,s){if(x.eatSpace())return null;var C=s.tokenize(x,s),K=s.stack[s.stack.length-1];if(u=="hash"&&K=="rule")C="css-colorcode";else if(C=="css-identifier")if(K=="rule")C="css-value";
else if(!K||K=="@media{")C="css-selector";K=="rule"&&/^[\{\};]$/.test(u)&&s.stack.pop();if(u=="{")if(K=="@media")s.stack[s.stack.length-1]="@media{";else s.stack.push("{");else if(u=="}")s.stack.pop();else if(u=="@media")s.stack.push("@media");else K!="rule"&&K!="@media"&&u!="comment"&&s.stack.push("rule");return C},indent:function(x,s){var C=x.stack.length;if(/^\}/.test(s))C-=x.stack[x.stack.length-1]=="rule"?2:1;return x.baseIndent+C*n},electricChars:"}"}});CodeMirror.defineMIME("text/css","css");
CodeMirror.defineMode("liquid",function(c){function a(x,s){var C=l.token(x,s.htmlState);if(C=="xml-tag"&&x.current()==">"&&s.htmlState.context)if(/^script$/i.test(s.htmlState.context.tagName)){s.token=g;s.localState=n.startState(l.indent(s.htmlState,""))}else if(/^style$/i.test(s.htmlState.context.tagName)){s.token=i;s.localState=u.startState(l.indent(s.htmlState,""))}return C}function e(x,s,C){var K=x.current();s=K.search(s);s>-1&&x.backUp(K.length-s);return C}function g(x,s){if(x.match(/^<\/\s*script\s*>/i,
false)){s.token=a;s.curState=null;return a(x,s)}return e(x,/<\/\s*script\s*>/,n.token(x,s.localState))}function i(x,s){if(x.match(/^<\/\s*style\s*>/i,false)){s.token=a;s.localState=null;return a(x,s)}return e(x,/<\/\s*style\s*>/,u.token(x,s.localState))}var l=CodeMirror.getMode(c,{name:"xml",htmlMode:true}),n=CodeMirror.getMode(c,"javascript"),u=CodeMirror.getMode(c,"css");return{startState:function(){var x=l.startState();return{token:a,localState:null,htmlState:x}},copyState:function(x){if(x.localState)var s=
CodeMirror.copyState(x.token==i?u:n,x.localState);return{token:x.token,localState:s,htmlState:CodeMirror.copyState(l,x.htmlState)}},token:function(x,s){return s.token(x,s)},indent:function(x,s){return x.token==a||/^\s*<\//.test(s)?l.indent(x.htmlState,s):x.token==g?n.indent(x.localState,s):u.indent(x.localState,s)},electricChars:"/{}:"}});CodeMirror.defineMIME("text/html","htmlmixed");
...
i=$(window).height();e=e.height()+42;e=(i-e)/2;$("#overlay").css({height:i}).hide().fadeIn("fast");g.hide().fadeIn("fast").css({top:e});a&&a.call(this)},close:function(){$("#modal_wrapper").fadeOut("fast",function(){$(this).remove();$("#overlay").fadeOut("fast",function(){$(this).remove()})})}},logger={info:function(c){window.console&&console.log(c)},debug:function(c){window.console&&console.log(c)}},Utilities={notice:function(){},keyboard_nav:function(){jQuery("body").keydown(function(c){switch(c.keyCode){case 37:$("a.previous").click();
break;case 39:$("a.next").click();break}})},check_browser_version:function(){if(!Modernizr.history){jQuery("a").live("click",function(a){a.preventDefault();document.location.hash=$(this).attr("href")});var c=jQuery("form#search-form");c.attr("action","#"+c.attr("action"))}},formObserver:function(c){jQuery(c).keyup(function(){delay(function(){jQuery(c).parents("form:first").submit()},800)})}},Loader={start:function(c){c.show();Loader.timer=setInterval(function(){var a=c.css("background-position-y").replace("px",
"");c.css({"background-position-y":a-40+"px"})},67)},stop:function(c){c.hide();clearInterval(Loader.timer)}},iFramer={initialize:function(c,a){var e=jQuery(c),g=jQuery("#loader"),i=this;if(e.length){Loader.start(g);e.load(function(){var l=$(this),n=l.contents(),u=n.find("span.part-editor"),x=u.find("a");Loader.stop(g);i.setEditFlags(u);l.fadeIn("fast");n.find('a[href^="/preview"]').click(function(s){var C=$(this).attr("href").split("?")[0].replace("/preview","");if(C=Page.find_by_path(C)){s.preventDefault();
C.id();document.location.hash=C.attr("admin_path")}});x.click(function(){window.top.trigger=$(this);window.top.location.hash=$(this).attr("href");return false});a&&a.call(this)})}},setEditFlags:function(c){c.css({position:"relative",display:"block",height:"0",width:"0","text-decoration":"none"}).find("span").css({display:"block",position:"absolute",top:"-24px",left:"12px",padding:"4px 8px",width:"100px",background:"rgba(255, 255, 0, 0.7)"})}},TabControl={initialize:function(c){if(c=jQuery(c)){c.each(function(a,
e){$(e).hide()});c.first().show();this.tabControl(jQuery(".tab-control"))}},tabControl:function(c){c.click(function(){var a="tab-"+$(this).find("label").attr("for").split("-")[1];jQuery(".tab").hide();jQuery("#"+a).show()})}},delay=function(){var c=0;return function(a,e){clearTimeout(c);c=setTimeout(a,e)}}(),Sites=Sammy(function(){var c=this;this.debug=false;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);this.helpers({loadSites:function(a){if(Site.all().length==0)Site.load(function(){a&&
a.call(this)});else a&&a.call(this)},renderSiteIndex:function(){this.load(jQuery("#admin-sites-index")).interpolate(Site.toMustache(),"mustache").replace("#sidebar")}});this.bind("run",function(){c.modal=false;c.refresh_pages=true});this.get("#/preview",function(){alert(document.location.hostname);window.open("/","preview")});this.get("/admin/sites",function(a){c.refresh_pages=true;Galerie.close();jQuery("#editor").html('<h1 class="section">Sites</div>');a.loadSites(function(){a.renderSiteIndex()})});
this.get("/admin/sites/new",function(a){a.loadSites(function(){$("#modal").length==0&&Galerie.open();a.load(jQuery("#admin-sites-new")).interpolate({},"mustache").replace("#modal");a.renderSiteIndex(Site.all())})});this.post("/admin/sites",function(a){(new Site(a.params.site)).save(function(e,g){var i=JSON.parse(g.responseText);i.errors?alert(JSON.stringify(i)):a.redirect("#/sites")})});this.get("/admin/sites/:id/edit",function(a){Galerie.close();a.loadSites(function(){site=Site.find(a.params.id);
a.load(jQuery("#admin-sites-edit")).interpolate({site:site.asJSON()},"mustache").replace("#editor");a.renderSiteIndex(Site.all())})});this.put("/admin/sites/:id",function(a){var e=Site.find(a.params.id);e.attr(a.params.site);e.save(function(g,i){var l=JSON.parse(i.responseText);l.errors?alert(JSON.stringify(l)):a.redirect("#/sites")})});this.get("/admin/sites/:id/remove",function(a){Galerie.open();a.loadSites(function(){site=Site.find(a.params.id);a.load(jQuery("#admin-sites-remove")).interpolate({site:site.asJSON()},
"mustache").replace("#modal");a.renderSiteIndex(Site.all())})});this.del("/admin/sites/:id",function(a){site.destroy(function(e){e?a.redirect("#/sites"):alert("there were issues")})})}),Layouts=Sammy(function(){var c=this;this.debug=false;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);this.helpers({loadLayouts:function(a){if(Layout.all().length==0)Layout.load(function(){a&&a.call(this)});else a&&a.call(this)},renderLayoutIndex:function(){this.load(jQuery("#admin-templates-index")).interpolate({layouts:Layout.find_all_by_class("Layout").map(function(a){return a.attributes}),
...
value:"sass",selected:a.attr("filter")=="sass"?'selected="selected"':""},{name:"Scss",value:"scss",selected:a.attr("filter")=="scss"?'selected="selected"':""}]},"mustache").replace("#editor").then(function(){var e=jQuery("#layout_content");e.attr("value",a.attr("content"));e=e.attr("class");jQuery("#layout_content").length>0&&CodeMirror.fromTextArea(document.getElementById("layout_content"),{mode:e,lineNumbers:true})})}});this.bind("run",function(){c.refresh_templates=true;c.refresh_pages=true;c.modal=
false});this.get("/admin/templates",function(a){Galerie.close();c.refresh_templates=false;c.refresh_pages=true;jQuery("#editor").html('<h1 class="section">Templates</div>');Layout.load(function(){a.renderLayoutIndex(Layout.all())})});this.get("/admin/templates/new/:klass",function(a){this.loadLayouts(function(){var e=$("<div />").attr({id:"new-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(e);a.load(jQuery("#admin-templates-new")).interpolate({klass:a.params.klass},"mustache").replace("#new-page-container");
a.renderLayoutIndex(Layout.all())})});this.post("/admin/templates",function(a){(new Layout(a.params.template)).save(function(e,g){var i=JSON.parse(g.responseText);if(i.errors)Utilities.notice(JSON.stringify(i));else{a.redirect("/admin/templates");Utilities.notice("Successfully saved template")}})});this.get("/admin/templates/:id/edit",function(a){Galerie.close();c.refresh_pages=true;this.loadLayouts(function(){var e=Layout.find(a.params.id);window.ninja=true;e.load(function(){a.renderLayout(e);a.renderLayoutIndex(Layout.all());
window.ninja=false})})});this.put("/admin/templates/:id",function(a){var e=Layout.find(a.params.id);e.attr(a.params.layout);e.save(function(g){if(g){a.renderLayoutIndex();Utilities.notice("Successfully saved template")}})});this.get("/admin/templates/:id/remove",function(a){this.loadLayouts(function(){var e=Layout.find(a.params.id);Galerie.open();a.load(jQuery("#admin-templates-remove")).interpolate({layout:e.asJSON()},"mustache").replace("#modal");a.renderLayoutIndex(Layout.all())})});this.del("/templates/:id",
function(a){Layout.find(a.params.id).destroy(function(e){if(e){a.renderLayoutIndex();Utilities.notice("Successfully deleted template");a.redirect("/admin/templates")}})});this.post("/layouts/:id/parts",function(a){this.loadLayouts(function(){var e=a.params.id;Part.create(a.params.part,function(){a.redirect("/admin/templates/"+e+"/edit")})})})}),Assets=Sammy(function(){var c=this;this.debug=false;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);this.swap=function(a){jQuery("#editor").html(a)};
this.helpers({loadAssets:function(a,e){if(Asset.all().length==0)Asset.searchAdmin(a,function(){e&&e.call(this)});else e&&e.call(this)},send_files:function(a,e,g){for(var i=0,l=0;l<a.length;l++){var n=a[l];Asset.create(n,{before:function(u){jQuery(".progress").append('<p id="progress-'+u+'">'+n.name+'<span class="percentage"></span></p>')},progress:function(u,x){jQuery("#progress-"+u+" .percentage").text(" "+x+"%")},success:function(){i+=1;i==a.length&&Asset.searchAdmin(e,function(){g&&g.call(this)})}})}}});
this.bind("run",function(){c.modal=false;c.first_run=true});this.bind("show_info",function(){var a=Asset.find(c.current_asset_id),e=jQuery("#asset-"+a.id()),g=e.find(".remove");jQuery(".modal-strip").remove();g.length||this.load(jQuery("#admin-assets-remove")).interpolate({asset:a.toMustache()},"mustache").appendTo(e).then(function(){jQuery(".modal-strip").fadeIn("fast")})});this.get("/admin/assets",function(a){var e=a.params.query,g=e?{query:e}:{};g.limit=a.params.limit||48;g.page=a.params.page||
1;c.modal||Asset.searchAdmin(g,function(){a.load(jQuery("#admin-assets-index")).interpolate(Asset.toMustache(e),"mustache").replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple");jQuery("a.info-icon").click(function(i){i.preventDefault();c.current_asset_id=this.id.split("-")[2];a.trigger("show_info")})})});c.modal=false;c.first_run=false});this.get("/admin/assets/new",function(a){a.load(jQuery("#admin-assets-new")).interpolate({},"mustache").replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple",
"multiple")});c.first_run=false});this.post("/admin/assets",function(a){var e=this,g=document.getElementById("ajax_uploader").files,i=a.params.query?a.params.query:null;jQuery("form#new_asset");var l=i?{query:i}:{};l.limit=a.params.limit||48;l.page=a.params.page||1;this.send_files(g,l,function(){e.render("/templates/admin/assets/index.mustache",Asset.toMustache(l.query)).replace("#editor").then(function(){jQuery("#ajax_uploader").attr("multiple","multiple")})});return false});this.get("/admin/assets/:id/edit",
function(a){var e=a.params.query?a.params.query:null;this.loadAssets(e?{query:a.params.query}:{},function(){var g=Asset.find(a.params.id);a.load(jQuery("#admin-assets-edit")).interpolate(g.toMustacheWithNeighbors(e),"mustache").replace("#editor").then(function(){setTimeout(function(){$("img.fade-in").fadeIn("slow")},100);Utilities.formObserver(".image-info input[type=text], .image-info textarea")})});c.modal=false});this.put("/admin/assets/:id",function(a){var e=Asset.find(a.params.id);e.attr(a.params.asset);
e.save(function(g){g&&Utilities.notice("Successfully saved asset")})});this.get("/admin/assets/:id/remove",function(a){var e=a.params.query?a.params.query:null,g=e?{query:a.params.query}:null;jQuery(".modal-strip").remove();this.loadAssets(g,function(){var i=Asset.find(a.params.id);if(c.first_run){var l=a.load(jQuery("#admin-assets-edit")).interpolate(Asset.toMustache(e),"mustache"),n=a.load(jQuery("#admin-assets-remove")).interpolate({asset:i.toMustache(e)},"mustache");l.replace("#editor").then(function(){var u=
jQuery("#asset-"+i.id());n.appendTo(u).then(function(){jQuery(".modal-strip").fadeIn("fast").css({})})})}else{l=jQuery("#asset-"+i.id());n=a.load(jQuery("#admin-assets-remove")).interpolate({asset:i.toMustache(e)},"mustache");n.appendTo(l).then(function(){jQuery(".modal-strip").fadeIn("fast").css({})})}})});this.del("/admin/assets/:id",function(a){var e=a.params.query?a.params.query:null,g=e?"?"+decodeURIComponent(jQuery.param({query:e})):"";Asset.find(a.params.id).destroy(function(i){if(i){Galerie.close();
Utilities.notice("Successfully saved asset");a.redirect("/admin/assets"+g)}})})}),Pages=Sammy(function(c){var a=this;this.debug=false;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);c.helpers({loadPages:function(e){if(Page.all().length==0)Page.load(function(){Layout.load(function(){e&&e.call(this)})});else e&&e.call(this)},renderTree:function(e,g){var i=this;i.load(jQuery("#admin-pages-node")).interpolate({pages:[e.asJSON()]},"mustache").replace("#sidebar").then(function(){jQuery("ul.page-children:first").attr("id",
"pages");i.renderNode(e,g);e.id()==g.id()&&jQuery("li#page-"+e.id()).addClass("active")})},renderNode:function(e,g){var i=this;i.load(jQuery("#admin-pages-node")).interpolate(e.children().toMustache(),"mustache").appendTo("#page-"+e.id()).then(function(){logger.info(e.id());$("ul#pages ul.page-children").sortable({items:"li"});e.children().each(function(l){l.id()==g.id()&&jQuery("li#page-"+l.id()).addClass("active");if(l.has_children()==true){jQuery("#page-"+l.id()).addClass("open");i.renderNode(l,
g)}})})},renderPagePreview:function(e,g){a.modal||this.load(jQuery("#admin-pages-show")).interpolate({page:e.asJSON(),layouts:Layout.asLayoutJSON(e.attr("layout_id")),base_page_id:e.id()},"mustache").replace("#editor").then(function(){iFramer.initialize(".preview iframe",function(){g&&g.call(this)});jQuery("li.node").removeClass("active");jQuery("#page-"+e.id()).addClass("active")})},open_page_editor:function(){var e=jQuery("#page-editor"),g=jQuery("#page-title");e.addClass("open").animate({height:"200px"},
300);g.removeAttr("disabled").focus()},close_page_editor:function(){var e=jQuery("#page-editor"),g=jQuery("#page-title");e.removeClass("open").animate({height:"0"},300);g.attr("disabled","disabled")}});c.bind("page-index",function(){var e=this;jQuery(".page-children").length||Page.load(function(){e.renderTree(Page.root(),Page.root())})});c.bind("set-active-tab",function(){});c.bind("run",function(){a.application=this;a.refresh_pages=true;a.modal=false;jQuery(".add-asset").live("click",function(){a.application.trigger("add-asset");
return false});jQuery("#sidebar .opener").live("click",function(){var e=(new Date).getTime();e=$(this).parents("li:first");console.log(e);var g=this.id.split("-")[2],i=Page.find(g),l=jQuery.cookie("active_page_ids");l=l?l.split(","):[];if(e.hasClass("open")){e.toggleClass("open");l=jQuery.grep(l,function(n){return n!=g});jQuery.cookie("active_page_ids",l.join(","),{path:"/admin"});i.children().each(function(n){Page.remove(n)});jQuery("#page-"+g+" ul").remove()}else{l.push(g);e.toggleClass("open");
e=(new Date).getTime();jQuery.cookie("active_page_ids",l.join(","),{path:"/admin"});Page.load(function(){a.application.renderNode(i,i)})}a.refresh_pages=false;return a.modal=false})});this.get("/admin/pages/?",function(e){Galerie.close();e.trigger("page-index");jQuery("#editor").html('<h1 class="section">Pages</div>');a.do_not_refresh=false});this.get("/admin/pages/:id/new/?",function(e){this.loadPages(function(){var g=Page.find(e.params.id),i=$("<div />").attr({id:"new-page-container","class":"small-modal"});
$("#modal").length==0&&Galerie.open(i);e.trigger("page-index");e.load(jQuery("#admin-pages-new")).interpolate({parent:g.asJSON(),layouts:Layout.asLayoutJSON(g.attr("layout_id"))},"mustache").replace("#new-page-container")})});this.post("/admin/pages/:page_id/?",function(e){Page.find(e.params.page_id);(new Page(e.params.page)).save(function(g,i){var l=JSON.parse(i.responseText);if(l.errors)alert(JSON.stringify(l));else{a.refresh_pages=true;Utilities.notice("Successfully saved page");e.redirect("#/pages/"+
l.id)}})});this.get("/admin/pages/:id/?",function(e){var g=this;Galerie.close();jQuery(".modal-editor").remove();this.loadPages(function(){var i=e.params.id,l=Page.find(i);if(a.do_not_refresh)g.close_page_editor();else{if(l)e.renderPagePreview(l);else{l=new Page({id:i});l.load(function(){e.renderPagePreview(l)})}a.modal=false;a.do_not_refresh=false;e.trigger("page-index")}});a.do_not_refresh=false});this.get("/admin/pages/:id/edit/?",function(e){var g=this;Galerie.close();a.modal=false;this.loadPages(function(){var i=
Page.find(e.params.id);if(jQuery("#preview-"+i.id()).length)g.open_page_editor();else{e.renderPagePreview(i,function(){g.open_page_editor()});e.trigger("page-index")}});a.do_not_refresh=true});this.put("/admin/pages/:page_id",function(e){var g=Page.find(e.params.page_id);g.attr(e.params.page);g.save(function(i){if(i){a.modal=false;Utilities.notice("Successfully saved page");e.renderTree(Page.root(),g)}})});this.get("/admin/pages/:id/remove",function(e){this.loadPages(function(){var g=Page.find(e.params.id),
i=$("<div />").attr({id:"remove-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(i);e.trigger("page-index");e.load(jQuery("#admin-pages-remove")).interpolate({page:g.asJSON()},"mustache").replace("#remove-page-container")})});this.del("/admin/pages/:id",function(e){var g=e.params.id;Page.find(g).destroy(function(i){if(i){jQuery("#page-"+g).remove();a.refresh_pages=false;e.redirect("#/pages")}})})}),Parts=Sammy(function(c){var a=this;this.debug=false;this.use(Sammy.JSON);
this.use(Sammy.Mustache);this.use(Sammy.NestedParams);c.helpers({render_part:function(e,g,i){var l=this;l.load(jQuery("#admin-"+i+"-edit")).interpolate({part:e.asJSON(),page:g.asJSON(),assets:Asset.asJSON()},"mustache").appendTo(jQuery("body")).then(function(){var n=jQuery(".modal-editor"),u=$("iframe").contents(),x=u.find("#editor-"+e.id());n.fadeIn("fast").css({top:x.offset().top-u.find("body").scrollTop()+"px",left:x.offset().left+400+"px"});l.set_asset_links(e,g);jQuery("#ajax_uploader").attr("multiple",
"multiple").change(function(){var s=jQuery(this).parents("form:first");jQuery(".progress").slideDown("slow",function(){s.submit()})});l.trigger("page-index");a.modal=true})},set_asset_links:function(e,g){jQuery("#search-results-container li.asset").each(function(i,l){var n=jQuery(l).find("a"),u=jQuery(l).attr("id").split("-")[1];n.click(function(x){x.preventDefault();x=g.attr("parts");for(var s=0,C=x.length;s<C;++s){var K=x[s];if(e.id()==K.id){K.asset_id=u;e.attr("asset_id",u);e.save()}}g.save(function(){jQuery(".modal-editor").remove();
a.modal=false;document.location.hash="#"+g.attr("admin_path")})})})}});this.get("#/pages/:page_id/parts/:id/edit",function(e){jQuery(".modal-editor").remove();var g=$("iframe");this.loadPages(function(){Page.load_by_id(e.params.page_id,function(){var i=Page.find(e.params.page_id),l=i.parts().find(e.params.id),n=jQuery("#page-updated_at").attr("value");if(g.length)if(n!=i.attr("updated_at")){var u=jQuery(".preview iframe");u.hide().attr("src",u.attr("src"));u.load(function(){u.fadeIn("fast");e.render_part(l,
i,"parts")})}else e.render_part(l,i,"parts");else e.renderPagePreview(i,function(){e.render_part(l,i,"parts")})})})});this.get("#/pages/:page_id/image_parts/:id/edit",function(e){var g=this;jQuery(".modal-editor").remove();g.loadPages(function(){var i=Page.find(e.params.page_id),l=i.parts().find(e.params.id),n=$("iframe");Asset.searchAdmin({limit:"8"},function(){n.length?g.render_part(l,i,"image_parts"):e.renderPagePreview(i,function(){g.render_part(l,i,"image_parts")})})})});this.get("/pages/:page_id/parts/:id/results",
function(e){var g=this;this.loadPages(function(){var i=Page.find(e.params.page_id),l=Part.find(e.params.id),n=e.params.query?e.params.query:null;Asset.searchAdmin(n?{query:n}:{},function(){e.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),"mustache").replace("#search-results-container").then(function(){g.set_asset_links(l,i)})})})});this.put("#/pages/:page_id/parts/:id",function(e){var g=e.params.id,i=e.params.page_id,l=Page.find(i),n=l.parts().find(g);logger.info(e.params.part);
for(var u=l.attr("parts"),x=0,s=u.length;x<s;++x){n=u[x];if(g==n.id){var C=e.params.part.content;n.content=C;n=Part.find(n.id);n.attr("content",C);n.save()}}l.save(function(K){if(K){a.modal=false;e.redirect("#/pages/"+i)}})});this.post("#/pages/:page_id/image_parts/:id/assets",function(e){var g=this,i=Page.find(e.params.page_id),l=i.parts().find(e.params.id),n=document.getElementById("ajax_uploader").files,u=e.params.query?e.params.query:null;jQuery("form#new_asset");u=u?{query:u}:{};u.limit=e.params.limit||
48;u.page=e.params.page||1;this.send_files(n,u,function(){e.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),"mustache").replace("#search-results-container").then(function(){jQuery("#ajax_uploader").attr("files",null);jQuery(".progress").slideUp("slow",function(){jQuery(this).html("")});g.set_asset_links(l,i)})})})}),Users=Sammy(function(){var c=this;this.debug=false;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);this.helpers({loadUsers:function(a){if(User.all().length==
0)User.load(function(){a&&a.call(this)});else a&&a.call(this)},renderUserIndex:function(a){this.load(jQuery("#admin-users-index")).interpolate(User.toMustache(),"mustache").replace("#editor").then(function(){a&&a.call(this)})}});this.bind("run",function(){c.modal=false;c.refresh_pages=true});this.get("/admin/users",function(a){Galerie.close();jQuery("#editor").html('<h1 class="section">Users</div>');a.loadUsers(function(){a.renderUserIndex()})});this.get("/admin/users/new",function(a){a.loadUsers(function(){jQuery("#modal").length||
Galerie.open();a.load(jQuery("#admin-users-new")).interpolate({},"mustache").replace("#modal");a.renderUserIndex()})});this.post("/admin/users",function(a){(new User(a.params.user)).save(function(e,g){var i=JSON.parse(g.responseText);i.errors?alert(JSON.stringify(i)):a.redirect("/admin/users")})});this.get("/admin/users/:id/edit",function(a){a.loadUsers(function(){user=User.find(a.params.id);if(jQuery("#users").length){jQuery(".user-form").html("");a.load(jQuery("#admin-users-edit")).interpolate({user:user.asJSON()},
"mustache").replace("#user-form-"+user.id())}else a.renderUserIndex(function(){jQuery(".user-form").html("");a.load(jQuery("#admin-users-edit")).interpolate({user:user.asJSON()},"mustache").replace("#user-form-"+user.id())})})});this.put("/admin/users/:id",function(a){var e=User.find(a.params.id);e.attr(a.params.user);e.save(function(g,i){var l=JSON.parse(i.responseText);l.errors?alert(JSON.stringify(l)):a.redirect("/admin/users")})})});
jQuery(document).ready(function(){jQuery("#login").length||jQuery.ajax({url:"/templates",success:function(a){jQuery("head").append(a);Pages.run()}});var c=jQuery("#loader");jQuery("body").ajaxStart(function(){window.ninja||Loader.start(c)}).ajaxSuccess(function(){Loader.stop(c)});Utilities.keyboard_nav();Utilities.check_browser_version()});
