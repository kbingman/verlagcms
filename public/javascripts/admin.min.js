...
e.removeClass("open").animate({height:"0"},300);g.attr("disabled","disabled")}});c.bind("page-index",function(){var e=this;jQuery(".page-children").length||e.loadPages(function(){e.renderTree(Page.root(),Page.root())})});c.bind("set-active-tab",function(){});c.before(function(){});c.bind("run",function(){a.application=this;a.refresh_pages=true;a.modal=false;jQuery(".add-asset").live("click",function(){a.application.trigger("add-asset");return false});jQuery("#sidebar .opener").live("click",function(){var e=
(new Date).getTime();e=$(this).parents("li:first");console.log(e);var g=this.id.split("-")[2],i=Page.find(g),l=jQuery.cookie("active_page_ids");l=l?l.split(","):[];if(e.hasClass("open")){e.toggleClass("open");l=jQuery.grep(l,function(n){return n!=g});jQuery.cookie("active_page_ids",l.join(","),{path:"/admin"});i.children().each(function(n){Page.remove(n)});jQuery("#page-"+g+" ul").remove()}else{l.push(g);e.toggleClass("open");e=(new Date).getTime();jQuery.cookie("active_page_ids",l.join(","),{path:"/admin"});
Page.load(function(){a.application.renderNode(i,i)})}a.refresh_pages=false;return a.modal=false})});this.get("#/pages/?",function(e){Galerie.close();e.trigger("page-index");jQuery("#editor").html('<h1 class="section">Pages</div>');a.do_not_refresh=false});this.get("#/pages/:id/new/?",function(e){this.loadPages(function(){var g=Page.find(e.params.id),i=$("<div />").attr({id:"new-page-container","class":"small-modal"});$("#modal").length==0&&Galerie.open(i);e.trigger("page-index");e.load(jQuery("#admin-pages-new")).interpolate({parent:g.asJSON(),
...
Asset.searchAdmin(n?{query:n}:{},function(){e.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),"mustache").replace("#search-results-container").then(function(){g.set_asset_links(l,i)})})})});this.put("#/pages/:page_id/parts/:id",function(e){var g=e.params.id,i=e.params.page_id,l=Page.find(i),n=l.parts().find(g);logger.info(e.params.part);for(var v=l.attr("parts"),x=0,s=v.length;x<s;++x){n=v[x];if(g==n.id){var D=e.params.part.content;n.content=D;n=Part.find(n.id);n.attr("content",
D);n.save()}}l.save(function(K){if(K){a.modal=false;e.redirect("#/pages/"+i)}})});this.post("#/pages/:page_id/image_parts/:id/assets",function(e){var g=this,i=Page.find(e.params.page_id),l=i.parts().find(e.params.id),n=document.getElementById("ajax_uploader").files,v=e.params.query?e.params.query:null;jQuery("form#new_asset");v=v?{query:v}:{};v.limit=e.params.limit||48;v.page=e.params.page||1;this.send_files(n,v,function(){e.load(jQuery("#admin-pages-search_results")).interpolate(Asset.toMustache(),
"mustache").replace("#search-results-container").then(function(){jQuery("#ajax_uploader").attr("files",null);jQuery(".progress").slideUp("slow",function(){jQuery(this).html("")});g.set_asset_links(l,i)})})})}),Users=Sammy(function(){var c=this;this.debug=false;this.disable_push_state=true;this.use(Sammy.JSON);this.use(Sammy.Mustache);this.use(Sammy.NestedParams);this.helpers({loadUsers:function(a){if(User.all().length==0)User.load(function(){a&&a.call(this)});else a&&a.call(this)},renderUserIndex:function(a){this.load(jQuery("#admin-users-index")).interpolate(User.toMustache(),
"mustache").replace("#editor").then(function(){a&&a.call(this)})}});this.bind("run",function(){c.modal=false;c.refresh_pages=true});this.get("#/users",function(a){Galerie.close();jQuery("#editor").html('<h1 class="section">Users</div>');a.loadUsers(function(){a.renderUserIndex()})});this.get("#/users/new",function(a){a.loadUsers(function(){jQuery("#modal").length||Galerie.open();a.load(jQuery("#admin-users-new")).interpolate({},"mustache").replace("#modal");a.renderUserIndex()})});this.post("#/users",
function(a){(new User(a.params.user)).save(function(e,g){var i=JSON.parse(g.responseText);i.errors?alert(JSON.stringify(i)):a.redirect("#/users")})});this.get("#/users/:id/edit",function(a){a.loadUsers(function(){user=User.find(a.params.id);if(jQuery("#users").length){jQuery(".user-form").html("");a.load(jQuery("#admin-users-edit")).interpolate({user:user.asJSON()},"mustache").replace("#user-form-"+user.id())}else a.renderUserIndex(function(){jQuery(".user-form").html("");a.load(jQuery("#admin-users-edit")).interpolate({user:user.asJSON()},
"mustache").replace("#user-form-"+user.id())})})});this.put("#/users/:id",function(a){var e=User.find(a.params.id);e.attr(a.params.user);e.save(function(g,i){var l=JSON.parse(i.responseText);l.errors?alert(JSON.stringify(l)):a.redirect("#/users")})})});
jQuery(document).ready(function(){var c=jQuery("#loader");jQuery("body").ajaxStart(function(){c.show()});jQuery("body").ajaxSuccess(function(){c.hide()});jQuery("#login").length||jQuery.ajax({url:"/templates",success:function(a){jQuery("head").append(a);Pages.run("#/pages")}});Utilities.keyboard_nav();Utilities.check_browser_version()});
